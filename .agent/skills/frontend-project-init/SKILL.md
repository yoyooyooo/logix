...
- 命令行一键初始化（默认模板 `vite-logix-empty`）：
  - `node .agent/skills/frontend-project-init/scripts/init-frontend-project.cjs <target-dir>`
  - 示例：`node .agent/skills/frontend-project-init/scripts/init-frontend-project.cjs examples/my-sandbox`
- 可选参数：
  - `node .agent/skills/frontend-project-init/scripts/init-frontend-project.cjs <target-dir> <template-id>`
...
- 在复制模板时，避免修改 Logix 运行时与 Vite 配置，除非已经在 `docs/specs/runtime-logix` 中更新对应规范。
