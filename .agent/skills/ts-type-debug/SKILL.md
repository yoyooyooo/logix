...
2. 在 skill 目录（例如 `.codex/skills/ts-type-debug`）的 `scripts/` 下创建/更新 `inspect-types.ts`（参考下文模板）。  
3. 在 skill 根目录运行：
   ```bash
...
- 调用时 `cwd` 为 skill 根目录（例如 `.codex/skills/ts-type-debug`）；  
...
在 skill 目录（如 `.codex/skills/ts-type-debug`）的 `scripts/` 下创建 `inspect-types.ts`，采用如下结构：
