...
- 内置模板根：`.codex/skills/unit-creator`
- 默认入口：通过 `skt generate` 基于模板 + 宏在 `apps/www2/registry/default` 下落盘源码。
- UI/Pro 组件完整骨架（含测试）可使用 `.codex/skills/project-guide/scripts/component-scaffold.ts`，或使用本 skill 提供的 `ui-component` 模板（两者骨架结构保持一致）。
- UI/Pro 组件也可通过脚本 `.codex/skills/unit-creator/scripts/scaffold-ui-component.mjs` 自动推导宏并调用 `ui-component` 模板。
...
> 实际模板名以 `skt templates list --template-root .codex/skills/unit-creator --json` 返回为准；LLM 可以根据用户描述选择最匹配的模板。
>
> 注意：`--template-root` 需要指向包含 `templates/**/template.md` 的上层目录，这里应使用 `.codex/skills/unit-creator`，而不是 `.codex/skills/unit-creator/templates`。
...
- UI/Pro 组件：
  - `node .codex/skills/unit-creator/scripts/scaffold-ui-component.mjs <name> [--layer ui|pro] [--with-docs] [--with-examples] [--write]`
  - 示例：`ui-hello-world` → 自动推导宏并调用 `ui-component` 模板。
- Hook：
  - `node .codex/skills/unit-creator/scripts/scaffold-hook.mjs <hook-file-name> [--write]`
  - 示例：`use-copy-advanced` → 生成 `hooks/use-copy-advanced.tsx` + `.meta.json`。
- lib 模块：
  - `node .codex/skills/unit-creator/scripts/scaffold-lib.mjs <lib-dir-name> [--write]`
  - 示例：`string-utils` → 生成 `lib/string-utils/index.ts` + `meta.json`。
- Block：
  - `node .codex/skills/unit-creator/scripts/scaffold-block.mjs <block-name> [--write]`
...
>
> - 固定 `--template-root .codex/skills/unit-creator`、`--root .`，保证只在 IMD 仓库工作区内写文件。
...
本节按照「先最少信息 → 再进阶控制」的顺序汇总 `skt` 调用方式，避免在 Skill 主体重复粘贴整行命令。除非特别说明，所有命令都默认带上 `--template-root .codex/skills/unit-creator --root . --json`。
...
- **发现模板列表**
  ...
  ```bash
  skt templates list --template-root .codex/skills/unit-creator --json
  ```
...
- **读取宏 / 插槽定义**
  ...
  skt list --template <name> \
    --template-root .codex/skills/unit-creator \
    [--required-only] \
...
1. 选模板：`skt list --template ui-component --template-root .codex/skills/unit-creator --required-only --json` → 需要 `COMPONENT/LAYER/DOC_SLUG/PASCAL`。
2. 推宏：脚本 `node .codex/skills/unit-creator/scripts/scaffold-ui-component.mjs ui-hello-world --dry-run` 会自动得到 `ui-hello-world / ui / hello-world / HelloWorld`。
3. JSON 调用（等价于脚本）：
   ```bash
   cat <<'REQ' | skt generate --input-json - --json
   {
     "template": "ui-component",
     "templateRoot": ".codex/skills/unit-creator",
     "root": ".",
     "macros": {
