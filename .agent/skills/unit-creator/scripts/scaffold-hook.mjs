...
function parseArgs(argv) {
  ...
  if (!args.length || args.includes('-h') || args.includes('--help')) {
    console.log(
      'Usage: scaffold-hook.mjs <hook-file-name> [--write] [--dry-run]\n' +
        'Example: scaffold-hook.mjs use-copy-advanced --write'
    )
    process.exit(0)
  }
  const name = args[0]
