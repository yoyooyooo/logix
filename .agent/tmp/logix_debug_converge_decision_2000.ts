...
const makeConvergeRuntime = (steps: number): ConvergeRuntime => {
...
const main = async () => {
  const rt = makeConvergeRuntime(2000)
  const dirtyRoots = 100

  // warmup (txnSeq=1 -> cold_start)
  ...
  // observe cache miss/hit behavior
  await rt.runtime.runPromise(
    runConvergeTxnCommit(rt, dirtyRoots).pipe(Effect.locally(Logix.Debug.internal.currentDiagnosticsLevel, 'off')) as any,
  )
  await rt.runtime.runPromise(
