...
type ConvergeMode = 'full' | 'dirty' | 'auto'
...
const makeConvergeRuntime = (steps: number, convergeMode: ConvergeMode): ConvergeRuntime => {
  ...
  const noopSink: Logix.Debug.Sink = {
    record: () => Effect.void,
  }
...
const measure = async (mode: ConvergeMode, steps: number, dirtyRoots: number) => {
  const rt = makeConvergeRuntime(steps, mode)
  ...
  for (let i = 0; i < runs; i++) {
    ...
    await rt.runtime.runPromise(
      runConvergeTxnCommit(rt, dirtyRoots).pipe(Effect.locally(Logix.Debug.internal.currentDiagnosticsLevel, 'off')) as any,
    )
    const t1 = nowMs()
  ...
  console.log(JSON.stringify({ mode, steps, dirtyRoots, p95Ms: p95(samples) }, null, 2))
}

const main = async () => {
  const steps = 200
  const dirtyRoots = 1
  await measure('full', steps, dirtyRoots)
  await measure('dirty', steps, dirtyRoots)
  await measure('auto', steps, dirtyRoots)
}
