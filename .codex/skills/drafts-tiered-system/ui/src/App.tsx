...
function KanbanBoard() {
  ...
  const navigateDraft = useCallback((link: string) => {
    const raw = typeof link === 'string' ? link : String(link);

    // SpecID navigation: if link is a 3-digit id, jump to the draft that declares it.
    if (/^[0-9]{3}$/.test(raw)) {
      const byId = localDrafts.find((d: Draft) => d.id === raw);
      if (byId) {
        setLocation(`/draft/${encodeURIComponent(byId.filename)}`);
        return;
      }
    }

    // Exact path navigation (relative to drafts root).
    const exactPath = localDrafts.find((d: Draft) => d.path === raw);
    if (exactPath) {
      setLocation(`/draft/${encodeURIComponent(exactPath.filename)}`);
      return;
    }

    // Repo-relative path navigation (docs/specs/drafts/...).
    const repoRel = raw.replace(/^docs\/specs\/drafts\//, '');
    const byRepoRel = localDrafts.find((d: Draft) => d.path === repoRel);
    if (byRepoRel) {
      setLocation(`/draft/${encodeURIComponent(byRepoRel.filename)}`);
      return;
    }

    let targetFilename = raw;

    // Resolve path logic (relative navigation from the currently selected draft)
    if (selectedDraft) {
      try {
        const resolvedPath = resolvePath(selectedDraft.path, raw);
        const targetDraft = localDrafts.find((d: Draft) => d.path === resolvedPath);

        if (targetDraft) {
          targetFilename = targetDraft.filename;
        } else {
          const absoluteDraft = localDrafts.find((d: Draft) => d.path === raw);
          if (absoluteDraft) {
            targetFilename = absoluteDraft.filename;
          } else {
            targetFilename = raw.split('/').pop() || raw;
          }
        }
      } catch (err) {
      ...
      <div className="h-screen flex flex-col bg-background text-foreground overflow-hidden font-sans selection:bg-primary/20">
        ...
        {activeView === 'board' ? (
          <div className="flex-1 overflow-x-auto overflow-y-hidden p-6">
            {isFiltered && (
              <div className="mb-4 px-3 py-1.5 bg-yellow-500/10 border border-yellow-500/20 rounded-md text-yellow-500 text-xs font-medium inline-flex items-center animate-in fade-in slide-in-from-top-2">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            <SpecGraphView drafts={localDrafts} onNavigate={navigateDraft} />
          </div>
        )}
