...
- Process 跨模块协作（declarative-first + blackbox fallback）
...
1. 把 `assets/feature-first-customer-search/src` 目录整体复制到你的项目 `src/` 下（按需重命名 feature）。
   - 或用本 skill 内置脚本自动复制（默认不覆盖）：`node <skill-root>/scripts/scaffold-feature-first.mjs --out ./src`（`<skill-root>` 即 `SKILL.md` 所在目录）
...
src/
  features/
    customer-search/
      ...
      processes/
        ...
        searchToDetail.declarative.process.ts
  ...
  scenarios/
    ...
    process-link-boundary-compare.ts
```
...
- `src/features/customer-search/processes/searchToDetail.declarative.process.ts`
...
- **Action key**：优先用短 key（如 `setKeyword/inc/add`），不要把 moduleId 再塞进 actionTag；执行面在 `dispatchers` 与 `actions` 中二选一并团队统一，监听用 `onAction(token)`。
...
- **协作语义**：默认 `searchToDetail.declarative.process.ts`（确定性优先）；仅在需要 async/external bridge 时使用 `searchToDetail.process.ts`。
- **语义对照验证**：可运行 `process-link-boundary-compare.ts`，直观看到 declarative same-tick 与 blackbox best-effort 的差异。
- **收尾自检**：交付前按 `references/consistency-checklist.md` 做一轮文档/样例/脚本一致性核对。
