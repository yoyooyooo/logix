...
const main = (): void => {
  ...
  const tasks: Task[] = [
    {
      ...
      command: ['pnpm', '-C', '.codex/skills/logix-perf-evidence', 'run', 'ci:decision-smoke'],
    },
    {
      label: 'replay',
      command: ['pnpm', '-C', '.codex/skills/logix-perf-evidence', 'run', 'ci:replay'],
    },
  ]
  ...
  if (includeTinyCollect) {
    tasks.push({
      label: 'tiny-collect',
      command: ['pnpm', '-C', '.codex/skills/logix-perf-evidence', 'run', 'ci:tiny-collect'],
    })
  }
