---
name: project-guide
description: 在 intent-flow 仓库做架构/规划/Runtime/示例/日常开发时，用最短导航定位 SSoT、代码落点、示例与质量门（平台 SSoT：docs/ssot/platform；runtime SSoT：docs/ssot/runtime；手册：docs/ssot/handbook）
---
...
## 内容边界（SKILL vs handbook）

- `SKILL.md`：只放入口导航 + 查询模板 + 最短路径（避免塞实现细节）。
- `docs/ssot/handbook/*`：工程手册/长链路/实现备忘录/完整示例；需要深入时再按需加载。
- `docs/ssot/handbook/README.md`：handbook 导航摘要（先从这里挑你要读的长文）。
  - 路径说明：本文中的 `docs/ssot/handbook/...` 均指仓库根目录下的绝对路径。
...
- 先定裁决层：硬约束看 `.specify/memory/constitution.md`；概念/术语/平台定位优先改 `docs/ssot/platform`；未定稿想法放 `docs/specs/drafts`；单个可交付特性用 `specs/<id>/spec.md`
- 再写可执行计划：把约束、落点目录、验收与质量门固化到 `plan.md`（尤其 Constitution Check / Project Structure），再拆 `tasks.md` 进入实现
- 最后同步三处“可交接事实源”：运行时规范（`docs/ssot/runtime/**`）→ 代码（`packages/*` / `examples/*`）→ 用户文档（`apps/docs/content/docs/*`）
...
- Runtime API/类型/语义：`packages/logix-core/src/*` + `docs/ssot/runtime/logix-core/api/*` + `docs/ssot/runtime/logix-core/runtime/*`
- logix-core 实现备忘录（实现细节/取舍/风险；非对外 API 契约）：`docs/ssot/runtime/logix-core/impl/README.md`
- React 适配：`packages/logix-react/*` + `docs/ssot/runtime/logix-react/01-react-integration.md`
- Devtools：`packages/logix-devtools-react/*` + `docs/ssot/runtime/logix-core/observability/09-debugging.md`
- Sandbox/Alignment Lab：`packages/logix-sandbox/*`（同时参考 docs/ssot/runtime 与 drafts 的 sandbox 主题）
