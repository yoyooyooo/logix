...
- `.codex/skills/project-guide/references/long-chain-index.md`：维护者/LLM 的长链路正交分解索引（A–K）与“分贝”指针。

## 检索与源码导航（可选）

- `.codex/skills/project-guide/references/auggie-playbook.md`：auggie 检索链路压缩包（smoke test + 黄金链路分步查询）。
- `.codex/skills/project-guide/references/codebase-playbook.md`：源码导航压缩包（入口 → 内核 → 回归）。
