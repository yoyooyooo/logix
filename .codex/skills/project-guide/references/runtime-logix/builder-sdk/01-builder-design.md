...
> **Note**: 本文基于 **Store / Logic / Flow** 三大运行时原语与 Effect 原生结构化控制流（含 `$.match`/`$.matchTag`）、`Effect.Service`、`Config` 等能力展开描述。Builder 被视为围绕这些原语的“工具链投影”，只负责**解析 / 生成**使用它们的代码本身，而不提供第二套运行时。

## 1. 核心定位：静态分析契约 (Static Analysis Contract)

在当前主线中，Builder SDK 被定义为一套 **DSL 语义标准**。
...
- **Unified Language**: Runtime 与 Builder 以同一套基于 Store / Logic / Flow 与结构化控制流（Effect.\* + `$.match`/`$.matchTag`）的 Effect-Native API 作为“语言”。Builder 只依赖这些 API 的类型与调用约定进行解析和代码生成，而不复刻一套独立 DSL。
...
- 对结构化控制流：使用 `$.match`/`$.matchTag`、`Effect.catch*`、`Effect.all` 明确表达分支、错误边界与并行结构；
...
export const runReliableSubmit = (input: {
  ...
  Effect.gen(function* (_) {
    ...
    // 也可以在调用侧由 Domain.logic 注入 state / flow / 结构化控制流等能力。
...
开发者使用 `Effect.gen` 配合 Runtime Core 提供的 Bound API（通过 `ModuleDef.logic(($)=>...)` 注入 `$`）编写逻辑：
...
```typescript
// 此处示意：`$` 由对应 ModuleDef.logic(($)=>...) 或调用方显式注入的 BoundApi<Shape, Env> 提供
...
2.  **Context Tracking**: 追踪 `Logic.Of` 生成的 Logic 程序，以及 Bound API `$`/相关 API 解构出的 `state/actions/flow/match` 等子域。
...
4.  **Fallback Strategy**:
    - **Fallback**：
    - 遇到无法识别的 Effect 组合 -> 标记为 `GrayBox` (Code Block)。
    - 遇到无法识别的语句 (如 `console.log`, `native fetch`) -> 归并为 `RawCodeNode`。
    - 遇到动态参数 (如 `$.match(someVar).with(...)`) -> 标记为 `GrayBox` (Ejected Node)。
