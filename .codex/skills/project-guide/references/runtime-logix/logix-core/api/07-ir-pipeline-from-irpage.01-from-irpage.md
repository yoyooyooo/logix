...
> 你在 `IrPage` 里看到的 JSON，99% 都来自 `@logix/core`；`@logix/sandbox` 负责把它“安全地跑起来并带回来”。
...
> **关键点**：`IrPage` 不是“从 sandbox 拿 IR”，而是“在 sandbox 里运行一段代码，而这段代码调用了 `@logix/core` 的 IR 提取/试跑能力”。
...
1. `createSandboxClient().init()`：创建 Worker，并把 `kernelUrl`（`@logix/core` bundle）与 `esbuild.wasm` 地址传给 Worker。
...
- **固定 runtime 入口**：`import * as Logix from "@logix/core"` 会被外部化为 `kernelUrl`（例如 `/sandbox/logix-core.js`）。
- **固定 effect 依赖**：`effect/*` 会被外部化为 sandbox 提供的 `effect.js` 子路径，避免版本漂移。
- **严格子路径策略**：禁止 `@logix/core/internal/*`；对 `@logix/core/<subpath>` 必须匹配 kernel 的 `logix-core.manifest.json`（否则直接编译失败）。
