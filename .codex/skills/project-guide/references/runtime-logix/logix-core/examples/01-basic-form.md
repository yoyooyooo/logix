...
> **Note**: 本示例是当前主线 Effect-Native Logix API 的 **黄金标准实现**。所有新业务逻辑都应以此为范本，它对齐 `@logix/core` 的 `Logix.ModuleShape` + Module-first + Bound API `$` + Fluent Intent（`$.on*().update/mutate/run*`）范式。
