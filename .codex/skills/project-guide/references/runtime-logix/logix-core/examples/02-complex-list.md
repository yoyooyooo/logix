...
> **Note**: 本示例基于当前主线 Effect-Native Logix 写法（`Logix.ModuleShape` + Bound API `$` + `Flow / 结构化控制流`）。实际代码应在对应 ModuleDef 上通过 `ModuleDef.logic(($)=>...)` 获取 `$`。
...
- **与 Core 一致**: 通过 `Logix.ModuleShape` + Bound API `$` + `Flow.Api` 表达行级联动、聚合与批量更新，直接对应运行时原语。
