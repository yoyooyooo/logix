# Matrix Examples: Collection & Batch (Standard Paradigm)

> **Focus**: 数组操作、行级联动、聚合计算、批量更新  
> **Note**: 本文示例展示了当前主线 Effect-Native 标准范式。所有集合操作都通过监听父级集合，并使用 `state.mutate` 进行原子化更新。实际代码应在对应 ModuleDef 上通过 `ModuleDef.logic(($)=>...)` 获取 `$`。

## S06 & S09: 行级联动与列表聚合 (Row Linkage & Aggregation)

**标准模式**: 监听整个 `items` 数组的变化。在 `flow.run` 中，使用一次 `state.mutate` 同时完成所有派生状态（行内 `total` 和整体 `summary`）的计算，以保证数据一致性并获得最佳性能。

```typescript
// 概念上，这里的 `$Cart` 表示针对 CartShape 预绑定的 Bound API，实际实现中通常由对应 ModuleDef 注入。
...
**标准模式**: 监听整个 `items` 数组。在 `flow.run` 中，遍历数组，识别出需要执行异步操作的项，并使用 `Effect.all` 并行处理它们。这需要开发者自行管理状态以避免重复请求。
...
**标准模式**: 监听触发全选的 `Action`。在 `flow.run` 中执行一次 `state.mutate`，即可原子化地遍历并更新所有行，实现批量更新效果。
