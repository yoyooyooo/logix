# Matrix Examples: Flow & Structured Control Flow (Standard Paradigm)

> **Focus**: 错误重试、竞态取消、错误回退  
> **Note**: 本文示例基于当前主线 Effect-Native 标准范式，使用 `Flow` API 和 `Effect` 组合子来声明式地处理复杂的异步控制流。实际代码应在对应 ModuleDef 上通过 `ModuleDef.logic(($)=>...)` 获取 `$`。

## S14: 失败重试 (Retry)

**标准模式**: 在传递给 `flow.run` 的 `Effect` 内部，使用 `Effect.retry` 组合子。
...
**标准模式**: 使用 `flow.runLatest` 代替 `flow.run`。`runLatest` 会自动取消前一个正在执行的 Effect，确保只有最新的流事件对应的逻辑在运行。
...
**标准模式**: 在 `Effect` 内部使用 `Effect.catchAll` 或 `Effect.try` 来处理错误分支。
