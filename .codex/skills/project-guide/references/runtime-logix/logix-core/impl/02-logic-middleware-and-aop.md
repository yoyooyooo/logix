...
- Runtime 在执行前**自动补齐**关键 meta：
  - `moduleId` / `instanceId` / `runtimeLabel` / `txnSeq` / `txnId` / `linkId` 等。
...
- `packages/logix-core/src/internal/runtime/core/ModuleRuntime.operation.ts`（`runOperation`：统一入口）
...
- `Runtime.make(root, { middleware })` → 注入 `EffectOpMiddlewareTag`（`root` 可为 program module 或其 `.impl`）
