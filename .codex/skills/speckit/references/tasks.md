...
如果本次目标 feature 是 **Spec Group**（其目录内存在 `spec-registry.md`，例如 `specs/046-core-ng-roadmap/`），则本阶段产出的 `tasks.md` 应当是**调度型 tasks**（index-only）：

- 任务粒度以“维护 registry / 生成执行清单 / 里程碑证据回写 / 创建或推进 member specs”为主。
- 严禁把 member spec 的实现 tasks 复制进来；只放**链接入口**（member 的 `tasks.md` / `quickstart.md` / 证据文件）。
- 如需生成/刷新 group 执行索引清单（`specs/<group>/checklists/*`），优先用：  
...
3. **Execute task generation workflow**:
   ...
   - Load spec.md and extract user stories with their priorities (P1, P2, P3, etc.) and traceability tags (NS-*/KF-*) if present
...
4. **Generate tasks.md**: Use `SKILL_DIR/assets/templates/tasks-template.md` as structure, fill with:
   ...
   - Outcome-level granularity for a high-intelligence implementation Agent: include result, file path, dependency, proof and writeback target; avoid pseudo-code or line-by-line algorithm steps unless an exact contract is already frozen in `implementation-details/*contract*.md`
