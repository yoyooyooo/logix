...
### VIII. CLI 作为薄控制面（Core-First / Scenario-Driven）

- `logix-cli` MUST 作为薄控制面：优先复用 `@logixjs/core` 的公开能力（IR/TrialRun/Reflection/Diagnostics），禁止在 CLI 层复制或分叉 core 语义实现。
- 新增/改造 CLI 能力 MUST 先定义对应业务场景与验收链路（至少一个真实场景或近真实样例），并以“可机器消费”的工件作为通过标准（而非仅人工日志）。
- CLI 命令设计 MUST 保持 primitive-first：先提供可组合的基础命令与结构化产物，再在外层提供可选编排命令；不得把 Agent 决策/策略硬编码进 CLI。
- 任何 CLI 扩展若触达 runtime 语义，MUST 同步验证 `core API → CLI 包装 → examples 场景` 三段一致性；若出现冲突，优先修正 core/规范，不做 CLI 兼容分支。

_Rationale_: CLI 的价值是给 Agent 与开发者提供“可验证武器库”，而不是第二套运行时或封闭工作流。
