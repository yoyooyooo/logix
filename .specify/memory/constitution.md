...
- Modified principles:
  - Core Principles:
    - 扩展「VII. 文档先行 & SSoT」：新增“退场即消失”约束，明确一旦某个 public surface 或 noun 被裁决退出当前形态，当前仓库的 live 区域不得继续保留其提及。
  - Spec-Driven 开发流程与质量门槛:
    - 扩展「Plan / Spec / Tasks 协同」：新增面向高智能 Agent 的规划粒度约束，默认冻结 owner、边界、质量门、proof 与回写，不强制伪代码级算法。
- Added sections: 无
- Removed sections: 无
- Templates:
  - ✅ .specify/templates/plan-template.md（已同步规划粒度与 discussion 可选结构）
  - ✅ .codex/skills/speckit/assets/templates/plan-template.md（已同步规划粒度）
  - ✅ .specify/templates/spec-template.md（已同步规划粒度与状态口径）
  - ✅ .codex/skills/speckit/assets/templates/spec-template.md（已同步规划粒度）
  - ✅ .specify/templates/tasks-template.md（已同步 outcome task 粒度）
  - ✅ .codex/skills/speckit/assets/templates/tasks-template.md（已同步 outcome task 粒度）
  - ✅ .specify/templates/checklist-template.md（已同步规划粒度检查项）
  - ✅ .codex/skills/speckit/assets/templates/checklist-template.md（已同步规划粒度检查项）
- Other sync:
  - ✅ AGENTS.md（已同步规划粒度原则）
  - ✅ specs/README.md（已同步规划粒度口径）
  - ✅ .codex/skills/speckit/references/plan.md（已同步执行阶段规划粒度）
  - ✅ .codex/skills/speckit/references/tasks.md（已同步任务粒度）
  - ✅ .codex/skills/speckit/references/specify.md（已同步 spec 质量检查）
  - ✅ docs/standards/logix-api-next-guardrails.md（无需改动，规划粒度口径不属于 API guardrails）
- Deferred TODOs: 无
-->
...
- `docs/specs` 是规范层单一事实源（SSoT），`apps/docs`
  ...
  `apps/docs`。
- 一旦某个 public surface、helper family、历史 noun 或旧 contract 被裁决退出当前形态，
  当前仓库的 live 区域 MUST 同步清掉它的提及；这条约束至少覆盖
  `docs/ssot/**`、`docs/standards/**`、package README、examples、public tests、
  public source 注释与导出面。需要保留历史轨迹时，只允许落到 proposal/review/archive
  一类明确的历史材料，不得继续混在当前 authority 或 live sample 中。
...
### VIII. 面向未来的完美收口与北极星可修订

- 当前阶段 MUST 以面向未来的最优设计为目标，在当下认知范围内追求极致收口；已有实现、既有 specs、
  现有 North Stars / Kill Features、命名 bucket 与阶段性路线图，都可被修订、替换或删除。
- North Stars / Kill Features / charters / guardrails / plans MUST 服务于更小、更一致、更可推导、
  更高性能、更可诊断的最终形态；当这些载体自身成为阻碍时，MUST 先更新事实源，再继续推进实现，
  禁止为迁就旧口径保留次优设计。
- 任何阻碍公开面压缩、相位收口、runtime clarity、performance 或 diagnostics 的残留，MUST
  进入“删除 / 腰斩 / 重写”优先队列；零存量用户阶段下，保留旧壳层需要额外证明。
- 实施与规划 MUST 采用单轨实施（single-track implementation）：
  - 直接朝最终形态 cutover；
  - 禁止为了执行组织引入兼容层、双轨运行、双写、影子路径、弃用期或“先并存后删除”的中间方案；
  - 若任务过大，只允许拆执行波次，不允许拆成语义并存的多轨方案。

_Rationale_: 当前仓库仍处在激进改造期。若把现有实现、现有 NS/KF 或既有治理文本视为不可调整边界，
只会把次优结构固化成新的包袱。治理文本必须服务终局设计，而不是反过来束缚收口。
...
- 规划默认面向高智能 Agent。`spec.md / plan.md / tasks.md` MUST 优先冻结 owner、边界、
  非目标、质量门、proof obligation、性能/内存/诊断约束、落点和回写路线；不默认展开为
  逐行伪代码、exact algorithm 或编码手册。
- 具体算法、数据结构和编码步骤默认由实施者在满足已冻结约束的前提下选择。只有当 DTO / wire shape、
  跨进程协议、生命周期所有权、跨 Agent handoff、持久化格式、安全规则或性能测量方法存在实质歧义时，
  才 SHOULD 新增 `specs/<###-feature>/implementation-details/*contract*.md` 冻结窄 contract。
- `implementation-details/*contract*.md` 不得变成第二事实源；一旦其中形成 owner law、公开边界、
  质量门或验收裁决，MUST 回写 `spec.md`、`plan.md`、`tasks.md` 或对应 SSoT。
- 对 cutover / convergence 类任务，`plan.md` MUST 明确：
  - 单轨实施路径
  - residue tombstone
  - 哪些旧入口 / 旧 contract / 旧命名必须被直接删除或降级
- `tasks.md` MUST
  将实现拆分为最小可独立交付的任务，按用户故事分组，确保每个故事可以
  被独立实现与测试。
- 对 SSoT 很多、authority 很分散的 cutover 类任务，允许把 `tasks.md` 降为薄索引与状态面；
  详细实施步骤 SHOULD 下沉到专门的 implementation plan，并继续保持单轨实施口径。
...
**Version**: 2.11.0 | **Ratified**: 2025-12-10 | **Last Amended**: 2026-05-04
