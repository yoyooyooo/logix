...
<!--
  Repo default:
  - Freeze owner / boundary / quality gates / proof / writeback.
  - Leave exact algorithms and data structures to implementation unless ambiguity
    would create incompatible contracts.
-->

- This plan targets a high-intelligence implementation Agent.
- This plan MUST freeze owner boundaries, non-goals, quality gates, proof obligations,
  performance / memory / diagnostics constraints, landing zones and writeback targets.
- This plan SHOULD NOT expand into pseudo-code, exact algorithms, or line-by-line
  coding recipes unless that is required to remove a concrete implementation ambiguity.
- Use `implementation-details/*contract*.md` only for narrow DTO / wire shape /
  lifecycle / handoff / persistence / security / measurement contracts that must be
  exact before implementation.
- Any owner-law or quality-gate decision discovered in an implementation-detail page
  MUST be written back into `spec.md`, this plan, `tasks.md`, or the relevant docs SSoT.
