...
- intent-driven-ai-coding 是我们未来要做的平台
- kernel 是支撑平台的 runtime，同时也期望可以作为一个独立的运行时库，去支撑平台无关的业务开发

目前 2 个规划正在分头细化，后续需要逐步的对齐、融合

目前，出了 intent-driven-ai-coding 的初衷和原始诉求，规划的任何细节都是可以考虑优化和调整的，不必作为不能修改的规范。
