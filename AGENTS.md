...
- 禁用否定前项加转折后项的固定句式，禁用破折号。
- 不吹捧，不写情绪价值开场白；先处理任务本身。
- 言简意赅。分歧时以逻辑、事实、可验证证据为准。
- 若缺少信息，只有在结果会被明显改变或继续执行会引入实质风险时才提问；其余情况按合理假设推进，并在汇报中说明假设。

## 文档同步门禁

- 涉及“统一口径”“系统性文档更新”或“相关文档一并调整”的任务，默认完成所有受影响文档的实际落盘、交叉引用回写和核对后再汇报。
- 只有存在明确且无法合理假设的决策阻塞，或继续落盘会引入口径错误时，才允许部分落盘。
- 部分落盘时，汇报必须列出已完成文件、待完成文件、阻塞原因和下一步需要用户确认的点。

## Subagent 使用策略

- 默认由主 agent 直接执行任务。
- 只有用户在当前请求中明确要求使用 subagent，或明确点名相关 skill 时，才启用 subagent 路径。
- 启用后由主 agent 负责消化输入、下发可执行 prompt、监控进度、做质量门和结果汇总。
- 有依赖关系时可串行或并行编排；未显式触发时继续由主 agent 直连执行。
- 若用户已显式要求但平台能力限制导致无法调用 subagent，主 agent 需说明原因并回退直接执行。

## 项目北极星

- AI Native first。
- 本仓核心目标是把 Logix 打造成 React 的逻辑层补充，支撑 Agent 更稳定地生成、理解、组合、调试和验证逻辑。
- 平台化暂不作为硬目标。若平台存在感与 API 清晰度、Agent 可用性、运行时性能、可诊断性冲突，优先后四者。
- 静态化只做必要部分，只服务类型边界、稳定诊断、最小 IR 和可复现验证。
- 拒绝为了平台叙事、表面完备度或未来假设去做过度静态化、过多锚点、额外壳层和第二模型。
- 公开 API 与内部结构优先追求更小、更一致、更可推导、更易被 LLM 生成和校验。

## 当前阶段

- 当前处于下一轮激进改造期，重点已经从提案讨论进入按新 SSoT 继续压缩公开面、清理旧壳层、准备推实现。
- 当前默认以面向未来的最优解为目标，在当下认知范围内追求极致收口。
- 最大北极星之外，已有实现、既有口径、已写 spec、局部北极星、charter、standards、命名 bucket，都可被挑战、修订、替换或删除。
- docs cutover 已完成，旧文档冻结在 `docs/archive/`，新裁决收口到 `docs/ssot/`、`docs/adr/`、`docs/standards/`。
- 当前允许文档领先代码。若旧实现、旧注释、旧目录结构与新 SSoT 冲突，优先以新事实源为准，不为迁就现状代码回退口径。
- 局部北极星、charter、standards、spec 与命名 bucket 只是当前裁决载体；一旦阻碍更小、更一致、更可推导、更高性能、更可诊断的方案，先改事实源，再继续推进实现。
- 当前默认继续清理旧命名、旧 facade、旧锚点化心智、旧平台优先残留；只保留能直接改善 Agent authoring、runtime clarity、performance 或 diagnostics 的部分。

## 裁决规则

- 当前阶段欢迎挑战既有实现、既有 spec、既有 SSoT、既有命名、既有目录结构和阶段性规划。
- 评审和规划必须主动寻找能支配当前方案的更小、更一致、更可验证候选。当前方案只有在经挑战后仍占优时才继续保留。
- 只要能让公开面更小、语义更纯、运行时更清晰、性能和诊断更强，推翻、压缩或升级已有设计就是可接受结果。
- 不得因为已经写过、已经实现、已经有文档而保守修补。若事实源与更优终局方案冲突，先修订事实源，再推进实现。
- 若需要在实现与文档之间裁决，默认优先推进更激进、更小、更一致、更终局的方案，再补验证。

## 规划粒度原则

- 规划默认面向高智能 Agent，不按低智能逐行脚本或伪代码手册编写。
- `spec.md / plan.md / tasks.md` 优先冻结 owner law、边界、非目标、质量门、proof obligation、性能/内存/诊断约束、落点与回写路线。
- 具体算法、数据结构、编码步骤默认留给实施者，只要实现满足已冻结的 owner、预算、生命周期、诊断和验证门。
- 只有当 DTO/wire shape、跨进程协议、生命周期所有权、跨 agent handoff、持久化格式、安全规则或性能测量方法存在实质歧义时，才新增 `implementation-details/*contract*.md` 冻结 exact contract。
- `implementation-details/*contract*.md` 只能消除窄边界歧义，不能变成第二事实源；一旦形成 owner 或质量门裁决，必须回写 `spec.md`、`plan.md`、`tasks.md` 或对应 SSoT。

## Forward-only 与零存量用户

- 当前仓库处于 forward-only 演进阶段，没有历史兼容负担。
- 当前按零存量用户前提推进，可以直接重做对外 API、协议与默认行为。
- forward-only 的对象也包括局部北极星、治理文本与阶段性路线图；它们一旦滞后于更优设计，先修订事实源，再继续推进实现。
- 不写兼容层，不设弃用期，不保留旧别名、旧协议、双入口、过渡壳层或双写期；必要说明只用迁移说明和决策日志承载。
...
- 保留最小 IR 与动态 trace，用于运行时解释、诊断、回放与证据对齐；禁止扩张成平台优先的第二真相源。
...
- 若某个能力会长出第二套 runtime、第二套事务语义或第二套调试事实源，默认拒绝。
...
- 测试文件名必须优先按最终行为、边界或合同命名，例如 `*.contract.test.ts`、`*.boundary.test.ts`、`*.guard.test.ts`。
...
- harness/proof/fixture/support 与 coverage inventory 的统一标准固定在 `docs/standards/harness-and-proof-assets-standard.md`；触及同一目录或证明链时，发现旧命名、旧注释或规划编号进入主阅读路径，默认顺手修正或记录阻塞。
...
- 任何新规划都必须包含最终读者路径检查：文件名、目录名、测试名、变量名、错误文案、trace 名、metric 名、docs 示例，都应优先表达最终运行时语义。
...
当前若出现旧代码与新文档冲突，先看上面这些新事实源，再决定实现如何继续收敛。
...
- 当前默认把本仓视为 AI Native 运行时实验场；平台化前置工程不构成当前定位。
...
- `Effect.Effect<A, E, R>` 的泛型顺序固定为成功值、业务错误、依赖环境。
...
- 若某个设计只是为了补齐未来可能存在的平台叙事，但当前不能直接改善 Agent authoring、runtime clarity、performance 或 diagnostics，默认先不做。
...
- 完成代码或文档改动前，运行能证明结果的最小验证；若无法验证，说明原因和下一步可行检查。
- 汇报时列出修改文件、关键改动、验证命令和结果。
- 遇到事实源冲突、权限风险、不可逆操作或无法合理假设的设计分歧时，停止并列出最小确认问题。
