...
function Page() {
  ...
  const child = useImportedModule(host, ChildModule)
...
- `parent` 必须是“有实例 scope 语义”的句柄（例如 `useModule(HostImpl, { key })` / `useModule(HostImpl)` / `useLocalModule(...)` 的返回值）；不要用 `useModule(HostModule)`（Tag 语义）作为 parent 再去解析 imports。
- `host.imports.get(ChildModule)` 返回稳定的 `ModuleRef`，可直接写在组件 render 内（无需 `useMemo`）。
