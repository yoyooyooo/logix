...
> 说明：本策略主要约束 Logix 提供的并行入口，例如状态和 action 的反应策略以及 TaskRunner 并行模式，不会自动改写你在业务代码里手写的并发（例如你自己写的 `Effect.all({ concurrency: "unbounded" })`）。
...
## 调度平面统一（queue / tick / concurrency）

从该版本开始，调度链路会把策略解析为同一份 scheduling surface，并在同一调度窗口内复用：

- 入口队列（入口背压）
- tick 调度器（tick 降级与恢复）
- 并发控制（并发上限与阈值）

这意味着你在诊断里看到的 `configScope` 与 `limit`，和实际调度行为是同源的，不再是“各模块各说各话”。

新增调度诊断：

- `scheduling::degrade`：本次 tick 发生降级（例如预算耗尽或检测到循环），会带上 `reason/backlogCount/configScope`。
- `scheduling::recover`：系统从降级状态恢复到稳定态，会带上 `fromTickSeq/fromReason` 便于对账。

诊断语义约束：事件必须对应真实行为（degrade/backlog/recover），不能出现“有事件无行为”或“有行为无事件”。
...
await Effect.runPromise(Logix.Runtime.setSchedulingPolicyOverride(runtime, "OrderForm", { concurrencyLimit: 4 }))
// 取消覆盖：传 undefined
await Effect.runPromise(Logix.Runtime.setSchedulingPolicyOverride(runtime, "OrderForm", undefined))
```
...
const overrides = Logix.Runtime.schedulingPolicyOverridesLayer({
  concurrencyLimit: 8,
  overridesByModuleId: {
    OrderForm: { concurrencyLimit: 4 },
  },
