...
The concurrency policy addresses two practical problems:
...
> Note: this policy primarily governs parallel entry points provided by Logix, such as state and action reaction policies plus TaskRunner parallel mode. It does not rewrite concurrency you manually create in business code (e.g. your own `Effect.all({ concurrency: "unbounded" })`).
