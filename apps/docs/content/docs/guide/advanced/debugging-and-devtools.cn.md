...
每条事件都会携带用于聚合与回放的“身份/事务”字段（按需出现）：

- `moduleId`：模块标识；
- `instanceId`：模块实例锚点（推荐使用它做聚合/关联）；
- `txnSeq` / `txnId`：同一实例内的事务序号与派生 ID；
- `runtimeLabel`：可选的运行时分组标签（例如 App 名称/场景标签）。

### 2.1.1 `state:update` 的性能字段（影响域与调度）

当你在 Devtools 或日志里看到 `state:update` 时，可以重点关注三类字段：

- `dirtySet`：本次提交影响到哪些字段（用于解释“为何增量/为何退化”）。
- `patchCount` / `patchesTruncated`：本次事务窗口内记录到的补丁数量，以及是否发生了“有界裁剪”。
- `commitMode`：本次提交属于哪种合并/调度模式（normal/batch/lowPriority）。
- `priority`：本次提交对 UI 的通知优先级（normal/low）。

它们的典型含义是：

- `commitMode = "batch"`：来自 `dispatchBatch([...])` 的显式批处理窗口，多次同步派发只形成一次可观察提交。
- `commitMode = "lowPriority"` + `priority = "low"`：来自 `dispatchLowPriority(action)`，不改变事务提交语义，只让 UI 通知更温和地合并（最终必达且有上界）。
- `dirtySet.rootIds`：影响域的“根集合”，以整数 id 表达（FieldPathId）。这是可 diff 的稳定锚点；只允许在展示/导出边界基于同一个 `staticIrDigest` 反解为可读 `rootPaths`。
- `dirtySet.rootIdsTruncated = true`：`rootIds` 在导出侧被裁剪为 TopK（默认 `light=3`、`full=32`），但 `rootCount/keyHash/keySize` 仍保持“完整 roots”的口径。
- `dirtySet.dirtyAll = true`：本次写入无法稳定追踪字段级影响域，派生/校验容易退化为全量；`dirtySet.reason` 会提示退化原因（例如 `unknownWrite` / `customMutation` / `nonTrackablePatch` / `fallbackPolicy`）。
- `patchesTruncated = true`：full 档位下补丁记录超过上限时会裁剪（默认最多 256 条），并通过 `patchesTruncatedReason="max_patches"` 给出可解释原因。

> 注意：这些字段会随 `diagnosticsLevel`（off/light/full）被裁剪；当你关闭诊断时，可能看不到它们。
...
需要特别注意一条边界：**DebugSink 收到的 `Event` 是“宿主内事件”，允许包含不可序列化字段**（例如 `action/state/cause` 的原始对象图）。

当你需要“可导出/可上传/可回放”的数据时，建议转换成 `RuntimeDebugEventRef` 或导出 `EvidencePackage`：

- `RuntimeDebugEventRef`：严格满足 JSON 可序列化（`meta` 是 JsonValue），并带有显式降级标记；
- `EvidencePackage`：从 DevtoolsHub 的窗口导出的证据包（可 `JSON.stringify`）。

> 实践建议：把事件写入日志/监控时，优先写入 `RuntimeDebugEventRef`，而不是直接透传 `Event`。

### 2.3 降级标记与错误摘要（避免导出崩溃）

当事件携带的原始信息不可序列化（循环引用、BigInt、函数等）或过大时，导出侧会执行“降级”：

- `downgrade.reason`：`non_serializable | oversized | unknown`
- `errorSummary`：错误原因的可序列化摘要（用于替代原始 `cause`）
...
### 2.5 Converge Performance Pane（收敛性能面板）

当你遇到“派生收敛（converge）”相关的性能/正确性回归时，可以在 DevTools 的 Timeline 区域切换到 **Converge** 视图做深挖。

#### 2.5.1 需要什么证据

- 需要存在 `kind="trait:converge"` 的证据事件；
- 当 `diagnosticsLevel="off"` 时不会产生该类事件；
- 在 `diagnosticsLevel="light"` 下会裁剪部分字段（例如 `dirty.rootCount/rootIds`），审计条目会以 `insufficient_evidence` 明确提示缺失字段。
- 关键字段速读：`configScope`（配置来源/作用域）、`reasons`（本次为何走 full/dirty）、`executionBudgetMs/decisionBudgetMs`（预算）、`thresholds.floorRatio`（auto 下界门槛，默认 1.05，与性能门禁同口径）。
...
- Lane：按 `moduleId + instanceId` 分组；
- Txn Bar：一笔事务（`txnSeq`），默认按实例内单调序号排序（时间戳仅用于展示）；
- 分段：决策（decision）与执行（execution）耗时（字段缺失时会显示为 `n/a`）；
- 详情面板：展示证据字段表、Audits（稳定 ID：`CNV-001..CNV-008`）与可复制代码片段。

#### 2.5.3 建议流程：止血 → 纳入边界地图 → 回收覆盖

1. **止血**：优先使用 Audits 给出的两档代码片段快速控制影响面：

- Provider override（优先级最高）：`Logix.Runtime.stateTransactionOverridesLayer(...)`
- Runtime module override（兜底止血）：`Logix.Runtime.setTraitConvergeOverride(runtime, moduleId, ...)`
- 覆盖优先级固定为：`provider > runtime_module > runtime_default > builtin`（可在证据字段 `configScope` 中直接看到本次生效来源）。

更完整、偏“配方化”的说明见：[收敛调度控制面](./converge-control-plane)。

2. **纳入边界地图**：把问题窗口导出成 `EvidencePackage`，在团队内共享复现；并把关键窗口固化到回归用例/基准测试中，防止再次漂移。

3. **回收覆盖**：根因修复后，移除止血 override，恢复到默认/auto 策略，让收敛决策重新覆盖真实线上路径。

### 2.6 Debug vs Effect.Logger：两条互补的通道
...
前面的内容都在讲 DebugSink 这条“调试事件管道”。Logix 还提供了一条更底层的 **运行时中间件总线**，基于 EffectOp 模型统一承载 Action / State / Service 等边界事件。
...
2. **在 Runtime 上一键启用 DevTools**
   ...
   - 默认 `diagnosticsLevel="light"`：只影响 DevtoolsHub / 证据包的“可导出事件”裁剪策略，不改变 DebugSink 的兜底日志语义。
