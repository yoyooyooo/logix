---
title: 字段收敛策略
description: 通过 Runtime/Provider/模块级覆盖，控制字段收敛策略（auto/full/dirty）与预算，支持止血回退与默认值调参。
---

字段收敛策略是一个运行时策略，主要承担两件事：

- 当某个页面或模块突然变慢、不稳定时，在局部范围内快速回退到更稳妥的模式。
- 在不改业务逻辑的前提下，逐步试探更合适的默认值，并且随时回滚。
...
### 2) 两个预算：`fieldConvergeBudgetMs` / `fieldConvergeDecisionBudgetMs`

你不需要知道具体怎么算，只要记住它们的“手感”：

- `fieldConvergeBudgetMs`：**这一笔交互最多愿意花多少时间做收敛**（越大越“肯算”，越小越“保响应”）。
- `fieldConvergeDecisionBudgetMs`：`auto` 在做“该不该局部重算”的判断时允许花的时间（越小越保守，越容易直接回退到 `"full"`）。

如果你完全不配置：运行时会使用默认值（目前 `fieldConvergeMode="auto"`、`fieldConvergeBudgetMs=200ms`、`fieldConvergeDecisionBudgetMs=0.5ms`）。

### 3) time-slicing：`fieldConvergeTimeSlicing`（显式 Opt-in）

当模块里 字段行为 数量很大（例如 1000+）且交互频繁时，你可能会遇到“每次输入都要检查/收敛一大堆派生”的硬上限。  
这时可以考虑 time-slicing。
...
- **把 字段行为 显式分两类**：`immediate`（必须同窗口收敛）与 `deferred`（允许短暂读到旧值）；
- **每次窗口只收敛 immediate**，把 deferred 的工作合并到后续窗口里补算（带上界，避免饿死）。

重要：**time-slicing 不会自动把任何字段行为变成 deferred**。只有你显式标了 `scheduling: "deferred"` 的 `computed` 才会被延后；没标的仍然按 immediate 处理。
...
- 字段行为/派生很多（比如 1000+）且交互高频，明显感觉“每次输入都在做很多不必要的派生计算”。
...
热切换只适合止血和排查。长期默认值应固化在 Runtime 或 Provider 配置中。
...
### 配方 F：如何把字段行为标成 deferred（必须显式声明）
...
const Fields = Module.logic("price-fields", ($) => {
  $.fields({
    ...
    // deferred：允许延后补算（需要先在 runtime 打开 fieldConvergeTimeSlicing）
...
1. 找到变慢的模块 `moduleId`；
2. 加一条 `fieldConvergeOverridesByModuleId[moduleId].fieldConvergeMode="full"`；
...
只改一个就够：把 `fieldConvergeMode` 固定为 `"full"`（全局或只对目标模块）。预算先别动。
...
### Q3：`fieldConvergeBudgetMs` 能不能随便调小？
