...
在大多数真实项目中，我们会把 “Module + 初始状态 + 逻辑” 封装成一个可复用的实现蓝图（ModuleImpl）：
...
后续无论是在 React 组件里，还是在测试里，你都可以拿着 `CounterImpl` 创建运行时实例。

## 5. 创建 Runtime，并在 React 中挂载

在应用入口，我们需要：

1. 基于根 ModuleImpl 构造一个 Runtime；
...
import { CounterImpl } from "./counter.impl"
