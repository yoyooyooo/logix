...
const ProfileLogic = Profile.logic(($) =>
  Effect.gen(function* () {
    ...
    yield* $.lifecycle.onDestroy(Effect.log('[Profile] module destroyed'))
  }),
)
```
...
const PollingLogic = PollingModule.logic(($) =>
  Effect.gen(function* () {
    // 平台挂起：暂停轮询
    yield* $.lifecycle.onSuspend($.state.update((s) => ({ ...s, paused: true })))

    // 平台恢复：恢复轮询
    yield* $.lifecycle.onResume($.state.update((s) => ({ ...s, paused: false })))
