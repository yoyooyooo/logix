...
This guide uses the minimal adapter provided by `@logix/i18n`.

## 1) Inject an external i18n instance

`@logix/i18n` defines a minimal `I18nDriver` shape and provides `I18n.layer(driver)` for injection:
...
External i18n instances may require async initialization. `@logix/i18n` provides two modes:
...
```ts
import { I18nModule } from "@logix/i18n"
