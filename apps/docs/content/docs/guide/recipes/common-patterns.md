...
const resetProvinceLogic = Effect.gen(function* (_) {
...
const validateUsernameLogic = Effect.gen(function* (_) {
...
```typescript
// 概念上，这里的 `$Form` 表示针对 FormShape 预绑定的 Bound API。
const validateDateRangeLogic = Effect.gen(function* (_) {
...
const calculateTotalsLogic = Effect.gen(function* (_) {
