...
1. 只从 `@logixjs/<pkg>` 或 `@logixjs/<pkg>/<Concept>` 导入；只使用文档明确允许的子路径入口（例如 `@logixjs/form/react`、`@logixjs/sandbox/vite`）。
2. 禁止导入 `@logixjs/*/internal/*`，也不要通过任何“未被明确允许的子路径”绕过 exports 收口（例如试图直接 import `dist/*` / `src/*` 等）。
...
- **Independent Entry Point**：独立子路径入口（例如 `@logixjs/form/react`、`@logixjs/sandbox/vite`）。
