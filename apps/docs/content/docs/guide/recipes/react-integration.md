...
- 了解 ModuleImpl / Runtime 的基础概念（可参考 Quick Start 与 Essentials）。
...
在你的应用入口，为这个 Module 构造一个 Root ModuleImpl，并通过 `Logix.Runtime.make` 组装 Runtime：
...
> 提示：`useLocalModule` 在内部使用一个“资源缓存 + Scope 管理”机制：
>
...
function CounterView() {
  ...
  const trend = useModule(CounterModule, (s) => s.trend)
  const dispatch = useDispatch(CounterModule)
...
```tsx
// CounterImpl 由 Module.make(...) 定义
import { useModule, useSelector, useDispatch } from '@logix/react'
import { CounterImpl } from '../runtime/counter'

export function LocalCounter() {
  // 每个组件实例各自持有一份 CounterImpl 的 ModuleRuntime
  const runtime = useModule(CounterImpl)
...
- `useModule(CounterImpl)` —— 传入的是 **ModuleImpl**；
- `useModule(CounterModule)` 或 `useModule(CounterImpl.module)` —— 传入的是 **Module Tag（ModuleInstance）**。
...
```ts
// runtime.ts
export const CounterImpl = CounterModule.implement({ initial: { count: 0 }, logics: [CounterLogic] })
export const AppRuntime = Logix.Runtime.make(CounterImpl, { layer: Layer.empty })
```
...
function CounterValue() {
  const count = useModule(CounterImpl.module, (s) => s.count)
  // ...
}
```
...
- `CounterImpl` 对应的 `ModuleRuntime` 由 `Runtime.make` 在应用级 Runtime 中统一创建和托管；
- `useModule(CounterImpl.module)` 只是把组件接到这份已经存在的全局 `ModuleRuntime` 上，不会再创建新的实例；
