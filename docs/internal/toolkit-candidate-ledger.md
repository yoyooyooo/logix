...
| --- | --- | --- | --- | --- |
| field error 读取 contract | `law-guard` | `closed-current-route` | `Form.Error.field` selector primitive 已属于 current route；完整 `useFormField` wrapper 不进入 core | toolkit 只能机械降解到 `useSelector(form, Form.Error.field(path))` |
| field-ui 叶子合同 | `law-guard` | `live-residue` | `ui` 叶子 contract 未冻结 | `touched / dirty` 仍只是观察值 |
| list row identity public contract | `law-guard` | `closed-current-route` | row identity current route is `fieldArray(...).byRowId(...)` plus row-scoped selectors; no public row token | `useFormList` wrapper 不进入 core；toolkit candidate must not own row identity |
...
| `useFormField` 完整 wrapper | `wrapper-family` | `rejected-for-core` | 混合 value/error/ui/event 四层语义；不得成为 core / domain route，只能作为 toolkit candidate 且必须机械降解 |
| `useFormList` 完整 binder | `wrapper-family` | `rejected-for-core` | 不得拥有 row identity 或 host truth；若进入 toolkit，必须直接组合 `fieldArray.byRowId` 与 row-scoped selectors |
