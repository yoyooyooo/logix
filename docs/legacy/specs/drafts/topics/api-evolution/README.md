---
title: API Evolution · Topic Overview
status: draft
version: 2025-12-03
value: vision
priority: later
related: []
---

# API Evolution · Topic Overview

本 Topic 专注于「下一代关键 API 形态」的探索，尤其是围绕 Bound API (`$`) 演进出的 Agent / AI / Flow 相关能力。

核心关注点：

- 以 `Agent.gen`、`$.whenAI` 等为代表的新 API 形态，如何在保持可解析性的前提下提升 DX；  
- 将 AI 调用从命令式 Effect 提升为声明式、一等公民的流式原语；  
- 为 v3/v4 以后的 Logix/平台 API 演进提供候选路径。

## 文档索引

- [next-gen-killer-apis.md](./next-gen-killer-apis.md) · Next-Gen Killer APIs（Agent.gen、Durable Flow、Invariants 等候选能力）  
- [when-ai-api.md](./when-ai-api.md) · `$.whenAI` 提案（将 AI 调用融入响应式体系的声明式 API）
