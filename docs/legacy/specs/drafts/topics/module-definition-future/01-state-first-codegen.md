...
- `02-reducer-redesign.md`：Module Action/Reducer 形态重构；
...
- 在 `docs/ssot/runtime` / `docs/ssot/platform` 中固化：
...
3. **与其他 Schema 能力（持久化/权限/埋点）的融合**  
   ...
   - 后续需要与 `topics/trait-system/01-current-coverage.md` 对齐，避免出现第二套“行为事实源”（以 007 为裁决）。
...
- `docs/ssot/platform/assets/00-assets-and-schemas.md` 中对 Module/Schema 的说明。
