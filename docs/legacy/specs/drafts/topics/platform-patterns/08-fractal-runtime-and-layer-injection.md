...
priority: next
related:
  - ./07-config-as-service.md
---

# Fractal Runtime and Layer Injection（分形运行时 / Layer 覆盖）

> 本文专注从 Runtime 视角梳理「分形运行时」与 Layer 注入/覆盖的语义。
...
const childRuntime = parentRuntime.fork({
  layer: Retry.layer({ defaultRetries: 5 }),
})
```
...
// main.ts
const runtime = Logix.Runtime.make(RootImpl, {
  layer: Layer.mergeAll(Retry.layer({ defaultRetries: 3 }), Router.layer({ type: 'history' })),
})
```
...
分形 Runtime + Layer 注入/覆盖提供了统一的能力注入模型，可以自然支持：
