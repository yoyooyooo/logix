...
- `docs/ssot/platform/`：**概念/模型/契约（最高优先级）**  
  - 三位一体（UI/Logic/Module）、资产与 Schema、最小系统方程（`C_T/Π/Δ⊕/tickSeq`）、RunResult/Trace/Tape/锚点等。
  - 入口：`docs/ssot/platform/foundation/01-the-one.md`（符号表）与 `docs/ssot/platform/contracts/00-execution-model.md`（执行模型）。
...
- 架构师（裁决者）：在上面基础上补 `docs/ssot/platform/contracts/00-execution-model.md` → `docs/ssot/platform/contracts/01-runresult-trace-tape.md` → `workbench/05-intent-pipeline.md`
- 开发（实施者）：`workbench/11-spec-to-code-mvp.md` → `workbench/01-module-traits-integration.md` → `docs/ssot/runtime/**`

## 冲突裁决（优先级）

1. `docs/ssot/platform/*`
2. `docs/ssot/runtime/**` 与 `packages/logix-*` 的真实类型/实现
