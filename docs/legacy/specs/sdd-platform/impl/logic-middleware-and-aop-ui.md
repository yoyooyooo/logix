...
本说明文档从平台视角整理当前主线中 Logic Middleware 的使用方式，以及 UI/出码器应如何与 runtime 规范对齐。

## 1. 核心信息来源

平台关于 Middleware/AOP 的信息主要来自三处：

1. **ModuleDef.meta.aop**（架构级配置）
   - 当前实现中 `ModuleDef` 没有一等字段 `middlewares`；平台侧可先将 AOP/审计/鉴权等“可视化配置”收敛到 `ModuleDef.meta.aop`（仅用于平台解析与出码，不参与运行时执行）；
   - 每个 Group/App 可在 `meta.aop.middlewares` 中声明一组默认中间件符号，例如 `LoggingMiddleware`、`AuthGuard` 等；
...
3. **Logic 中的调用模式**（实际使用）
   - 例如：
     ...
     yield * $.flow.fromAction('delete').pipe($.flow.run(secure(deleteEffect, { name: 'deleteUser' })))
     ```
...
- 在 ModuleDef 中：
  ...
  ```ts
  export const UserDef = Logix.Module.make("User", {
    state: UserState,
    actions: UserActions,
    meta: {
      aop: {
...
- 当前模块启用的能力（来自 `ModuleDef.meta.aop.middlewares` 的解析结果）。

UI 交互：

- 勾选/取消勾选某个能力 → 更新模块配置中的 `meta.aop.middlewares` 字段；
...
- 该逻辑实际使用的中间件列表：
  ...
  - 若无显式调用，则回退到模块级默认 Middleware（`ModuleDef.meta.aop.middlewares`）。
...
1. 读取某 Module 的 `meta.aop.middlewares` 配置，得到一个 Middleware 列表；
2. 对该模块内所有 Logic 定义：
...
如 Runtime 实现备忘中所述，当前阶段不计划在运行时自动从 `ModuleDef.meta.aop.middlewares` 注入 Middleware。
因此平台层在设计时要注意：

- **所有 AOP 配置最终都应体现在 TS 代码中**（即通过 Middleware 调用显式表达）；
- `ModuleDef.meta.aop.middlewares` 只是辅助平台在“模块层面”存储这些配置，Runtime 不基于它做任何隐式行为；
- 如果未来引入运行时 Middleware Registry，平台可以继续复用这些配置，但不需要修改既有已生成的代码。
...
- 推荐 Pattern：
  ...
  ```ts
  const secure = Logic.compose(LoggingMiddleware, AuthGuard /*, ... */)
  yield * source$.pipe($.flow.run(secure(effect, { name: 'xxx', storeId: 'StoreId' })))
  ```
