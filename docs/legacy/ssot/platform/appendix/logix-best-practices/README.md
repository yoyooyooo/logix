...
- **运行时语义 / 类型**：以 `docs/ssot/runtime/**` 与 `packages/logix-*` 的真实导出为准。
- **Pattern 资产形态**：以 `docs/ssot/platform/design/pattern-system.md` 为准（当前共识：Pattern 是普通 `(input) => Effect` 函数，平台只包 metadata）。
...
> 深入：`ModuleDef / Module / ModuleImpl / $` 的契约与差异见 `docs/ssot/runtime/logix-core/api/02-module-and-logic-api.md`。
...
> 规范入口（SSoT）：`docs/ssot/runtime/logix-core/api/02-module-and-logic-api.03-module-logic.md`（Phase Guard）。
...
> 参考：`docs/ssot/runtime/logix-core/guides/08-usage-guidelines.md`。
...
- 规范详解：`docs/ssot/runtime/logix-core/patterns/10-pattern-multi-instance.md`
...
- 规范入口：`docs/ssot/runtime/logix-react/01-react-integration.04-di-and-entry.md`
...
参照：`docs/ssot/platform/design/pattern-system.md`（第 5 节“工程约定”）。
