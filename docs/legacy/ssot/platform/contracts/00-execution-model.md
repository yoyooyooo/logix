...
- RunResult / Trace / Tape：`docs/ssot/platform/contracts/01-runresult-trace-tape.md`
- Tape 最小口径（program 侧）：`specs/075-workflow-codegen-ir/contracts/tape.md`
...
- `specs/075-workflow-codegen-ir/`：把 $\Pi$ 落成 `WorkflowDef → Workflow Static IR（Π slice）` 的闭环（对外 DX 入口为 `Workflow` 值对象；时间算子进入证据链）。
