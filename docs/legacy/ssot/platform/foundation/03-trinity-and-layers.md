...
interface LogicIntentNode {
  ...
  // 流程编排（实现载体在代码）
  // 当前主线中，这里不再是 JSON，而是指向源码的引用
  source: {
    file: string;
    exportName: string;
  };
  // 说明：源码是“可执行实现载体”，但不是概念/模型层的术语裁决点；
  // 概念与边界以 `foundation/02-glossary.md` 为准。
...
2.  **固态 (Solid)**：Dev (或 LLM) 将其转化为精确的组件树、Logic 程序（基于 Store / Logic / Flow / Effect 原生结构化控制流 / Effect.Service / Config）和 Schema。
