...
> 收敛说明：
> 当前实现里，`Logix.Module.make(...)` 返回 `ModuleDef`，装配期对象来自 `ModuleDef.implement(...)`。
> proposal 已收敛到未来公开主链 `Module / Logic / Program`：
> 未来公开 `Module` 承接当前 `ModuleDef` 的定义期角色，未来公开 `Program` 承接当前装配期对象。
...
// 1. 当前实现里先定义领域 ModuleDef（未来公开口径里，这层会统一叫 Module）
...
// 2. 在该定义期对象上编写 Logic 程序（使用 Bound API `$`）
...
// 3. 当前实现里可直接生成 Live Layer（未来公开口径还会补 Program 这层装配对象）
...
- [`api/02-module-and-logic-api.md`](api/02-module-and-logic-api.md)：Module / Logic / Program / `$`
