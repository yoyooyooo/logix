...
> 收敛说明：
> 当前实现里，`Logix.Module.make(...)` 返回的是 `ModuleDef`。
> 未来公开口径里，公开 `Module` 会直接承接这层“定义期锚点”角色。

在代码层，一个领域模块统一由 `Logix.Module.make` 定义，并返回当前实现中的 `ModuleDef`：
...
- `CounterDef`（当前实现中的 `ModuleDef`）本身同时是：
