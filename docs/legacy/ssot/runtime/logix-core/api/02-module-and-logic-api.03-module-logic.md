...
> 收敛说明：
> 当前实现主要仍表现为 `ModuleDef.logic(($)=>...)`。
> 未来公开口径里，公开 `Module` 会直接承接这层定义期对象，因此这里统一按 `Module.logic` 叙述。

## 3.1 基本形态

当前实现里的 `ModuleDef.logic`，以及带 `.impl` 的 `Module.logic`，本质上都在表达“在该领域上挂载一段 Logic 程序”的入口。
...
- **O-007 canonical 执行模型**：`ModuleRuntime` 采用“先 normalize，再统一执行”的单一管线：
  1) raw logic（单相 / LogicPlan / LogicPlanEffect）先归一为 canonical `LogicPlan`；
  2) 统一执行 `setup`（同步注册）；
  3) 在 init gate 后统一 fork `run`。
  兼容分支已收敛：不再支持“单相 logic 在 run 成功后再二次解释返回值为 plan”的隐式行为；如需“effect 先产出 plan”，必须显式使用 LogicPlanEffect 语义。
