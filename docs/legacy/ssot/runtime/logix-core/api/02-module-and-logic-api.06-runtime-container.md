...
> 收敛说明：
> 当前实现里，Root 运行入口常由 `ModuleDef.implement(...)` 产出的装配对象提供。
> 未来公开口径里，这一层会被显式命名为 `Program`，并作为 `Runtime.make(...)` 的唯一公开输入。
> 公开装配入口固定为 `Program.make(Module, config)`，不新增 `Module.program(config)` 一类 sugar。
...
- 运行时层：
  ...
  - 在分形 Runtime 模型下：
    - 推荐通过 `Logix.Runtime.make(root, { layer, onError })` 以某个 Root program object 为入口构造一颗 Runtime（App / Page / Feature 均视为“Root + Runtime”）；
...
对日常业务开发而言，只需通过 Module / Logic / Program / Runtime / `$` 这条主链进行思考与编码。
