...
- `yield* $.use(module)`：取其他 Module 的只读句柄（等价 `yield* $.use(module.tag)`）。
  - `module` 在当前实现里可以是 `ModuleDef`（`Logix.Module.make(...)` 返回）或带 `.impl` 的 `Module`。
  - 未来公开口径里，定义期对象统一收敛为 `Module`，装配期对象统一收敛为 `Program`。
...
- 当前实现中，主要仍在 `ModuleDef.logic(($)=>...)` 注入的 `$` 上使用；
- 未来公开口径里，对应写法统一表述为 `Module.logic(($)=>...)`；
...
- `$` 必须是 **Logic 文件顶层绑定的常量**（当前实现通常来自 `ModuleDef.logic(($) => ...)` 的参数）；
- 不允许对 `$` 重新赋值；
