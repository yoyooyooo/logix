...
> 收敛说明：
> 当前实现里，定义期对象仍主要表现为 `ModuleDef`。
> 未来公开主链会统一收敛成 `Module / Logic / Program`，因此本节按未来公开口径叙述，并在必要处提示当前实现名。

为了帮助使用者快速建立整体心智，可以将 Logix 的“主链路”理解为：

> **定义 Module → 编写 Logic（当前实现多为 `ModuleDef.logic(($)=>...)`）→ 在其他 Logic 中通过 `$.use(Module)` 协作 → 平台从 Fluent 链生成 IntentRule。**
