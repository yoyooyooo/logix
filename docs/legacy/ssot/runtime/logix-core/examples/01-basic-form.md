# Example: Basic Form Logic (Gold Standard)

> **Scenario**: 用户注册表单
> **Features**: 字段联动、异步校验、多字段约束
> **Note**: 本示例是当前主线 Effect-Native Logix API 的 **黄金标准实现**。所有新业务逻辑都应以此为范本，它对齐 `@logixjs/core` 的 `Logix.ModuleShape` + Module-first + Bound API `$` + Fluent Intent（`$.on*().update/mutate/run*`）范式。

## Schema Definition

```typescript
import { Schema, Context, Effect } from 'effect';
import { Store, Logic, Logix } from '~/logix-core'; // 概念性路径
...
## Logic Implementation (Effect-Native)
...
```typescript
export const RegisterLive = RegisterDef.live(
  {
    username: "",
    password: "",
    confirmPassword: "",
