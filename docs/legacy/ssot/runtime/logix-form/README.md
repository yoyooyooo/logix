...
当前实现仍暴露 `derived / rules / traits` 三套入口。

下一代 canonical authoring 已在 proposal 中收敛为：

- 顶层只保留统一的 `logic` 家族
- `derived` 退出顶层 authoring 入口
- `rules` 退为校验子族或迁移壳层
- `traits` 继续停留在 expert / internal 层

在实现落地前，本文件仍保留当前 API 事实描述；涉及未来 authoring 方向，以 proposal 为准：

- `docs/proposals/logix-api-next/13-form-authoring-after-derived-exit.md`
