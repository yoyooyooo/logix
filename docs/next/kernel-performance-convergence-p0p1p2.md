...
These bridge raw local reports and sentinel snapshots into the final convergence manifest. They do not create performance proof by themselves. Local agents should not reimplement the stage plan from `tasks.md`; they should apply the patch, run the focused tests, collect default/soak or adversarial-default/soak evidence, assemble the manifest, and run the final gate.
