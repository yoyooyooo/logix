...
## Final Form Single-Track Cutover Note (2026-05-11)

Form now treats the API from `docs/ssot/form/13-exact-surface-contract.md` as accepted single-track authority. Historical public helpers and compatibility-shaped source files are no longer watch-only; they must be absent from live source/package exports:
...
Internal path/schema lowering may still exist under `packages/logix-form/src/internal/**`. It is not a public API route.
...
| `Form.Companion.field(path)` | `frozen` | `implemented` | `partial` | `proven` | `none` | returned-carrier path is canonical teaching route; imperative `void` callback remains runtime-valid with honest `unknown` selector result and no compatibility accumulator | `watch-only` |
| `Form.Companion.byRowId(listPath, rowId, fieldPath)` | `frozen` | `implemented` | `partial` | `proven` | `none` | returned-carrier path is canonical row companion inference route; imperative `void` callback remains runtime-valid with honest `unknown` selector result and no compatibility accumulator | `watch-only` |
