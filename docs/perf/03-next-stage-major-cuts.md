...
- [x] D-3：no-trackBy list RowId gate（`validate/source` 窗口内跳过重复 reconcile）。
...
- `docs/perf/archive/2026-03/2026-03-06-s11-post-s10-blocker-probe.md`：post-S10 real probe；remaining browser blocker queue 已清空。
...
- `externalStore`：已由 `U-1` 收口。
  - `2026-03-14` 前置失败试探仍保留为负样本：
    - `docs/perf/archive/2026-03/2026-03-14-d4-external-store-raw-direct-failed.md`
    - `docs/perf/archive/2026-03/2026-03-14-t1-txn-phase-gate-failed.md`
...
证据与实现记录：
- `docs/perf/archive/2026-03/2026-03-05-d2-dirtyevidence-snapshot.md`

### D-3（已做）：no-trackBy list RowId gate（仅 validate/source 窗口）

目标：
- 把 no-trackBy row edit 在 `validate` / `source.refresh` 里的重复 `ensureList(...)` 去掉；
- 不改变 D-1 的 commit-time `rowIdStore.updateAll(...)` 触发条件。

核心做法：
1. 在 `RowIdStore` 增加 `canSkipNoTrackByListReconcile(...)`；
2. `validate.impl.ts` 的 root/nested list no-trackBy 路径先问 gate，再决定是否 `ensureList(...)`；
3. `source.impl.ts` 的 list.item refresh 先问 gate；若跳过，则按 index 读取既有 rowId。

使用条件：
- 有 `trackBy` 时一律不跳过；
- `items` 引用未变时直接跳过；
- txn dirty evidence 已明确“结构没动”时跳过；
- 没有 txn 证据时，只做 reference-level 的保守复用。
...
证据与实现记录：
- `docs/perf/archive/2026-03/2026-03-05-e1-mutative-index-evidence.md`
