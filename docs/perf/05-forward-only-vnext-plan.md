...
- [ ] R-1：`txnLanes` backlog policy split（区分 backlog 启动期与 steady-state 的 urgent 调度策略，继续打 `urgent.p95<=50ms` 硬门）。
  ...
  - `2026-03-06` handoff-lite checkpoint：`docs/perf/2026-03-06-r1-txn-lanes-handoff-lite-failed.md` 记录了 queue-snapshot + handoff-lite 版本的 3/3 quick audit 回归。
- [ ] R-1：`txnLanes` backlog policy split（区分 backlog 启动期与 steady-state 的 urgent 调度策略，继续打 `urgent.p95<=50ms` 硬门；`2026-03-06` 已新增 handoff-lite 失败证据，见 `docs/perf/2026-03-06-r1-txn-lanes-handoff-lite-failed.md`）。
...
- 当前仍优先沿 `txnQueue snapshot -> urgent-aware handoff` 收口；blind first-host-yield、显式 startup-phase 与 handoff-lite 版本都已保留为失败证据，不直接落 runtime。
- 当前暂不需要动表面 API。
