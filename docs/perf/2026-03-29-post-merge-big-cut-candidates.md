...
- `selector_snapshot_mirror_plane`
  ...
  - strengthened probe 已证明 React + Flow 平面没有出现消费面放大；Module-as-Source probe 给出 `1 / 8 / 32` 线性信号；non-React plane dedupe PoC 已把它压到 `1 / 1 / 1`
  - reading:
...
- current continuation rule:
  - React + Flow 平面已判 flat；当前 non-React plane PoC 已正向，下一步改为 focused local 补 declarative-link fanout probe
