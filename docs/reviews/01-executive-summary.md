...
3. **实现与 specs 漂移**：`.codex/skills/project-guide/references/runtime-logix/logix-core/impl/06-package-structure.md` 描述的 `src/api|dsl|runtime` 与当前 `packages/logix-core/src/*.ts + src/internal/**` 结构不一致，SSoT 无法作为可依赖的真相源；必须立刻收敛（要么改文档，要么改代码结构）。
