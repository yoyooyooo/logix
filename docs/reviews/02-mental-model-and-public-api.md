...
`docs/specs/runtime-logix/impl/package-structure.md` 仍描述 `src/api|dsl|runtime` 的分层，但当前实际实现以 `src/*.ts` + `src/internal/**` 为核心，且大量关键逻辑直接落在 `internal/runtime/*`。
...
- Flow API 风格（与 Fluent 功能重叠）：`flow.fromState/fromAction` + `flow.run*`  
  - `docs/specs/runtime-logix/core/examples/05-matrix-basic.md` 明确以 `flow.fromState(...).pipe(flow.run(state.mutate(...)))` 作为“v3 标准模式”
...
两套写法在 `docs/specs/runtime-logix/core/03-logic-and-flow.md` 中同时出现，并明确写了“等价”。
...
- `$.use(Module)`：返回 `ModuleHandle`，再用 `$.on($Handle.changes(...))` 拼装  
  ...
  - `examples/logix/src/scenarios/coordinated-search-detail.ts`（CoordinatorModule 里 use 两个模块做联动）
...
- specs/文档中大量出现 `.then(...)` 链式写法（例如 `docs/specs/intent-driven-ai-coding/v3/03-module-assets.md`、部分 scenario 注释），但当前 `IntentBuilder` 实际暴露的是 `.run/.update/.mutate/...` 与 `.andThen`（并且 `andThen` 还带 arity 隐式分派）。
...
`packages/logix-core/src/Logic.ts` 导出 `RuntimeTag = Context.GenericTag("@logix/Runtime")`，而当前 `Module.logic`/`ModuleRuntime.make` 的装配主要提供的是“模块自身的 Tag（ModuleInstance）”，并未看到对 `@logix/Runtime` 的注入。
