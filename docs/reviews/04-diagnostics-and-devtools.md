...
- `moduleId/runtimeId/runtimeLabel`
- `txnId`（若存在活跃事务）
...
`packages/logix-core/src/internal/runtime/core/DebugSink.ts` 里还存在 `lastTxnIdByRuntime` 的兜底逻辑：用于在某些 trace 事件缺失 txnId 时，为其补齐最近一次 `state:update` 的 txnId，帮助 Devtools 对齐“事务 → 渲染”。
...
- Instance：`runtimeId`（当前实现为随机 id）
- Transaction：`txnId`（时间戳+随机）
- EffectOp：`linkId`（跨模块共享 FiberRef）
- Trait：`stepId` / `traitNodeId`（Graph/Plan 侧）
- Debug 事件自身：`eventId`（当前实现为进程内自增序号，非稳定）
- EffectOp：`op.id`（当前默认 `Math.random()`，非稳定）

更重要的是：这些 id 不仅“不稳定”，还会直接影响**性能分析、回放对齐、冲突合并**三件事：

- `runtimeId/txnId/op.id/linkId` 大量依赖 `Math.random()/Date.now()`，难以在“同一份 IR”里确定性重建；
- Devtools 侧只能做“最佳努力的对齐”（例如用 `lastTxnIdByRuntime` 推断 react-render 属于哪个 txn），而不是强协议；
- 在 Sandbox/跨进程桥接时，`Math.random()` 产生的 id 既不可比对，也不便于对齐“同一份代码生成的同一条链路”。
...
- `ModuleRuntime.make` 不再随机生成 `runtimeId`，改为：`instanceId` 由外部装配显式传入（React local key / sandbox runId / host instance key），并作为 Debug/Devtools 的唯一实例键；  
- `StateTransaction` 用“单调递增序号”生成 `txnId`（例如 `${instanceId}::${txnSeq}`），时间戳只作为 `startedAt` 字段；  
- `EffectOp.make` 默认 id 不再 `Math.random()`，而是 `linkId + opSeq` 或 `txnId + opSeq`（保证顺序与可对齐）。
...
- `Middleware.makeDebugObserver` 当前会把 **完整的 EffectOp 对象**直接塞进 `Debug.record({ type:"trace:effectop", data: op })`；
- EffectOp 对象里包含 `effect: Effect.Effect<...>`（闭包/引用链），这类对象：
  - 既不可跨进程序列化；
  - 也会在 DevtoolsHub ring buffer 中形成高保留引用（延长 GC 生命周期、放大内存占用）；
  - 更可能把“用户程序捕获的对象图”偷偷挂在 Devtools 上，造成隐性泄漏与性能抖动。

建议（不兼容）：`trace:effectop` 只允许上报 **SlimOp**（结构化元信息 + start/end/status + 关键 meta），严禁携带 `effect` 本体；需要更丰富观测时，用专门的“诊断级别开关”显式开启，并在 Hub 侧做 ring buffer + 裁剪/脱敏。
...
- 时间旅行能力：`packages/logix-devtools-react/src/state/logic.ts`
  ...
  - 其中 `instanceId` 实际上就是 core 的 `runtimeId`（`ModuleRuntime.ts` 内部 map 的 key 形如 `${moduleId}::${runtimeId}`）。
