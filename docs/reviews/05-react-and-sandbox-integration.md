...
### D) `runtime.id` 目前仍是随机值：不利于稳定对齐与 time-travel

React/Devtools 已经把 `runtime.id` 当作“实例标识”使用：

- trace 事件用它来分组（`trace:react-render`/`trace:react-selector`/`trace:instanceLabel`）；  
- time-travel 入口 `Logix.Runtime.applyTransactionSnapshot(moduleId, instanceId, txnId, ...)` 里，`instanceId` 实际就是 `runtime.id`。

但 core 的 `ModuleRuntime` 当前仍用 `Math.random()` 生成 `id`（见 `packages/logix-core/src/internal/runtime/ModuleRuntime.ts`），导致：

- 无法在“同一份 IR / 同一份用例”下稳定复现实例键；  
- 影响跨进程（Sandbox）与回放对齐；  
- 让 Devtools 只能依赖 `trace:instanceLabel` 做展示层补丁，而不是引擎层的稳定标识。

建议（不兼容）：允许 React 在构造模块实例时把 “key/label” 下沉为 core 的 `instanceId`（即 `runtime.id` 的稳定来源），并把这件事写进 Runtime Identity Model（详见 `04-diagnostics-and-devtools.md`）。
...
- 观测：
  ...
  - Effect Logger → `LOG`；
  - 简化 span → `TRACE`；
  - `self.__logixSandboxUiIntent(packet)` → `UI_INTENT` + `TRACE`；
...
编译阶段把 `effect` external 到 `https://esm.sh/effect@3.19.8/*`，运行阶段 worker 又单独 `import("https://esm.sh/effect@3.19.8")`。在 Alignment Lab 场景，任何“多份 Effect 实例导致 Tag/类型身份不一致”的问题都会被放大（尤其是 Context.Tag 体系与层级注入）。
