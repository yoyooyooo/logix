...
- `Module`：`packages/logix-core/src/Module.ts`
  ...
  - `ModuleInstance.logic(($)=>...)`：在 Logic Effect 内 `yield* moduleTag` 获取 runtime，再 `BoundApiRuntime.make(...)` 构造 `$`
...
- `StateTrait`：`packages/logix-core/src/state-trait.ts`
  - DSL：`computed/source/link/node/list/from`
  - 编译：`internal/state-trait/build.ts`
  - 安装：`internal/state-trait/install.ts`
- `TraitLifecycle`：`packages/logix-core/src/trait-lifecycle.ts`（Phase 2 占位）
...
- `internal/runtime/core/StateTransaction.ts`
  - `beginTransaction(...)` 生成 `txnId = startedAt + Math.random()`（当前不稳定）
...
- 公共 DSL：`packages/logix-core/src/state-trait.ts`
...
- Debug 事件模型：`packages/logix-core/src/internal/runtime/core/DebugSink.ts`
  - `Debug.currentTxnId` FiberRef、`lastTxnIdByRuntime` 兜底对齐 React trace
...
- EffectOp 总线：
  ...
  - `linkId` 生成：`Date.now() + Math.random()`（当前不稳定）
...
7. runtime/txn/effectop 的 id 生成普遍依赖 `Math.random()`/`Date.now()`（不利于可复放/可 diff 的稳定标识）
