...
证据入口：`packages/logix-core/src/Module.ts`、`packages/logix-core/src/state-trait.ts`。
...
- `__logixSandboxUiIntent` / `__logixSandboxSpy` 这两个“未来双工协议”的桥接钩子。
...
- 约束 A（旧）：`...then(...)` / `.when(...)`
  - `docs/specs/intent-driven-ai-coding/v3/06-codegen-and-parser.md`
  - `docs/specs/intent-driven-ai-coding/v3/platform/impl/README.md`
  - `scripts/intent-fluent-parser.ts`
- 约束 B（现行实现）：`...update/mutate/run*(...)`
  - `docs/specs/runtime-logix/core/03-logic-and-flow.md`（Hard Constraints 明确写 `.update/mutate/run*`）
...
目前 `docs/specs/runtime-logix/core/03-logic-and-flow.md` 已经把 `andThen` 定位为非白盒子集（Gray/Black Box），但它仍然属于 `IntentBuilder` 的公开接口：一旦业务使用，就会把平台的全双工能力打回黑盒。
...
- **建议选择现实实现路线作为唯一白盒锚点**：
  ...
  - 与 `docs/specs/runtime-logix/core/03-logic-and-flow.md` 的 Hard Constraints 一致；
  - 终端算子 `.run* / .update / .mutate` 语义更显式（比 `.then` 更利于诊断与平台推断）。

随之必须执行的“不兼容清理”：

- 更新/重写 `docs/specs/intent-driven-ai-coding/v3/06-codegen-and-parser.md` 与 `docs/specs/intent-driven-ai-coding/v3/platform/impl/README.md`：移除 `when/then` 约束，统一为 `.run*/update/mutate`；
...
参考入口：`docs/specs/drafts/topics/sdd-platform/06-dev-server-and-digital-twin.md`、`docs/specs/intent-driven-ai-coding/v3/platform/impl/README.md`（需先修正其中的 then/when 漂移）。
