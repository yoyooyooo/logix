...
本文件的目标不是把 `docs/philosophy/*` “升级为规格”，而是把它作为 **SSoT 之上的架构设计原则层**，与 `docs/reviews/*` 的实现评估结论做一次 **双向拉齐**：
...
2. **Philosophy（原则层，裁决 Why/Trade-off）**：`docs/philosophy/**` 给出价值观、不变量与“取舍方向”，不直接规定 API 细节。
3. **Reviews（证据层，裁决 Now/Gap/Next）**：`docs/reviews/**` 以代码为证据，指出当前偏离点与不兼容改造顺序。
...
- 事务与 trait 增量优化：`docs/reviews/03-transactions-and-traits.md`
- 诊断与回放：`docs/reviews/04-diagnostics-and-devtools.md`
- 逃逸通道破坏确定性与回放：`docs/reviews/02-mental-model-and-public-api.md`（`state.ref()` 可写）、`docs/reviews/06-evidence-map.md`
...
- 平台全双工与最小 IR：`docs/reviews/07-platform-full-duplex-and-alignment-lab.md`
- “统一最小 IR”作为路线图硬门槛：`docs/reviews/99-roadmap-and-breaking-changes.md`
...
- React 1+1>2 的关键前提（事务边界、订阅收敛、严格多实例语义）：`docs/reviews/05-react-and-sandbox-integration.md`、`docs/reviews/07-phase3-react-1p1gt2.md`
- Sandbox 的 UI_INTENT/回放闭环：`docs/reviews/07-platform-full-duplex-and-alignment-lab.md`
...
- White/Gray/Black 的降级策略与锚点系统：`docs/reviews/07-platform-full-duplex-and-alignment-lab.md`
- “歧义 API 会破坏全双工”→ 必须删 `andThen`：`docs/reviews/07-platform-full-duplex-and-alignment-lab.md`
...
- 现状最大阻塞：锚点写法在 docs/scripts 与实现之间漂移：`docs/reviews/07-platform-full-duplex-and-alignment-lab.md`
...
- “阻断漂移”被列为 Phase 0：`docs/reviews/99-roadmap-and-breaking-changes.md`
- 漂移已发生且阻塞平台化：`docs/reviews/07-platform-full-duplex-and-alignment-lab.md`
...
- 多实例隔离与作用域语义：`docs/reviews/03-transactions-and-traits.md`、`docs/reviews/05-react-and-sandbox-integration.md`
- Tag/Identity 的一致性与稳定性：`docs/reviews/04-diagnostics-and-devtools.md`、`docs/reviews/99-roadmap-and-breaking-changes.md`
...
1. `docs/philosophy/05-architecture-as-view.md` 示例与表述已统一为 “Fluent 规则链 + 显式终端 sink（`.run/.update/.mutate`）”。
2. `docs/philosophy/05-architecture-as-view.md` 的 Black Box 已明确为“降级为代码块节点（保留 loc/Module/Logic/最小诊断锚点）”，而不是 Invisible。
3. philosophy/reviews 的入口互链已补齐：`docs/philosophy/README.md` ↔ `docs/reviews/README.md`，并以 `docs/reviews/08-philosophy-alignment.md` 作为长期维护的对照总表。
...
- 每次 specs 发生重大演进，应跑一次全库 drift 扫描：docs/specs + docs/philosophy + scripts + examples 必须同步收敛，否则平台全双工会被最薄弱一环拖死。
