...
- Identity 去随机化：`runtimeId/txnId/opId/linkId` 改为“可注入 instanceId + 单调序号”，禁止 `Math.random()` 作为默认标识来源（详见 `04-diagnostics-and-devtools.md`）。
...
- （008 收口）全仓统一 root/global 的示例写法为 `Logix.Root.resolve(Tag)`（避免“Root 未导入/歧义符号”导致的二次心智噪音），并确保 React 错误 fix 文案与 `docs/specs/runtime-logix/*`、`apps/docs/*` 的推荐写法一致。
...
- （已完成：008）跨模块协作入口收敛：默认 strict（`$.use`），显式 root/global 用 `Root.resolve`；跨模块/IR 承载使用 `Link.make`，删除 `$.useRemote`（证据：`specs/008-hierarchical-injector/tasks.md` / `specs/008-hierarchical-injector/review.md`）。
