...
| :---------- | :----------------------- | :-------------------------- | :----------------------------------------------------- |
| **Domain**  | **Module** (Data/Schema) | `ModuleImpl` / `Store`      | [`Module.ts`](../../packages/logix-core/src/Module.ts) |
...
- **Spec SSoT**: `docs/specs/intent-driven-ai-coding/v3` (意图模型事实源)
- **Runtime SSoT**: `docs/specs/runtime-logix` (运行时设计事实源)
...
- **Execution Engine**: [`packages/logix-core/src/internal/runtime/EffectOpCore.ts`](../../packages/logix-core/src/internal/runtime/EffectOpCore.ts)
