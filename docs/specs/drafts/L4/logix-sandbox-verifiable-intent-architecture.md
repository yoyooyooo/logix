...
- `docs/specs/intent-driven-ai-coding/v3/platform/impl/code-runner-and-sandbox.md`（平台层实现规划）；
- `docs/specs/runtime-logix/core/*` 与相关 impl 文档（前端 Runtime 契约与 Platform 集成）；
...
上述结构暂不对具体 API 形态（函数签名、类型参数）做最终结论，但可作为后续在 `docs/specs/runtime-logix` 与 `apps/docs` 中设计用户级 API 时的参考。
...
2. 与 runtime-logix：
   ...
   - 需要在 `runtime-logix/core` 或 impl 文档中明确：
...
3. 与 drafts topics 的对接：
   - 风险与观测相关部分可后续收敛到 `runtime-readiness` 与 `runtime-observability` topics；
