...
version: 0.2.0
supersedes:
  - logix-sugar-possibilities（已删除，历史版本见 Git）
  - logic-for-shape-env-first-roadmap（已删除，历史版本见 Git）
related: []
---
...
- 真正“把蓝图变成代码/行为”的过程，可以发生在两处：
  ...
  - Build-Time：本地 generator（CLI / Vite 插件）扫描同一份元数据，生成普通 TS 代码和 `.d.ts`（例如 `topics/module-definition-future/01-state-first-codegen.md` 提出的 State-First Module Codegen）。
...
- [ ] Sugar 与 Trait/事务/回放/诊断口径如何对齐（见 `topics/trait-system/01-current-coverage.md`）？
