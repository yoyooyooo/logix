...
> 草稿定位：L6（问题拆解为主，结论弱），聚焦于「如何在不破坏 Effect / React 不变量的前提下，为 `@logix/react` 提供**真正安全的 runtime 级 React 配置覆盖能力**」，是后续对 `ReactModuleConfig` / `ReactRuntimeConfigTag` / `RuntimeProvider` 组合方案的前置分析。
...
5) 文档与 API 一致性  
   - 本稿与 `docs/specs/runtime-logix/react/README.md`、`apps/docs` 中的 React 集成章节在以下方面保持一致：  
