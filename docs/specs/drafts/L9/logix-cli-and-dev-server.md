...
priority: next
related:
  - ./runtime-studio-full-duplex-digital-twin.md
  - ../topics/platform-vision/10-platform-deep-dive.md
...
- 本地源码（TS/React + LogixRuntime）  
- 平台 Studio（Spec Studio / Galaxy / IntentRule Explorer）  
- 运行中的 Runtime（LogixRuntime.make(...) + Debug 事件流）  
...
1. **项目感知（Project Awareness）**
   ...
   - 扫描 `Logix.Module` / `ModuleImpl` / `LogixRuntime.make` 等定义，构建 Universe 级别的模块索引；
...
2. **Code ↔ IR 双向桥接**
   ...
   - IR → Code：
     ...
     - 通过 AST patch 修改对应的 Fluent 链，保持 **Code is Truth** 与项目风格（Prettier/ESLint）不变；
     - 支持 dry-run diff 与回滚。

3. **Runtime 事件中继**
   - 与运行中的 LogixRuntime 建立连接（例如 WebSocket / DebugSink + HTTP）；
...
1. **不重复实现 Runtime**
   - Dev Server 不运行 LogixRuntime 本身，不重新实现 Flow/Effect 行为；
...
- **Runtime（浏览器或 Node 中运行的 LogixRuntime）**  
  - `LogixRuntime.make(RootImpl, { layer })` 创建的应用级 Runtime；
...
# 5. 与 LogixRuntime 的耦合点

目前 Runtime 规范已经明确：

- `LogixRuntime.make(rootImpl, { layer, onError })` 是应用/页面级 Runtime 的 Composition Root；
- React 场景下，由 `RuntimeProvider runtime={LogixRuntime.make(...)}`
  管理 Scope 与生命周期；
...
2. **Runtime 标识与映射**
   - 每个 `LogixRuntime` 实例在创建时应生成可稳定识别的 `runtimeId`；
   - Dev Server 按 `runtimeId` 管理多个运行实例（例如不同页面、不同 Storybook 场景），并把事件映射回 Studio 中对应的 Universe/Galaxy 节点。
