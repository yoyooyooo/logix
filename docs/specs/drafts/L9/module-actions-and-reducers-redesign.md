...
- 当前 Module API（概念层）：
  ...
  - 实现：`Module.implement({ initial, logics, imports?, processes? })`
- Reducer 的引入动机：
...
也就是说：`L9/logix-state-first-module-codegen.md` 描述的 State-First Module Codegen，并没有引入第二种 Action/Reducer 语义，只是在 **工程化层面** 自动化了 ActionMap/Reducers 的构造。SSoT 仍然是 State Schema + 注解，执行语义仍由 Reducer 函数与 Logic 承载。
