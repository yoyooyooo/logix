...
related:
  ...
  - ../../intent-driven-ai-coding/v3/platform/impl/app-and-universe-view.md
---
...
- 如何在现有 `ModuleImpl` / `LogixRuntime.make` / AppRuntime 设计下，以最小增量拿到可用的 TagIndex 与拓扑信息；
...
- **AppRuntime 级 TagIndex**（已实现）：  
  - 代码：`packages/logix-core/src/runtime/AppRuntime.ts`；
  - 行为：
...
- **分形 ModuleImpl 组合**（已实现）：  
  - 代码：`ModuleImpl.make` + `withLayer/withLayers`（`ModuleFactory.ts`）；
  - Root Runtime 构建：`LogixRuntime.make(rootImpl, { layer, onError })` 仅把 **一个 Root ModuleImpl** 作为 `AppModuleEntry` 传给 `makeApp`；
