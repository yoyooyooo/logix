...
1. Module 层
   - 扩展 `ModuleShape` / `ModuleInstance`：
     - 增加 `reducers` 字段或等价结构：`Record<ActionTag, ReducerFn>`；
     - 在 `ModuleFactory.Module` 中接收并存入 ModuleInstance。
   - `ModuleInstance.live(initial, ...logics)`：
   ...
   - `ModuleRuntime.make<S, A, R>(initial, { tag, logics, moduleId, reducers })`：
     ...
     - 改造 `dispatch`，建议时序：
       ...
       2. 若存在对应 `_tag` 的 reducer，则：
          ...
          - 对 `stateRef` 做同步 `SubscriptionRef.update` 或 `SubscriptionRef.set(next)`；
...
3. BoundApi / Logic 层
   - 引入 `$.reducer`：
     ...
     - 实现上仅在 ModuleRuntime 或 ModuleInstance 持有的 reducer map 上注册；
...
1. 在 `docs/specs/runtime-logix/core/02-module-and-logic-api.md` 中补充 primary reducer 概念与 API 草案；
2. 在 `docs/specs/runtime-logix/core/03-logic-and-flow.md` 中：
   - 把 `$.reducer` 与 `$.onAction/onState` 的职责边界写清楚；
   - 更新 watcher 性能基线，注明 primary reducer 不计入 watcher 数量统计。
3. 在 `docs/specs/runtime-logix/impl/watcher-performance-and-flow.md` 及新建的 impl 文档中细化：
