...
- Runtime 侧：
  - 开发者通过 `Logix.Module.make("Order", { state: Schema.Struct(...), actions: { ... } })` 定义 Module；
...
> 已有草稿关联：  
> - `docs/specs/drafts/topics/devtools-and-studio/04-devtools-and-runtime-tree.md`  
> - `docs/specs/drafts/topics/runtime-v3-core/README.md`
...
- `topics/runtime-v3-core` / `topics/devtools-and-studio/04-devtools-and-runtime-tree.md` 提供了 Runtime 侧的“接线端口”（Debug 事件流、RuntimeAdapter、Runtime Tree），本草稿应在后续演进中把这些能力明确映射到 Studio 的视图和交互（例如：Runtime Tree 视图如何与 Galaxy 视图联动、高亮某条 IntentRule 的实际运行路径）。
