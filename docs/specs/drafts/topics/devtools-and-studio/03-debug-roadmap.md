...
- 引擎层：
  - `packages/logix-core/src/Debug.ts`（公开 API）与 `packages/logix-core/src/internal/runtime/core/DebugSink.ts`（事件模型 + sinks）已提供 `Debug.record` / `Debug.layer` / `Debug.noopLayer` 等入口；
  ...
  - Debug 的默认兜底输出（例如浏览器 console 分组、Node 下对错误类事件兜底）应被视为 Debug 能力的一部分，而不是散落在运行时代码中的 `console.log(...)`。
- 测试层：
...
5. **与 Runtime Tree / TagIndex 一致的标识体系**  
   - Debug 事件中的 `moduleId` / `instanceId` / `tag` 等标识应与 Runtime Tree / TagIndex 使用同一命名与 ID 体系；
...
- 约定一份轻量协议（草稿）：
  - 顶层字段：`version` / `instanceId` / `timestamp` / `event` / `traceId?`；
...
- Module 列表与状态快照：
  - 当前实现直接依赖 Debug 事件中的 `moduleId` / `instanceId` 与 `state:update.state`，而不是复用业务 Runtime 中的 `useModuleList`；
