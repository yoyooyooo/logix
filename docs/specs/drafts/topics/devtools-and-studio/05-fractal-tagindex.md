...
- **分形 ModuleImpl 组合**（已实现）：  
  - 代码：`ModuleInstance.implement` + `withLayer/withLayers`（`ModuleFactory.ts`）；  
  - Root Runtime 构建：`Logix.Runtime.make(rootImpl, { layer, onError })` 仅把 **一个 Root ModuleImpl** 作为 `AppModuleEntry` 传给 `makeApp`；
