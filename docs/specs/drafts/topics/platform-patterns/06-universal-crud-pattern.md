...
// 2. 一个用于生成 Tag 的工厂函数
export const makeCrudTag = <T, C, U>(key: string) => Context.Tag<CrudService<T, C, U>>(key)
```
...
export const makeCrudModule = <T, C, U>(
  ...
  const Module = Logix.Module.make(name, {
  ...
  const Logic = Module.logic(($) =>
    Effect.gen(function* () {
      const api = yield* ServiceTag

      yield* $.onAction('fetch').pipe(
        $.flow.run(({ page }) =>
          Effect.gen(function* () {
            yield* $.state.patch({ loading: true, error: null })
            const { items, total } = yield* api.list({ page, size: 10 })
            yield* $.state.patch({ items, total, page, loading: false })
          }).pipe(Effect.catchAll((err) => $.state.patch({ loading: false, error: err.message }))),
        ),
      )
  ...
  // 返回一个预配置好的 Impl 工厂，方便用户直接 useModule
  // 用户只需要提供 initial state，Logic 已经挂载好了
  return {
    Module,
    Logic,
...
export const ComplexUserLayer = Layer.succeed(UserService, {
  list: ({ page }) =>
    Effect.gen(function* () {
      ...
      const raw = yield* HttpClient.get(`/api/${tenantId}/users`, { params: { page } })
      return { items: raw.data.map(adaptUser), total: raw.meta.total }
    }),
  // ...
})
```
...
export const ExportLogic = UserModule.logic(($) => Effect.gen(function*() {
  yield* $.onAction('export').pipe(
    $.flow.run(() => Effect.log("Exporting..."))
  )
}))
...
export const OverrideLogic = UserModule.logic(($) =>
  Effect.gen(function* () {
    // 拦截 delete 动作
    yield* $.onAction('delete').pipe(
      // 使用 .prevent() 或类似机制阻止 Template 的默认 Flow (需 Flow 引擎支持优先级或拦截)
      // 或者，更简单地：Template 设计时提供 Hook 点，或者我们直接不使用 Template 的 Delete Logic
