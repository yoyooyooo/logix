...
**结论/设计**：
1.  **Prompt as Schema**: Prompt 不只是字符串模板，而是 Schema 驱动的资产（Prompt Intent）。  
    参考草案：`docs/specs/drafts/topics/ai-native-core/00-architecture.md`（`Prompt.make` / Prompt Intent 映射）。
2.  **Agent as Pattern**: Agent 是一种标准 Pattern，拥有 Input (Goal) / Output (Result)，内部封装 Loop（ReAct）。  
    参考草案：`docs/specs/drafts/topics/ai-native-core/00-architecture.md`（`Agent.make` / Tool Registry / Memory）。
...
**结论/设计**：
1.  **Strict Subset**: 只有使用 `$.onState / $.onAction / $.on`、（草案）`Agent.make` 等特定 API 的代码才会被 Parser 识别为图节点。
...
**路线图 (Phasing)**：
1.  **Phase 1 (Kernel)**: 先收敛 AI Native Core 的最小可执行 PoC（建议新增 `packages/logix-ai` 或 `examples/ai-native`），打通 Agent Loop 与 Prompt/Tool 原语。 (Current)
