...
类型示例以 `docs/specs/intent-driven-ai-coding/v3/effect-poc/shared/logix-v3-core.ts` 中的 `Logix.Module` / `ModuleRuntime` 等为基准。
...
在 React Adapter 中，`useModule` 推荐以统一的句柄类型（如 `Logix.ModuleInstance`）作为参数，底层根据实际形态（Tag / 运行时实例）决定是“查找环境”还是“直接使用实例”。
