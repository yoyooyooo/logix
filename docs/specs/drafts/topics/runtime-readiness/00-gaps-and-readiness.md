...
- `@logix/core`
  - Module / Runtime：
    ...
    - `ModuleImpl`：`Module.implement({ initial, logics, imports?, processes? })` + `withLayer/withLayers`。
...
### 2.1 Tag 冲突与 Env 拓扑校验
  ...
  - 分形 Module 目前通过 `ModuleImpl.imports` / `withLayer` 组合 Env，`Logix.Runtime.make(rootImpl, options)` 仅以 Root ModuleImpl 生成单一 `AppModuleEntry`，不会递归分析 imports 中的 Tag。
- 风险：
...
- TODO（v1 必要 + v1 之后可选拆分）：
  - v1 范围（必要）：
    ...
    - 在更贴近真实使用的场景下补充集成测试：Root ModuleImpl + `Logix.Runtime.make` 组合时，显式使用 `provideWithTags` 的 App 模块发生冲突应 fail，并暴露结构化错误。
...
- 现状：
  - `Module.implement({ initial, logics })` + `ModuleImpl.withLayer/withLayers` + `Logix.provide(ModuleImpl)` 在 core 层已经有成体系的实现与单测兜底：
...
- 现状：
  - `RuntimeProvider` 在实现上考虑了 StrictMode 下的 “双调用” 问题；
  - **[已落地]** L9 引入了 `ModuleCache`，通过 Reference Counting + Delayed GC 机制在原理上解决了 StrictMode 下的资源抖动与泄漏问题，并支持了 Suspense；
  - **[缺口]** 尽管实现已就绪，但仍缺少覆盖 StrictMode + Suspense 复杂组合场景的**系统化集成测试**；
  - **[缺口]** ModuleImpl + `$.lifecycle.onInit/onDestroy` 与 ModuleCache 的组合仅在单元层面有覆盖（Lifecycle / BoundApi 测试），尚缺少“Async ModuleImpl + 局部 ModuleRuntime + GC 驱动 Scope 关闭”这一端到端用例，用于验证：
    - onInit 在 ModuleRuntime 首次构建时按注册顺序执行；
    - 由 ModuleCache 触发的 Scope 关闭能够稳定触发一次 onDestroy，且不会因 StrictMode 双调用而重复。
...
- TODO：
  ...
  - 补充 React 层测试或 demo：
    - 在 StrictMode + Suspense 环境下 mount/unmount RuntimeProvider 和使用中的模块组件（包括含 `$.lifecycle.onInit/onDestroy` 的 Async ModuleImpl）；
