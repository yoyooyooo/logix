...
> 本文将 `specs/000-module-traits-runtime` 及旧「module-traits-sdd-roadmap」草案的讨论统一纳入 `sdd-platform` 主题下，聚焦一个问题：  
...
- 规范层：  
  - 在 `docs/specs/intent-driven-ai-coding` 中补充 `TraitHint`、`Module Blueprint` 与 AlignmentReport 的概念定义；  
  - 在 `.codex/skills/project-guide/references/runtime-logix/logix-core` 中补充 IntentRule ↔ Trait Anchor、ResourceSpec/Query 契约与 StateTraitProgram 导出格式。  
- 实现层：  
