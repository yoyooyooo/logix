...
- 用户角色：  
  - PM：只在 Spec Studio 中写需求、看演示与验收结果；  
  - Dev：在本地仓库实现 Module/Logic/Traits，使用 Dev Server 与 Sandbox。  
- 技术边界（MVP 不做的）：  
...
**目标**：提供一个最小可用的 Live Spec 编辑器，让 PM 能用“写文档 + 插卡片”的方式把 RegionSelector 需求说清楚，并形成结构化 Spec。
...
- 功能范围：  
  - 在 `effect-runtime-poc` 或 `examples` 中定义一个 `RegionSelectorModule`：  
...
- 输出契约：  
  - RunResult 至少包含：EffectOp Timeline + StateSnapshot；  
...
1. PM 能在 Spec Studio 中用实体块 + 场景块描述 “省市区联动” 需求，无需写任何代码或配置变量名；  
2. Dev 能从这份 Spec 出发，在本地得到一份可编译的 RegionSelector 模块代码（state/actions/traits 至少一条 source/link）；  
3. 在 Playground/Sandbox 中，PM/Dev 能一键运行该 Scenario，看到联动生效与否的红/绿反馈；  
