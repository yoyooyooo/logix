...
- **Traceability**: 所有的 Runtime Trace、Agent Log、Review Comment 都挂载在 Track ID 下。
...
| **Blocked**     | `depends_on` 上游变为 `Active` 以外状态 | **Halt**: 自动挂起当前任务，向 PM 发送 Blocked Alert。                          |
...
### 3.2 Trace-Driven Governance (行为驱动的治理)

Spec Graph 中的状态流转不应仅靠手动或 GIT Merge 触发，应由 **Runtime Behavior (Effect Trace)** 驱动。

- **Concept**: **Spec-Trace Duality (规约-轨迹对偶性)**。
- **Mechanism**:
  - Spec 定义了 "Success Criteria" (e.g., "Click Submit -> State becomes Loading -> Call API").
  - 平台在 Sandbox 中试运行代码，捕获 **Trace IR**。
  - **Governance Gate**: 只有当 Trace IR 能够 **拓扑匹配** Spec 中的 Intent Graph 时，Track 状态才自动转为 `Verified`。
...
3.  **Governance Sentinel**: 一个基于 Runtime Trace 验证状态流转的守护进程。
