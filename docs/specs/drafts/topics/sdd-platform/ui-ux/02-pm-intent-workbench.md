---
title: 09 · Logix Intent Studio (Project: Excel Killer)
status: draft
version: 1.0
target_audience: Product Managers (Primary), Frontend Devs (Secondary)
---

> **核心价值**：将非结构化的业务逻辑转化为可执行的意图，并提供严格的校验。
> **定位**：L0 (需求) 到 L1 (逻辑) 的转换器，旨在替代 Excel 成为 PM 定义逻辑的首选工具。
...
- **ADR-01: 无 UI 预览 (No UI Preview)**
  - **决策**: 不提供实时 UI 视觉预览。
  - **理由**: PM 应关注 **数据完整性** 和 **逻辑一致性**，而非像素级样式。视觉预览会分散对逻辑漏洞（如脏数据、死逻辑）的注意力。Payload 预览足矣。
