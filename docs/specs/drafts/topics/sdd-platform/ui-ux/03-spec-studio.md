...
priority: next
related:
  - ../spec-studio/00-design.md
---

# L0 Spec Studio：SDD 平台的需求入口

> 本文作为 `topics/sdd-platform` 的子篇，吸收 `topics/spec-studio/00-design.md` 中与 **平台形态** 相关的内容，只保留 Spec Studio 在 **L0 / SPECIFY 阶段** 的角色与边界。  
> 具体的 UI 组件与交互细节仍以 `topics/spec-studio/00-design.md` 为参考实现。
