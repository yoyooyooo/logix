---
title: 01 · 意图驱动开发平台 (Intent-Driven Development Platform)
status: living
---
...
## 6. 一条黄金链路（ModuleDef → Logic → Fluent → IntentRule）

在当前主线中，平台与 Runtime 围绕一条“黄金链路”协同：

> **ModuleDef → ModuleDef.logic(($) => ...) → $.use(ModuleDef) → Fluent DSL (`$.onState`/`$.onAction` / `$.on`) → IntentRule**

直观理解：

- **Define**：用 `Logix.Module.make('Id', { state, actions })` 定义领域资产（返回 `ModuleDef`：纯定义），同时承载 Id / Schema / `ModuleTag`（`moduleDef.tag`）；
- **Logic**：在 ModuleDef 上通过 `ModuleDef.logic(($)=>Effect.gen(...))` 编写 Logic，`$` 是 BoundContext（绑定当前 Module Runtime + 业务 Env 的统一入口）；
- **Collaborate**：在其他 Logic 中通过 `$.use(OtherDef)` 获取跨模块句柄（等价于使用 `OtherDef.tag`）；
...
- Runtime 视角：`.codex/skills/project-guide/references/runtime-logix/logix-core/api/03-logic-and-flow.md`（第 9 节）；
- Intent & Asset 视角：`03-assets-and-schemas.md`、`03-module-assets.md`；
- 解析 & 出码视角：`06-codegen-and-parser.md`、`.codex/skills/project-guide/references/runtime-logix/logix-core/platform/06-platform-integration.md`。
...
- `04-intent-to-code-example.md`：演练示例（含锚点代码）。
- `06-codegen-and-parser.md`：全双工引擎与锚点规范。
- `97-effect-runtime-and-flow-execution.md`：Logic 意图的运行时实现。
- `99-glossary-and-ssot.md`：平台概念与术语表（Platform Glossary SSoT）。
- `.codex/skills/project-guide/references/runtime-logix/logix-core/concepts/10-runtime-glossary.md`：运行时术语表（Runtime Glossary SSoT）。
- `EVOLUTION_POLICY.md`：演进策略（允许 breaking、收敛事实源与清理规则）。

## 8. 单一事实源与优先级（SSoT & Priority）

主线规范仍在演进中，难免存在历史写法与新规划并存的情况。为避免歧义，出现冲突时应按以下优先级判断“谁说了算”：

1. **平台与意图模型（概念层）**
   - 以本目录下的主规格文档为最高优先级，特别是：
   ...
   - 历史探索只保留“提炼后的结论”（见 `decisions/`），不作为事实源。

2. **Runtime 与 Intent API（类型与行为层）**
   - 以 `.codex/skills/project-guide/references/runtime-logix/logix-core/*` 中标记为 `Status: Definitive` 的文档为准，例如：
     - `.codex/skills/project-guide/references/runtime-logix/logix-core/api/02-module-and-logic-api.md`：Module / Logic / Bound API `$` 与 Module-first 编程模型；
     - `.codex/skills/project-guide/references/runtime-logix/logix-core/api/03-logic-and-flow.md`：Logic / Flow / Intent L1/L2 API 形态与 Fluent DSL；
     - `.codex/skills/project-guide/references/runtime-logix/logix-core/platform/06-platform-integration.md`：IntentRule IR 及平台集成规范。
...
3. **平台交互与资产体系（产品/UX 层）**
   ...
   - 若平台交互设计与 Runtime/API 细节有冲突，优先保证 Intent 模型与 Runtime 语义一致，然后再调整交互方案。

4. **示例与实现细节**
   - `examples/logix` 下的场景与 Pattern（`src/scenarios/*`、`src/patterns/*`）是当前实现的参考样例，但不等同于长期 API 设计承诺；
   - 在演进过程中，可先在示例中试验新写法，再根据验证结果回写/调整上述规范文档。

简言之，当遇到“多处不一致”的情况时，优先级排序为：

> **平台宣言 & 意图模型 ≥ Runtime 规范 & 类型定义 ≥ 平台交互规范 ≥ 示例与实现细节。**

任何对 Intent 模型、Runtime API 或平台交互有破坏性影响的改动，都应先在对应规范文档中达成共识，再回写到示例代码与后续实现中。
