...
- **Pattern 资产形态**：以 `docs/specs/intent-driven-ai-coding/design/pattern-system.md` 为准（当前共识：Pattern 是普通 `(input) => Effect` 函数，平台只包 metadata）。
...
参照：`docs/specs/intent-driven-ai-coding/design/pattern-system.md`（第 5 节“工程约定”）。
