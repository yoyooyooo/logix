...
1. **文档先于实现**
   - 任何涉及 Intent 模型、Logix Runtime、平台交互的变更，必须先在 `docs/specs/intent-driven-ai-coding` 中达成共识，再进入示例/实现代码。
   - SSoT 优先级以 `01-overview.md` 的规则为准。
...
- 模型层：
  - 三位一体模型：`02-intent-layers.md`；
  - 资产结构：`03-assets-and-schemas.md`；
  - 长期蓝图：`decisions/blueprint.md` + `00-platform-manifesto.md`。
- Runtime & Intent API：
  - Logix 核心：`.codex/skills/project-guide/references/runtime-logix/logix-core/api/02-module-and-logic-api.md`、`.codex/skills/project-guide/references/runtime-logix/logix-core/api/03-logic-and-flow.md`；
  - Intent 原语（L1/L2）：以 Fluent DSL（`$.onState / $.onAction / $.on + $.state/dispatch`）表达的标准规则，在 IR 中映射为 L1/L2 IntentRule；
  - 平台 IR：`.codex/skills/project-guide/references/runtime-logix/logix-core/platform/06-platform-integration.md` 中的 `IntentRule`。
- 平台视角：
  - 视图体系与交互原则：`06-platform-ui-and-interactions.md`；
  - Intent & UX 规划 + L0–L3 资产链路：`platform/README.md`。
...
2. **Runtime 能力收敛到 logix-* 包**
   ...
   - 确保 `.codex/skills/project-guide/references/runtime-logix/logix-core/api/*`（特别是 `02-module-and-logic-api.md` / `03-logic-and-flow.md`）紧跟这些能力更新。
...
- `.codex/skills/project-guide/references/runtime-logix/logix-core/` 文档中对这些模式的总结与推荐用法；
...
1. **是否破坏 Intent 模型 / Runtime 语义？**
   - 如有冲突，先回到 `02-intent-layers.md`、`03-assets-and-schemas.md`、`.codex/skills/project-guide/references/runtime-logix` 修模型，再动实现。
...
4. **是否可以先在示例中验证，再沉淀为规范？**
   - 新 API / 新 Pattern / 新 IntentRule 类型，统一先在 `examples/logix` 中试验，再升级到 `.codex/skills/project-guide/references/runtime-logix` / 主规格文档。

本 Roadmap 本身也是“living document”：
- 每完成一个 Phase 或经历一次较大方向调整时，应回到本文件更新目标与优先级；
- 若 Roadmap 与 `01-overview.md` / `.codex/skills/project-guide/references/runtime-logix` / `platform/*` 规范产生冲突，以这些 SSOT 为准，并同步修订本文件。
...
- **运行时侧**：
  - 所有围绕 `ModuleDef` / AppRuntime / Module 展开、以及 Logic Middleware、Store 生命周期的实现思路与取舍，沉淀在 `.codex/skills/project-guide/references/runtime-logix/logix-core/impl/` 下（例如 `01-app-runtime-and-modules.md`、`02-logic-middleware-and-aop.md`）；
  - 这些文档可以比 core/ 规范更细、更工程化，但一旦发现与核心规范冲突，应先修 core/ 再修 impl/。

- **平台侧**：
  - 围绕 Universe View / Galaxy View、IntentRule ↔ TS 代码生成、AOP 配置与可视化等能力的解析/生成逻辑，沉淀在 `platform/impl/` 下（例如 `app-and-universe-view.md`、`intent-rule-and-codegen.md`）；
  - 平台实现者在设计解析器与出码链路时，优先查阅这些实现备忘，并在方案确定后回写主规格（`platform/*` 与 `.codex/skills/project-guide/references/runtime-logix/logix-core/`）。
