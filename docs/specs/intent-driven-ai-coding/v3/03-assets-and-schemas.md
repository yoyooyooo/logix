...
- 形态：将 Level 0 的自然语言需求投影到 v3 三维 Intent 模型与 Logic Intent Graph 上，形成“用例级蓝图”：  
  - 明确涉及哪些 UI/Interaction 节点、哪些 Domain 实体/字段、哪些 Logic 步骤；  
...
- 形态：在 Level 1 蓝图的基础上，落到具体的 Store / Pattern / Intent API 选择上：  
  - 确定使用哪些 Store.Shape / Logic 模块；  
  - 为每条规则选择对应的表现形式：L1（`Intent.andUpdate*`）、L2（`Intent.Coordinate.*`）、L3（Flow/Pattern 组合）；  
  - 形成结构化、可出码的 `IntentRule` 集合。  
- 资产类型：  
  - **Pattern 资产**：`(input) => Effect` 行为积木，配有 `configSchema` 与 meta，可在多个模块/项目中复用；  
  - **Logic 模板**：针对某一类场景的 `Logic.make<Shape>` 模板（如标准搜索场景 Logic），适合在一个产品线内部复用；  
  - **Store 模板**：包含特定 State/Action 形状与一组 Logic 的“模块级模板”（如标准分页列表 Store）；  
...
- 形态：具体项目中的 Store / Logic / Pattern / UI 代码与运行时配置：  
  - 类型安全的 State/Action Schema、`Store.make(...)` 调用；  
  - `Logic.make<Shape>(...)` 中的 `Effect.gen` 逻辑，结合 `Intent.*` / Flow / Control；  
