...
status: draft
version: 15 (Effect-Native · Fluent DSL)
---
...
1.  **分支 (Branch / Switch)**
    ...
    - **解析逻辑**: 提取每个 `.when(predicate, body)` 中的 `body` 作为子图 (Subgraph)。
...
为保证解析器实现的简洁与鲁棒性，v3 采用 **显式的白盒/黑盒分界**：
...
v3 之后，锚点系统不再依赖大量魔法注释，而是以内建结构为主、注释为辅：

1. **上下文锚点：`$` 与 Module**
   - 对 Parser 而言，`Logix.Module` + `.logic(($) => ...)` 明确标记了 Logic 的上下文（Module Id）；
...
1. **Code → Graph（Fluent 优先）**
   - 解析入口：扫描 `Logix.Module` / `Module.logic(($) => ...)` / `$.use` / Fluent 链；
   - 构建过程：
