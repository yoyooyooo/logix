...
为了让产品 / 设计 / 架构角色也能在平台侧直接表达业务意图，同时与 Logix / Effect Runtime 严格对齐，本平台在「画布视图 → 规则 IR → TypeScript Intent API」之间引入了统一的中间表示 `IntentRule`（见 `runtime-logix/core/06-platform-integration.md`）。
