...
> 本文档定义了如何将业界 **Spec-Driven Development (SDD)** 方法论映射到 **Intent Flow V3** 架构中。
...
| Classic SDD Layer             | Intent Flow V3 Mapping           | Artifact / Asset                     |
| :---------------------------- | :------------------------------- | :----------------------------------- |
...
我们将 GitHub Spec Kit 定义的四阶段工作流 (Specify -> Plan -> Tasks -> Implement) 映射到 Intent 模型的逐层展开过程（L0/L1/L2/L3 的详细定义见 `02-intent-layers.md`）。
