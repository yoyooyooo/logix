---
title: 数据/状态意图 (Module Intent)
status: draft
version: 3
---

> 本文定义了 v2 架构下的数据模型：作为业务实体的单一事实源，为 Signal Payload 提供类型定义，并映射为 Logix Module 的状态结构（stateSchema + initialState）。

## 1. 核心定义

Module Intent 回答：**“系统中有哪些业务实体？它们的状态如何流转？”**

它不仅仅是数据库表结构的映射，更是**全链路类型系统 (Type System)** 的基石。从 UI 组件到 Signal Payload，再到 Logix Module 和后端 API，所有数据结构都必须引用 Module Intent 中定义的 Schema。

### 1.1 关键特性

1.  **Schema First**：所有数据交互（API、Signal、Module）必须先定义 Schema。消灭 `any` 和隐式类型。
2.  **Signal 类型源**：Signal Intent 中的 `payloadSchemaId` 直接引用本层的 Entity 定义。
3.  **Logix 映射**：Entity Schema 直接映射为 Logix Module 的 `stateSchema` 和初始值（`initialState`）。
...
Module Intent 是整个系统的“类型字典”。以下是它在不同层级的落点：
...
Module Intent 中的 `apis` 定义不仅生成 DTO，还驱动整个服务层的生成：

1.  **Interface (Module)**：生成 `OrderApi` Tag，定义 `create: (input: OrderInput) => Effect<Order>`。
...
在平台的“自由画布”视图中，Module Intent 表现为**实体节点 (Entity Node)**：
