...
export type ReliableSubmitConfig = Schema.Schema.Type<typeof ReliableSubmitConfigSchema>;
...
export type GuardedReliableSubmitConfig = Schema.Schema.Type<typeof GuardedReliableSubmitConfigSchema>;
  ...
  definePattern<GuardedReliableSubmitConfig>({
    ...
    body: config =>
      Effect.gen(function*(_) {
        ...
        if (current === expected) {
          yield* dsl.run(ReliableSubmitPattern, config.submit);
        } else {
