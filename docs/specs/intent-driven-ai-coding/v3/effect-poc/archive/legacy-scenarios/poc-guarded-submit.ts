...
import {
  GuardedSubmitPattern,
  type GuardedSubmitConfig
} from "../patterns/guarded-submit";
...
const program = Effect.gen(function*(_) {
  ...
  const blockedConfig: GuardedSubmitConfig = {
    guardStatePath: "ui.formValid",
    service: "OrderService",
    method: "submit",
    fallbackMessage: "表单未通过校验，已拦截提交"
  };
  yield* dsl.run(GuardedSubmitPattern, blockedConfig);

  // 场景 2：guard 为 true，允许提交
  yield* dsl.set("ui.formValid", true);
  const allowedConfig: GuardedSubmitConfig = {
    guardStatePath: "ui.formValid",
    service: "OrderService",
    method: "submit",
    fallbackMessage: "表单未通过校验，已拦截提交"
  };
  yield* dsl.run(GuardedSubmitPattern, allowedConfig);
});
