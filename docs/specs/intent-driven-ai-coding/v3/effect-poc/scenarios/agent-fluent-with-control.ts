 ...
 *   2）国家变化 → 重置城市 + 加载城市列表失败时弹 Toast（Flow + Control）。
 *
 *   代码遵守 v3 的硬约束：
 *   - 使用 Bound API (`$`) 作为唯一入口；
...
export class SearchService extends Context.Tag('@poc/SearchService')<
  SearchService,
...
export const SearchLogicAgent = AgentModule.logic<SearchService>(($) =>
  Effect.gen(function* () {
...
export const SearchLiveAgent = AgentModule.live(
  {
    keyword: '',
    results: [],
  },
...
export class LocationService extends Context.Tag('@poc/LocationService')<
  LocationService,
...
export const ProfileLogicAgent = ProfileModule.logic<LocationService>(($) =>
  Effect.gen(function* () {
    yield* $.onState((s) => s.country).run(
      Effect.gen(function* () {
        ...
        yield* loadCities.pipe(
          Effect.catchAll(() =>
            $.state.update((d) => ({ ...d, toast: '加载城市失败' })),
          ),
        )
      }),
    )
...
export const ProfileLiveAgent = ProfileModule.live(
  {
    country: '',
    city: '',
    cities: [],
