...
- **IntentRule（IR）**：平台侧的统一规则表示（详见 `runtime-logix/core/06-platform-integration.md`）：  
  ...
  代码中的 `Intent.*` / Flow 组合都尽量被解析为 IntentRule；画布 / DSL 编辑的也是 IntentRule。

- **L1 / L2 / L3 分层 API（代码视图）**：  
  - L1：`Intent.andUpdateOnChanges / andUpdateOnAction` —— 单 Store 内同步联动；  
  - L2：`Intent.Coordinate.onChangesDispatch / onActionDispatch` —— 跨 Store 协作；  
  - L3：Flow/Stream/Effect —— 极度定制化逻辑的逃逸口。  

本目录后续所有平台交互设计，都默认建立在这套模型之上：

> 画布 / 表单 / 列表 操作的是业务语义 → 映射为 `IntentRule` → 再由生成器落到 `Intent.*` / Flow / Pattern 上。
...
3. **UI & Domain Studio（屏幕与数据模型工作台）**  
   - 视角：页面结构、组件、字段与交互入口。  
   - 目标用户：产品、设计、前端。  
   - 核心能力：  
     - 声明/编辑页面布局与 UI Intent（参照 `07-ui-ux-detailed-specs.md`）；  
     - 基于 Domain Schema 管理字段、校验、初值；  
...
4. **Pattern Studio（模式资产工作台）**  
   ...
   - 核心能力：  
     ...
     - Pattern 内部可使用 `Intent.*` / Flow / Control / Service，平台仅需关心 Pattern 的输入/输出契约与 meta。
...
- 形态：将自然语言需求投影到 v3 Intent 模型（UI / Logic / Domain / Constraint），并初步拆成「用例级蓝图」：  
...
- 形态：在 Requirement Intent 的基础上，显式落到具体的 Store / Pattern / Intent API 选择上：  
  - 确认涉及到哪些 Store.Shape（State/Action Schema）；  
...
- 形态：将 Level 2 的开发意图自动或半自动生成为具体代码与运行时配置：  
  - 类型安全的 Store Schema / Store.make 调用；  
  - `Logic.make<Shape>(...)` 内部的 `Intent.*` / Flow / Control 调用；  
  - Pattern 定义文件、测试用例、React 绑定代码等。  
- 平台职责：  
  - 提供稳定的 Codegen 模板：从 `IntentRule` 到 `Intent.andUpdate* / Intent.Coordinate / flow.*` 的标准化生成；  
