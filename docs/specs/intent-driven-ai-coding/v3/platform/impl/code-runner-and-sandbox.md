...
针对“如何运行用户代码”这一问题，v3 架构明确选择 **前端方案 (Browser / WebWorker)** 作为首选运行时，而非后端沙箱 (Node/Docker)。
...
在 `esbuild-wasm` 与 `swc-wasm` 之间，v3 明确选择 **`esbuild-wasm`**。
...
v3 平台通过 **Web Worker + Effect Layer** 实现了一个零成本、毫秒级响应的 **全双工代码演练场**。这既提升了 DX，又倒逼了架构的纯洁性。
