...
> **Status**: Draft  
> **Purpose**: 覆盖核心场景之外，在真实业务中高频出现、且对 Kernel API 设计有较强约束力的复杂场景；用于在设计/实现阶段对 API 做压力测试与对齐。
...
**目标**：压测 Kernel 是否需要提供字典级别的监听能力（如 `watch('itemsById.*.status', ...)`），以及通配符路径在类型与实现上的可行性。
...
**目标**：压测 Kernel 对“聚合派生状态”的表达能力，以及是否需要专门的聚合工具（如 `derive` / `selector` 风格）。
...
**目标**：验证 Kernel 是否提供提交级并发策略（如 `exhaust` / `restart`），以及与 `watch`/`on` 的组合方式。
...
**目标**：验证在 `watch` 中组合 `Effect.all` / `Effect.allWith` 等并发原语时，Kernel 对错误通道路由、部分成功写入与回滚策略的兼容性。
...
**目标**：验证 Kernel 是否需要内建“init-on-first-subscribe / auto-dispose”策略，还是通过 Adapter / 外层 Runtime 负责。
...
**目标**：压测 Kernel 在“状态源桥接”场景下的双向交互能力，以及对第三方库生命周期的适配难度。

### Scenario 6.3: 资源清理与异常恢复
**描述**：WebSocket、EventSource 等长连接在网络抖动、服务端断开等异常情况下，需要正确重连或释放资源，关闭 Store 时必须彻底清理。  
**目标**：验证 Scope + finalizer 在异常路径上的表现，以及是否需要 Kernel 提供统一的 retry/backoff 策略挂点。
...
**目标**：验证 Kernel 对 debounce/throttle、队列长度、丢弃策略（drop/latest/buffer 等）的配置能力及可组合性。
...
**目标**：从 Kernel 角度明确 `state$` 是否只提供整体流，还是需要提供局部投影/选择器机制以支撑高性能 Adapter。
...
**描述**：一个 Kernel 进程内为多个租户提供服务，每个租户拥有独立的 Store 与 Services，实现与配置完全隔离。  
...
**目标**：压测 Kernel 在 Trace/日志接口上是否提供足够的扩展点，以支撑脱敏策略和合规要求。
