# Runtime Kernel 用户手册 · 导航

> 面向「使用 Kernel 写业务」的一线工程师与平台集成方，帮助你在不深挖全部设计文档的情况下，用几个典型工作流把 Kernel 用起来。

## 文档结构

- `01-overview-and-roles.md`  
  从使用者视角解释 Kernel 在整个架构中的位置、适用场景，以及和 Effect Flow Runtime、React/表单库的关系。
...
- `05-working-with-effect-flow-runtime.md`  
  说明如何在 Kernel 中触发服务侧 Flow（Effect Flow Runtime），以及如何回填状态、展示执行结果，对应 `v2/97-effect-runtime-and-flow-execution.md` 的 Runtime 家族说明。
