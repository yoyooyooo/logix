...
> **StateTransaction / Debug 事务边界**：事务边界与长链路拆分心智已在 §3.5 收敛；`txnId` 绑定与 Debug 事件聚合策略见 `core/09-debugging.md` 与 `core/05-runtime-implementation.md`。本文件不再保留 003 特性的 TODO 标记。

本节描述 Logix v3 中的 `Logic` / `Flow` / `Control` 原语，以及围绕 **Universal Bound API (`$`)** 的最终编程模型。
类型以 `@logix/core` 的导出为准（见 `packages/logix-core/src/index.ts`），本文件给出概念视图；场景/示例以 `examples/logix/src/scenarios/*` 为准。
...
> 本小节从 Logic/Flow 视角补充 StateTransaction 边界的直觉心智；运行时实现细节见 `core/05-runtime-implementation.md#15-statetransaction-与状态提交路径（v3-内核）` 与 `core/09-debugging.md`。
