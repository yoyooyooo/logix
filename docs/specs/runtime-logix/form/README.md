...
Form Store 本质上是一个标准的 Logix Store，它可以根据业务需求选择生命周期与接入方式。
...
- **生命周期**：随宿主组件销毁而销毁；  
- **接入方式**：  
  - 在组件中使用 `useLocalStore`（或 `useForm` 封装）创建 `Store.Runtime<FormState, FormAction>`；  
...
- **接入方式**：
  - 使用 `makeFormStore` 创建实例，得到 `Store.Runtime<FormState, FormAction>`；  
  - 使用 `Logix.provide` 将该实例注册进 `Logix.app` 的 `modules`：  
    - `modules: [Logix.provide(GlobalSearchFormTag, GlobalSearchFormStore), ...]`；  
  - 在 `Logix.app` 的 `processes` 中，使用 `Intent.Coordinate` 或普通 Logic Effect 编写该表单与其他 Store 的联动逻辑。
