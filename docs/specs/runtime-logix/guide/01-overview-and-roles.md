...
- 负责在业务项目中接入 Logix 的前端工程师；  
- 负责把 Intent/Flow 工具链落到实际代码仓库中的平台/基础设施开发者。

你可以假设：

- 已经了解项目整体在推进「Intent → Flow/Effect → 代码」的方向；  
- 不需要深入理解 Logix 内部 Scope/Hub/Stream 的实现，只需要知道如何在业务场景中使用 Store/Logic/API。

## 2. Logix 在架构里的位置（使用者视角）

从使用者视角，简单记两点即可：

- Logix 是**前端应用里的唯一状态与本地行为运行时**：字段联动、表单校验、列表加载策略、UI 级状态（loading/error/dialog 等）都统一交给 Store + Logic 来管理；  
- 对「跨系统 / 强审计 / 长流程」的行为（如导出任务、审批流），Logix 只负责：  
  - 触发服务侧 Flow（Effect Flow Runtime）；  
  - 接收结果、更新本地状态、驱动 UI。

可以把 Logix 理解为「更强版的表单/Store」，但：

- 不再允许在 React 组件里散落 `useEffect` 编排业务级联动；  
- 所有业务副作用（调用 API、刷新列表、发埋点等）都通过 Logix 的 Logic 来声明。
...
- **可以暂时不用 Logix 的场景**：  
  ...
  - 纯展示页，几乎没有业务逻辑（这类场景 Logix 不会带来明显收益）。

> 经验法则：只要你开始考虑“这个逻辑是写在组件 A 里，还是放到 store 里？”——优先考虑 Logix，把「业务行为」写成 Logic 规则，而不是继续扩散 useEffect。
...
- `05-working-with-effect-flow-runtime.md` 则站在用户视角说明如何在 Logix 中触发 Flow Runtime。
