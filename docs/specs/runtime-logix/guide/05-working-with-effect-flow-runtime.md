# 05 · 与 Effect Flow Runtime 协同 (v3 标准范式)

> **目标**: 说明在前端 Logix 中，如何以 v3 标准范式触发服务侧 Flow (Effect Flow Runtime)，并管理状态反馈。
> **上游规范**: `docs/specs/intent-driven-ai-coding/v3/97-effect-runtime-and-flow-execution.md`。
...
interface RunFlowInput { flowId: string; input: any; }
interface RunFlowOutput { success: boolean; data?: any; error?: any; }

// 定义服务接口
class FlowRunner extends Context.Tag("FlowRunner")<FlowRunner, {
  ...
  Effect.gen(function* (_) {
    ...
    const submitEffect = Effect.gen(function* (_) {
      ...
      const result = yield* Effect.either(
        runner.run({ flowId: 'createOrder', input: current.form })
      );
...
- **声明式并发**: 利用 `runExhaust` (防重提交) 或 `runLatest` (取消旧请求) 等 `Flow` API 来声明式地管理并发，而不是手动编写 `if (isLoading) return;` 逻辑。
