...
> 裁决口径：平台概念与契约以本目录为准；运行时与类型语义以 `.codex/skills/project-guide/references/runtime-logix/**` 与 `packages/logix-*` 的真实导出为准。
...
6. `.codex/skills/project-guide/references/runtime-logix/logix-core/concepts/10-runtime-glossary.md`：运行时术语裁决（Runtime Glossary SSoT）
...
- **实现备忘**：平台侧放 `docs/specs/sdd-platform/impl/*`；运行时侧放 `.codex/skills/project-guide/references/runtime-logix/*/impl/*`
