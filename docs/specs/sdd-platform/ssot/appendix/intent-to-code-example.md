...
```typescript
	...
	// 1. 定义领域 Module（state/actions 形状；返回 ModuleDef：纯定义）
	export const ArticleDef = Logix.Module.make('Article', {
	  state: ArticleStateSchema,
	  actions: ArticleActionSchema,
	});
	
	// 2. 定义 Logic 程序（基于 Fluent Intent + Pattern）
	export const ArticleLogic = ArticleDef.logic(($) =>
	  Effect.gen(function* () {
	...
	// 3. 组装 program module（wrap module，带 `.impl`，可直接给 React `useModule` 创建局部实例）
	export const ArticleModule = ArticleDef.implement({
	  initial: {
	    status: 'idle',
	    title: '',
	    content: '',
