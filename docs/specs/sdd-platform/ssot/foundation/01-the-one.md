...
- $\operatorname{Close}_{C_T}$：在约束下求收敛（closure / fixpoint）。
  - 现实裁决：允许**预算与软降级**（例如 partial fixpoint、分帧、优先级），但必须可解释、可诊断、可回放（口径见 `docs/specs/sdd-platform/ssot/contracts/00-execution-model.md` 与 `docs/specs/sdd-platform/ssot/contracts/01-runresult-trace-tape.md`）。
...
- 运行时物理模型：`docs/specs/sdd-platform/ssot/contracts/00-execution-model.md`
- RunResult 契约：`docs/specs/sdd-platform/ssot/contracts/01-runresult-trace-tape.md`
