...
1. **文档先于实现**
   - 任何涉及 Intent 模型、Logix Runtime、平台交互的变更，必须先在 `docs/specs/sdd-platform/ssot` 中达成共识，再进入示例/实现代码。
...
1. **是否破坏 Intent 模型 / Runtime 语义？**
   - 如有冲突，先回到 `foundation/03-trinity-and-layers.md`、`assets/00-assets-and-schemas.md`、`.codex/skills/project-guide/references/runtime-logix` 修模型，再动实现。
...
- 每完成一个 Phase 或经历一次较大方向调整时，应回到本文件更新目标与优先级；
  - 若 Roadmap 与 `foundation/00-overview.md` / `.codex/skills/project-guide/references/runtime-logix` / `docs/specs/sdd-platform/workbench/*` 规范产生冲突，以这些 SSOT 为准，并同步修订本文件。
...
- **平台侧**：
  ...
  - 平台实现者在设计解析器与出码链路时，优先查阅这些实现备忘，并在方案确定后回写主规格（`docs/specs/sdd-platform/workbench/*` 与 `.codex/skills/project-guide/references/runtime-logix/logix-core/`）。
