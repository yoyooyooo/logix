...
> 视角：站在平台“最终用户”（需求负责人（PM/业务负责人/架构师）/ 开发者 / QA）的角度，自上而下按**业务价值**而不是按实现分层，梳理几条可独立感知的高价值路线。  
> 目标：为后续产品规划与实现排期提供“从体验出发”的骨架，具体技术细节仍以本主题其他文档为准。

## 1. 从业务需求到「跑得起来的页面」

- **目标用户**：需求负责人（PM/业务负责人/架构师）+ 对应前端开发  
- **核心承诺**：从“把需求写清楚”到“看到能点得动的页面”，主要由平台/AI 完成，开发者只补关键逻辑。  
- **主链路**：
  - 在 Spec Studio 中录入/迭代 FeatureSpec + ScenarioSpec（`ui-ux/03-spec-studio.md`）；  
...
- **主链路**：
  ...
  - 通过 StateTraitGraph 构建模块内/跨模块的数据依赖视图（见 `01-module-traits-integration.md#6` 的治理/路线收敛）；  
...
- **主链路**：
  ...
  - Intent Compiler 将 JSON Definition 注水为 TypeScript + Effect Program（方向见 `07-intent-compiler-and-json-definition.md`，边界见 `05-intent-pipeline.md#3`），并通过 Dev Server 写入仓库；  
...
> - 在 `docs/specs/sdd-platform/workbench/00-overview.md` 中添加简短引用，让新读者可以从“愿景 → 用户价值路线 → 技术子篇”顺畅导航。 
