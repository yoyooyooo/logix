...
> 未来如需提炼“精简版”，建议新增 `10-platform-shapes-and-interactions.md` 并从本 Backlog 逐步裁剪；新的发散想法建议优先追加到本 Backlog。
...
2. **Blueprint → Module 图纸 / Traits**（`01-module-traits-integration.md`）：  
   - 为 Blueprint 中的 Module 补全 state/actions/traits 骨架；  
   - Traits 提供字段能力图，为后续数据治理和 Alignment 提供锚点。  
3. **Blueprint/Module → Code & Dev Server**（`02-full-duplex-architecture.md#6` + `docs/specs/sdd-platform/impl/README.md`）：  
...
- 导航层：  
  - 需求录入者（PM/架构师）登录：默认只露出 Spec Studio / Blueprint / Scenarios / Reports；  
  - Dev 登录：可以看到 Intent/Definition、DevTools、Sandbox/Alignment 等更多入口。  
- 权限层：  
  - Product View 不直接暴露 Schema/Traits/IntentRule 等实现词汇（必要时仅显示自然语言摘要与示例）；  
...
### 10.6 需求视图的“翻译层”：把技术事件流变成可理解的隐喻

> `UI_INTENT` 事件流是给 Sandbox/Alignment/DevTools 看的“技术层”，**绝不能直接给需求录入者看**。对需求录入者来说，平台应该更像“能自动验收的墨刀/原型工具”，而不是事件监控台。

**设计原则**：需求录入者操作的是“页面/组件之间的因果关系”，平台在后台悄悄编译为 UiBinding/UiSignal/IntentRule/UI_INTENT。

- **交互隐喻：连线 = 逻辑**  
  - 需求录入者在画布上只看到页面和组件：列表页、详情页、搜索按钮、API 方块；  
  - 从按钮拖一根线到“详情页”或“API 请求”节点，心智是“点这个就去那”；  
  - 平台在后台生成 `UiSignal` + IntentRule/dispatch 绑定，并在 Sandbox 中转成 `UI_INTENT`。  

- **属性隐喻：填空 = 绑定**  
  - 需求录入者在属性面板只看到“数据内容”下拉框，选项是“用户列表/订单列表”等自然语言（来自 Domain/Module Intent 推断）；  
  - 选择完成后，平台生成 `UiBinding`，绑定到具体 state/trait 路径；  
  - 技术视图里可以看到精确路径，需求录入者永远只看到“内容来自某某列表”。  

- **行为隐喻：预制动作 = Flow 套餐**  
  - 对“提交表单并返回列表”这类逻辑，需求录入者在 UI 中只选择“点击后执行：提交表单”、“成功后：返回列表”；  
  - 平台用预设 Pattern 生成对应 IntentRule/Logic 代码，需求录入者不需要懂 Flow DSL。  

- **验证隐喻：红绿灯 = UI_INTENT 对齐结果**  
  - 在 Playground/Alignment Lab 里，需求录入者点击“运行演示”：  
    - 左侧是自己的 Scenario 文案；  
...
凡是落在左列的概念，都应该在需求视图里有一个“自然语言/图形隐喻”的对映，而不是以技术名词出现。
