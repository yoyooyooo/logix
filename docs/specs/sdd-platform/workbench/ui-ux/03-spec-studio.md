...
priority: next
related:
  - docs/specs/sdd-platform/workbench/ui-ux/00-platform-ui-and-interactions.md
---

# L0 Spec Studio：SDD 平台的需求入口

> 本文作为 `docs/specs/sdd-platform/workbench` 的子篇，吸收旧「spec-studio」草案中与 **平台形态** 相关的内容，只保留 Spec Studio 在 **L0 / SPECIFY 阶段** 的角色与边界。  
> 具体的 UI 组件与交互细节建议以 `docs/specs/sdd-platform/workbench/ui-ux/00-platform-ui-and-interactions.md` 为准。

## 1. 在 SDD 生命周期中的角色

- 上游：需求录入者（PM/架构师）在 Spec Studio 中以自然语言 + Intent Widgets 方式，逐步结晶出 `FeatureSpec` / `ScenarioSpec` / `Step`；  
- 下游：一旦用户在 Studio 中点击 “Approve Spec”，平台根据 `../00-overview.md` 中的约定，将 Spec 交给 Architect/Task/Coder Agents，进入 PLAN/TASKS/IMPLEMENT 阶段。  
...
- **对需求录入者（PM/架构师）**：像写 PRD 一样自然（大段文本 + 少量结构化块），但每个嵌入块都是“可以生成/验证代码”的意图单元；  
...
- 交接点：当 Spec Studio 触发 “Approve Spec” 时，平台根据 `../00-overview.md` 中的 Context Pack 规范，将上述资产打包为 **Spec Context Pack**，交给 Architect Agent。
...
- 原 `topics/spec-studio/00-design.md` 中关于 “四阶段 SDD” 与 “全双工平台架构” 的内容，已被统一收束到 `docs/specs/sdd-platform/workbench/00-overview.md` 与 `docs/specs/sdd-platform/workbench/02-full-duplex-architecture.md`；  
- Spec Studio 主题后续仅负责：  
  - 具体组件设计（Tiptap Node、画布交互、Traceability Matrix UI）；  
  - 与 Runtime/Devtools 的前端集成细节（例如如何从 Runtime 回放结果到画布）。  
- 平台级流程、Agent 职责与 Context Supply Chain 的定义，一律以 `docs/specs/sdd-platform/workbench/README.md` 的 Owner Map 为准。
