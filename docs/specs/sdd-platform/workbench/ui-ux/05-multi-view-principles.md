...
> **目标**：在不拆成“两套平台”的前提下，让需求侧与开发侧都能在同一平台内“只看到自己该看的东西”，但共享一套 Spec/Intent/Blueprint/Code/Runtime 资产。
...
- **视图层次（按职责变化）**：
  ...
  - **Dev View（实现视图）**：展示与编辑实现细节（Schema/Traits/IntentRule/运行结果），并保持与 Product View 的锚点。

结果是：**数据只有一份，视图按职责切换**，避免“需求系统”和“开发配置系统”各自漂移。
...
- **架构模式（在同一页面开启）**：
  - 右侧出现 Architect Panel（比 Dev Panel 更高层）：
    - Scenario ↔ 模块/服务/接口的归属与依赖图（只到“模块/服务”粒度，不展开字段/Schema）；
    - 范围裁决（本迭代做/不做）、关键路径与风险登记；
    - 质量约束（预算/一致性/可观测性要求）的“勾选 + 简述”，并把约束锚定到对应 Scenario。
- **开发模式（在同一页面开启）**：
  - 右侧出现 Tech Panel：
...
- **架构看到**：
  - 在同一图上叠加边界/依赖/质量约束 Overlay；
  - 可以把 Scenario 映射到“模块/服务/接口”，并给出裁决理由；
  - 可以从节点跳转回 Spec 文档的对应段落（双向锚点）。
- **开发看到**：
...
- **架构模式**：右侧侧边栏是拆解与关联：
  - “这条 Scenario 属于哪个模块/服务？接口边界是什么？”；
  - “需要哪些质量约束（预算/一致性/可观测）？它们如何验收？”；
  - “哪些地方需要降级策略/兜底规则？”。
- **开发模式**：右侧侧边栏变成技术映射：
...
- **导航层**：
  ...
  - 开发登录：可以看到 Intent/Definition、DevTools、Sandbox/Alignment 等更多入口。
- **权限层**：
  - Product View 不直接暴露 Schema/Traits/IntentRule 等实现词汇（必要时仅显示自然语言摘要与示例）；
  - Architect View 可以编辑“归属/依赖/约束/验收口径”，但仍避免直接编辑底层实现工件；
  - Dev View 可以编辑实现工件，并触发出码/运行/对齐；
