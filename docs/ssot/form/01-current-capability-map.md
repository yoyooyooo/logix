...
## Final Surface Read Rule (2026-05-11)

Read every capability below through the final single-track surface in `13-exact-surface-contract.md`. Form capabilities map to one of seven owner lanes only: declaration, source, companion, final truth/reason, row identity, host selector, or verification. No capability is allowed to justify `@logixjs/form/react`, package-local `useForm*`, public `Form.Path`, `Form.Source`, public row token, generic `Fact/SoftFact`, or second report/evidence surface.
...
example-side `useFormField / useFormList / useFormMeta` 若仍在历史材料中出现，只能作为 toolkit-candidate / historical-residue 讨论；current source 不应保留 Form-owned hook family，且它们不构成 canonical owner。
