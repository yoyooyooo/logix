...
> **Final cutover note (2026-05-11):** Current Form API authority is `ssot/form/13-exact-surface-contract.md`. Historical gap wording about Form-owned hook families or helper families is superseded by the single-track route: `Form.make -> FormProgram -> FormHandle -> useModule + useSelector`.
...
- exact noun and import shape are now frozen by `13-exact-surface-contract.md`
- `useFormList` / Form-owned hook family is excluded from current core/domain route; if pursued, it must live as toolkit-only sugar and mechanically lower to core host selectors
