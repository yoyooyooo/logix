...
## Final API Resolution (2026-05-11)

The capability/scenario matrix is now closed against the single-track Form API from [13-exact-surface-contract.md](./13-exact-surface-contract.md):

```text
Form.make -> field.rule / field.source / field.companion / root / list / submit
FormHandle -> validate / submit / mutation / fieldArray.byRowId
React read -> useModule + useSelector + fieldValue/rawFormMeta/Form.Error/Form.Companion descriptors
Verification -> Runtime.check / Runtime.trial / Runtime.compare
```

All `CAP-*`, `SC-*`, `PF-*`, and `VOB-*` references in this file must be interpreted through that exact surface. Rejected shapes such as `@logixjs/form/react`, package-local `useForm*`, public `Form.Path`, public `Form.Source`, public row tokens, generic `Fact/SoftFact`, second report objects, and raw evidence default compare surfaces are negative-space examples only; they are not alternate coverage lanes.
