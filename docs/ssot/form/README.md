...
> **Final cutover note (2026-05-11):** The current Form SSoT is single-track. Read `13-exact-surface-contract.md` first; old candidate wording that keeps Form-owned hooks, `Form.Path`, `Form.Source`, schema mapping helpers, or package-local React entrypoints is superseded.
...
- `list row identity public projection contract`
  ...
  - 已冻结：exact noun、import shape、single-track host read route；`useFormList` / Form-owned hook family 不进入 current core/domain route
...
## Final single-track cutover pointer

Current final Form execution must read [13-exact-surface-contract.md](./13-exact-surface-contract.md) and [14-final-single-track-cutover-gate.md](./14-final-single-track-cutover-gate.md) before any older route, proposal, challenge packet, demo, or implementation handoff. Historical files that mention forbidden old public nouns are trace-only unless explicitly restated in those two pages.
