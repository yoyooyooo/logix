...
2. **需要源码入口/回归索引时先读压缩包**：先读 `docs/ssot/handbook/codebase-playbook.md`，再让 auggie 去命中其中列出的入口/内核/测试落点。
...
- `请检索并返回 docs/ssot/handbook/codebase-playbook.md 的标题与“核心链路索引”目录（确认源码导航压缩包可命中）。`
