...
- internal-only field-kernel declaration asset 经 `Logic` 声明，并在 `Program.make(...)` 统一 merge / compile
- `Runtime` 不再承担 internal-only field-kernel declaration merge / build
...
- internal-only field-kernel 只作为内部 declaration compiler 层存在，不额外抬升成公开家族
