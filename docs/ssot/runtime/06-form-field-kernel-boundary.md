...
- 本页只负责 form 与 internal-only field-kernel 的边界
...
- 下层共享 internal-only compile-time `field-kernel`
...
- `Form.Path / Form.SchemaPathMapping / Form.SchemaErrorMapping` 已退出 root public surface；current source 不应保留这些 public helper files，路径/错误路径 helper 只允许在 package-internal implementation 中存在
- React exact host law 继续停留在 [./10-react-host-projection-boundary.md](./10-react-host-projection-boundary.md)

`@logixjs/form/react` 不再作为 current route；任何 UI binding sugar 只能进入 toolkit candidate，并且必须机械降解到 core host law。
...
internal-only field-kernel 继续只承接：
...
- `Form.from(...).logic`、`config.logic` 不再作为 current route；若历史材料仍出现，只能作为 archived migration residue
...
## Final cutover kernel boundary

Final Form cutover keeps field-kernel internal. Source scheduling, companion lowering, list-scope validation, dirtyPlan execution, and fallback reason evidence may be optimized internally, but they must not create public Form field-kernel nouns, second owner laws, or compatibility routes. Full-topo fallback must be reason-coded and diagnostics-off paths must avoid trace payload allocation.
