...
- app-level runtime object 由 app 拥有，通常在模块作用域创建并传给 `RuntimeProvider`
- route/page runtime object 只有在该 route/page 自己创建时才由该 route/page 拥有
- React host component 通过 props、context 或上层 module scope 接收 runtime 时，只借用这个 runtime
- `RuntimeProvider` 卸载、DEV carrier reset 或 host cleanup 只关闭 host-owned resources，不释放借用的 runtime
- Program 与 ModuleTag module instances 默认由 `ModuleCache` 拥有；`gcTime` 只管理 module instance scope，不改变 base runtime ownership
- `runtimeOwnership: "owned"` 只作为 DEV lifecycle carrier 的内部 dispose guard，不是标准业务 authoring choice
