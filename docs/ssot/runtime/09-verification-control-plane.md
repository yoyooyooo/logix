...
`KernelStabilityReport` 是 repo-internal release gate artifact，不是新的 runtime control-plane public facade。
它只汇总既有 guard、contract test、static sweep 和 explicit evidence refs。
它不得暴露为 `Runtime.*` API，不得运行 benchmark suite，不得把 `UNKNOWN` 当成 `PASS`，也不得在缺少 comparable perf artifact 时声明 broad performance improvement。
...
Live bridge selector-route observation、host commit evidence、profile evidence、snapshot evidence 都是 canonical evidence inputs，不能默认成为 check/startup verdicts。

- 它们可以作为 evidence artifact/event facets、degradation notices 或 evidence gaps 进入 Workbench projection。
- 它们只能通过 admissible captured reports、evidence summaries 或 artifact refs 进入 compare。
- 除非存在 explicit scenario evidence 或 repo-internal host-harness artifact authority，它们不得产生 `runtime.check` 或 `runtime.trial(mode="startup")` verdicts。
- 任何带 selector-quality 或 host-quality claim 的 selector-route、host-commit、profile 或 snapshot facet 都必须携带 `stageClass`: `static`、`startup`、`scenario`、`host-harness` 或 `drilldown-only`。
- `static` 与 `startup` stage classes 需要 control-plane report 或 artifact authority。live bridge observation 不能铸造这些 authority。
- `scenario` 与 `host-harness` stage classes 需要 explicit scenario evidence 或 repo-internal host-harness artifact authority。
- `drilldown-only` 可以由 Workbench 或 DVTools 展示，但不能作为 check/startup verdict；它只能通过 captured evidence summary 或 artifact refs 影响 compare。
...
Live operation evidence summary 是 `evidence` coordinate 的一部分。它可以引用 manifest digest、action tag、payload schema 或 validator availability refs、runtime coordinate、binding status、structured denial reason、result summary digest、budget、dropped、degraded、redaction markers。它不创建新的 report、stage、verdict 或 evidence envelope。

当前 repo-internal static-live binding proof 由 167 `RuntimeReflectionManifest` 派生 binding header。运行时 live bridge 只能消费该 header 并把它写入 canonical live operation facet；reflection 不拥有 active runtime event、operation result、session identity 或 Workbench truth。

171 Batch 6 operation safety 对 live operation evidence 的范围收窄为：

- P1 operation kinds 是 `target.discover`、`capture.eventWindow`、`snapshot.read`、`wait.condition`、`evidence.export`、`dispatch.declaredAction` 和 local-only `profile.runtimeSummary`。
- Mutation-capable `dispatch.declaredAction` 在 runtime scheduling 前必须通过 actor、capability、target coordinate、manifest digest、action tag、payload schema 或 validator availability、binding status、budget、redaction gates。
- 任何 precondition failure 都产出 `operation.denied` 且 no mutation。它不得被报告为 `operation.failed`。
- `operation.failed` 只表示 post-admission runtime failure evidence。
- Browser CPU profile integration、heap snapshot、remote/cloud mutation、long-running stream、cross-process aggregation 不属于 171 的 P1 evidence closure。

171 Batch 7 researchability header 只能作为 evidence metadata 进入 compare：

- allowed header data 包含 evidence digest、capture window、stage/admissibility class、runtime coordinate、manifest/source/build/env/budget/sampling/redaction refs、proof command refs、带 owner/unit 的 metric refs、dropped/degraded/redacted markers 和 evidence gaps。
- `metricRef[]` 不定义 metric family authority。
- decision trace family、adoption algebra、adoption ledger、adopt/discard verdict、autonomous loop state 都不是 171 的 verification control-plane authority。
- 当 headers 显示 environment、budget、sampling、window、digest 或 proof command refs 不匹配时，Compare 可以 reject evidence 或标记 inconclusive。

### Hot lifecycle evidence

Hot lifecycle evidence 由现有 canonical evidence envelope 承载。

- Event type: `runtime.hot-lifecycle`
- Decision set: `reset | dispose`
- 必需 payload coordinates: owner id、lifecycle event id、cleanup id、previous runtime instance id、optional next runtime instance id、cleanup status、resource summary、residual active count
- 可选 host payload: host cleanup summary，用于 React-owned binding cleanup
- Evidence producer: runtime internals 通过 Effect DI 或等价 internal layer boundary，从 host dev lifecycle carrier 接收 lifecycle owner、registry 和 evidence services
- 它不创建第二套 HMR report protocol。
- 它不创建新的 `runtime.*` root command。
- Browser HMR evidence artifacts 必须把 evidence 映射回该 envelope。
- Example-visible lifecycle helpers 不能作为本能力的有效 closure evidence。Closure evidence 必须展示普通 example authoring code 加 host dev lifecycle carrier activation。
- Production import checks 必须证明 dev lifecycle carrier modules 不存在，除非导入的是 dev-only entrypoint、conditional export 或等价 static boundary。

`runtime.hot-lifecycle` 是 evidence event shape，不是 authoring surface。
...
Harness、proof、fixture、support、coverage inventory 的通用资产卫生固定看 [../../standards/harness-and-proof-assets-standard.md](../../standards/harness-and-proof-assets-standard.md)。本页只定义 verification control plane 的 authority boundary，不定义文件命名、编号保留或顺手纠偏细则。
...
这条路径表达能力升级顺序，不表示每次 offline repair closure 都必须执行 scenario。

184-186 terminal offline Agent self-verification family 的 repair closure path 可以在 `check` 或 `trial(mode="startup")` 的 before/after reports 之间显式运行 `runtime.compare`，用于证明修复闭合、回归、mismatch 或 admissibility inconclusive。该 compare closure 不要求 scenario proof。`trial(mode="startup")` PASS 只表示 default gate complete，不等同于 before/after repair-loop closed。
...
`190-kernel-release-gate-profile` 可以把默认门禁、explicit compare、live evidence handoff、perf gate 与 text sweep 聚合为 repo-local release gate projection。该 projection 不新增 `runtime.*` stage，不改变默认门禁，不创建第二 report shape，也不把 live evidence 提升为 verification verdict。
...
Live-derived evidence 对齐规则：

- `logix live` 只能提供 repair clues，例如 operation denial reason、evidence gap、degraded marker、target coordinate、binding header、source/declaration refs 和 artifact refs。
- `logix live` 不得输出 `repairHints`、`nextRecommendedStage` 或 verification verdict。
- live-derived evidence 要产生修复建议，必须先进入 canonical evidence package，再由 `runtime.trial` 或 `runtime.compare` 产出 `VerificationControlPlaneReport.repairHints`。
- 当 live-derived evidence 能局部化失败时，`repairHints[].focusRef` 与 `repairHints[].relatedArtifactOutputKeys` 必须尽量保留到 live evidence artifact、binding header 或 owner-approved stable coordinate 的追溯链。
- 当前 targeted proof 覆盖 `logix live export evidence -> trial --evidence` 与 examples/logix-react live bridge route；`logix compare` 消费 live-derived before/after evidence 的完整 closure 仍属后续 proof。

CLI 对齐约束：

- `@logixjs/cli` 的 verification lane 只认 `check / trial / compare`；live runtime collaboration lane 归 `15` 的 `logix live <task>`
...
## Form final cutover wording

For Form final cutover, `Runtime.check` and `Runtime.trial` are the current verification routes. `Runtime.compare` remains part of the runtime control-plane family only when admitted by explicit authority; it is not a default Form root compare product and must not create a second report shell or raw-evidence default truth.
