...
`188-react-host-adjunct-evidence` 将 React host evidence 接入 live runtime evidence 时，它仍只能作为 SSoT 18 定义的 adjunct sidecar：

- selector evidence 只能引用 core route identity、selector fingerprint 或诊断 label
- render evidence 只能引用 host render boundary ref 或 host harness artifact ref
- host evidence 不得自建 selector authority、runtime ordering、timeline completeness、field semantics 或 verification verdict
- public machine artifact route 必须保持在 `15-cli-agent-first-control-plane.md` 已准入的 live artifact kind、canonical evidence package ref 或 structured gap 内；若需要 `HostEvidence` / `HostAdjunctEvidence` public artifact kind，必须同时重开 18 与 15
- disabled host evidence capture 必须证明无额外 render subscription fanout、无 host capture buffer、无 transaction-window IO
- host attachment、interaction linkage、source/module refs 和 route refs 只能作为 provenance marker 或 gap，不能成为 Runtime truth
...
- `RuntimeProvider runtime={runtime}` 借用传入的 runtime，不拥有也不释放这个 runtime
...
- Provider unmount, carrier reset, and DEV cleanup may close host-owned resources, but must not call `runtime.dispose()` for a runtime supplied by app, route, page, or parent scope.
- `runtimeOwnership` is a carrier-internal dispose authority. The default carrier binding is `borrowed`; only an explicitly `owned` internal binding may call `runtime.dispose()`.
- `runtimeOwnership` must stay out of route discovery, live inspect target snapshots, CLI machine output, and public React authoring props.
- `useModule(Program)` module instance lifetime remains owned by `ModuleCache`; `gcTime` retain/release/GC closes the module instance scope without transferring base runtime ownership to the carrier.
...
仓内任何 repo-local `useForm*`、list helper 或薄封装，若仍在历史材料中出现，只算 residue；current source 不应保留 Form-owned hook family，它们不构成 exact host contract。
...
- Form corollary：
  ...
  - `@logixjs/form/react` 与仓内任何 repo-local `useForm*` / list wrapper 不属于 current route；如需 UI sugar，只能作为 toolkit candidate 且不得拥有 host truth
...
## Form host read final gate

Form does not own a React hook family. Form descriptors such as `Form.Error.*` and `Form.Companion.*` are data-support for the core `useSelector` route. `@logixjs/form/react`, `useForm*`, `useCompanion`, `useFieldSource`, and `useFormSelector` are forbidden as current public or toolkit-canonical host law.
