...
它的核心任务是把 runtime evidence、control-plane report、canonical evidence event/artifact facet 和 artifact 转成可定位、可比较、可行动的诊断视图。
...
- 所有结论都能回到统一 evidence envelope、`VerificationControlPlaneReport`、canonical evidence event/artifact facet 或已归一化的 repo-internal debug drilldown feed
...
- canonical live evidence event or artifact facets for operation, selector-route, host-commit, profile and snapshot evidence
...
- session、finding、artifact attachment、source coordinate 与 evidence gap 的跨宿主投影语义委托给 Runtime Workbench Kernel；manifest/action/payload 的共享解释语义委托给 167 repo-internal reflection contract；active operation、selector-route、host-commit、profile 与 snapshot 事实只能来自 canonical live evidence facets。
...
DVTools 可以展示 canonical evidence event facet、state snapshot artifact、transaction summary、field graph、converge lane、hot lifecycle event 等下钻视图。

这些视图必须满足：

- 只从 canonical evidence、canonical evidence artifact refs、degradation notices、evidence gaps 或已经归一化的 repo-internal debug drilldown feed 派生
- 不拥有新 truth
- 不影响 runtime 执行语义
- 不回流为 public authoring surface

Repo-internal debug drilldown feed 是 pre-normalized drilldown source。它进入 Runtime Workbench Kernel `truthInputs` 前必须先变成 canonical evidence event/artifact facet、artifact ref、degradation notice 或 evidence gap。DVTools 不得把 raw debug feed 作为 Workbench truth input。

171 targeted implementation 进一步固定：DVTools host normalization 可接受 debug drilldown live facets，但只能输出 Workbench `live-evidence` truth input 或 `evidence-gap` truth input。DVTools 的 session、finding、artifact attachment model 只是对 Runtime Workbench Kernel projection index 的 view model，不是 panel-owned truth。Selection export 继续只产生 canonical evidence package 和 `selectionHints`，不能把 selected session/finding/artifact 写回 truth input。
...
171 采纳 `C171-I Shared Workbench Projection Hosts` 后，DVTools 处置固定为：

| capability | disposition | rule |
| --- | --- | --- |
| Session Workbench | 保留 | 只消费 Runtime Workbench Kernel projection index |
| Evidence Package Import / Export | 保留 | 只能输出 canonical evidence package 和 selection manifest |
| Control-Plane Report Explainer | 保留 | 展示 `VerificationControlPlaneReport`，不展开成第二 report protocol |
| Runtime Debug Drilldown | 保留为 drilldown | raw feed 必须先归一成 canonical facet、artifact ref、degradation 或 gap；171 targeted proof 已覆盖 live facet/gap normalization |
| Agent Repair Coordinate Surface | 保留 | 只镜像 focusRef、artifact outputKey、runtime coordinate 和 owner locator |
| raw timeline | 下沉 | 二级 tab/drawer，不作为默认主心智 |
| state snapshot JSON | 下沉 | artifact drilldown，不作为 session truth |
| FieldGraph / converge lane | 下沉 | 内部联动视图 |
| settings / thresholds | 下沉 | host view state，不改变 evidence truth |
| time travel / state mutation | 暂停 | 默认关停；任何重开必须先通过 Batch 6 allowlist 或未来 operation spec |
| panel-only fact derivation | 裁剪 | 不能创建 session/finding/artifact truth |
| public package surface | 裁剪 | survivor set 继续归零 |

DVTools 与 Playground、Agent route 的共同证明义务是：同一 canonical evidence package 或 live session projection 得到等价 session、finding、artifact、coordinate、metric、degradation 和 evidence gap ids。
...
| 层 | owner | 角色 |
| --- | --- | --- |
| evidence contract | `@logixjs/core` | evidence envelope、canonical evidence event/artifact facets、control-plane report |
