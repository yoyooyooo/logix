...
| live bridge target discovery、operation admission、canonical evidence export、Workbench projection、compare handoff | [../../../specs/171-agent-live-runtime-bridge/spec.md](../../../specs/171-agent-live-runtime-bridge/spec.md) |
...
- 本页不得把 live bridge debug flow 写成默认 verification stage 或 `CommandResult` authority；public live CLI surface 归 [15-cli-agent-first-control-plane.md](./15-cli-agent-first-control-plane.md) 持有。
...
进入 `covered` 的 repair-loop cluster 必须至少证明一条端到端 proof pack：
...
只证明单个 row 会失败不等于 repair-loop closure 成立。没有 before/after 修复声明时，`trial startup` PASS 只能说明 default gate complete。
...
| `INV-LIVE` | live bridge closes through evidence | live target discovery、operation admission、capture 和 export 只产生 live command transport、canonical evidence、Workbench projection、structured denial 或 evidence gap，不产生 report/verdict/stage truth |
...
| `TP-ENTRY-01` | `INV-ENTRY`, `INV-ROUTE` | entry 必须拒绝 Module、Logic、fake Program、missing blueprint；locator failure 不伪造 next stage | `check` | `entry` | `covered` | none | `PA-REPORT`, `PA-STAGE`, `PA-RERUN`, `PA-REPEAT`, `PA-SURFACE` | `packages/logix-cli/test/Integration/program-entry.contract.test.ts`; `packages/logix-cli/test/Integration/command-result-transport.contract.test.ts` |
...
| `TP-LIVE-01` | `INV-LIVE`, `INV-ARTIFACT` | `logix live` target discovery、static-live binding、operation admission/denial、capture window、canonical evidence export、Workbench projection 与 trial evidence handoff 必须形成 dogfood proof pack；live-derived before/after compare deep closure 仍是 future extension | `none` | `evidence` | `covered` | none | `PA-REPORT`, `PA-FOCUS`, `PA-ARTIFACT`, `PA-SURFACE` | core `test/internal/LiveBridge`; CLI `live-*` integration tests; DevTools/Playground live projection tests; `examples/logix-react` targeted live bridge Playwright proof; `examples/logix-react test:browser:playground`; `packages/logix-react/test/browser/perf-boundaries/live-bridge-disabled-overhead.test.tsx`; `specs/171-agent-live-runtime-bridge/perf/diff.worktree.live-bridge.default.json`; `packages/logix-cli/test/Integration/live-evidence-handoff.e2e.test.ts` |
...
## Kernel Release Gate Projection

`190-kernel-release-gate-profile` 把外部 CLI/kernel 审查意见吸收到 repo-local release gate 视角。它只是本页 pressure rows、`15` CLI 合同、`09` report/compare authority、`18` live evidence owner law、`02` hot-path perf gate 与 `189` discovery recipe 的派生投影。

它不得定义新的 stage、command、report schema、live artifact truth、perf budget 或 domain package owner law。任何实现或 release note 使用该 profile 时，必须按下表回到 owner 页面取证：

| gate row | owner authority | required evidence |
| --- | --- | --- |
| `publicSurface` | `01`, `15`, guardrails | public roots 仍为 `check / trial / compare / live`，无 `describe/debug/challenge` 支持路线 |
| `authoringSpine` | `01`, `03` | CLI entry 与 domain projection 沿 `Module.logic -> Program.make -> Runtime.make` |
| `entryDeclarationAuthority` | `184`, `16` rows `TP-ENTRY-01`, `TP-STATIC-01` | Program entry、fake Program、missing export、missing blueprint 与 static declaration proof |
| `fieldDeclarationCompiler` | `01`, `09`, field-kernel cutover ADR | declaration asset 仍由 `Program.make` 编译，Runtime 不重建 field truth |
| `runtimeLifecycle` | `09`, `16` row `TP-LIFECYCLE-01` | startup boot/close/readiness failure 保持 structured report 与 artifact linking |
| `transactionSafety` | guardrails, runtime owner specs | 事务窗口无 IO，业务层不直接写 `SubscriptionRef`，无第二事务语义 |
| `selectorPrecision` | `02`, `10`, selector route specs | selector route、dirty/read overlap、fallback policy 有 proof 或 explicit perf gate |
| `dependencyTrial` | `09`, `16` row `TP-DEPENDENCY-01` | service/config/import/child dependency 进入 typed dependency causes |
| `liveEvidenceBoundary` | `15`, `18`, `187`, `16` row `TP-LIVE-01` | live 只产 evidence/gap/operation facet，repair 只能由 trial/compare report 产生 |
| `diagnosticsOffCost` | `02`, active perf spec | touched diagnostics/live rows 必须证明 disabled/off path 不引入未预算开销 |
| `perfBroadStrict` | `02`, active perf spec | selector、dirty、external-store、RuntimeStore、Form list 等 touched suites 必须有 comparable evidence |
| `domainBoundary` | `08`, guardrails | Form/Query/i18n/domain 只作同一 spine 投影，不产生第二 runtime/host/report truth |
| `discoveryConsumption` | `189`, `15` | static schema、skill mirror、docs recipe 同步，无 help-text machine discovery |
| `legacyResidueSweep` | guardrails, harness standard | 旧 CLI/toolbox/Probe/Witness/Pressure 等命中必须分类为 rejected/internal-only/negative-only/archived/future |

外部分析中的 challenge packs 对应关系固定为：

- Authoring / declaration -> `TP-ENTRY-01`, `TP-SOURCE-01`, `TP-STATIC-01`
- Dependency / lifecycle -> `TP-DEPENDENCY-01`, `TP-LIFECYCLE-01`
- Repair closure -> `TP-LOOP-01`, `TP-COMPARE-01`, `TP-SCHED-01`
- Live evidence handoff -> `TP-LIVE-01`
- Hot path budget -> `02-hot-path-direction.md` 和 active perf spec

任何 `Kernel Release Gate Profile` artifact 都只能作为 release/handoff evidence bundle。它不能命名为 public CLI command、public report schema 或 runtime truth owner。
...
- current terminal offline Agent self-verification family：[../../../specs/184-entry-declaration-authority/spec.md](../../../specs/184-entry-declaration-authority/spec.md)、[../../../specs/185-repair-intent-contract/spec.md](../../../specs/185-repair-intent-contract/spec.md)、[../../../specs/186-verification-loop-orchestration/spec.md](../../../specs/186-verification-loop-orchestration/spec.md)
...
| --- | --- | --- | --- | --- |
| `GAP-LOOP-CLOSURE` | `186 + 09 + 15` | `TP-LOOP-01`, `TP-COMPARE-01` | `closed` | terminal family closure 由 `186` 持有并消费 184/185 proof lanes；`161` 提供 core report/compare proof substrate，`162` 提供 CLI exact rerun、before/after report refs 与 transport proof substrate |
| `GAP-ENTRY-BLUEPRINT` | `184 + 15 + 09` | `TP-ENTRY-01` | `closed` | `184` 已覆盖 Module、Logic、fake Program、missing export、import failure、missing blueprint transport gate 与 no-next-stage；`162` 提供 transport gate artifact substrate；missing blueprint 的 core check pressure 仍由 `161` 覆盖 |
...
| `GAP-CHECK-STATIC` | `09` | `TP-ENTRY-01`, `TP-SOURCE-01`, `TP-STATIC-01` | `closed` | `Runtime.check` static pressure 已由 `161` 覆盖；CLI entry boundary 已由 `184` proof 覆盖 |
...
| `GAP-LIVE-BRIDGE-DOGFOOD` | `171 + 15 + 09 + 165` | `TP-LIVE-01` | `closed` | core/CLI in-process handoff、static-live binding、targeted examples/logix-react live bridge Playwright route、full playground browser route suite、run -> live evidence -> trial repair handoff 与 live bridge disabled overhead perf proof 已覆盖；full before/after live compare deep closure 是 future extension |
| `GAP-LIVE-OPERATION-ADMISSION` | `171` | `TP-LIVE-01` | `closed` | mutation-capable `dispatch.declaredAction` 已覆盖 static-live binding、structured denial、missing validator、digest mismatch、stale manifest、unauthorized target 与 no-mutation |
| `GAP-LIVE-EVIDENCE-EXPORT` | `171 + 09` | `TP-LIVE-01` | `closed` | live operation/capture/gap facets 已通过 Workbench truth input、trial evidence handoff 与 `VerificationControlPlaneReport.repairHints` 回链；host-commit/profile/snapshot 的更深矩阵和 compare before/after deep closure 归 future extension |
| `GAP-LIVE-DVTOOLS-PLAYGROUND-PARITY` | `171 + 14 + 165 + 168` | `TP-LIVE-01` | `closed` | 同一 live evidence truth input 已在 Core Workbench、DVTools、Playground targeted tests 得到等价 projection；full playground browser route suite 已通过 |
| `GAP-LIVE-RESEARCHABILITY-HEADER` | `171 + future research spec` | `TP-LIVE-01` | `future` | 171 只提供 comparable evidence header 和 gap；metric family、decision trace family、adoption algebra 与 adoption ledger 归 future spec |

Live bridge proof route:

- 171 必须采用 harness-first 路线，具体入口见 [../../../specs/171-agent-live-runtime-bridge/implementation-details/harness-path.md](../../../specs/171-agent-live-runtime-bridge/implementation-details/harness-path.md)。
- Browser dogfood proof 的可关闭证据必须来自可重复命令，例如 Playwright/browser test、Vitest、CLI proof script、perf command 或 text sweep。
- `agent-browser` 只作为 exploratory QA 与 failure inspection 工具；它产出的 screenshot、snapshot、console/error/network、trace/profile/recording 或 stream artifact 不能单独关闭 `TP-LIVE-01`、`W171-001`、`W171-006` 或 `W171-009`。
- `run -> evidence -> repair` 关闭时，live output 只能提供 repair clues；最终 repair hints 必须来自 `VerificationControlPlaneReport.repairHints`，并能追溯到 live-derived evidence artifact 或 owner-approved stable coordinate。
...
- `CommandResult` 继续只作 verification transport；machine report authority 仍是 `VerificationControlPlaneReport`。`logix live` 使用 `LiveCommandResult`，不进入 report authority。
...
- 让 CLI 自己定义 dependency truth、import truth、runtime truth 或第二 report shape。
...
Agent 自验证闭环的终局压力已经从 failure catalog 收敛为 invariant-first matrix：`184 -> 185 -> 186` 是当前 terminal offline Agent self-verification family，分别关闭 entry/declaration lane、repair intent lane 与唯一 loop closure gate。`161` 与 `162` 继续作为 core/kernel 与 CLI transport proof substrate。剩余明确 future 项指向 scenario executor、host deep trial，以及 live-derived before/after compare deep closure。
