...
- 行动（Actuation）：`$.state` / `$.actions` / `$.dispatchers` / `$.dispatch` / `$.effect` —— 读写当前 Module 状态（`read / update / mutate / ref`）；`$.actions` 只创建可序列化 action object（定义视图/纯数据）；`$.dispatchers` 返回可 `yield*` 的 Effect（执行视图/派发，默认推荐）；`$.dispatch(...)` 仅兼容/低阶派发入口（非默认写法）；`$.effect(...)` 是副作用注册面（事务外触发、可诊断/可去重）；
