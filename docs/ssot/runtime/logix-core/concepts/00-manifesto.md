...
- **在平台视角下**: 作为 `SDD Platform` 体系中的一个前端 Runtime Target（`logix-engine`），与 Effect Flow Runtime 一同承接 Logic Intent / Flow 编排的执行职责。Intent/Flow/Runtime 的概念模型与 Schema 以 `docs/ssot/platform` 为事实源（尤其是 `docs/ssot/platform/foundation/03-trinity-and-layers.md`、`docs/ssot/platform/assets/00-assets-and-schemas.md` 与 `docs/ssot/platform/contracts/00-execution-model.md`），Logix 文档只说明“如何在前端运行时实现这些契约”。
