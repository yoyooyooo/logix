...
- Logix 不重新定义 Intent / Flow / Constraint 的模型，相关概念与 Schema 以平台 SSoT 为事实源：
  - 三位一体 Intent 模型：`docs/ssot/platform/foundation/03-trinity-and-layers.md`；
  - 资产与 Schema 映射：`docs/ssot/platform/assets/00-assets-and-schemas.md`；
  - Runtime 家族与执行口径：`docs/ssot/platform/contracts/00-execution-model.md`。
- 如需回顾历史探索与教训，统一参考：`docs/ssot/platform/governance/decisions/history-and-lessons.md`。  
  Logix 只定义“这些 Intent 如何在前端运行时中落地”为 Module 定义（State/Action Schema）、ModuleRuntime 配置（初始状态等）与 Logic 规则。

## 2. Intent 三维与 Logix 的关系

参考平台 SSoT 中的三位一体模型（UI / Logic / Module），Logix 的覆盖范围可以概括为：
...
| **Logic Intent**  | 运行时目标之一          | 当 `runtimeTarget = 'logix-engine'` 时，Logic Intent 的实现以 `ModuleDef.logic(($)=>...)` 的 Fluent DSL（`$.onState/$.onAction/$.on + .update/.mutate/.run*`）为白盒入口，平台侧解析为 IntentRule/Graph；当目标为其他 Runtime 时，Logix 只负责触发/展示结果。 |
...
- Layout / View / Interaction Intent 现在被统一纳入 UI / Logic 维度（参见 `docs/ssot/platform/foundation/03-trinity-and-layers.md` 中的说明）；
...
3. **状态结构**（Data & State Intent）
   - Data & State Intent 声明的字段/实体 Schema 映射到 Module 定义与初始状态：
     - `stateSchema: Schema<S>` 与 `initialState: S`（对应 `Logix.ModuleShape` 与 `ModuleDef.live(initial, ...)`）；
...
最终结果是在某个 feature 下生成一个或多个 Logix Module 定义 + Logic 配置（或 JSON DSL），由 UI 通过 `@logixjs/react` 适配层接入。
...
Logix 端的 JSON 解释器与 Logic DSL 仅是“承接这些 Intent 的一种落地形式”，其 Schema 设计应以上游平台 SSoT 的 Intent / Flow / Constraint 模型为准（参见 `docs/ssot/platform/foundation/03-trinity-and-layers.md`、`docs/ssot/platform/assets/00-assets-and-schemas.md` 与 `docs/ssot/platform/contracts/00-execution-model.md`），避免再出现第二套事实源。

## 6. Intent ↔ Logix API 映射表

为方便在实现与出码时快速定位，下面给出一份从 Intent 模型到 Logix 核心 API 的直观映射表（只列出常用字段，详细语义以各自规范为准）：

| Intent / Schema 字段                                                                 | Logix 对应位置                                                                 | 典型写法 / 说明                                                                                                                                                                                                                                                             |
| :----------------------------------------------------------------------------------- | :----------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `IntentSpec.ui[*]`（UI Intent 节点）                                                 | Module 使用侧（UI 层）                                                         | 由平台生成的 React 组件树，通过 `@logixjs/react` 订阅某个 ModuleRuntime 的状态、派发 Actions；Logix 不直接建模 UI 节点，只消费其发出的事件。                                                                                                                                  |
...
| `DomainIntentNode` / `DomainImplConfig.fields`                                       | `Logix.Module.make('Id', { state, actions })` / `ModuleDef.live(initial, ...)` | Domain 字段 Schema 编译为 `stateSchema: Schema.Schema<S>`，同时生成对应的 `initialState: S`，共同决定某个 Logix.Module 的 State 形状与初始值。                                                                                                                              |
| `DomainImplConfig.services`                                                          | ModuleRuntime Env + Logic 中 `$.use`                                           | Domain 服务契约映射为 `Services` 接口与具体实现对象，通过 Layer 注入到 ModuleRuntime 的环境，在 Logic 中通过 `$.use(ServiceTag)` 或 `services.Xxx` 调用。                                                                                                                   |
...
| `LogicImplConfig.testCases`                                                          | `@logixjs/test` / Round-trip 测试                                                | 平台可将这些用例转换为基于 Logix ModuleRuntime 的 Round-trip 测试（参见 `docs/ssot/runtime/logix-test` 系列文档），通过注入 Mock Services、构造输入、断言 State/Signals。                                                                                                             |

在实际出码链路中，平台会先根据 Intent/Flow Schema 生成中间层的 Flow DSL/JSON（与 `docs/ssot/platform/appendix/intent-to-code-example.md` 的演练一致），再由 Logix 编译器将其中 `runtimeTarget = 'logix-engine'` 的部分落地为上述 Module 定义 / ModuleRuntime 配置与 Logic 规则。
