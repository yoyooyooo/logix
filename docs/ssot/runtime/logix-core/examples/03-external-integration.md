# Example: External Integration (Standard Paradigm)

> **Scenario**: 实时股票看板
> **Features**: WebSocket 接入、外部事件响应、资源管理
> **Note**: 本示例是当前主线中处理外部事件源的 **黄金标准实现**。它演示了如何将外部流（如 WebSocket）封装为 `Effect.Service`，并通过依赖注入在 `Logic` 中安全地消费。实际代码应在对应 ModuleDef 上通过 `ModuleDef.logic(($)=>...)` 获取 `$`。

## 1. Schema Definition

```typescript
import { Schema, Context, Effect, Stream } from 'effect';
import { Store, Logic } from '~/logix-core'; // 概念性路径
...
## 2. Service Definition (Effect Service)
...
export const StockDef = Logix.Module.make('Stock', {
  state: StockStateSchema,
  actions: StockActionSchema,
});

export const StockLive = StockDef.live(
  { ...initialValues },
  StockLogic,
);
...
- **Architectural Purity**: 将外部源视为标准的 `Effect.Service`。这确保了所有逻辑，无论是响应内部状态还是外部事件，都遵循统一的依赖注入和生命周期管理模型，使得 `Logic` 单元保持纯粹，只关注业务规则。
