# Matrix Examples: System Capabilities (Standard Paradigm)

> **Focus**: 多租户隔离、错误流路由、权限控制  
> **Note**: 本文示例基于当前主线 Effect-Native 标准范式，展示了如何利用 `Layer` 和 `Effect` 的原生能力来实现系统级的横切关注点。实际代码应在对应 ModuleDef 上通过 `ModuleDef.logic(($)=>...)` 获取 `$`。

## S22: 多租户隔离 (Multi-Tenant)

**标准模式**: 通过 `Layer` 的组合来为不同租户提供不同的服务实现。`Logic` 代码本身保持租户无关，只依赖于服务的抽象接口 (`Tag`)。
...
**标准模式**: `Logic` 中返回的 `Effect` 的错误通道 (`E`) 会被 `Store` 的运行时捕获。可以通过订阅 `Store` 暴露的全局错误流来构建统一的错误处理机制。
...
**标准模式**: 在 `Effect` 内部通过调用注入的 `AuthService` 来实现运行时的权限检查，并使用 `Effect.if` 或 `$.match` 来执行相应的逻辑分支。
