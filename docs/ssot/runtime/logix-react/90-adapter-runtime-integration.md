...
- Props 约定：
  - `runtime?: ManagedRuntime<never, any>`：复用外部创建的 Runtime（推荐形态，通常来自 `Logix.Runtime.make(root, { layer, onError })`；`root` 可为 program module 或其 `.impl`）；
...
- 支持三类句柄：
  - `ModuleTag`：身份锚点（Context.Tag），用于接入当前 React Runtime 环境中的“全局实例”；
  - `ModuleImpl` / `Module`（wrap）：用于创建/管理局部（会话/组件级）实例；
  - `ModuleRuntime`：已存在的运行时实例。
- 调用形态：
  - `useModule(ModuleTag | ModuleRuntime)`：返回稳定的 `ModuleRuntime` 引用；
...
- 基于 `useSyncExternalStoreWithSelector` 实现，签名为：
  ...
  - `handle` 可以是 ModuleRuntime 或 ModuleTag；
...
- 签名：`useDispatch(handle)`，其中 `handle` 为 ModuleTag 或 ModuleRuntime；
- 内部行为：
