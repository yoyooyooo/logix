...
- 超时保护：Host 维护 `timeout`（可配置；默认 10s）计时器，`init/compile/run` 任一阶段超时则 **hard reset**：`worker.terminate()` + 清空状态/映射 + Promise reject；不尝试软中断。  
- 复位：Host 侧 `terminate()` 直接 hard reset 并把状态回到 `idle`（用于 Cancel/Reset）；主线程 Hook 到 React/DevTools 时要避免内存泄漏。  
- 有界缓存：Host 侧收集到的 `logs/traces/uiIntents` 默认有界（`maxLogs/maxTraces/maxUiIntents`），避免长时间运行导致内存无上限增长。
