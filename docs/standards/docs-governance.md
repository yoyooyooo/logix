...
## Final cutover trace-only governance

For final single-track cutovers, stale files must not remain as active narrative. Use one of: `authority`, `accepted`, `implemented`, `consumed`, `trace-only`, `superseded`, `archived`, `deferred-authority-intake`, `blocked`. Files marked `trace-only`, `superseded`, or `archived` that mention obsolete public API must put a current-authority tombstone in the first 1600 bytes.
