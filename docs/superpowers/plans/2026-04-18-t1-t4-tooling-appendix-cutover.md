...
要求：
- `check.command`、`trial.command`、`compare.command`、`output-contract` 继续证明 `check / trial / compare` 是 verification lane 的唯一 public route；live runtime collaboration route 由后续 `logix live <task>` 合同单独证明
