...
export const AddressFormPage: React.FC<AddressFormPageProps> = (props) => {
  ...
  const country = FormReact.useField<AddressForm.Values, string>(controller, 'defaultCountry')

  const addresses = FormReact.useFieldArray<AddressForm.Values, AddressForm.AddressLine>(controller, 'addresses')
  ...
  return (
    <form onSubmit={handleSubmit}>
      <input value={country.value} onChange={(e) => country.onChange({ target: { value: e.target.value } })} />
      ...
      {addresses.fields.map((item, index) => (
        <div key={item.id}>
          <input
            ...
            onChange={(e) =>
              addresses.replace(
                addresses.fields.map((field, i) => (i === index ? { ...field, label: e.target.value } : field)),
              )
            }
          />
