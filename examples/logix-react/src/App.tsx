...
import {
  formItems,
  logixReactDemoCategories,
  logixReactDirectDemoRoutes,
  type DemoCategory,
  type DemoItem,
} from './demoRouteRegistry'
import './style.css'
...
const LogixReactPlaygroundRoute = React.lazy(() =>
  import('./playground/routes').then((module) => ({ default: module.LogixReactPlaygroundRoute })),
)

const allKnownRoutes = new Set(logixReactDirectDemoRoutes)
...
const getActiveCategory = (pathname: string): DemoCategory => {
  if (isPlaygroundPath(pathname)) return logixReactDemoCategories[1]!
  if (pathname === '/') return logixReactDemoCategories[0]!
  return logixReactDemoCategories.find((category) => category.items.some((item) => item.route === pathname) || category.route === pathname) ?? logixReactDemoCategories[0]!
}
...
function PrimaryNavigation({
  ...
  return (
    <nav className="space-y-1">
      {logixReactDemoCategories.map((category) => {
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        {logixReactDemoCategories.filter((category) => category.id !== 'start').map((category) => {
          const Icon = category.icon
          return (
        ...
        <ScrollArea className="min-h-0 flex-1">
          <div className="mx-auto max-w-6xl p-4 lg:p-8">
            ...
            <Routes>
              ...
              <Route path="/host-root-provider" element={<DiShowcaseLayout view="root-provider" />} />
              <Route path="/host-imports-scope" element={<DiShowcaseLayout view="imports-scope" />} />
