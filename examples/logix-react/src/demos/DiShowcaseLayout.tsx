...
import { CounterImpl, CounterModule } from '../modules/counter'
import { CounterMultiImpl, CounterMultiModule } from '../modules/counterMulti'
...
const ImportedHostModule = Logix.Module.make('DiShowcaseImportedHost', {
  state: Schema.Void,
  actions: { noop: Schema.Void },
})

const ImportedHostImpl = ImportedHostModule.implement({
  initial: undefined,
  imports: [CounterMultiImpl],
})

const importsRuntime = Logix.Runtime.make(ImportedHostImpl, {
...
const CounterPanel: React.FC<{
  ...
  const ref =
    kind === 'global'
      ...
      : options
        ? useModule(CounterImpl, options)
        : useModule(CounterImpl)
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="space-y-1">
          ...
          <div className="text-[11px] text-gray-500 dark:text-gray-400 font-mono break-all">
            runtimeId: {String(ref.runtime.id)}
          </div>
        </div>
...
const RootResolveProbe: React.FC = () => {
  ...
  React.useEffect(() => {
    ...
    void runtime
      .runPromise(
        Logix.Root.resolve(EnvTag).pipe(
          Effect.map((env) => env.name),
        ),
      )
      .then(setRootEnv)
  }, [runtime])
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
      ...
      <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
        <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-[11px] font-mono">RuntimeProvider.layer</code>{' '}
        只影响“当前子树”的 Env；当你需要显式拿到 root provider 的单例（类似 Angular 的 root provider），使用{' '}
        <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-[11px] font-mono">Logix.Root.resolve(Tag)</code>
        。
      </div>
    </div>
  )
...
const ImportsPanel: React.FC = () => {
  const host = useModule(ImportedHostModule)
  const childByGet = host.imports.get(CounterMultiModule)
  const childByHook = useImportedModule(host, CounterMultiModule)
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-2">
        ...
        <div className="text-[11px] text-gray-500 dark:text-gray-400 font-mono break-all">runtimeId: {String(host.runtime.id)}</div>
        <div className="text-xs text-gray-500 dark:text-gray-400 leading-relaxed">
          子模块只在 Host 的 imports-scope 内可见：在 React 里优先通过{' '}
          <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-[11px] font-mono">host.imports.get(Child.module)</code>{' '}
          或{' '}
          <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-[11px] font-mono">useImportedModule(host, Child.module)</code>{' '}
          解析（避免串实例）。
        </div>
      </div>
      ...
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 p-4 space-y-3">
        <div className="flex items-start justify-between gap-3">
          <div className="space-y-1">
            ...
            <div className="text-[11px] text-gray-500 dark:text-gray-400 font-mono break-all">
              runtimeId(get): {String(childByGet.runtime.id)}
              <br />
              runtimeId(hook): {String(childByHook.runtime.id)}
            </div>
          </div>
  ...
  return (
    <div className="space-y-10">
      <div className="border-b border-gray-200 dark:border-gray-800 pb-6">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">DI Showcase（Root / Imports / Instances）</h2>
      ...
      <RuntimeProvider runtime={counterRuntime} layer={RootEnvLayer}>
        <div className="space-y-6">
          ...
          >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <CounterPanel title='同 key 复用（key="shared" · 视图 1）' kind="local" options={{ key: 'shared', label: 'Shared' }} />
              <CounterPanel title='同 key 复用（key="shared" · 视图 2）' kind="local" options={{ key: 'shared', label: 'Shared' }} />
            </div>
          </Card>
          ...
          >
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <RuntimeProvider layer={FeatureEnvLayer}>
                <div className="space-y-3">
                  <div className="text-xs text-gray-500 dark:text-gray-400">
      ...
      <RuntimeProvider runtime={importsRuntime}>
        <Card
