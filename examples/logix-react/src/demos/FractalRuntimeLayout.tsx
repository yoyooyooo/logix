  ...
  React.useEffect(() => {
    ...
    try {
      void runtime
        ...
        .then((nextThemeName) => {
          if (!cancelled) setThemeName(nextThemeName)
        })
        .catch((error) => {
          if (!cancelled) setThemeName(error instanceof Error ? error.message : String(error))
        })
    } catch (error) {
      if (!cancelled) setThemeName(error instanceof Error ? error.message : String(error))
    }
    ...
    <RuntimeProvider runtime={appRuntime} layer={GlobalThemeLayer}>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <RuntimeProvider runtime={appRuntime} layer={FeatureThemeLayer}>
