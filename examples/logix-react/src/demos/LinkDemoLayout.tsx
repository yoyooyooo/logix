...
import {
  AuditDef,
  AuditModule,
  ReactMultiModuleLink,
  SourceDef,
  SourceModule,
  TargetDef,
  TargetModule,
} from '../modules/linkModules'

// 组合三个 ModuleImpl 的 Layer，构造应用级 Runtime
const appLayer = Layer.mergeAll(
  Logix.Debug.runtimeLabel('LinkDemo'),
  Logix.Debug.devtoolsHubLayer(),
  SourceModule.impl.layer,
  TargetModule.impl.layer,
  AuditModule.impl.layer,
)
...
const SourcePanel: React.FC = () => {
  const source = useModule(SourceDef)
...
const TargetPanel: React.FC = () => {
  const count = useModule(TargetDef, (s) => s.count)
...
const AuditPanel: React.FC = () => {
  const logs = useModule(AuditDef, (s) => s.logs)
      ...
      <div className="mt-1 space-y-1 max-h-40 overflow-auto text-xs text-gray-600 dark:text-gray-300 font-mono">
        {logs.length === 0 ? (
          ...
          logs.map((line, idx) => (
            <div
              key={idx}
              className="px-2 py-1 rounded bg-gray-50 dark:bg-gray-800/60 border border-gray-100 dark:border-gray-700/70"
            >
              {line}
            </div>
          ))
      ...
      <div className="space-y-8">
        <div className="border-b border-gray-200 dark:border-gray-800 pb-6">
          ...
          <p className="text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed">
            ...
            实现跨模块编排：点击 <code>ReactLinkSource</code> 的 ping，会驱动 <code>ReactLinkTarget</code> 计数并在{' '}
            <code>ReactLinkAudit</code> 中记录日志。
          </p>
        </div>
