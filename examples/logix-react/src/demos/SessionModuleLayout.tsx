...
import { RuntimeProvider, useModule, useSelector, useDispatch, useRuntime, ReactPlatformLayer } from '@logixjs/react'

// Session 级 ModuleImpl 示例：演示 useModule(Impl, { key, gcTime }) 的会话保活语义。

const SessionCounterDef = Logix.Module.make('SessionCounter', {
  state: Schema.Struct({
    count: Schema.Number,
...
const SessionCounterLogic = SessionCounterDef.logic(($) => ({
  setup: Effect.gen(function* () {
    yield* $.lifecycle.onStart(
      Effect.gen(function* () {
        // 模拟一次较重的初始化过程（例如远程配置拉取），仅影响
        // 模块内部状态（ready），不阻塞后续 Watcher 的注册与 Action 处理。
...
const SessionCounterModule = SessionCounterDef.implement({
  initial: { count: 0, ready: false },
  logics: [SessionCounterLogic],
})

const SessionCounterImpl = SessionCounterModule.impl
...
const SessionTab: React.FC<SessionTabProps> = ({ tabId, gcTimeMs }) => {
  const ref = useModule(SessionCounterImpl, {
    key: `session:${tabId}`,
    label: `Session ${tabId}`,
    gcTime: gcTimeMs,
  })

  const state = useSelector(ref)
