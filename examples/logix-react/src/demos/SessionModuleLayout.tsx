...
import { RuntimeProvider, fieldValue, useModule, useSelector, useDispatch } from '@logixjs/react'

// Session 级 Program 示例：演示 useModule(Program, { key, gcTime }) 的会话保活语义。

const SessionCounter = Logix.Module.make('SessionCounter', {
  state: Schema.Struct({
    count: Schema.Number,
...
const SessionCounterLogic = SessionCounter.logic('session-counter-logic', ($) => {
  return Effect.gen(function* () {
    yield* Effect.log('SessionCounterLogic setup')
    // 模拟一次较重的初始化过程（例如远程配置拉取），仅影响模块内部状态。
...
const SessionCounterProgram = Logix.Program.make(SessionCounter, {
  initial: { count: 0, ready: false },
  logics: [SessionCounterLogic],
})

// 简单的应用级 Runtime：提供基础 Env，
// ModuleRuntime 由 useModule(Program) 构造并交给 ModuleCache 管理。
const sessionRuntime = ManagedRuntime.make(
  Layer.mergeAll(
    CoreDebug.runtimeLabel('SessionModuleDemo'),
...
const SessionTab: React.FC<SessionTabProps> = ({ tabId, gcTimeMs }) => {
  const ref = useModule(SessionCounterProgram, {
    key: `session:${tabId}`,
    label: `Session ${tabId}`,
    gcTime: gcTimeMs,
  })

  const count = useSelector(ref, fieldValue('count'))
  const ready = useSelector(ref, fieldValue('ready'))
  const dispatch = useDispatch(ref)
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 flex flex-col gap-4">
      ...
      <div className="flex flex-col items-center justify-center py-6 bg-gray-50 dark:bg-gray-800/50 rounded-lg border border-gray-100 dark:border-gray-800">
        ...
        <span className="text-[11px] text-gray-500 dark:text-gray-400">
          ready:{' '}
          <code className="px-1.5 py-0.5 bg-gray-100 dark:bg-gray-800 rounded text-[11px] font-mono text-emerald-700 dark:text-emerald-300">
            {ready ? 'true' : 'false'}
          </code>
        </span>
        <span className="text-[11px] text-gray-500 dark:text-gray-400">
    ...
    <RuntimeProvider runtime={sessionRuntime}>
      <div className="space-y-6">
        <div className="border-b border-gray-200 dark:border-gray-800 pb-6">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">会话级 Program 示例（key + gcTime）</h2>
