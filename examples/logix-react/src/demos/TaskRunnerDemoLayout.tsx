...
const TaskRunnerDemoView: React.FC = () => {
  ...
  const fireRefresh = () => {
    const id = nextId
    setNextId((n) => n + 1)
    dispatch({ _tag: 'refresh', payload: id })
  }
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed">
          ...
          <code className="font-mono text-[12px]">runExhaustTask</code>：每次触发会被拆分成 pending（立即提交）→
          IO（事务外）→ writeback（再次提交）。打开右下角 Devtools 的 Timeline， 可以看到两笔 state:update：
          <code className="font-mono text-[12px]">task:pending</code> 与{' '}
          <code className="font-mono text-[12px]">task:success</code>。
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">runLatestTask（最新优先）</h3>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-5 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-gray-900 dark:text-white">runExhaustTask（防重）</h3>
