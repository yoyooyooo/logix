...
const Panel: React.FC = () => {
  ...
  const host = useModule(QuerySearchDemoHostImpl)

  const runtime = host.imports.get(SearchQuery.module)
  const dispatch = useDispatch(runtime)

  const params = useSelector(runtime, (s) => s.params)
  const ui = useSelector(runtime, (s) => s.ui)
  const search = useSelector(runtime, (s) => s.search)
  const detail = useSelector(runtime, (s) => s.detail)
  ...
  const refresh = (target?: string) => dispatch({ _tag: 'refresh', payload: target })
