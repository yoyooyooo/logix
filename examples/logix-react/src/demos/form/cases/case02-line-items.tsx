...
const LineItemsForm = Form.make("FormCase.LineItems", {
  ...
  traits: Logix.StateTrait.from(LineItemsValues)({
    ...
    "summary.itemCount": Logix.StateTrait.computed({
      deps: ["items"],
      get: (s: Readonly<LineItemsV>) => s.items.length,
    }),
    "summary.totalQty": Logix.StateTrait.computed({
      deps: ["items"],
      get: (s: Readonly<LineItemsV>) => s.items.reduce((acc, it) => acc + it.quantity, 0),
    }),
    "summary.totalAmount": Logix.StateTrait.computed({
      deps: ["items"],
      get: (s: Readonly<LineItemsV>) => s.items.reduce((acc, it) => acc + it.quantity * it.price, 0),
    }),
  }),
})

const runtime = Logix.Runtime.make(LineItemsForm.impl, {
...
const LineItemsCase: React.FC = () => {
  const form = useForm(LineItemsForm)
  ...
  return (
    <div className="space-y-6">
      <SectionTitle title="Case 02 · 订单行表格" desc="动态数组（Form.array*）+ trackBy(id) + items[] check + 汇总 computed。" />
          ...
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-3 text-xs font-mono">
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3">
                itemCount: <span className="text-blue-600 dark:text-blue-300">{String(state?.summary?.itemCount ?? 0)}</span>
              </div>
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3">
                totalQty: <span className="text-blue-600 dark:text-blue-300">{String(state?.summary?.totalQty ?? 0)}</span>
              </div>
              <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-3">
                totalAmount:{" "}
                <span className="text-blue-600 dark:text-blue-300">{Number(state?.summary?.totalAmount ?? 0).toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
