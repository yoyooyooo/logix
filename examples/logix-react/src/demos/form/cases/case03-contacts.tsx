...
const $ = Form.from(ContactsValues)
const z = $.rules
    ...
    z.list('contacts', {
      ...
      list: {
        ...
        validate: (rows: ReadonlyArray<any>) => {
          const contacts: ReadonlyArray<any> = Array.isArray(rows) ? rows : []
          const primaryIndices: Array<number> = []
          for (let i = 0; i < contacts.length; i++) {
            if (contacts[i]?.isPrimary === true) primaryIndices.push(i)
          }
          if (primaryIndices.length <= 1) return undefined

          const rowErrors: Array<Record<string, unknown> | undefined> = contacts.map(() => undefined)
          for (const i of primaryIndices) {
            rowErrors[i] = { $item: '主联系人只能有一个' }
          }
...
const runtime = Logix.Runtime.make(ContactsForm, {
...
const ContactRow: React.FC<{
  ...
  const rowError = useSelector(form.runtime as any, (s) => (s as any)?.errors?.contacts?.rows?.[index]) as any
  ...
  return (
    ...
    >
      ...
      {rowError?.$item ? (
        <div className="text-[11px] text-rose-600 dark:text-rose-400">{String(rowError.$item)}</div>
      ) : null}
...
const ContactsCase: React.FC = () => {
  const form = useModule(ContactsForm)
  ...
  return (
    <div className="space-y-6">
      <SectionTitle
        title="Case 03 · 联系人列表"
        desc="list scope check（跨行约束）+ trackBy(id) + errors/ui 数组对齐。"
      />
