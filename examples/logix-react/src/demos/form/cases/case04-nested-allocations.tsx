...
const NestedForm = Form.make("FormCase.NestedAllocations", {
  ...
  traits: Logix.StateTrait.from(NestedValues)({
...
const runtime = Logix.Runtime.make(NestedForm.impl, {
...
const AllocationRow: React.FC<{
  ...
  return (
    <div key={fieldsId} className="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/40 p-3 space-y-2">
...
const CostItemCard: React.FC<{
  ...
  return (
    <div key={fieldsId} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/60 p-4 space-y-3">
      <div className="flex items-center justify-between">
  ...
  return (
    <div className="space-y-6">
      <SectionTitle
        ...
        desc="items[] → allocations[] 的增删改；观察 errors/ui 的数组对齐与清理行为（当前 nested item check 仍以 computed/自定义为主）。"
      />
