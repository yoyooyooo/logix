...
const UniqueForm = Form.make("FormCase.UniqueCode", {
  ...
  traits: Logix.StateTrait.from(UniqueValues)({
    codeCheck: Logix.StateTrait.source({
      resource: CodeAvailabilitySpec.id,
      deps: ["code"],
      triggers: ["onValueChange"],
      debounceMs: 200,
      concurrency: "switch",
      key: (s: Readonly<UniqueV>) => {
...
const runtime = Logix.Runtime.make(UniqueForm.impl, {
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm p-6 space-y-4">
            ...
            <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/40 p-4 space-y-2">
              ...
              {check?.status === "loading" ? <div className="text-sm text-amber-700 dark:text-amber-300">Loading…</div> : null}
              {check?.status === "error" ? <div className="text-sm text-rose-700 dark:text-rose-300">Error</div> : null}
              {check?.status === "success" ? (
                <div className={availability ? "text-sm text-emerald-700 dark:text-emerald-300" : "text-sm text-rose-700 dark:text-rose-300"}>
                  {availability ? "可用" : "已占用"}
                </div>
              ) : null}
            </div>
            ...
            <div className="flex gap-2">
              ...
              <GhostButton type="button" onClick={() => dispatch({ _tag: "setValue", payload: { path: "code", value: "" } })}>
                清空 code（应回到 idle）
              </GhostButton>
            </div>
          </div>
