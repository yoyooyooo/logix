...
const UploadForm = Form.make("FormCase.AttachmentsUpload", {
  ...
  traits: Logix.StateTrait.from(UploadValues)({
    attachments: Logix.StateTrait.list({
      identityHint: { trackBy: "id" },
      item: Logix.StateTrait.node({
        source: {
          upload: Logix.StateTrait.source({
...
const runtime = Logix.Runtime.make(UploadForm.impl, {
...
const AttachmentRow: React.FC<{
  ...
  return (
    <div key={fieldsId} className="rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/60 p-4 space-y-3">
      ...
      {status === "success" ? <div className="text-xs font-mono text-emerald-700 dark:text-emerald-300 break-all">{url}</div> : null}
    </div>
  )
}
