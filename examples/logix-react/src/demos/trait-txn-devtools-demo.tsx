    ...
    <div className="space-y-8">
      <section className="border-b border-gray-200 dark:border-gray-800 pb-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed">
          这个页面把表单 Traits 与 StateTransaction / Devtools 组合在一起，方便你按照文档中的
          「事务视图 → 时间旅行 → 渲染事件 → 时间轴总览条」路径做端到端验证。
          建议在浏览器右下角打开 Logix Devtools 面板，然后依次体验下方两个示例。
        </p>
      ...
      <section className="space-y-4">
        ...
        <p className="text-xs text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed">
          先在「表单脏标记 Traits」区域中缓慢输入几次文本，然后在 Devtools 中选中对应的事务：
          你应该只看到一条 <code className="px-1 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">state:update</code>{" "}
          事件，以及与该事务共享同一 <code className="px-1 py-0.5 bg-gray-100 dark:bg-gray-800 rounded">txnId</code>{" "}
          的 Trait / 渲染事件。
        </p>
        <TraitFormDemoLayout />
      </section>
      ...
      <section className="space-y-4">
        ...
        <p className="text-xs text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed">
          在 ComplexTraitForm 中尝试添加多条订单行项目或频繁修改联系人信息，再结合 Devtools 的 Timeline、
          React 渲染事件视图和 Overview Strip，观察一次交互引出的 Trait 行为、字段 Patch 数量和渲染次数。
          如有需要，可以在应用代码中切换 StateTransaction 的观测策略（full / light），对比性能表现。
        </p>
        <ComplexTraitFormDemoLayout />
      </section>
    </div>
  )
}
