...
export const ComplexTraitFormTraits = StateTrait.from(ComplexTraitFormStateSchema)({
  ...
  'validation.formValid': StateTrait.computed({
    ...
    get: (email, preferredChannel, phone, items) => {
      ...
      const hasBadItem =
        ...
        (items as any)?.some(
          (item: any) => String(item?.name ?? '').trim() === '' || (item?.quantity ?? 0) <= 0 || (item?.price ?? 0) < 0,
        )
  ...
  'summary.totalQuantity': StateTrait.computed({
    deps: ['items'],
    get: (items) =>
      (Array.isArray(items) ? items : []).reduce((acc: number, item: any) => acc + (item?.quantity ?? 0), 0),
  }),
  'summary.totalAmount': StateTrait.computed({
    deps: ['items'],
    get: (items) =>
      (Array.isArray(items) ? items : []).reduce(
        (acc: number, item: any) => acc + (item?.quantity ?? 0) * (item?.price ?? 0),
        0,
      ),
  }),
...
export const ComplexTraitFormDef = Module.make('ComplexTraitFormModule', {
  state: ComplexTraitFormStateSchema,
  actions: ComplexTraitFormActions,
  traits: ComplexTraitFormTraits,
})

const ComplexTraitFormLogic = ComplexTraitFormDef.logic(($) =>
  Effect.gen(function* () {
    yield* Effect.all(
      [
...
export const ComplexTraitFormModule = ComplexTraitFormDef.implement({
  initial: {
    profile: {
      firstName: '',
      lastName: '',
