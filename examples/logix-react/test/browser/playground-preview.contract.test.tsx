...
const toTreePath = (workspacePath: string): string => workspacePath.replace(/^\//, '')

const getFileTreePathElement = (workspacePath: string): HTMLElement => {
  const treePath = toTreePath(workspacePath)
  const element = document.querySelector('file-tree-container')?.shadowRoot?.querySelector(`[data-item-path="${treePath}"]`)
  if (!(element instanceof HTMLElement)) throw new Error(`Expected file tree path ${workspacePath} to be an HTMLElement`)
  return element
}

const expectFileTreePath = (workspacePath: string): void => {
  expect(getFileTreePathElement(workspacePath)).toBeTruthy()
}

const clickFileTreePath = (workspacePath: string): void => {
  getFileTreePathElement(workspacePath).click()
}
...
test('registered React project renders the runtime workbench', async () => {
  ...
  try {
    ...
    expectFileTreePath('/src/logic/localCounter.logic.ts')
...
test('bare Playground route opens the default React project', async () => {
  ...
  try {
    await expect.element(screen.getByText('Logix Playground')).toBeInTheDocument()
    expectFileTreePath('/src/main.program.ts')
...
test('shared source edit updates runtime Run output', async () => {
  ...
  try {
    clickFileTreePath('/src/logic/localCounter.logic.ts')
