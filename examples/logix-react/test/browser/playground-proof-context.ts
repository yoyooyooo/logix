...
export const toFileTreePath = (workspacePath: string): string =>
  workspacePath.replace(/^\//, '')

export const waitForFileTreePath = async (
  page: Page,
  workspacePath: string,
  label: string,
): Promise<void> => {
  const treePath = toFileTreePath(workspacePath)
  ...
  const visible = await page.evaluate((targetPath) => {
    const element = document.querySelector('file-tree-container')?.shadowRoot?.querySelector(`[data-item-path="${targetPath}"]`)
    return element instanceof HTMLElement && element.offsetParent !== null
  }, treePath)
  assertCondition(visible, `${label} should be visible`)
}

export const clickFirstNonActiveFileTreePath = async (page: Page): Promise<void> => {
  const clicked = await page.evaluate(() => {
    const host = document.querySelector('file-tree-container')
    const rows = Array.from(host?.shadowRoot?.querySelectorAll<HTMLElement>('[data-item-path][data-item-type="file"]') ?? [])
    const target = rows.find((row) => row.getAttribute('aria-selected') !== 'true') ?? rows[1]
    target?.click()
    return target != null
  })
  assertCondition(clicked, 'file navigator should expose a non-active file row')
}
