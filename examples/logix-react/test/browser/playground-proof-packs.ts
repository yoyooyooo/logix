...
export const assertAllRouteInvariants = async ({ page, baseUrl, project, recipe }: RunProofPackInput): Promise<void> => {
  ...
  if (project.program?.entry) {
    await waitForFileTreePath(page, project.program.entry, `${project.id} program entry`)
  }
...
export const runFacetProofPack = async (
  ...
  if (packId === 'serviceFiles') {
    await waitForFileTreePath(page, '/src/services/search.service.ts', `${project.id} service source file`)
    return
  }
  ...
  if (packId === 'liveBridgeDogfood') {
    await getHostCommand(page, 'Run').click()
    const result = page.getByRole('region', { name: 'Program result' }).getByLabel('Run result')
    await result.waitFor({ state: 'visible' })
    const text = await result.textContent()
    assertCondition(text?.includes('runtime:example-runtime/module:LiveBridgeFixture/instance:default'), `${project.id} should expose live target coordinate`)
    assertCondition(text?.includes('live-evidence:example-runtime'), `${project.id} should expose live evidence package id`)
    assertCondition(text?.includes('operation.denied'), `${project.id} should expose denied dispatch facet`)
    assertCondition(!text?.includes('repairHints'), `${project.id} live result should not contain repair hints`)
    assertCondition(!text?.includes('nextRecommendedStage'), `${project.id} live result should not contain verification scheduling`)
    return
  }
