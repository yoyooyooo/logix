...
const routes = {
  ...
  bare: '/playground',
  hostNestedProviders: '/host-nested-providers',
  hostRootProvider: '/host-root-provider',
  hostImportsScope: '/host-imports-scope',
  hostEnvOverride: '/host-env-override',
...
async function main() {
  ...
  try {
    ...
    const directDemoRoutes = await loadDirectDemoRoutes(server)
    ...
    await run('direct demo routes render first shell', () => assertDirectDemoRoutesRender(page, baseUrl, directDemoRoutes))
    await run('host scenario direct routes', () => assertHostScenarioDirectRoutes(page, baseUrl))
    await run('bare route opens default project', () => assertBareRoute(page, baseUrl))
    await run('bare route remounts after gallery navigation', () => assertBareRouteRemountsAfterGalleryNavigation(page, baseUrl))
...
async function loadDirectDemoRoutes(server: ViteDevServer): Promise<ReadonlyArray<string>> {
  const mod = await server.ssrLoadModule('/src/demoRouteRegistry.tsx') as {
    readonly logixReactDirectDemoRoutes: ReadonlyArray<string>
  }
  return mod.logixReactDirectDemoRoutes
}
...
async function gotoExample(page: Page, baseUrl: string, path: string, heading: string): Promise<void> {
  await page.goto(`${baseUrl}${path}`, { waitUntil: 'networkidle' })
...
async function assertBareRoute(page: Page, baseUrl: string): Promise<void> {
  ...
  await expectFileTreePath(page, 'src/main.program.ts', 'default program entry')
...
async function assertBareRouteRemountsAfterGalleryNavigation(page: Page, baseUrl: string): Promise<void> {
  ...
  await page.getByRole('heading', { name: 'Explore Logix through stable user-facing paths.' }).waitFor({ state: 'visible' })
  await page.getByRole('link', { name: /Open Playground/ }).click()
  await page.waitForURL(`${baseUrl}${routes.bare}`)
  await page.getByText('Logix Playground').waitFor({ state: 'visible' })
  ...
  await page.waitForURL(`${baseUrl}${routes.bare}`)
  await page.getByText('Logix Playground').waitFor({ state: 'visible' })
  await expectVisible(page.getByLabel('Playground project').getByText('new project'), 'default project label after remount')
  await expectFileTreePath(page, 'src/main.program.ts', 'default program entry after remount')

  assert.deepEqual(pageErrors, [], 'playground route should not raise page errors after gallery remount')
  page.removeAllListeners('pageerror')
}
...
async function assertTopProjectSwitcher(page: Page, baseUrl: string): Promise<void> {
  ...
  await expectFileTreePath(page, 'src/services/search.service.ts', 'service source file')
...
async function assertDirectDemoRoutesRender(
  page: Page,
  baseUrl: string,
  directDemoRoutes: ReadonlyArray<string>,
): Promise<void> {
  for (const route of directDemoRoutes) {
    const pageErrors: Array<string> = []
    page.removeAllListeners('pageerror')
    page.on('pageerror', (error) => {
      pageErrors.push(error.message)
    ...
    await page.waitForTimeout(route === '/async-local-program' || route === '/session-program' ? 2500 : 800)

    const bodyText = ((await page.locator('#app').innerText().catch(() => '')) || '').trim()
    const hasHeading = await page.locator('main h1, main h2').first().isVisible().catch(() => false)
    const interactiveCount = await page.locator('main button, main input, main a').count().catch(() => 0)

    assert(bodyText.length > 0, `${route} should not render a blank app shell`)
    assert(hasHeading, `${route} should render a stable heading`)
    assert(interactiveCount > 0, `${route} should expose an interactive first shell`)
    assert.deepEqual(pageErrors, [], `${route} should not raise page errors`)
  }

  page.removeAllListeners('pageerror')
}

async function assertHostScenarioDirectRoutes(page: Page, baseUrl: string): Promise<void> {
  await gotoExample(page, baseUrl, routes.hostNestedProviders, 'Host Projection · Nested Providers')
  await page.getByText('主题：').first().waitFor({ state: 'visible' })
  await expectVisible(page.getByText('GlobalTheme'), 'nested providers root theme')
  await expectVisible(page.getByText('FeatureTheme'), 'nested providers subtree theme')
  await expectVisible(page.getByRole('button', { name: '+1' }).first(), 'nested providers counter command')

  await gotoExample(page, baseUrl, routes.hostEnvOverride, 'Host Projection · Env Override')
  await expectVisible(page.getByRole('button', { name: /根据 Step 增加/ }).first(), 'env override root command')
  await expectVisible(page.getByText('根 Provider 子树 · Step = 1'), 'env override root panel')
  await expectVisible(page.getByText('嵌套 Provider 子树 · Step = 5'), 'env override nested panel')

  await gotoExample(page, baseUrl, routes.hostRootProvider, 'Host Projection · Root Provider')
  await expectVisible(page.getByRole('button', { name: '+1' }).first(), 'root provider counter command')
  assert.equal(
    await page.getByText('imports scope：host.imports.get / useImportedModule').count(),
    0,
    'root provider route should not require reaching imports scope through the same page',
  )

  await gotoExample(page, baseUrl, routes.hostImportsScope, 'Host Projection · Imports Scope')
  await expectVisible(page.getByRole('button', { name: 'increment' }), 'imports scope increment command')
  assert.equal(
    await page.getByText('Root provider singleton').count(),
    0,
    'imports scope route should not require reaching root provider content through the same page',
  )
}
...
async function assertPressureFileTree(page: Page, context: string): Promise<void> {
  await expectVisible(page.getByRole('navigation', { name: 'File navigator' }), `${context} file navigator`)
  await expectFileTreePath(page, 'src/', `${context} src folder`)
  await expectFileTreePath(page, 'src/actions.ts', `${context} actions source`)
  await expectFileTreePath(page, 'src/drivers.ts', `${context} drivers source`)
  await expectFileTreePath(page, 'README.md', `${context} readme source`)
}

async function expectFileTreePath(page: Page, path: string, label: string): Promise<void> {
  const visible = await page.evaluate((targetPath) => {
    const host = document.querySelector('file-tree-container')
    return host?.shadowRoot?.querySelector(`[data-item-path="${targetPath}"]`) instanceof HTMLElement
  }, path)
  assert(visible, `${label} should be visible`)
}
