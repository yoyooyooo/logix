...
describe('example HMR host carrier setup', () => {
  ...
  it('keeps host activation in Vite and Vitest boundaries', () => {
    ...
    expect(viteConfig).toContain('LOGIX_LIVE_PORT')
    expect(viteConfig).toContain('@logixjs\\/react\\/dev\\/live')
  ...
  it('provides a Vite serve plugin that injects the live bridge bootstrap', () => {
    const plugin = logixReactDevLifecycle({ live: { host: '127.0.0.1', port: 8098, projectId: 'example' } })
    const html = plugin.transformIndexHtml?.('<html><head></head><body></body></html>')

    expect(html).toContain('@logixjs/react/dev/live')
    expect(html).toContain('@logixjs/react/dev-live-browser-adapter-options')
    expect(html).toContain('projectId: "example"')
  })
