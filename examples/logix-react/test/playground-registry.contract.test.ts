...
import { logixReactPlaygroundScenarioMappability as playgroundScenarioMappability } from '../src/playground/scenarioMappability'
...
describe('logix-react Playground registry', () => {
  ...
  it('maps local counter Playground scenario metadata to the verification corpus without creating executable input', async () => {
    const row = playgroundScenarioMappability.find(
  ...
  it('registers live bridge dogfood route as Logic-first evidence handoff fixture', () => {
    const project = resolvePlaygroundProject(logixReactPlaygroundRegistry, 'logix-react.live-bridge')

    expect(project?.program?.entry).toBe('/src/main.program.ts')
    expect(project?.preview).toBeUndefined()
    expect(project?.files['/src/main.program.ts']?.content).toContain('liveBridgeDogfood')
    expect(project?.files['/src/main.program.ts']?.content).toContain('operation.denied')
    expect(project?.files['/src/main.program.ts']?.content).toContain('CanonicalEvidencePackage')
    expect(JSON.stringify(project)).not.toContain('repairHints')
    expect(JSON.stringify(project)).not.toContain('nextRecommendedStage')
