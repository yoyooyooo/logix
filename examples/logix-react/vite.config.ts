...
export default defineConfig({
  plugins: [
    ...
    logixReactDevLifecycle({
      live: {
        ...
        port: Number(process.env.LOGIX_LIVE_PORT ?? '8098'),
        projectId: process.env.LOGIX_LIVE_PROJECT_ID ?? 'examples/logix-react',
      },
    }),
    jsToTsResolver(),
    react(),
  ...
  resolve: {
    alias: [
      ...
      { find: /^@logixjs\/react\/dev\/live$/, replacement: path.resolve(__dirname, '../../packages/logix-react/src/dev/live.ts') },
