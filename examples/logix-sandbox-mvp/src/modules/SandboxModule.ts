...
export const SandboxState = Schema.Struct({
  ...
  stepIntentScriptDraft: Schema.String,
})
...
export const SandboxDef = Logix.Module.make('SandboxModule', {
  state: SandboxState,
  actions: {
    init: Schema.Void,
    ...
    setStepIntentScriptDraft: Schema.String,
  },
  ...
  reducers: {
    ...
    setAutoImportLogix: (state, action) => ({ ...state, autoImportLogix: action.payload }),
    addLog: (state, action) => ({
      ...state,
    ...
    setScenarioSteps: (state, action) => ({
      ...state,
      scenarioSteps: action.payload,
    }),
    setMockManifestSource: (state, action) => ({
      ...state,
      mockManifestSource: action.payload,
    }),
    setSemanticWidgets: (state, action) => ({
      ...state,
      semanticWidgets: action.payload,
    }),
    setIntentScript: (state, action) => ({
      ...state,
      intentScript: action.payload,
    }),
    setKernelId: (state, action) => ({ ...state, kernelId: action.payload }),
    setKernelStrict: (state, action) => ({ ...state, strict: action.payload }),
    setKernelAllowFallback: (state, action) => ({
      ...state,
      allowFallback: action.payload,
    }),
    ...
    updateStepIntentScript: (state, action) => {
      ...
      const newFeatures = state.specFeatures.map((feature) => ({
        ...feature,
        stories: feature.stories.map((story) => ({
          ...story,
          scenarios: story.scenarios.map((scenario) => ({
            ...scenario,
            steps: scenario.steps.map((step) => (step.id === stepId ? { ...step, intentScript } : step)),
          })),
        })),
      }))
      return { ...state, specFeatures: newFeatures }
    },
    setStepIntentScriptDraft: (state, action) => ({
      ...state,
      stepIntentScriptDraft: action.payload,
    }),
  },
})
