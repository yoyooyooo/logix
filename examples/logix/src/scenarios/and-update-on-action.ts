...
export const DirtyFormLive = DirtyFormImpl.layer
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const program = Effect.gen(function* () {
    const runtime = yield* FormModule

    // Log state changes
    yield* Effect.fork(
      runtime.changes((s) => s).pipe(
        Stream.runForEach((s) => Effect.log(`[State] value="${s.value}", isDirty=${s.isDirty}`))
      )
    )
  ...
  void Effect.runPromise(
    program.pipe(Effect.provide(DirtyFormLive)) as Effect.Effect<void, never, never>
  )
}
