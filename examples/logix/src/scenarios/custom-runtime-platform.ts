...
const ReactPlatformLive: Layer.Layer<Logix.Platform.Service, never, never> =
  Layer.succeed(Logix.Platform.tag, {
    lifecycle: {
      onSuspend: (eff: Effect.Effect<void, never, any>) =>
        ...
        Stream.make(1).pipe(
          // Simulating an event
          Stream.tap(() =>
            Console.log('[ReactPlatform] Detected visibility: hidden'),
          ),
...
const NodePlatformLive: Layer.Layer<Logix.Platform.Service, never, never> =
  Layer.succeed(Logix.Platform.tag, {
    lifecycle: {
...
const PollingImpl = PollingModule.implement({
  initial: {},
  logics: [PollingLogic],
})
...
export const WebRuntime = Logix.Runtime.make(PollingImpl, {
  // ✨ MAGIC HERE: We inject the React behavior
  layer: ReactPlatformLive,
})

// Runtime 2: CLI Tool (Uses Node Platform)
export const CliRuntime = Logix.Runtime.make(PollingImpl, {
