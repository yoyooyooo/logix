...
PROBE_THRESHOLD_FAILURE_RETURN_CODE = 42
...
class BrowserProbeRun:
    ...
    threshold_anomaly_count: int
    threshold_anomalies: list[dict[str, Any]]
    perf_report_parse_errors: list[str]
...
def _parse_perf_reports(stdout: str) -> tuple[list[dict[str, Any]], list[str]]:
    reports: list[dict[str, Any]] = []
    parse_errors: list[str] = []
    for line_no, raw_line in enumerate(stdout.splitlines(), start=1):
        line = raw_line.strip()
        if not line.startswith(LOGIX_PERF_REPORT_PREFIX):
            continue
        payload = line[len(LOGIX_PERF_REPORT_PREFIX) :].strip()
        if not payload:
            parse_errors.append(f'line {line_no}: empty perf payload')
            continue
        try:
        ...
        if not isinstance(report, dict):
            parse_errors.append(f'line {line_no}: perf payload is not object')
            continue
        reports.append(report)
    return reports, parse_errors


def _collect_suite_threshold_anomalies(reports: list[dict[str, Any]], suite_id: str) -> list[dict[str, Any]]:
        ...
        for suite in suites:
            ...
            for threshold in thresholds:
                if not isinstance(threshold, dict):
                    continue
                first_fail_level = threshold.get('firstFailLevel')
                if first_fail_level is None:
                    continue
                budget = threshold.get('budget')
                budget_id = budget.get('id') if isinstance(budget, dict) else None
                gate_class = budget.get('gateClass') if isinstance(budget, dict) else None
                cost_class = budget.get('costClass') if isinstance(budget, dict) else None
                normalized_gate_class = gate_class if gate_class in {'hard', 'soft'} else 'hard'
                where = threshold.get('where')
                anomalies.append(
                    {
                        'report_index': report_index,
                        'suite_id': suite_id,
                        'budget_id': budget_id if isinstance(budget_id, str) else '(unknown)',
                        'gate_class': normalized_gate_class,
                        'cost_class': cost_class if isinstance(cost_class, str) else None,
                        'blocks_probe': normalized_gate_class != 'soft',
                        'first_fail_level': first_fail_level,
                        'max_level': threshold.get('maxLevel'),
...
def _run_probe_target(target: BrowserProbeTarget, tail_lines: int) -> BrowserProbeRun:
    ...
    perf_reports, perf_report_parse_errors = _parse_perf_reports(result.stdout)
    threshold_anomalies = _collect_suite_threshold_anomalies(perf_reports, target.suite_id)
    blocking_threshold_anomalies = [anomaly for anomaly in threshold_anomalies if anomaly.get('blocks_probe', True)]
    ...
    elif blocking_threshold_anomalies:
        status = 'failed'
        failure_kind = 'threshold'
        effective_returncode = PROBE_THRESHOLD_FAILURE_RETURN_CODE
    return BrowserProbeRun(
        ...
        perf_report_count=len(perf_reports),
        threshold_anomaly_count=len(threshold_anomalies),
        threshold_anomalies=threshold_anomalies,
        perf_report_parse_errors=perf_report_parse_errors,
...
def cmd_probe_next_blocker(json_mode: bool, dry_run: bool, tail_lines: int) -> int:
    ...
    threshold_anomalies: list[dict[str, Any]] = []
    for run in runs:
        for anomaly in run.threshold_anomalies:
            threshold_anomalies.append(
                {
                    'probe_order': run.order,
    ...
    for run in runs:
        print(
            f'- [{run.order}/{len(targets)}] {run.suite_id} | gate={run.gate} | status={run.status} | failure_kind={run.failure_kind or "-"} | duration_ms={run.duration_ms} | threshold_anomalies={run.threshold_anomaly_count}',
        )
        print(f'  command={run.command}')
        if run.threshold_anomalies:
            print('  threshold_anomalies:')
            for anomaly in run.threshold_anomalies:
                where = anomaly.get('where')
                where_text = json.dumps(where, ensure_ascii=False) if where else '{}'
                print(
                    f"  - budget={anomaly['budget_id']} first_fail_level={anomaly['first_fail_level']} max_level={anomaly['max_level']} where={where_text}",
                )
    if blocker:
        ...
        print(f'process_returncode: {blocker.process_returncode}')
        if blocker.perf_report_parse_errors:
            print('perf_report_parse_errors:')
            for error in blocker.perf_report_parse_errors:
                print(f'- {error}')
