...
import type { Draft } from '../../../../logix-core/src/Logic.js'
import type { Tag as StateTag } from '../../../../logix-core/src/State.js'
...
export type OptimisticCrudServices<Entity extends object, QueryInput, EntityId> = {
  readonly api: StateTag<CrudApi<Entity, QueryInput, EntityId>>
}
...
export const makeOptimisticCrudProgram = <
  Id extends string,
  Entity extends object,
...
>(
  ...
  const toDraft = <T>(value: T): Draft<T> => value as unknown as Draft<T>

  const getId = (entity: Entity | Draft<Entity>): EntityId =>
  ...
  const install = module.logic('install', ($) =>
      Effect.gen(function* () {
  ...
  ;(module.tag as unknown as Record<PropertyKey, unknown>)[EXTEND_HANDLE] = (
    ...
    const ext = commands.make(runtime as any)
    return { ...base, commands: ext.commands, services }
  }

  return Logix.Program.make<Id, typeof module.shape, OptimisticCrudHandleExt<Entity, QueryInput, EntityId>, unknown>(module, {
    initial: {
    ...
    logics: [install],
  }) as unknown as Logix.Program.Program<
    Id,
    typeof module.shape,
    OptimisticCrudHandleExt<Entity, QueryInput, EntityId>,
    unknown
