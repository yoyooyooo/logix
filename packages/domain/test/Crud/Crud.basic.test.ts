...
import type { CrudApi } from '../../src/Crud.js'
import * as CrudKit from '../../src/Crud.js'
...
describe('Crud.basic', () => {
  it.effect('Runtime.make/$.use/withLogic basics', () =>
    Effect.gen(function* () {
      ...
      const Crud = CrudKit.make('Crud.basic', {
        entity: Entity,
        initial: [],
      })
      ...
      const Seed = Crud.logic('seed', ($) =>
          Effect.gen(function* () {
            const self = yield* $.self
            expect(typeof self.commands.save).toBe('function')
      ...
      const HostLogic = Host.logic('host-logic', ($) =>
        Effect.gen(function* () {
          ...
          yield* crud.commands.list({ pageSize: 10 })
          const items1 = yield* waitForItems(1)
          expect(items1[0]?.id).toBe('e1')

          yield* crud.commands.save({ id: 'e2', name: 'Bob' } satisfies Entity)
          yield* waitForItems(2)

          yield* crud.commands.remove('e1')
