...
export const runTrial = (inv: TrialInvocation): Effect.Effect<CommandResult, never> =>
  Effect.gen(function* () {
    ...
    const reportArtifactsWithEvidence = evidenceInputs.evidence
      ? [
          ...report.artifacts,
          {
    ...
    const repairHintsWithEvidenceRefs = evidenceInputs.evidence
      ? report.repairHints.map((hint) => ({
          ...hint,
          relatedArtifactOutputKeys: Array.from(new Set([...(hint.relatedArtifactOutputKeys ?? []), 'evidenceInput'])).sort(),
        }))
      : report.repairHints
    ...
    const reportWithProvenance = {
      ...
      artifacts: reportArtifactsWithEvidence,
      repairHints: repairHintsWithEvidenceRefs,
    }
