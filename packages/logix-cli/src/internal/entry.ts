...
import { sortLiveArtifactsByOutputKey } from './liveResult.js'
...
import { runLive as runLiveCommand } from './commands/live.js'
...
export type RunOutcome =
  ...
  | { readonly kind: 'result'; readonly result: CommandResult | LiveCommandResult; readonly exitCode: 0 | 1 | 2 }
...
export type LiveRunOutcome =
  | { readonly kind: 'help'; readonly text: string; readonly exitCode: 0 }
  | { readonly kind: 'result'; readonly result: LiveCommandResult; readonly exitCode: 0 | 1 | 2 }
...
export const formatCliResult = (result: CommandResult | LiveCommandResult): string => stableStringifyJson(result)
...
用法:
  ...
  logix live <task> --runId <id> [--target <runtime:r/module:m/instance:i>] [--attachment <attachmentId>] [--cursor <token>] [--out <dir>]
...
const runLive = (inv: LiveInvocation): Effect.Effect<LiveCommandResult, unknown> => runLiveCommand(inv)

const runCommand = (inv: CliInvocation): Effect.Effect<CommandResult | LiveCommandResult, unknown> => {
  switch (inv.command) {
    case 'check':
      return runCheck(inv)
    case 'compare':
...
const sortCliArtifacts = (result: CommandResult | LiveCommandResult): CommandResult | LiveCommandResult => {
  if (result.kind === 'LiveCommandResult') {
    return {
      ...result,
      artifacts: sortLiveArtifactsByOutputKey(result.artifacts),
    }
  }
  return {
    ...result,
    artifacts: sortArtifactsByOutputKey(result.artifacts),
  }
}
...
const runParsedCommand = (args: {
  ...
  runCommand(withArgvSnapshot(args.parsed, args.argv2)).pipe(
    Effect.map((result) => {
      const result2 = result.kind === 'CommandResult' ? withHostDiagnosticsIfNeeded(result, args.argv2) : result
      return {
        kind: 'result' as const,
        result: sortCliArtifacts(result2),
...
export function runCli(argv: readonly ['live', ...string[]]): Effect.Effect<LiveRunOutcome, never>
export function runCli(argv: ReadonlyArray<string>): Effect.Effect<CommandRunOutcome, never>
export function runCli(argv: ReadonlyArray<string>): Effect.Effect<RunOutcome, never> {
  return runCliWithPolicy({
    argv,
    helpText: printHelp(),
  })
}

export const runCliAny = (argv: ReadonlyArray<string>): Effect.Effect<RunOutcome, never> =>
  runCliWithPolicy({
    argv,
    helpText: printHelp(),
  })

export const runInternalCli = runCliAny

export const main = runCliAny
