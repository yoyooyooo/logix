...
const isCliViolationCode = (code: string | undefined): boolean =>
  typeof code === 'string' && (code === 'CLI_VIOLATION' || code.startsWith('CLI_VIOLATION_'))

const isCliUsageCode = (code: string | undefined): boolean =>
  typeof code === 'string' &&
...
export const asSerializableErrorSummary = (cause: unknown): SerializableErrorSummary => {
  ...
  if (cause && typeof cause === 'object') {
    ...
    return {
      ...(typeof anyCause.name === 'string' && anyCause.name.length > 0 ? { name: anyCause.name } : null),
      message,
      ...(typeof anyCause.code === 'string' && anyCause.code.length > 0 ? { code: anyCause.code } : null),
      ...(typeof anyCause.hint === 'string' && anyCause.hint.length > 0 ? { hint: anyCause.hint } : null),
    }
  }
