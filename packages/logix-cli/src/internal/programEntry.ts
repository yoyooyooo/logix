...
import { hasProgramRuntimeBlueprint } from '@logixjs/core/repo-internal/runtime-contracts'
...
export const loadProgramEntry = (
  ...
  Effect.tryPromise({
    try: async () => {
      ...
      if (!hasProgramRuntimeBlueprint(value)) {
        throw makeCliError({
          code: 'PROGRAM_BLUEPRINT_MISSING',
          message: `[Logix][CLI] entry Program is missing runtime blueprint authority: ${entry.modulePath}#${entry.exportName}`,
        })
      }
      return value
    },
