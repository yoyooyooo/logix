...
describe('logix-devserver (101) allowWrite enables --mode write', () => {
  it(
    'should execute dev.run(argv contains --mode write) when allowWrite is enabled (not blocked by ERR_FORBIDDEN)',
    async () => {
      ...
      const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), 'intent-flow-logix-devserver-allowwrite-'))
      ...
      try {
        ...
        expect(info?.result?.capabilities?.write).toBe(true)
        ...
        const { stdout: runStdout } = await execFileAsync(
          ...
          [
            ...
            'write',
            '--ops',
            '__missing_ops__.json',
          ],
          { cwd: workspaceRoot, timeout: 60_000, maxBuffer: 10_000_000 },
        )
        const resp = JSON.parse(runStdout.trim())
        expect(resp?.ok).toBe(true)
        expect(resp?.result?.outcome?.exitCode).not.toBeUndefined()
      } finally {
        child.kill('SIGTERM')
