import { describe, expect, it } from 'vitest'

import {
  ExtensionHotReloadController,
  migrateExtensionState,
  revalidateBeforeSwap,
  ResourceBudgetExecutor,
  type ExtensionHostRuntime,
} from '../../src/internal/extension-host/index.js'

const makeRuntime = (args: {
  readonly revision: string
  readonly net?: boolean
}): ExtensionHostRuntime => ({
  manifest: {
    manifestVersion: 'ext.v1',
    extensionId: 'demo-extension',
    revision: args.revision,
    runtime: {
      apiVersion: 'v1',
      entry: './extension-entry.js',
      hooks: ['setup', 'healthcheck', 'teardown'],
    },
    capabilities: {
      hostApis: ['emit-control-event'],
      ...(typeof args.net === 'boolean' ? { net: args.net } : null),
    },
    limits: {
      timeoutMs: 20,
      maxQueueSize: 8,
    },
  },
  hooks: {
    healthcheck: async () => true,
  },
})

describe('logix-cli integration (extension policy drift revalidate)', () => {
  it('rejects swap when policy drifts before revalidate and keeps previous revision', async () => {
    const controller = new ExtensionHotReloadController(makeRuntime({ revision: 'r1' }))
    let policyCall = 0

    const result = await controller.reload({
      baseContext: {
        runId: 'policy-drift-1',
        instanceId: 'policy-drift-1',
        command: 'verify-loop',
      },
      loadShadow: async () => makeRuntime({ revision: 'r2', net: true }),
      budget: new ResourceBudgetExecutor({ timeoutMs: 50, maxQueueSize: 8 }),
      resolvePolicy: async () => {
        policyCall += 1
        if (policyCall === 1) {
          return { hostApis: ['emit-control-event'], net: true }
        }
        return { hostApis: ['emit-control-event'], net: false }
      },
      migrateState: migrateExtensionState,
      revalidate: revalidateBeforeSwap,
    })

    expect(result.ok).toBe(false)
    if (result.ok) throw new Error('expected failure')

    expect(result.failedStage).toBe('revalidate-before-swap')
    expect(result.reasonCode).toBe('EXT_MANIFEST_INVALID')
    expect(controller.getActiveRuntime().manifest.revision).toBe('r1')
  })

  it('allows swap when policy remains stable across preflight and revalidate', async () => {
    const controller = new ExtensionHotReloadController(makeRuntime({ revision: 'r1' }))

    const result = await controller.reload({
      baseContext: {
        runId: 'policy-drift-2',
        instanceId: 'policy-drift-2',
        command: 'verify-loop',
      },
      loadShadow: async () => makeRuntime({ revision: 'r2', net: true }),
      budget: new ResourceBudgetExecutor({ timeoutMs: 50, maxQueueSize: 8 }),
      resolvePolicy: async () => ({
        hostApis: ['emit-control-event'],
        net: true,
      }),
      migrateState: migrateExtensionState,
      revalidate: revalidateBeforeSwap,
    })

    expect(result.ok).toBe(true)
    if (!result.ok) throw new Error('expected success')

    expect(result.active.manifest.revision).toBe('r2')
    expect(controller.getActiveRuntime().manifest.revision).toBe('r2')
  })
})
