...
import {
  hasReadQueryActivationRetainer,
  readQueryActivationRetainSymbol,
} from './runtime/core/ReadQueryActivationRetention.js'
...
export const retainReadQueryActivation = <S, V>(
  moduleRuntime: unknown,
  readQuery: ReadQueryCompiled<S, V>,
): Effect.Effect<() => void, never, never> | undefined => {
  if (!hasReadQueryActivationRetainer<S>(moduleRuntime)) {
    return undefined
  }
  return moduleRuntime[readQueryActivationRetainSymbol](readQuery)
}
