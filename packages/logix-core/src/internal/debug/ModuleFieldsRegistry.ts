...
import { exportStaticIr } from '../field-kernel/ir.js'

// Debug/Devtools only: records FieldProgram by moduleId.
// - Not used for runtime behavior decisions.
// - In production, stores nothing by default to avoid extra memory footprint.

const programsByIdAndDigest = new Map<string, Map<string, FieldProgram<any>>>()
...
export const registerModuleProgram = (moduleId: string, program: FieldProgram<any>): void => {
  if (!isDevEnv()) {
...
export const getModuleProgramById = (moduleId: string): FieldProgram<any> | undefined => {
...
export const getModuleProgramByIdAndDigest = (moduleId: string, digest: string): FieldProgram<any> | undefined =>
