...
const resolveModuleIdOrThrow = (module: unknown): string => {
  ...
  throw makeError(
    ...
    '[ExternalStore.fromModule] Failed to resolve moduleId.\n' +
      'Fix: pass a ModuleTag, Module, or runtime-aware handle carrying .id/.tag.id/.moduleId, not a read-only ModuleHandle.',
  )
}
 ...
 *
 * Notes:
 * - This sugar is intended to be consumed by compiled field external-store declarations during install/runtime.
...
export const fromService = <Id, Svc, T>(
  ...
  const store: ExternalStore<T> = {
    getSnapshot: () => {
      throw makeError(
        'external_store::unresolved',
        `[ExternalStore.fromService] Store is unresolved (tagId=${tagId}). Use it via compiled field external-store install/runtime.`,
      )
    },
    subscribe: () => {
      throw makeError(
...
export const fromSubscriptionRef = <T>(ref: LogixModule.ReadonlySubscriptionRef<T> | SubscriptionRef.SubscriptionRef<T>): ExternalStore<T> => {
  ...
  const runtime = makeDetachedRuntimeRunner()
    ...
    getSnapshot: () => {
      throw makeError(
        ...
        `[ExternalStore.fromModule] Store is unresolved (moduleId=${moduleId}, selectorId=${compiled.selectorId}). ` +
          'Use it via compiled field external-store + TickScheduler/RuntimeStore (Module-as-Source).',
      )
    },
    subscribe: () => {
      throw makeError(
