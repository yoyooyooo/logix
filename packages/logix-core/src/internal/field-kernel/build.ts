...
import {
  ...
  type FieldPlanStep,
  type FieldResource,
  type FieldSchemaPathRef,
  type SourceDepIr,
  type ValidateStaticIr,
  collectNodeMeta,
  normalizeSpec,
} from './model.js'
...
// declaration-authority boundary: FieldKernel.build compiles already-declared field specs only.
// It does not own Form declaration authority, public exact acts, or semantic-owner decisions.
type ConvergeWriter = Extract<FieldEntry<any, string>, { readonly kind: 'computed' | 'link' }>

const collectMultipleWritersError = (
  entries: ReadonlyArray<FieldEntry<any, string>>,
): ConvergeStaticIrRegistry['configError'] | undefined => {
  const kindsByFieldPath = new Map<string, Set<FieldEntryKind>>()

  for (const entry of entries) {
    if (entry.kind !== 'computed' && entry.kind !== 'link' && entry.kind !== 'source' && entry.kind !== 'externalStore') {
      continue
    }
    const set = kindsByFieldPath.get(entry.fieldPath) ?? new Set<FieldEntryKind>()
    set.add(entry.kind)
    kindsByFieldPath.set(entry.fieldPath, set)
  }

  const conflicts: Array<{ readonly fieldPath: string; readonly kinds: ReadonlyArray<FieldEntryKind> }> = []
  ...
  return {
    ...
    message:
      `[FieldKernel.build] Multiple writers for field "${primary.fieldPath}" (${kindSummary}). ` +
...
const computeConvergeTopoOrder = (
  ...
  for (const entry of writers) {
    ...
    if (existing) {
      return {
        ...
        configError: {
          code: 'MULTIPLE_WRITERS',
          message: `[FieldKernel.converge] Multiple writers for field "${entry.fieldPath}" (${existing.kind} + ${entry.kind}).`,
  ...
  if (order.length !== nodes.size) {
    ...
    return {
      ...
      configError: {
        code: 'CYCLE_DETECTED',
        message: `[FieldKernel.converge] Cycle detected in computed/link graph: ${remaining.join(', ')}`,
        fields: remaining,
      },
    }
  }
...
const buildConvergeIr = (
  ...
  const fieldPathsKey = fieldPathIdRegistry.fieldPathsKey
  ...
  for (const entry of stepsById) {
    ...
    if (outId == null) {
      throw new Error(`[FieldKernel.build] Failed to map converge output fieldPath "${entry.fieldPath}" to FieldPathId.`)
    }
...
/**
 * Builds a normalized FieldBehavior from a FieldEntry.
 *
 ...
 */
const toFieldBehavior = (entry: FieldEntry<any, string>): FieldBehavior => {
...
const buildGraph = (
  entries: ReadonlyArray<FieldEntry<any, string>>,
  nodeMetaByFieldPath: ReadonlyMap<string, Meta.FieldMeta>,
): {
  readonly graph: FieldGraph
  readonly plan: FieldPlan
} => {
  const fieldMap = new Map<string, FieldDescriptor>()
  const nodes: Array<FieldGraphNode> = []
  const edges: Array<FieldGraphEdge> = []
  const resourcesById = new Map<string, FieldResource>()
  const planSteps: Array<FieldPlanStep> = []

  const ensureField = (fieldPath: string): FieldDescriptor => {
  ...
  for (const entry of entries) {
    ...
    const behavior = toFieldBehavior(entry)

    ;(field.behaviors as Array<FieldBehavior>).push(behavior)
    ...
    if (entry.kind === 'computed') {
      ...
      if (existing) {
        ...
        resourcesById.set(resourceId, {
          ...
          keySelector: `FieldKernel.source@${fieldPath}`,
      ...
      planSteps.push({
        ...
        targetFieldPath: fieldPath,
        resourceId,
        keySelectorId: `FieldKernel.source@${fieldPath}`,
      })
...
const getOverlappingDirtyRootIdsForDep = (
  registry: ReturnType<typeof makeFieldPathIdRegistry>,
  id: FieldPathId,
): ReadonlyArray<FieldPathId> => {
  const path = registry.fieldPaths[id]
  if (!path || path.length === 0) return []
  const out: Array<FieldPathId> = []
  for (let candidateId = 0; candidateId < registry.fieldPaths.length; candidateId++) {
    const candidate = registry.fieldPaths[candidateId]
    if (!candidate || candidate.length === 0) continue
    if (isPrefixOf(candidate, path) || isPrefixOf(path, candidate)) {
...
const buildSourceDepIr = (
  entries: ReadonlyArray<FieldEntry<any, string>>,
  registry: ReturnType<typeof makeFieldPathIdRegistry>,
  fieldPathsKey?: string,
): SourceDepIr | undefined => {
  const sources: Array<CompiledSource> = []
  const idsByRoot = new Map<FieldPathId, Array<number>>()
  ...
  for (const entry of entries) {
    ...
    const depPathIds: Array<FieldPathId> = []
    for (const dep of deps) {
      const normalized = normalizeFieldPath(dep)
      const depId = normalized ? getFieldPathId(registry, normalized) : undefined
      if (depId == null) {
        throw new Error(`[FieldKernel.build] Source "${entry.fieldPath}" has unmappable dep "${dep}".`)
      }
      depPathIds.push(depId)
    }

    const dirtyRootMatchIds = Array.from(
      new Set(depPathIds.flatMap((id) => getOverlappingDirtyRootIdsForDep(registry, id))),
    ).sort((a, b) => a - b)
...
const classifyCheckKind = (fieldPath: string): CompiledCheck['kind'] => {
  if (fieldPath === '$root') return 'root'
  if (fieldPath.endsWith('[]')) return 'item'
  if (fieldPath.includes('.')) return 'field'
  return 'list'
}

const buildValidateStaticIr = (
  programLike: Pick<FieldProgram<any>, 'entries' | 'graph'>,
  fieldPathCount: number,
  fieldPathsKey?: string,
): ValidateStaticIr | undefined => {
  const checks = programLike.entries.filter(
    (e): e is Extract<FieldEntry<any, string>, { readonly kind: 'check' }> => e.kind === 'check',
  )
  if (checks.length === 0) return undefined

  const graph = buildDependencyGraph(programLike as FieldProgram<any>)
  const compiledChecks: Array<CompiledCheck> = []
  const reverseCheckIdsByTargetPath = new Map<string, Array<number>>()
  const listCheckIdsByListPath = new Map<string, Array<number>>()
  const itemCheckIdsByListPath = new Map<string, Array<number>>()

  for (const entry of checks) {
    const id = compiledChecks.length
    const kind = classifyCheckKind(entry.fieldPath)
    const rules = entry.meta.rules as Record<string, any>
    const ruleKeySets = new Map<string, ReadonlyArray<string>>()
    for (const ruleName of Object.keys(rules).sort()) {
      const rule = rules[ruleName]
      const deps = Array.isArray(rule?.deps) ? (rule.deps as ReadonlyArray<string>) : []
      const keys = Array.from(
        new Set(
          deps
            .map((dep) => {
              const listPrefix = entry.fieldPath === '$root' ? '' : entry.fieldPath.endsWith('[]') ? entry.fieldPath.slice(0, -2) : entry.fieldPath
              if (!listPrefix) return dep
              if (dep === listPrefix) return '$list'
              if (dep.startsWith(`${listPrefix}.`)) return dep.slice(listPrefix.length + 1)
              return dep
            })
            .filter((dep) => dep !== '$root'),
        ),
      ).sort()
      ruleKeySets.set(ruleName, keys)
    }
    ...
    if (kind === 'list') {
      const bucket = listCheckIdsByListPath.get(entry.fieldPath) ?? []
      bucket.push(id)
      listCheckIdsByListPath.set(entry.fieldPath, bucket)
    } else if (kind === 'item') {
      const listPath = entry.fieldPath.slice(0, -2)
      const bucket = itemCheckIdsByListPath.get(listPath) ?? []
      bucket.push(id)
      itemCheckIdsByListPath.set(listPath, bucket)
    }
  }

  const uniqueTargets = new Set<string>()
  for (const target of graph.reverseAdj.keys()) {
    uniqueTargets.add(target)
  }
  for (const check of compiledChecks) {
    uniqueTargets.add(check.fieldPath)
  }

  for (const target of uniqueTargets) {
    const closure = reverseClosure(graph, target)
    for (const check of compiledChecks) {
      if (!closure.has(check.fieldPath)) continue
      const bucket = reverseCheckIdsByTargetPath.get(target) ?? []
      bucket.push(check.id)
      reverseCheckIdsByTargetPath.set(target, bucket)
    }
  }

  const freezeBuckets = (source: ReadonlyMap<string, ReadonlyArray<number>>): ReadonlyMap<string, Int32Array> =>
    new Map(Array.from(source, ([k, v]) => [k, Int32Array.from(v)]))

  return {
    checkEntries: compiledChecks,
    reverseCheckIdsByTargetPath: freezeBuckets(reverseCheckIdsByTargetPath),
    listCheckIdsByListPath: freezeBuckets(listCheckIdsByListPath),
    itemCheckIdsByListPath: freezeBuckets(itemCheckIdsByListPath),
    ...(fieldPathsKey ? { fieldPathsKey } : null),
    fieldPathCount,
  }
}
 ...
 */
const assertNoLinkCycles = (edges: ReadonlyArray<FieldGraphEdge>): void => {
  ...
  const dfs = (node: string): void => {
    if (stack.has(node)) {
      throw new Error(
        `[FieldKernel.build] link cycle detected at field "${node}". ` +
          'Please check field links for circular dependencies.',
      )
    }
...
const collectSchemaPaths = (
  entries: ReadonlyArray<FieldEntry<any, string>>,
): ReadonlyArray<FieldSchemaPathRef> => {
  const byKey = new Map<string, FieldSchemaPathRef>()

  const add = (ref: FieldSchemaPathRef): void => {
    if (!ref.path) return
    const k = `${ref.kind}|${ref.entryKind}|${ref.entryFieldPath}|${ref.ruleName ?? ''}|${ref.path}`
    byKey.set(k, ref)
  }

  const getCheckWritebackPath = (entry: Extract<FieldEntry<any, string>, { readonly kind: 'check' }>): string => {
...
/**
 * Builds a FieldProgram from the given stateSchema and field spec.
 *
...
export const build = <S extends object>(
  ...
  const entries = normalizeSpec(spec) as ReadonlyArray<FieldEntry<S, string>>
  ...
  for (const entry of entries) {
    if (entry.kind === 'computed') {
      ...
      if (deps === undefined) {
        throw new Error(
          `[FieldKernel.build] Missing explicit deps for computed "${entry.fieldPath}". ` +
            'Please use FieldKernel.computed({ deps: [...], get: ... }).',
        )
      }
    }
    if (entry.kind === 'source') {
      ...
      if (deps === undefined) {
        throw new Error(
          `[FieldKernel.build] Missing explicit deps for source "${entry.fieldPath}". ` +
            'Please provide meta.deps for FieldKernel.source({ deps: [...], ... }).',
        )
      }
    }
    if (entry.kind === 'check') {
      ...
      for (const name of Object.keys(rules)) {
        ...
        if (typeof rule === 'function' || !rule || typeof rule !== 'object') {
          throw new Error(
            `[FieldKernel.build] Missing explicit deps for check "${entry.fieldPath}" rule "${name}". ` +
  ...
  const convergeIr = buildConvergeIr(stateSchema as any, entries as ReadonlyArray<FieldEntry<any, string>>)
  const sourceDepIr = buildSourceDepIr(entries as ReadonlyArray<FieldEntry<any, string>>, convergeIr.fieldPathIdRegistry, convergeIr.fieldPathsKey)
  const validateIr = buildValidateStaticIr(
    { entries: entries as ReadonlyArray<FieldEntry<any, string>>, graph },
    convergeIr.fieldPaths.length,
    convergeIr.fieldPathsKey,
  )
  ...
  return {
    stateSchema,
    spec,
    entries: entries as ReadonlyArray<FieldEntry<any, string>>,
    graph,
    plan,
    ...(sourceDepIr ? { sourceDepIr } : null),
    ...(validateIr ? { validateIr } : null),
    convergeIr,
    schemaPaths: collectSchemaPaths(entries as ReadonlyArray<FieldEntry<any, string>>),
  }
}
