...
import type { FieldProgram, FieldSchemaPathRef } from './model.js'
...
const formatSchemaMismatchLine = (ref: FieldSchemaPathRef): string => {
...
export const emitSchemaMismatch = <S extends object>(
  ...
  const level = yield* Effect.service(Debug.currentDiagnosticsLevel).pipe(Effect.orDie)
    ...
    const refs = (program.schemaPaths ?? []) as ReadonlyArray<FieldSchemaPathRef>
    if (refs.length === 0) return

    const stateSchemaAst = program.stateSchema.ast as unknown as SchemaAST.AST

    const mismatches: Array<FieldSchemaPathRef> = []
    ...
    yield* Debug.record({
      ...
      code: 'field_kernel::schema_mismatch',
      severity: 'warning',
      message: `[schema] The following paths are not declared in stateSchema (total ${mismatches.length}):\n${lines.join('\n')}`,
      hint: 'Field-kernel writeback will create missing objects/fields. Declare all fieldPath/deps/link.from and errors.* writeback paths in stateSchema, or fix typos in field paths.',
