...
import { type DirtyAllReason, type FieldPath } from '../field-path.js'
...
import { dirtyRootIdsFromLegacyDirtyPaths } from './converge-legacy-dirty-adapter.js'
import { getMiddlewareStack, runWriterStep, runWriterStepOffFast } from './converge-step.js'
import {
  FieldKernelConfigError,
...
import type {
  ...
  FieldConvergeDiagnosticsSamplingSummary,
  FieldConvergeDirtySummary,
  FieldConvergeGenerationEvidence,
  FieldConvergeHotspot,
  FieldConvergeOutcome as FieldConvergeOutcomeTag,
  FieldConvergePlanCacheEvidence,
  FieldConvergeReason,
  FieldConvergeRequestedMode,
  FieldConvergeStaticIrEvidence,
  FieldConvergeStepStats,
} from './model.js'

const EMPTY_PLAN_STEP_IDS = new Int32Array(0)
...
const insertTopKHotspot = (args: {
  readonly hotspots: Array<FieldConvergeHotspot>
  readonly next: FieldConvergeHotspot
...
export const convergeInTransaction = <S extends object>(
  ...
  Effect.gen(function* () {
    ...
    const requestedMode: FieldConvergeRequestedMode = ctx.requestedMode ?? 'auto'
    const reasons: Array<FieldConvergeReason> = []
    ...
    if (ir.configError) {
      throw new FieldKernelConfigError(ir.configError.code, ir.configError.message, ir.configError.fields)
    }

    const stepsInTopoOrder = (ir.stepsById ?? []) as ReadonlyArray<FieldEntry<any, string>>
    ...
    let diagnosticsSampling: FieldConvergeDiagnosticsSamplingSummary | undefined
    if (diagnosticsLevel === 'sampled') {
      const cfg = yield* Effect.service(Debug.currentFieldConvergeDiagnosticsSampling).pipe(Effect.orDie)
    ...
    const hotspots: Array<FieldConvergeHotspot> | undefined = shouldTimeStepsForHotspots ? [] : undefined
    ...
    const emitFieldConvergeTraceEvent = (decision: FieldConvergeDecisionSummary): Effect.Effect<void> =>
    ...
    const dirtyPathCountHint = Array.isArray(dirtyPaths)
      ...
      : typeof (dirtyPaths as any)?.size === 'number'
        ...
        : undefined
    const resolvedDirtyPlanState = resolveDirtyPlanState({
      dirtyPlan: ctx.dirtyPlan,
    })
    const dirtyPlanExactEmpty = resolvedDirtyPlanState?.kind === 'exact-empty'
    const dirtyCardinalityHint =
      resolvedDirtyPlanState?.kind === 'exact-roots' ? resolvedDirtyPlanState.rootCount : dirtyPathCountHint
    ...
    const configScope: FieldConvergeConfigScope = ctx.configScope ?? 'builtin'
    const generationEvidence: FieldConvergeGenerationEvidence = ctx.generation ?? {
    ...
    const ensureDirtyRootIds = (): DirtyRootIds => {
      ...
      if (ctx.dirtyPlan) {
        dirtyRootIds = dirtyRootIdsFromDirtyPlan(ctx.dirtyPlan)
      }

      if (!dirtyRootIds && ctx.dirtyAllReason) {
        dirtyRootIds = makeDirtyAll(ctx.dirtyAllReason)
      } else if (!dirtyRootIds && dirtyPaths instanceof Set) {
        dirtyPrefixBitSet.clear()
      ...
      } else if (!dirtyRootIds) {
        dirtyRootIds = dirtyRootIdsFromLegacyDirtyPaths({
          execIr,
      ...
      canUseCache =
        ...
        cacheableBySize
      planKeyHash = resolvePlanKeyHash({
        dirtyRoots: dirtyRootIds,
        schedulingScope,
      })
    ...
    let cacheEvidence: FieldConvergePlanCacheEvidence | undefined = shouldCollectDecisionHeavyDetails
      ? {
          capacity: 0,
          size: 0,
          hits: 0,
    ...
    const getOrComputePlan = (options?: {
      readonly missReason?: FieldConvergePlanCacheEvidence['missReason']
    ...
    const rootRatioHint =
      typeof dirtyCardinalityHint === 'number' && dirtyCardinalityHint > 0
        ? scopeStepCount > 0
          ? dirtyCardinalityHint / scopeStepCount
          : 1
        : undefined
    ...
    if (requestedMode === 'auto') {
      ...
      } else if (dirtyPlanExactEmpty) {
        // Exact empty dirty plan is a proven no-op for non-cold transaction windows.
        // Do not degrade it into unknown_write/full just because legacy dirtyPaths is empty.
        mode = 'dirty'
        planStepIds = EMPTY_PLAN_STEP_IDS
        planStepCount = 0
        affectedSteps = 0
      } else if (scopeStepCount <= AUTO_TINY_GRAPH_FULL_STEP_THRESHOLD) {
        mode = 'full'
        reasons.push('near_full')
      } else if (resolvedDirtyPlanState?.kind === 'dirty-all') {
        mode = 'full'
        reasons.push('dirty_all')
        reasons.push('unknown_write')
      } else if (dirtyCardinalityHint === 0) {
        mode = 'full'
        reasons.push('unknown_write')
    ...
    const getDirtySummary = (): FieldConvergeDirtySummary | undefined => {
      ...
      if (resolvedDirtyPlanState?.kind === 'dirty-all') {
        return {
          dirtyAll: true,
          reason: resolvedDirtyPlanState.reason,
          rootCount: 0,
          ...(requiresRootIds ? { rootIds: [], rootIdsTruncated: false } : null),
        }
      }

      if (dirtyPlanExactEmpty) {
        return {
          dirtyAll: false,
          rootCount: 0,
          ...(requiresRootIds ? { rootIds: [], rootIdsTruncated: false } : null),
    ...
    const buildStepStats = (executedSteps: number): FieldConvergeStepStats => ({
      totalSteps,
      executedSteps,
    ...
    const makeDecisionSummary = (params: {
      readonly outcome: FieldConvergeOutcomeTag
      ...
      return {
        ...
        top3:
          shouldCollectDecisionHeavyDetails && hotspots && hotspots.length > 0
            ? hotspots.slice()
            : undefined,
      } satisfies FieldConvergeDecisionSummary
    }
    ...
    try {
      ...
      const convergePlan = planConverge({
        execIr,
        dirtyPlan: ctx.dirtyPlan,
        requestedMode,
        schedulingScope,
        ...(ctx.schedulingScopeStepIds ? { schedulingScopeStepIds: ctx.schedulingScopeStepIds } : null),
        diagnosticsLevel,
        ...
        scopeStepIds,
        scopeStepCount,
        dirtyStepIds: planStepIds ?? (planStepCount != null ? planScratch : undefined),
        dirtyStepCount: planStepCount ?? planStepIds?.length,
        affectedStepCount: affectedSteps,
        deferredStepIds: execIr.topoOrderDeferredInt32,
      })

      const stepIds = convergePlan.stepIds ?? (convergePlan.mode === 'dirty' ? planScratch : scopeStepIds)
      const stepCount = convergePlan.stepCount
        ...
        if (steps) {
          if (ctx.now() - executionStartedAt > ctx.budgetMs) {
            ...
            if (decision && diagnosticsLevel !== 'off') {
              yield* emitFieldConvergeTraceEvent(decision)
            }
            return {
              _tag: 'Degraded',
          ...
          if (budgetChecks >= 32) {
            ...
            if (ctx.now() - executionStartedAt > ctx.budgetMs) {
              ...
              const decision = shouldCollectDecision
                ...
                : undefined
              if (decision && diagnosticsLevel !== 'off') yield* emitFieldConvergeTraceEvent(decision)
              return {
                _tag: 'Degraded',
                reason: 'budget_exceeded',
                ...(decision ? { decision } : null),
      ...
      if (e instanceof FieldKernelConfigError) {
        throw e
      }
      const error = toSerializableErrorSummary(e)
    ...
    const totalDurationMs = Math.max(0, ctx.now() - executionStartedAt)
    const outcome: FieldConvergeOutcomeTag = changedCount > 0 ? 'Converged' : 'Noop'
    ...
    const finalConvergePlan = planConverge({
      execIr,
      dirtyPlan: ctx.dirtyPlan,
      requestedMode,
      schedulingScope,
    ...
    const deferredReachableStepIds = finalConvergePlan.deferredReachableStepIds
