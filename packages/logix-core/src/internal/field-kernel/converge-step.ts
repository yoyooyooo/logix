...
const enumerateConcreteValuePaths = (
  ...
  const visit = (segmentIndex: number, currentPath: string, rowIndices: ReadonlyArray<number>): void => {
    ...
    if (segment.endsWith('[]')) {
      ...
      const indices =
        Array.isArray(changedIndices) && rowIndices.length === 0 ? changedIndices : Array.from({ length: items.length }, (_, i) => i)
      for (const rowIndex of indices) {
        if (!Number.isInteger(rowIndex) || rowIndex < 0 || rowIndex >= items.length) continue
...
const getChangedRowIndicesFromDirtyPlan = <S extends object>(ctx: ConvergeContext<S>, listPath: string): Int32Array | undefined => {
  const plan = ctx.dirtyPlan
  if (!plan || plan.dirtyAll || plan.authority !== 'field-path-registry') return undefined
  const list = plan.list
  if (!list) return undefined
  const key = `${listPath}@@`
  if (list.rootTouched.has(key)) return undefined
  return list.indexBindingsSorted.get(key)
}
...
const runRowScopedComputedStep = <S extends object>(params: {
  ...
  const meta = params.entry.meta as {
    ...
    readonly _rowScopeIncrementalPolicy?: unknown
  ...
  const canUseChangedRows = meta._rowScopeIncrementalPolicy === 'changed-rows'
  ...
  if (valuePatternPath && deriveAtPath) {
    let changed = false
    const listItem = RowId.parseListItemFieldPath(valuePatternPath)
    const changedIndices = canUseChangedRows && listItem ? getChangedRowIndicesFromDirtyPlan(params.ctx, listItem.listPath) : undefined
    for (const concrete of enumerateConcreteValuePaths(
      params.draft.getRoot(),
      valuePatternPath,
      changedIndices ? Array.from(changedIndices) : undefined,
    )) {
  ...
  const items = Array.isArray(rows) ? rows : []
  let changed = false
  const changedIndices = canUseChangedRows ? getChangedRowIndicesFromDirtyPlan(params.ctx, sourceListPath) : undefined
  const indices = changedIndices ? Array.from(changedIndices) : Array.from({ length: items.length }, (_, i) => i)
