...
import type { DeclarativeLinkRuntimeService, HostSchedulerService } from '../runtime/core/env.js'
import { getGlobalHostScheduler } from '../runtime/core/HostScheduler.js'
import * as RowId from './rowid.js'
import type { FieldEntry, FieldPlanStep } from './model.js'

const isFn = (value: unknown): value is (...args: ReadonlyArray<any>) => unknown => typeof value === 'function'

type ExternalStoreEntry<S> = Extract<FieldEntry<S, string>, { readonly kind: 'externalStore' }>
...
const getOrCreateExternalStoreWritebackCoordinator = (args: {
  ...
  Effect.gen(function* () {
    ...
    const hostSchedulerOpt = yield* Effect.serviceOption(
      HostSchedulerTag as unknown as ServiceMap.Key<any, HostSchedulerService>,
    )
    ...
    let scheduledPriority: ExternalStoreWritebackCommitPriority | undefined
    let cancelScheduledHostTask: (() => void) | undefined
    let scheduledFiber: Fiber.Fiber<void, never> | undefined

    const priorityRank = (priority: ExternalStoreWritebackCommitPriority | undefined): number =>
      priority === 'normal' ? 1 : 0

    const clearScheduledFlushState = (): void => {
    ...
    const coordinator: ExternalStoreWritebackCoordinator = {
      ...
      scheduleFlush: (policy) =>
        Effect.gen(function* () {
          ...
          const delayMsRaw = policy?.coalesceWindowMs ?? 0
          const delayMs = typeof delayMsRaw === 'number' && Number.isFinite(delayMsRaw) ? Math.max(0, delayMsRaw) : 0
          const commitPriority = policy?.commitPriority ?? 'normal'
          ...
          scheduledPriority = commitPriority
          const generation = scheduledGeneration + 1
            ...
            yield* Effect.promise(
              () =>
                new Promise<void>((resolve) => {
                  ...
                  cancelScheduledHostTask = hostScheduler.scheduleTimeout(delayMs, resolve)
                }),
            )
            if (closed || generation !== scheduledGeneration) return
            cancelScheduledHostTask = undefined
            scheduledFiber = undefined
            clearScheduledFlushState()
            yield* coordinator.flush()
          ...
          scheduledFiber = yield* Effect.forkDetach(Effect.provideServices(scheduledFlush, args.env), { startImmediately: true })
        }),
      flush: () =>
        Effect.gen(function* () {
          if (closed) return
      ...
      enqueue: (request, policy) =>
        Effect.gen(function* () {
          ...
          yield* coordinator.scheduleFlush(policy)
        }),
    }
...
export const installExternalStoreSync = <S>(
  ...
  Effect.gen(function* () {
    ...
    const fieldLane = (entry.meta as any)?.priority as 'urgent' | 'nonUrgent' | undefined
    ...
    if (rawDescriptor?.kind === 'module') {
      ...
      if (Option.isNone(linkRuntimeOpt)) {
        return yield* Effect.die(new Error('[FieldKernel.externalStore] Missing DeclarativeLinkRuntime service (073).'))
      }
      ...
      if (!sourceRuntime) {
        return yield* Effect.die(new Error(`[FieldKernel.externalStore] Module-as-Source store is unresolved for "${fieldPath}". ` +
      ...
      if (rawDescriptor.instanceId && rawDescriptor.instanceId !== sourceRuntime.instanceId) {
        return yield* Effect.die(new Error(`[FieldKernel.externalStore] Module-as-Source instanceId mismatch for "${fieldPath}". ` +
      ...
      if (sourceRuntime.moduleId !== rawDescriptor.moduleId) {
        return yield* Effect.die(new Error(`[FieldKernel.externalStore] Module-as-Source moduleId mismatch for "${fieldPath}". ` +
      ...
      if (!isRecognizableStatic) {
        yield* Debug.record({
          ...
          severity: 'warning',
          message:
            '[fieldExternalStore] Module-as-Source selector is not fully IR-recognizable; falling back to module-topic edge (may reduce perf).',
          hint:
      ...
      const commitPriority: ExternalStoreWritebackCommitPriority = fieldLane === 'nonUrgent' ? 'low' : 'normal'
      ...
      const writeValue = (nextValue: unknown): Effect.Effect<void, never, never> =>
        Effect.gen(function* () {
          ...
          const body = Effect.gen(function* () {
            ...
            internals.txn.recordStatePatch(normalizedPatchPath, 'field-external-store', prevValue, nextValue, step.id)
    ...
    if (!store || typeof store !== 'object') {
      return yield* Effect.die(new Error(`[FieldKernel.externalStore] Invalid store for "${fieldPath}". Expected { getSnapshot, subscribe }.`))
    }
    ...
    const recordFuseDiagnostic = Effect.gen(function* () {
      ...
      yield* Debug.record({
        ...
        severity: 'warning',
        message:
          `[FieldKernel.externalStore] store.getSnapshot() threw; field sync is fused and will stop syncing for "${fieldPath}".`,
        hint: 'Fix: ensure getSnapshot is synchronous and non-throwing; async resources should use FieldKernel.source or ExternalStore.fromStream({ current/initial }).',
