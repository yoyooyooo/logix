...
import type { FieldProgram } from './model.js'
import * as Meta from './meta.js'

type MetaIssue = Readonly<{
  readonly origin: string
  readonly report: Meta.FieldMetaSanitizeReport
}>
...
const collectNodeMetaIssues = (program: FieldProgram<any>): Array<MetaIssue> => {
  const spec: any = program.spec as any
  if (!spec || typeof spec !== 'object') return []

  const isNode = (value: unknown): value is { readonly _tag: 'FieldNode'; readonly meta?: unknown } =>
    typeof value === 'object' && value !== null && (value as any)._tag === 'FieldNode'

  const isList = (value: unknown): value is { readonly _tag: 'FieldList'; readonly item?: unknown; readonly list?: unknown } =>
...
const collectEntryMetaIssues = (program: FieldProgram<any>): Array<MetaIssue> => {
  const issues: Array<MetaIssue> = []
...
export const emitMetaSanitizeDiagnostics = (
  ...
  Effect.gen(function* () {
    ...
    yield* Debug.record({
      ...
      code: 'field_kernel::meta_sanitized',
      severity: 'warning',
      message: `[meta] FieldMeta contains non-exportable fields/values and will be sanitized (total ${issues.length}):\n${lines.join('\n')}`,
      hint:
        'FieldMeta is export-only (Devtools/Static IR) and MUST be JsonValue. ' +
