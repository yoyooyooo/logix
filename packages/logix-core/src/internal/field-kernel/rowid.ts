...
export class RowIdStore {
  ...
  resetList(listPath: string, parentRowId?: RowId): void {
    const key = this.listKey(listPath, parentRowId)
    const state = this.lists.get(key)
    if (!state) return

    this.lists.delete(key)
    for (const rowId of state.ids) {
      this.rowIdIndex.delete(rowId)
      this.notifyRemoved(state.listPath, rowId)
      this.removeDescendants(rowId)
    }
  }
   ...
   * - configs come from list declarations in FieldProgram.spec (may include trackBy).
   */
...
const getListConfigDirtyMatchPlan = (listConfigs: ReadonlyArray<ListConfig>): ListConfigDirtyMatchPlan => {
  // Runtime keeps listConfigs identity stable between field program rebuilds; cache keying by array reference is effective on hot commits.
  ...
  for (const [rootKey, dirtyRootsForRoot] of dirtyRootsToProcessByRoot) {
    ...
    for (const dirtyRoot of dirtyRootsForRoot) {
      for (const listPath of candidates) {
        ...
        // 3) No trackBy: keep the index-based default behavior.
