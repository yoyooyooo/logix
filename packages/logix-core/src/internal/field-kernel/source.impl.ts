...
import type { PatchReason, TxnDirtyPlanSnapshot } from '../runtime/core/StateTransaction.js'
import { recordKernelHotPathFallback } from '../runtime/core/KernelHotPathAudit.js'
import { toSourceKernelFallbackReason, type KernelFallbackReason } from '../runtime/core/kernelFallbackReason.js'
...
export type SourceDirtyGateFallbackReason =
  | 'missing_dirty_plan'
  | 'missing_source_dep_ir'
  | 'dirty_all'
  | 'non_field_path_authority'
  | 'field_paths_key_mismatch'
  | 'field_path_count_mismatch'
  | 'invalid_root_id'

export type SourceDirtyGateDecision =
  | { readonly mode: 'exact'; readonly affectedSourceIds: ReadonlySet<number> }
  | {
      readonly mode: 'full'
      readonly fallbackReason: SourceDirtyGateFallbackReason
      readonly kernelFallbackReason: KernelFallbackReason
    }

const makeSourceDirtyGateFallback = (fallbackReason: SourceDirtyGateFallbackReason): SourceDirtyGateDecision => ({
  mode: 'full',
  fallbackReason,
  kernelFallbackReason: toSourceKernelFallbackReason(fallbackReason),
})

const recordSourceDirtyGateFallback = (decision: SourceDirtyGateDecision): SourceDirtyGateDecision => {
  if (decision.mode === 'full') {
    recordKernelHotPathFallback('source_dirty_gate', decision.kernelFallbackReason)
  }
  return decision
}

export const collectAffectedSourceDecision = <S extends object>(
  program: FieldProgram<S>,
  dirtyPlan: TxnDirtyPlanSnapshot | undefined,
): SourceDirtyGateDecision => {
  const ir = program.sourceDepIr
  if (!dirtyPlan) return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('missing_dirty_plan'))
  if (!ir) return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('missing_source_dep_ir'))
  if (dirtyPlan.dirtyAll) return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('dirty_all'))
  if (dirtyPlan.authority !== 'field-path-registry') {
    return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('non_field_path_authority'))
  }
  if (dirtyPlan.rootCount === 0) return { mode: 'exact', affectedSourceIds: new Set() }
  if (!ir.fieldPathsKey || !dirtyPlan.fieldPathsKey || ir.fieldPathsKey !== dirtyPlan.fieldPathsKey) {
    return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('field_paths_key_mismatch'))
  }
  if (ir.fieldPathCount !== dirtyPlan.fieldPathCount) {
    return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('field_path_count_mismatch'))
  }

  const out = new Set<number>()
  for (const rootId of dirtyPlan.rootIds) {
    if (!Number.isFinite(rootId) || rootId < 0 || rootId >= ir.fieldPathCount) {
      return recordSourceDirtyGateFallback(makeSourceDirtyGateFallback('invalid_root_id'))
    }
    const bucket = ir.sourceIdsByDirtyRootId.get(rootId)
    if (!bucket) continue
    for (let i = 0; i < bucket.length; i++) {
      out.add(bucket[i]!)
    }
  }
  return { mode: 'exact', affectedSourceIds: out }
}

export type SourceRowScopeFallbackReason =
  | SourceDirtyGateFallbackReason
  | 'missing_list_evidence'
  | 'list_root_touched'
  | 'missing_changed_indices'

export type SourceRowScopeRunPlan =
  | {
      readonly mode: 'full'
      readonly fallbackReason: SourceRowScopeFallbackReason
      readonly kernelFallbackReason: KernelFallbackReason
    }
  | { readonly mode: 'changed'; readonly indices: Int32Array }

const makeSourceRowScopeFallback = (fallbackReason: SourceRowScopeFallbackReason): SourceRowScopeRunPlan => ({
  mode: 'full',
  fallbackReason,
  kernelFallbackReason: toSourceKernelFallbackReason(fallbackReason),
})

const recordSourceRowScopeFallback = (plan: SourceRowScopeRunPlan): SourceRowScopeRunPlan => {
  if (plan.mode === 'full') {
    recordKernelHotPathFallback('source_dirty_gate', plan.kernelFallbackReason)
  }
  return plan
}

export const getSourceRowScopeRunPlan = (args: {
  readonly listPath: string
  readonly dirtyPlan?: TxnDirtyPlanSnapshot
}): SourceRowScopeRunPlan => {
  const dirtyPlan = args.dirtyPlan
  if (!dirtyPlan) return recordSourceRowScopeFallback(makeSourceRowScopeFallback('missing_dirty_plan'))
  if (dirtyPlan.dirtyAll) return recordSourceRowScopeFallback(makeSourceRowScopeFallback('dirty_all'))
  if (dirtyPlan.authority !== 'field-path-registry') {
    return recordSourceRowScopeFallback(makeSourceRowScopeFallback('non_field_path_authority'))
  }
  const list = dirtyPlan.list
  if (!list) return recordSourceRowScopeFallback(makeSourceRowScopeFallback('missing_list_evidence'))

  const key = `${args.listPath}@@`
  if (list.rootTouched.has(key)) return recordSourceRowScopeFallback(makeSourceRowScopeFallback('list_root_touched'))

  const indices = list.indexBindingsSorted.get(key)
  if (!indices || indices.length === 0) return recordSourceRowScopeFallback(makeSourceRowScopeFallback('missing_changed_indices'))
  return { mode: 'changed', indices }
}
...
export const syncIdleInTransaction = <S extends object>(
  ...
  Effect.sync(() => {
    ...
    const sourceGate = collectAffectedSourceDecision(program, ctx.dirtyPlan)
    const affectedSourceIds = sourceGate.mode === 'exact' ? sourceGate.affectedSourceIds : undefined

    for (const entry of program.entries) {
      if (entry.kind !== 'source') continue
      let compiledSource = undefined as NonNullable<FieldProgram<S>['sourceDepIr']>['sourcesById'][number] | undefined
      if (affectedSourceIds && program.sourceDepIr) {
        const sourceId = program.sourceDepIr.sourceIdsByFieldPath.get(entry.fieldPath)
        if (sourceId == null) continue
        compiledSource = program.sourceDepIr.sourcesById[sourceId]
        if (compiledSource?.depsKind === 'explicit-empty') continue
        if (!affectedSourceIds.has(sourceId)) continue
      }
      ...
      if (listItem) {
        ...
        const rowPlan = getSourceRowScopeRunPlan({
          listPath: listItem.listPath,
          dirtyPlan: ctx.dirtyPlan,
        })
        const rowCount = rowPlan.mode === 'changed' ? rowPlan.indices.length : items.length

        for (let cursor = 0; cursor < rowCount; cursor++) {
          const index = rowPlan.mode === 'changed' ? rowPlan.indices[cursor]! : cursor
