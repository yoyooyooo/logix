...
export function make<Sh extends Logix.AnyModuleShape, R = never>(
  ...
  const topLevelStateKeys = (() => {
    const fields = (shape.stateSchema as any)?.fields
    if (!fields || typeof fields !== 'object' || Array.isArray(fields)) return new Set<string>()
    return new Set<string>(Object.keys(fields as Record<string, unknown>))
  })()
  ...
  const toTopLevelListRootKey = (path: unknown): string | undefined => {
    if (typeof path !== 'string') return undefined
    const trimmed = path.trim()
    if (!trimmed) return undefined

    const dotIndex = trimmed.indexOf('.')
    let head = dotIndex >= 0 ? trimmed.slice(0, dotIndex) : trimmed
    ...
    if (bracketIndex >= 0) {
      head = head.slice(0, bracketIndex)
    }

    return head.length > 0 ? head : undefined
  }

  const isTopLevelListRootKey = (key: string): boolean => {
    if (!runtimeInternals) return false
    const listConfigs = runtimeInternals.traits.getListConfigs() as ReadonlyArray<unknown>
    for (let index = 0; index < listConfigs.length; index += 1) {
      if (toTopLevelListRootKey((listConfigs[index] as any)?.path) === key) {
        return true
      }
    }
    return false
  ...
  const applyKnownTopLevelDirtyStateUpdate = (
    prev: Logix.StateOf<Sh>,
    next: Logix.StateOf<Sh>,
  ): Effect.Effect<void, never, any> => {
    if (Object.is(prev, next)) return Effect.void
    if (!runtimeInternals) return runtime.setState(next)

    const changes = StateTransaction.collectKnownTopLevelDirtyChanges(prev, next)
    if (!changes || changes.length === 0) {
      return Effect.sync(() => {
        runtimeInternals.txn.recordStatePatch(undefined, 'unknown')
        runtimeInternals.txn.updateDraft(next)
      })
    }
    ...
    for (let index = 0; index < changes.length; index += 1) {
      ...
      if (topLevelStateKeys.size > 0 && !topLevelStateKeys.has(key)) {
        return Effect.sync(() => {
          runtimeInternals.txn.recordStatePatch(undefined, 'unknown')
          runtimeInternals.txn.updateDraft(next)
        })
    ...
    return Effect.sync(() => {
      for (let index = 0; index < changes.length; index += 1) {
        const [key, prevValue, nextValue] = changes[index]!
        const patchPath = isTopLevelListRootKey(key) ? `${key}[]` : key
        runtimeInternals.txn.recordStatePatch(patchPath, 'unknown', prevValue, nextValue)
      }
      runtimeInternals.txn.updateDraft(next)
    })
  }
      ...
      Effect.gen(function* () {
        ...
        for (let i = 0; i < batch.length; i++) {
          ...
          if (req.kind === 'update') {
            ...
            yield* applyKnownTopLevelDirtyStateUpdate(current, next)
            current = next
            continue
          }
      ...
      markDirectStateWriteEffect<Sh, Effect.Effect<void, never, any>>(
        Effect.gen(function* () {
          ...
          if (inTxn) {
            ...
            return yield* applyKnownTopLevelDirtyStateUpdate(prev, f(prev))
          }

          const body = () =>
            Effect.flatMap(runtime.getState, (prev) => applyKnownTopLevelDirtyStateUpdate(prev, f(prev)))
            ...
            const prev = yield* runtime.getState
              ...
              for (const path of patchPaths) {
                if (path !== '*' && Array.isArray(path)) {
                  recordPatch?.(StateTransaction.prefetchProducerPatchArrayPath(path), 'unknown')
                  continue
                }
                recordPatch?.(path, 'unknown')
              }
