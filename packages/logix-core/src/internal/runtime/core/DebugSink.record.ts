...
import type { TxnLaneEvidencePolicyV2 } from './ModuleRuntime.txnLanePolicy.js'
...
export type Event =
  ...
  | {
      ...
      /**
       * Optional: unified state-write intent summary for degradation-ledger / devtools / perf gates.
       * - Must stay slim and serializable.
       */
      readonly stateWrite?: unknown
      /**
...
export type DiagnosticsCostClass = 'runtime_core' | 'controlplane_phase' | 'devtools_projection'
export type DiagnosticsGateClass = 'hard' | 'soft'
export type DiagnosticsSamplingPolicy = 'always' | 'budgeted' | 'sampled'
...
export interface RuntimeDebugEventRef {
  ...
  readonly costClass?: DiagnosticsCostClass
  readonly gateClass?: DiagnosticsGateClass
  readonly samplingPolicy?: DiagnosticsSamplingPolicy
...
export type TxnLaneEvidence = {
  ...
  readonly kind: string
  readonly policy: TxnLaneEvidencePolicyV2
  readonly backlog: {
...
const classifyDiagnosticsBudget = (
  ...
  const isControlplane =
    ...
    kind === 'trait-source' ||
    label === 'trace:tick' ||
    label === 'trace:txn-phase' ||
    label.startsWith('trait:')

  if (isControlplane) {
    return {
      costClass: 'controlplane_phase',
      gateClass: 'soft',
      samplingPolicy: 'budgeted',
    }
  }

  if (kind === 'devtools' || kind === 'react-render' || kind === 'react-selector') {
    return {
...
export const toRuntimeDebugEventRef = (
  ...
  const withDowngrade = (ref: Omit<RuntimeDebugEventRef, 'downgrade'>): RuntimeDebugEventRef => {
    const classifiedRef = {
      ...ref,
      ...classifyDiagnosticsBudget(ref.kind, ref.label),
    } satisfies Omit<RuntimeDebugEventRef, 'downgrade'>

    if (!downgrade) return classifiedRef
    return { ...classifiedRef, downgrade: { reason: downgrade } }
  }
  ...
  switch (event.type) {
    ...
    case 'state:update': {
      ...
      if (e.stateWrite !== undefined) slimMetaInput.stateWrite = e.stateWrite
