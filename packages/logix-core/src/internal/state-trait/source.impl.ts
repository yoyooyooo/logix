  ...
  if (listItem) {
    ...
    register(fieldPath, (state: any) =>
      Effect.gen(function* () {
        ...
        const items: ReadonlyArray<unknown> = Array.isArray(listValue) ? listValue : []
        const canSkipNoTrackByRowIdReconcile = store.canSkipNoTrackByListReconcile({
          listPath,
          items,
        })
        const ids = canSkipNoTrackByRowIdReconcile ? undefined : store.ensureList(listPath, items)
        ...
        for (let index = 0; index < items.length; index++) {
          const rowId = ids?.[index] ?? store.getRowId(listPath, index)
          if (!rowId) continue
