...
export interface RuntimeWorkbenchLiveEvidenceInput {
  readonly kind: 'live-evidence'
  readonly facet: {
    readonly kind:
      | 'operation.accepted'
      | 'operation.completed'
...
export const authorityRefOf = (input: RuntimeWorkbenchTruthInput): RuntimeWorkbenchAuthorityRef => {
  switch (input.kind) {
    ...
    case 'live-evidence':
      return { kind: input.kind, id: liveEvidenceAuthorityId(input) }
    case 'evidence-gap':
...
const liveEvidenceAuthorityId = (input: RuntimeWorkbenchLiveEvidenceInput): string =>
  input.facet.operationId ?? input.facet.captureId ?? `${input.facet.kind}:unknown`
