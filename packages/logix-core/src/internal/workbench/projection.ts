import type {
  ...
  RuntimeWorkbenchLiveEvidenceInput,
...
const projectTruthInput = (
  ...
  switch (input.kind) {
    ...
    case 'live-evidence':
      return projectLiveEvidence(input, authorityRef)
    case 'evidence-gap':
...
const projectLiveEvidence = (
  input: RuntimeWorkbenchLiveEvidenceInput,
  authorityRef: ReturnType<typeof authorityRefOf>,
): ReturnType<typeof projectTruthInput> => {
  const facet = input.facet
  const target = facet.target
  const runtimeCoordinate =
    target && target.runtimeId && target.moduleId && target.instanceId
      ? {
      ...
      : undefined
  const artifactOutputKey =
    facet.artifactRef?.outputKey ??
    (facet.kind === 'live.capture'
      ? `live-capture:${facet.captureId ?? 'unknown'}`
      : `live-operation:${facet.operationId ?? 'unknown'}`)
  const artifact: RuntimeWorkbenchArtifactProjection = {
    id: `artifact:${artifactOutputKey}`,
    authorityRef,
    artifactOutputKey,
    artifactRef: facet.artifactRef?.file,
    kind: 'evidence',
    sourceRefs: [],
    preview: {
      kind: facet.artifactRef?.kind ?? facet.kind,
      liveFacetKind: facet.kind,
      ...(facet.operationKind ? { operationKind: facet.operationKind } : null),
      ...(facet.captureKind ? { captureKind: facet.captureKind } : null),
      ...(facet.reason ? { reason: facet.reason } : null),
      ...(facet.noMutation ? { noMutation: facet.noMutation } : null),
      ...(facet.stageClass ? { stageClass: facet.stageClass } : null),
      ...(facet.budget ? { budget: facet.budget } : null),
      ...(facet.dropped ? { dropped: facet.dropped } : null),
      ...(facet.degraded ? { degraded: facet.degraded } : null),
      ...(facet.redacted ? { redacted: facet.redacted } : null),
      ...(facet.binding ? { binding: facet.binding } : null),
      ...(facet.artifactRef?.digest ? { digest: facet.artifactRef.digest } : null),
      ...(facet.artifactRef?.reasonCodes ? { reasonCodes: facet.artifactRef.reasonCodes } : null),
    },
  }
  const findings =
    facet.degraded || facet.kind === 'operation.denied' || facet.kind === 'operation.failed'
      ? [
          degradationFinding({
            id: facet.operationId ?? facet.captureId ?? facet.kind,
            authorityRef,
            summary:
              facet.kind === 'operation.denied'
                ? `Live operation denied: ${facet.reason ?? 'unknown'}`
                : facet.kind === 'operation.failed'
                  ? 'Live operation failed'
                  : `Live evidence degraded: ${facet.degraded?.reason ?? 'unknown'}`,
          }),
        ]
      : []

  return {
    status: facet.kind === 'operation.denied' || facet.degraded ? 'inconclusive' : 'unknown',
    findings,
    artifacts: [artifact],
    gaps: [],
    drilldownRefs: [`artifact:${artifactOutputKey}`],
    ...(runtimeCoordinate ? { runtimeCoordinate } : null),
  }
}
