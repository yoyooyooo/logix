...
describe('Verification control-plane compare authority', () => {
  ...
  it('treats FAIL to PASS with matching admissibility as repair closure', () => {
    ...
    expect(result.nextRecommendedStage).toBeNull()
  })

  it('distinguishes regression and unresolved mismatches from closed repairs', () => {
    const env = { host: 'node', declarationDigest: 'decl:same', scenarioPlanDigest: 'scenario:none' }
    const regression = compareVerificationControlPlaneReports({
      runId: 'core-compare-regression',
      before: makeReport('before-pass', env),
      after: makeFailReport('after-fail', env),
    })
    const unresolved = compareVerificationControlPlaneReports({
      runId: 'core-compare-unresolved',
      before: makeFailReport('before-fail', env),
      after: makeFailReport('after-fail', env, {
    ...
    expect(regression.repairHints[0]).toMatchObject({
    ...
    expect(unresolved.repairHints[0]).toMatchObject({
