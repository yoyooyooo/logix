...
describe('DevtoolsHub (core)', () => {
  ...
  it.effect('exportBudget should accumulate dropped/oversized counts from JsonValue projection', () =>
    Effect.gen(function* () {
      ...
      const firstEntry = Array.from(snapshot.exportBudget.byEvent.values())[0] as any
      expect(firstEntry.costClass).toBe('runtime_core')
      expect(firstEntry.gateClass).toBe('hard')
      expect(firstEntry.samplingPolicy).toBe('always')
      expect(() => JSON.stringify(snapshot)).not.toThrow()
    }),
  )
