...
describe('FieldKernel converge auto decision budget (small steps)', () => {
  it.effect('small steps should not be forced into long-term full fallback', () =>
    Effect.gen(function* () {
      ...
      const { M, runtime, ring } = makeSmallStepsFixture({
        moduleId: 'FieldKernelConvergeAuto_DecisionBudget_SmallSteps',
        steps,
        diagnosticsLevel: 'light',
        stateTransaction: {
          fieldConvergeMode: 'auto',
          fieldConvergeDecisionBudgetMs: 2,
        },
      })
