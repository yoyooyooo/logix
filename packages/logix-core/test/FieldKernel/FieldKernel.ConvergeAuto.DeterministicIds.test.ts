...
describe('FieldKernel converge auto deterministic ids', () => {
  it.effect('uses stable instanceId/txnSeq/opSeq anchors (no random/time defaults)', () =>
    Effect.gen(function* () {
      ...
      const M = FieldContracts.withModuleFieldDeclarations(Logix.Module.make('FieldKernelConvergeAuto_DeterministicIds', {
  state: State,
  actions: Actions,
  reducers: {
          noop: (s: any) => s,
      ...
      const effectOps = pkg.events
        ...
        .filter((p) => p && typeof p === 'object' && (p.kind === 'field-computed' || p.kind === 'field-link'))
  ...
  it.effect('staticIrDigest is stable across independent runs (same converge shape)', () =>
    Effect.gen(function* () {
      ...
      const M = FieldContracts.withModuleFieldDeclarations(Logix.Module.make('FieldKernelConvergeAuto_DeterministicIds_Digest', {
  state: State,
  actions: Actions,
  reducers: {
          noop: (s: any) => s,
