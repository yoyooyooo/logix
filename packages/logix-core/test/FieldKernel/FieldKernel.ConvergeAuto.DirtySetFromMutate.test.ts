...
describe('FieldKernel converge · dirty-set from $.state.mutate', () => {
  it.effect('$.state.mutate and plain reducer replace should both produce field-level dirty roots', () =>
    Effect.gen(function* () {
  ...
  reducers: {
      ...
      const program = Effect.gen(function* () {
        ...
        // dispatch txn: plain reducer whole-state replace should now infer exact field-level dirty evidence too
        ...
        expect(afterReducer?.fieldSummary?.converge?.dirty?.dirtyAll).toBe(false)
        expect(afterReducer?.fieldSummary?.converge?.dirty?.rootCount).toBe(1)
        expect(afterReducer?.fieldSummary?.converge?.stepStats?.totalSteps).toBe(2)
        expect(afterReducer?.fieldSummary?.converge?.stepStats?.executedSteps).toBe(1)
        expect(afterReducer?.fieldSummary?.converge?.stepStats?.skippedSteps).toBe(1)
      })
