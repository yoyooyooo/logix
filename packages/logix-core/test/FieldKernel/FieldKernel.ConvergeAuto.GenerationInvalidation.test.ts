...
describe('FieldKernel converge auto generation invalidation', () => {
  it.effect('generation bump strictly invalidates cache (missReason=generation_bumped)', () =>
    Effect.gen(function* () {
      ...
      const { M, runtime, ring, State, baseSpec } = makeGenerationFixture({
        moduleId: 'FieldKernelConvergeAuto_GenerationInvalidation',
        steps,
      ...
      const program = Effect.gen(function* () {
        ...
        const nextSpec = FieldContracts.fieldFrom(State as any)({
          ...(baseSpec as any),
          dExtra: FieldContracts.fieldComputed<any, any, any>({
            deps: ['inExtra'],
            get: (inExtra: any) => (inExtra as number) + 1,
          }),
        })

        const nextProgram = FieldContracts.buildFieldProgram(State as any, nextSpec as any)
        FieldContracts.registerFieldProgram(rt, nextProgram)
      ...
      // t4 is emitted after registerFieldProgram(nextProgram), so staticIrDigest must switch.
  ...
  it.effect('generation thrash triggers cache self-protection (disableReason=generation_thrash)', () =>
    Effect.gen(function* () {
      const steps = 10
      const { M, runtime, ring, State, baseSpec } = makeGenerationFixture({
        moduleId: 'FieldKernelConvergeAuto_GenerationThrash',
        steps,
        diagnosticsLevel: 'light',
        stateTransaction: {
          fieldConvergeMode: 'auto',
      ...
      const program = Effect.gen(function* () {
        ...
        const stableProgram = FieldContracts.buildFieldProgram(State as any, baseSpec as any)

        FieldContracts.registerFieldProgram(rt, stableProgram, {
