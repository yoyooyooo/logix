import { describe, it, expect } from '@effect/vitest'
import * as FieldContracts from '@logixjs/core/repo-internal/field-contracts'
import { Effect, Layer, Schema } from 'effect'
import * as Logix from '../../src/index.js'
import * as Debug from '../../src/internal/debug-api.js'

describe('FieldKernel converge dirty-set scheduling', () => {
  const State = Schema.Struct({
    a: Schema.Number,
    b: Schema.Number,
    derivedA: Schema.Number,
    derivedB: Schema.Number,
  })

  type S = Schema.Schema.Type<typeof State>

  const Actions = { noop: Schema.Void, setA: Schema.Number }

  const M = FieldContracts.withModuleFieldDeclarations(Logix.Module.make('FieldKernelConvergeDirtySet', {
  state: State,
  actions: Actions,
  reducers: {
      noop: (s: any) => s,
      setA: Logix.Module.Reducer.mutate((draft, value: number) => {
        draft.a = value
      }),
    }
}), FieldContracts.fieldFrom(State)({
      derivedA: FieldContracts.fieldComputed({
        deps: ['a'],
        get: (a) => a + 1,
      }),
      derivedB: FieldContracts.fieldComputed({
        deps: ['b'],
        get: (b) => b + 1,
      }),
    }))

  const programModule = Logix.Program.make(M, {
    initial: { a: 0, b: 0, derivedA: 1, derivedB: 1 },
    logics: [],
  })

  it.effect('runs only affected writers when dirtyPaths are specific', () =>
    Effect.gen(function* () {
      const ring = Debug.makeRingBufferSink(64)

      const runtime = Logix.Runtime.make(programModule, {
        stateTransaction: { fieldConvergeMode: 'dirty' },
        layer: Layer.mergeAll(
          Debug.replace([ring.sink]) as Layer.Layer<any, never, never>,
          Debug.diagnosticsLevel('light'),
        ) as Layer.Layer<any, never, never>,
      })

      const program = Effect.gen(function* () {
        const rt = yield* Effect.service(M.tag).pipe(Effect.orDie)
        yield* rt.dispatch({ _tag: 'setA', payload: 1 } as any)

        const updates = ring
          .getSnapshot()
          .filter((e) => e.type === 'state:update' && (e as any).txnId) as ReadonlyArray<any>
        expect(updates.length).toBeGreaterThan(0)

        const last = updates[updates.length - 1]
        expect(last?.fieldSummary?.converge?.requestedMode).toBe('dirty')
        expect(last?.fieldSummary?.converge?.executedMode).toBe('dirty')
        expect(last?.fieldSummary?.converge?.dirty?.dirtyAll).toBe(false)
        expect(last?.fieldSummary?.converge?.dirty?.rootCount).toBe(1)
        expect(Array.isArray(last?.fieldSummary?.converge?.dirty?.rootIds)).toBe(true)
        expect(last?.fieldSummary?.converge?.dirty?.rootIds?.length).toBe(1)
        expect(last?.fieldSummary?.converge?.dirty?.rootIdsTruncated).toBe(false)
        expect(last?.fieldSummary?.converge?.stepStats?.totalSteps).toBe(2)
        expect(last?.fieldSummary?.converge?.stepStats?.executedSteps).toBe(1)
        expect(last?.fieldSummary?.converge?.stepStats?.skippedSteps).toBe(1)

        expect(last?.state?.a).toBe(1)
        expect(last?.state?.derivedA).toBe(2)
        expect(last?.state?.derivedB).toBe(1)
      })

      yield* Effect.promise(() => runtime.runPromise(program as Effect.Effect<void, never, any>))
    }),
  )

  it.effect('falls back to full scheduling when only wildcard dirtyPaths exist', () =>
    Effect.gen(function* () {
      const ring = Debug.makeRingBufferSink(64)

      const runtime = Logix.Runtime.make(programModule, {
        stateTransaction: { fieldConvergeMode: 'dirty' },
        layer: Layer.mergeAll(
          Debug.replace([ring.sink]) as Layer.Layer<any, never, never>,
          Debug.diagnosticsLevel('light'),
        ) as Layer.Layer<any, never, never>,
      })

      const program = Effect.gen(function* () {
        const rt = yield* Effect.service(M.tag).pipe(Effect.orDie)

        yield* FieldContracts.runWithStateTransaction(
          rt as any,
          { kind: 'test', name: 'dirty-wildcard' },
          () =>
            Effect.gen(function* () {
              const prev = yield* rt.getState
              yield* rt.setState({ ...prev, a: 2 })
            }),
        )

        const updates = ring
          .getSnapshot()
          .filter((e) => e.type === 'state:update' && (e as any).txnId) as ReadonlyArray<any>
        expect(updates.length).toBeGreaterThan(0)

        const last = updates[updates.length - 1]
        expect(last?.fieldSummary?.converge?.requestedMode).toBe('dirty')
        expect(last?.fieldSummary?.converge?.executedMode).toBe('dirty')
        expect(last?.fieldSummary?.converge?.dirty?.dirtyAll).toBe(true)
        expect(last?.fieldSummary?.converge?.dirty?.rootCount).toBe(0)
        expect(last?.fieldSummary?.converge?.dirty?.rootIds).toEqual([])
        expect(last?.fieldSummary?.converge?.dirty?.rootIdsTruncated).toBe(false)
        expect(last?.fieldSummary?.converge?.stepStats?.totalSteps).toBe(2)
        expect(last?.fieldSummary?.converge?.stepStats?.executedSteps).toBe(2)
        expect(last?.fieldSummary?.converge?.stepStats?.skippedSteps).toBe(0)
      })

      yield* Effect.promise(() => runtime.runPromise(program as Effect.Effect<void, never, any>))
    }),
  )
})
