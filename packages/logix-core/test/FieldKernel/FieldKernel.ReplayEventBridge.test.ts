...
describe('ReplayEvent ↔ state:update bridge', () => {
  it.effect('records last ResourceSnapshot as state:update.replayEvent', () =>
    Effect.gen(function* () {
      ...
      const spec = Resource.make<Schema.Schema.Type<typeof KeySchema>, { readonly name: string }, never, never>({
        id: 'user/profile',
        keySchema: KeySchema,
      ...
      const M = FieldContracts.withModuleFieldDeclarations(Logix.Module.make('FieldKernelReplayEventBridge', {
  state: State,
  actions: Actions,
  reducers: {
          refresh: (s: any) => s,
      ...
      const RefreshLogic = M.logic('refresh-logic', ($) =>
        Effect.gen(function* () {
          yield* $.onAction('refresh').run(() => $.fields.source.refresh('profileResource'))
        }),
      )
