...
describe('LogicFields setup · perf baseline (Diagnostics=off)', () => {
  it.effect('module init + fields merge/install p95 baseline', () =>
    Effect.gen(function* () {
      ...
      const allTraits = FieldContracts.fieldFrom(State)({
        ...Object.fromEntries(
          Array.from({ length: 50 }, (_, i) => [
            `d${i}`,
            FieldContracts.fieldComputed({
              deps: [`v${i}`],
              get: (v) => v,
            }),
          ]),
      ...
      const M = Logix.Module.make('LogicFieldsPerfOff', {
        state: State,
        actions: Actions,
      })

      const makeLogic = (id: string, fields: Record<string, unknown>) =>
        M.logic(id, ($) => {
          $.fields(fields)
          return Effect.void
        })
      ...
      // Intended for manually filling the current LogicFields perf baseline notes.
      console.log(
        `[perf] LogicFields.Setup.off fields=100 contribs=4 iters=${iterations} p50=${p50.toFixed(
          2,
