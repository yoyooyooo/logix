...
describe('Runtime.run signals (US1)', () => {
  it.effect('handleSignals triggers graceful shutdown and removes listeners', () =>
    Effect.gen(function* () {
      ...
      const Root = Logix.Module.make('Runtime.run.signals', {
        state: Schema.Void,
        actions: {},
      })
      ...
      const p = Logix.Runtime.run(program, () => Effect.never, {
        layer: slowBootLayer,
        handleSignals: true,
      ...
      const timeoutError = new Error('run did not settle in time')
