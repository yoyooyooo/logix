...
describe('Runtime.runProgram error taxonomy (US1)', () => {
  it.effect('BootError is distinguishable and includes stable identity fields', () =>
    Effect.gen(function* () {
      ...
      const DummyTag = ServiceMap.Service<{}>('@test/Runtime.runProgram.errorCategory/Boot')
  ...
  it.effect('MainError is distinguishable and includes stable identity fields', () =>
    Effect.gen(function* () {
      ...
      const mainOutcome = yield* Effect.promise(() =>
        Logix.Runtime.runProgram(impl, () => Effect.die(new Error('main fail')), {
  ...
  it.effect('DisposeTimeout is distinguishable and includes actionable suggestions', () =>
    Effect.gen(function* () {
      ...
      const disposeOutcome = yield* closeProgramScope({
        scope,
        timeoutMs: 10,
        identity: { moduleId: 'Runtime.runProgram.errorCategory.DisposeTimeout', instanceId: 'i1' },
      }).pipe(Effect.exit)
