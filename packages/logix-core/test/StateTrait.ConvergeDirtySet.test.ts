  ...
  const M = Logix.Module.make("StateTraitConvergeDirtySet", {
    ...
    traits: Logix.StateTrait.from(State)({
      derivedA: Logix.StateTrait.computed({
        deps: ["a"],
        get: (s: Readonly<S>) => s.a + 1,
      }),
      derivedB: Logix.StateTrait.computed({
        deps: ["b"],
        get: (s: Readonly<S>) => s.b + 1,
      }),
    }),
  })
  ...
  it.scoped("runs only affected writers when dirtyPaths are specific", () =>
    Effect.gen(function* () {
      ...
      const program = Effect.gen(function* () {
        ...
        yield* (rt as any).__runWithStateTransaction(
          { kind: "test", name: "dirty-specific" },
          () =>
            Effect.gen(function* () {
              const prev = yield* rt.getState
              yield* rt.setState({ ...prev, a: 1 })
              ;(rt as any).__recordStatePatch({
                path: "a",
                reason: "test:manual-path",
              })
            }),
        )
        ...
        expect(last?.traitSummary?.converge?.mode).toBe("dirty")
        expect(last?.traitSummary?.converge?.dirtyRoots).toEqual(["a"])
        expect(last?.traitSummary?.converge?.totalSteps).toBe(2)
        expect(last?.traitSummary?.converge?.executedSteps).toBe(1)
        expect(last?.traitSummary?.converge?.skippedSteps).toBe(1)
  ...
  it.scoped("falls back to full scheduling when only wildcard dirtyPaths exist", () =>
    Effect.gen(function* () {
      ...
      const program = Effect.gen(function* () {
        ...
        expect(last?.traitSummary?.converge?.mode).toBe("dirty")
        expect(last?.traitSummary?.converge?.totalSteps).toBe(2)
        expect(last?.traitSummary?.converge?.executedSteps).toBe(2)
        expect(last?.traitSummary?.converge?.skippedSteps).toBe(0)
      })
