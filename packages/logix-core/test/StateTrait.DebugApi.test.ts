...
describe("Debug.getModuleTraits", () => {
  it("should expose StateTraitProgram / graph / plan for a Module with traits", () => {
    const traits = Logix.Debug.getModuleTraits(
      CounterWithProfile as any,
    )
    ...
    const hasComputedStep = program.plan.steps.some(
      (s) =>
        s.kind === "computed-update" &&
        s.targetFieldPath === "sum",
    )
