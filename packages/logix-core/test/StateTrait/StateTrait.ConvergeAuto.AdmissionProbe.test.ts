...
const makeAdmissionProbeFixture = (options: {
...
const printProbeTable = (summaries: ReadonlyArray<ScenarioAggregate>): void => {
  console.table(
...
describe('StateTrait converge auto admission probe', () => {
  it.effect('quantifies unknown_write / dirty_all / near_full against covered single-field writes', () =>
    Effect.gen(function* () {
      ...
      const collectScenario = (
        ...
        Effect.gen(function* () {
          const { M, runtime, ring } = makeAdmissionProbeFixture({
            moduleId: `StateTraitConvergeAutoAdmissionProbe_${name}`,
      ...
      expect(byName.unknown_write_setState?.executedModes).toEqual(['full'])
      expect(byName.unknown_write_setState?.reasonCounts.unknown_write).toBe(ANALYZED_SAMPLES)
      expect(byName.unknown_write_setState?.reasonCounts.dirty_all ?? 0).toBe(0)
      expect(byName.unknown_write_setState?.dirtyReasonCounts.unknownWrite).toBe(ANALYZED_SAMPLES)
      expect(byName.unknown_write_setState?.dirtyAllCount).toBe(ANALYZED_SAMPLES)
      expect(byName.unknown_write_setState?.medianDirtyRootCount).toBe(0)
      expect(byName.unknown_write_setState?.medianExecutedSteps).toBe(INPUT_COUNT)
      expect(byName.unknown_write_setState?.medianSkippedSteps).toBe(0)
      ...
      expect(byName.plain_replace_single_field?.executedModes).toEqual(['full'])
      expect(byName.plain_replace_single_field?.reasonCounts.unknown_write).toBeGreaterThan(0)
      expect(byName.plain_replace_single_field?.dirtyAllCount).toBe(ANALYZED_SAMPLES)
      expect(byName.plain_replace_single_field?.medianDirtyRootCount).toBe(0)
      expect(byName.plain_replace_single_field?.medianExecutedSteps).toBe(INPUT_COUNT)
      expect(byName.plain_replace_single_field?.medianSkippedSteps).toBe(0)
      ...
      expect(byName.unknown_write_setState!.medianExecutedSteps).toBeGreaterThan(
