...
describe('FieldKernel field:check diagnostics levels', () => {
  ...
  const runWithDiagnostics = (options: {
    readonly level: CoreDebug.DiagnosticsLevel
    ...
    Effect.gen(function* () {
      ...
      const ring = CoreDebug.makeRingBufferSink(200)
      const layer = Layer.mergeAll(
        CoreDebug.replace([ring.sink]),
        CoreDebug.diagnosticsLevel(options.level),
      ) as Layer.Layer<any, never, any>

      const ctx: FieldValidate.ValidateContext<any> = {
        moduleId: 'M',
        instanceId: 'i-field-check',
        txnSeq: 1,
        txnId: 'i-field-check::t1',
        origin: {
          kind: 'action',
          name: 'dispatch',
          details: { _tag: 'setValue', path: options.originPath, op: options.originOp },
        },
        rowIdStore: new RowId.RowIdStore('i-field-check'),
      ...
      const events = ring.getSnapshot().filter((e) => e.type === 'trace:field:check')
      return events
    })

  it.effect('Diagnostics=off does not emit field:check events', () =>
    Effect.gen(function* () {
  ...
  it.effect('Diagnostics=light emits exportable field:check events', () =>
    Effect.gen(function* () {
      const events = yield* runWithDiagnostics({
        level: 'light',
        originPath: 'items.0.warehouseId',
      ...
      const ref = CoreDebug.internal.toRuntimeDebugEventRef(events[0] as any, {
        diagnosticsLevel: 'light',
      }) as any
      expect(ref?.kind).toBe('field:check')
  ...
  it.effect('Diagnostics=full emits exportable field:check events', () =>
    Effect.gen(function* () {
      const events = yield* runWithDiagnostics({
        level: 'full',
        originPath: 'items.0.warehouseId',
