...
describe('FieldKernel converge auto NoAsyncGuard', () => {
  it('auto decision path stays sync-only (no Promise/Effect.async/Effect.promise/Effect.tryPromise)', () => {
    assertNoAsync(readText(new URL('../../../src/internal/field-kernel/converge.ts', import.meta.url)), 'converge.ts')

    // Future-proof: once plan-cache.ts exists, keep it sync-only as well.
    const planCacheUrl = new URL('../../../src/internal/field-kernel/plan-cache.ts', import.meta.url)
