...
import { makeConvergeAutoFixture, pickConvergeTraceEvents } from '../../FieldKernel/FieldKernel.ConvergeAuto.fixtures.js'

const lastConvergeEvent = (
  ring: Debug.RingBufferSink,
): Extract<Debug.Event, { readonly type: 'trace:field:converge' }> | undefined => {
  const events = pickConvergeTraceEvents(ring.getSnapshot())
  return events.length > 0 ? events[events.length - 1] : undefined
}

describe('FieldKernel converge auto trace event', () => {
