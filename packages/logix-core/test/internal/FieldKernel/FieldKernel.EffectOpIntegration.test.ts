  ...
  it('should emit EffectOp events for computed and link steps via middleware', async () => {
    ...
    const testEffect = Effect.gen(function* () {
      ...
      const runtime = yield* ModuleRuntimeImpl.make<State, Action>(initial, {
        moduleId: 'FieldKernelEffectOpIntegrationTest',
      })
      ...
      // Install the FieldKernel program behavior (forks watchers within the current Scope).
    ...
    expect(computedOp.kind).toBe('field-computed')
    expect(computedOp.meta?.fieldPath).toBe('sum')
    expect(computedOp.meta?.moduleId).toBe('FieldKernelEffectOpIntegrationTest')

    expect(linkOp.kind).toBe('field-link')
    expect(linkOp.meta?.from).toBe('source.name')
    expect(linkOp.meta?.to).toBe('target')
    expect(linkOp.meta?.moduleId).toBe('FieldKernelEffectOpIntegrationTest')
  })
})
