...
describe('FieldKernel.externalStore external-owned governance', () => {
  it('detects multiple writers (computed + externalStore)', () => {
    ...
    const fieldSpec = FieldContracts.fieldFrom(State)({
      value: FieldContracts.fieldNode({
        computed: FieldContracts.fieldComputed({
          deps: ['base'],
          get: (base) => base + 1,
        }),
        externalStore: FieldContracts.fieldExternalStore({ store }),
      }),
    })

    const program = FieldContracts.buildFieldProgram(State, fieldSpec)
  ...
  it.effect('fails fast on non-trait writes overlapping external-owned fields', () =>
    Effect.gen(function* () {
      ...
      const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
        { value: -1, other: 0 },
        {
          moduleId: 'FieldKernelExternalStoreOwnershipTest',
        },
      )
  ...
  it.effect('fails fast with external-owned fields containing special key characters', () =>
    Effect.gen(function* () {
      ...
      const program = FieldContracts.buildFieldProgram(
        StateSchema,
        FieldContracts.fieldFrom(StateSchema)({
          [ownedKey]: FieldContracts.fieldExternalStore({ store }),
        } as any),
      )
      ...
      const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
        ...
        {
          moduleId: 'FieldKernelExternalStoreOwnershipSpecialKeyTest',
        },
      )
