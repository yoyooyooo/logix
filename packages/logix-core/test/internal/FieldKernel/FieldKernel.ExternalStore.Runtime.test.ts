...
const CoreContracts = {
  withModuleFieldDeclarations: FieldContracts.withModuleFieldDeclarations,
  fieldFrom: FieldContracts.fieldFrom,
  fieldExternalStore: FieldContracts.fieldExternalStore,
  buildFieldProgram: FieldContracts.buildFieldProgram,
  installFieldProgram: FieldContracts.installFieldProgram,
  exportFieldStaticIr: FieldContracts.exportFieldStaticIr,
  getRuntimeStore: RuntimeContracts.getRuntimeStore,
};
  ...
  Effect.scoped(
    Effect.gen(function* () {
      ...
      const modules = stores.map((entry, index) =>
        CoreContracts.withModuleFieldDeclarations(Logix.Module.make(`ExternalStoreIngestProxyPerf.M${index}`, {
  state: StateSchema,
  actions: {}
}), CoreContracts.fieldFrom(StateSchema)({
            value: CoreContracts.fieldExternalStore({ store: entry.store }),
          })),
      );
      ...
      const modulePrograms = modules.map((moduleDef, index) =>
        Logix.Program.make(moduleDef, {
          initial: { value: index },
          capabilities: {
            imports: [],
          },
      ...
      const runtimeStore = CoreContracts.getRuntimeStore(runtime as any);
...
describe("FieldKernel.externalStore runtime semantics", () => {
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const program = CoreContracts.buildFieldProgram(
          StateSchema,
          CoreContracts.fieldFrom(StateSchema)({
            value: CoreContracts.fieldExternalStore({ store }),
          })
        );
        ...
        const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
          { value: -1 },
          {
            moduleId: "FieldKernelExternalStoreAtomicInitTest",
          }
        );
        ...
        yield* CoreContracts.installFieldProgram(bound as any, program);
  ...
  it.effect("select/equals gating avoids redundant commits", () =>
    Effect.gen(function* () {
      ...
      const program = CoreContracts.buildFieldProgram(
        StateSchema,
        CoreContracts.fieldFrom(StateSchema)({
          value: CoreContracts.fieldExternalStore({
            store,
      ...
      const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
        { value: 0 },
        {
          moduleId: "FieldKernelExternalStoreSelectEqualsTest",
        }
      );
    ...
    () =>
      Effect.gen(function* () {
        ...
        const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
          { value: -1 },
          {
            moduleId: "FieldKernelExternalStoreSignalDedupTest",
          }
        );
  ...
  it.effect("getSnapshot throw fuses the trait (no further writebacks)", () => {
    const ring = CoreDebug.makeRingBufferSink(256);
    const DebugLayer = CoreDebug.replace([ring.sink]);
    ...
    return Effect.gen(function* () {
      ...
      const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
        { value: 0 },
        {
          moduleId: "FieldKernelExternalStoreFuseTest",
        }
      );
  ...
  it("ExternalStore.fromStream fails fast when missing { initial/current }", () => {
    let thrown: unknown;
    try {
      RuntimeContracts.ExternalInput.fromStream(Stream.empty as any);
    } catch (err) {
      thrown = err;
    }
  ...
  it("Static IR export includes ExternalStore policy (ownership/lane/source)", () => {
    const StateSchema = Schema.Struct({ value: Schema.Number });
    const store = RuntimeContracts.ExternalInput.fromStream(Stream.empty as any, {
      current: 1,
    });
    ...
    const ir = CoreContracts.exportFieldStaticIr(
      program,
      "FieldKernelExternalStoreStaticIrTest",
      { version: "009" }
    );
    ...
    () =>
      Effect.gen(function* () {
        ...
        const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
          { value: 0 },
          {
            moduleId: "FieldKernelExternalStoreNonUrgentPriorityTest",
          }
        );
    ...
    () =>
      Effect.gen(function* () {
        ...
        const runtime = yield* ModuleRuntimeImpl.make<State, Action>(
          { value: 0 },
          {
            moduleId: "FieldKernelExternalStoreGuardDefectTest",
          }
        );
