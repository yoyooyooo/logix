  ...
  it('two modules under load should both make progress and diagnostics should be distinguishable', async () => {
    ...
    const program = Effect.provideService(Effect.scoped(
      Effect.gen(function* () {
        ...
        const consumerFiberA = yield* Effect.forkChild(consumerA)
        const consumerFiberB = yield* Effect.forkChild(consumerB)
        ...
        const dispatchFiber = yield* Effect.forkChild(Effect.all(
          [
