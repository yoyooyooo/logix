...
describe('ModuleRuntime dispatch outer shell perf boundary', () => {
  it.effect('compares public dispatch vs public setState vs direct txn setState', () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        const M = Logix.Module.make('ModuleRuntime.DispatchOuterShell.PerfBoundary', {
          state: Schema.Struct({
            count: Schema.Number,
          }),
          actions: {
        ...
        const summary = [
          `[perf] dispatch-outer-shell-boundary iters=${iterations} warmup=${warmup}`,
