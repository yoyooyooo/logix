    ...
    it.effect('should support dev-only time-travel: applyTransactionSnapshot(before) restores previous state', () =>
      Effect.gen(function* () {
        ...
        const program = Effect.scoped(
          ...
          Effect.provideService(Debug.internal.currentDiagnosticsLevel as any, 'light'),
          Effect.provideService(Debug.internal.currentDebugSinks as any, [ring.sink as Debug.Sink]),
        )
    ...
    it.effect('fields.source.refresh should not deadlock when called inside an existing transaction', () =>
      Effect.gen(function* () {
        ...
        const program = Effect.scoped(
          ...
          Effect.provideService(Debug.internal.currentDiagnosticsLevel as any, 'light'),
          Effect.provideService(Debug.internal.currentDebugSinks as any, [ring.sink as Debug.Sink]),
        ) as unknown as Effect.Effect<void, never, any>
