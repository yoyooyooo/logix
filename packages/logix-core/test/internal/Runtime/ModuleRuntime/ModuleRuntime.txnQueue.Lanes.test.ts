...
describe('ModuleRuntime.txnQueue (lanes)', () => {
  ...
  it.effect('should resolve policy only for the blocked acquire path after an idle fast-path execution', () =>
    Effect.gen(function* () {
      const policy: ResolvedConcurrencyPolicy = {
        concurrencyLimit: 16,
        losslessBackpressureCapacity: 1,
  ...
  it.effect('should skip policy resolution for uncontended idle executions when diagnostics are off', () =>
    Effect.gen(function* () {
      const policy: ResolvedConcurrencyPolicy = {
        concurrencyLimit: 16,
        losslessBackpressureCapacity: 16,
      ...
      const runOff = <A, E>(eff: Effect.Effect<A, E, never>) =>
        Effect.provideService(eff, Debug.internal.currentDiagnosticsLevel, 'off')

      yield* runOff(enqueueTransaction(Effect.sync(() => {
        executed.push('urgent')
      })))

      yield* runOff(enqueueTransaction(
        'nonUrgent',
        Effect.sync(() => {
          executed.push('nonUrgent')
        }),
      ))

      expect(executed).toEqual(['urgent', 'nonUrgent'])
