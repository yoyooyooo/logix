...
describe('StateTrait.externalStore runtime semantics', () => {
  it.scoped('init is atomic (no missed updates between getSnapshot and subscribe)', () =>
    Effect.gen(function* () {
  ...
  it.scoped('select/equals gating avoids redundant commits', () =>
    Effect.gen(function* () {
      ...
      const commits: Array<{ readonly value: number; readonly originKind?: string }> = []
      yield* Effect.forkScoped(
        Stream.runForEach(runtime.changesWithMeta((s) => s.value), ({ value, meta }) =>
          Effect.sync(() => commits.push({ value, originKind: meta.originKind })),
        ),
      )
  ...
  it.scoped('Signal Dirty dedups bursts within the same microtask (writes latest only once)', () =>
    Effect.gen(function* () {
      const StateSchema = Schema.Struct({ value: Schema.Number })
      type State = Schema.Schema.Type<typeof StateSchema>
      ...
      yield* Effect.forkScoped(
        Stream.runForEach(runtime.changesWithMeta((s) => s.value), ({ value }) => Effect.sync(() => commits.push(value))),
      )
      yield* flushMicrotasks(1)
      commits.length = 0
  ...
  it('Static IR export includes ExternalStore policy (ownership/lane/source)', () => {
    const StateSchema = Schema.Struct({ value: Schema.Number })
    const store = Logix.ExternalStore.fromStream(Stream.empty as any, { current: 1 })
    ...
    const ir = Logix.StateTrait.exportStaticIr(program, 'StateTraitExternalStoreStaticIrTest', { version: '009' })
    const node = ir.nodes.find((n) => n.kind === 'externalStore')

    expect(node?.nodeId).toBe('external-store:value')
    expect(node?.policy && (node.policy as any).ownership).toBe('external-owned')
  ...
  it.scoped('ownership guard surfaces as a defect (ExternalOwnedWriteError)', () =>
    Effect.gen(function* () {
      const StateSchema = Schema.Struct({ value: Schema.Number })
      type State = Schema.Schema.Type<typeof StateSchema>
