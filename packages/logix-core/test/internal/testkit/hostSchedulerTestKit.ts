...
import { makeDeterministicHostScheduler, type DeterministicHostScheduler } from '../../../src/internal/runtime/core/HostScheduler.js'
import { hostSchedulerTestStubLayer } from '../../../src/internal/runtime/core/env.js'

export const makeTestHostScheduler = (): DeterministicHostScheduler => makeDeterministicHostScheduler()
...
export const flushAllHostScheduler = (
  ...
  Effect.gen(function* () {
    ...
    const settleYields = options?.settleYields ?? 256
    ...
    const { microtasks, macrotasks } = scheduler.getQueueSize()
    throw new Error(
      `[TestKit.flushAllHostScheduler] Exceeded maxTurns=${maxTurns} (microtasks=${microtasks}, macrotasks=${macrotasks}).`,
    )
  }).pipe(Effect.asVoid)
...
export const advanceTicks = (args: {
  ...
  Effect.gen(function* () {
    ...
    throw new Error(`[TestKit.advanceTicks] Expected >=${args.n} ticks, got ${end - start}.`)
  }).pipe(Effect.asVoid)
