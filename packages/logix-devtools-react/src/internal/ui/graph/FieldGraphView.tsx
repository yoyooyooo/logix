...
export interface FieldGraphViewProps {
  /**
   * Program output from Debug.getModuleFieldProgram(module).
   * - If not provided, the component only renders a placeholder.
   */
  readonly program?: FieldContracts.FieldProgram<any>
  /**
...
export const FieldGraphView: React.FC<FieldGraphViewProps> = ({
  program,
  onSelectNode,
  selectedFieldPath,
}) => {
  if (!program) {
    return (
      ...
      >
        No field program available for this module.
      </div>
    )
  }
              ...
              <li key={node.id} className="flex items-center justify-between gap-2">
                ...
                >
                  ...
                  <span className="text-[9px]" style={{ color: 'var(--dt-text-muted)' }}>
                    {node.behaviors.map((t) => t.kind).join(', ') || '—'}
