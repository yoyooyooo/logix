...
/**
 * OperationSummaryBar：
 * - Top pinned info bar that shows the latest "operation window" summary.
 * - Controlled component; visibility is decided by the parent.
 * - Does not auto-dismiss by default; it is replaced by the next operation or manually closed by the user.
 */
export const OperationSummaryBar: React.FC<OperationSummaryBarProps> = ({ summary, onClose }) => {
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-3 text-[10px] font-mono">
        <span className="uppercase tracking-wider" style={{ color: 'var(--dt-text-muted)' }}>
          Last Operation
        </span>
        <span>events: {summary.eventCount}</span>
        <span>txns: {summary.txnCount}</span>
        <span title="trace:react-render event count (per useModule call, per commit)">
          react-render: {summary.renderCount}
        </span>
