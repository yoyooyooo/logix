...
export const loadSettingsFromStorage = (): DevtoolsSettings | undefined => {
  ...
  try {
    ...
    const operationWindowMs = clampOperationWindowMs((parsed as any).operationWindowMs ?? defaultSettings.operationWindowMs)

    const thresholds = parsed.overviewThresholds ?? defaultSettings.overviewThresholds
    const overviewThresholds = {
      txnPerSecondWarn:
        typeof thresholds.txnPerSecondWarn === 'number' ? thresholds.txnPerSecondWarn : defaultSettings.overviewThresholds.txnPerSecondWarn,
      txnPerSecondDanger:
        typeof thresholds.txnPerSecondDanger === 'number' ? thresholds.txnPerSecondDanger : defaultSettings.overviewThresholds.txnPerSecondDanger,
      renderPerTxnWarn:
        typeof thresholds.renderPerTxnWarn === 'number' ? thresholds.renderPerTxnWarn : defaultSettings.overviewThresholds.renderPerTxnWarn,
      renderPerTxnDanger:
        typeof thresholds.renderPerTxnDanger === 'number' ? thresholds.renderPerTxnDanger : defaultSettings.overviewThresholds.renderPerTxnDanger,
    }
    ...
    const reactRenderSampleRate =
      typeof sampling.reactRenderSampleRate === 'number' ? sampling.reactRenderSampleRate : defaultSettings.sampling.reactRenderSampleRate
