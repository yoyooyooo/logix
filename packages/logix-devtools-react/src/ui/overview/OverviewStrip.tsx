...
// Overview 的时间桶：
// - 目的：让「新事件到来」体现为“新增/滑动”的柱子，而不是永远 24 根柱子只做重分桶导致横向不动；
// - 使用 WeakMap 记录事件“首次观察时间”，保证同一事件在多次 render/compute 中不会重新分桶。
...
export const OverviewStrip: React.FC = () => {
  ...
  const bucketMs = React.useMemo(() => {
    ...
    const first = timeline[0]?.event as Logix.Debug.Event | undefined
    const last = timeline[timeline.length - 1]?.event as Logix.Debug.Event | undefined
  ...
  const buckets = React.useMemo<Bucket[]>(() => {
    ...
    const lastEvent = timeline[timeline.length - 1]?.event as Logix.Debug.Event | undefined
    ...
    for (let idx = 0; idx < timeline.length; idx++) {
      ...
      const event = entry.event as Logix.Debug.Event
      ...
      // 未能归一化的事件：仍然为 Overview 贡献“活动信号”，避免出现“明明有事件但柱子全空”的情况。
      if (!ref) {
        bucket.txnIds.add(`unknown@${idx}`)
      ...
      // 事务计数：
      // - 优先使用 txnId：同一事务内的多条事件只算一次；
      // - 若缺少 txnId：
      //   - state/action：用索引兜底（至少能看到密度信号）；
      //   - 其他（trait/service/...）：尝试用 EffectOp.linkId 归并，否则退化为按事件索引计数。
      ...
      // 渲染事件计数：仅统计 react-render，用于评估渲染密度。
    ...
    const buckets = rawBuckets.map((b) => {
      ...
      // level 基于“每秒”阈值：按 bucketMs 时间桶换算为每秒值。
      ...
      if (
        ...
        (txnCount > 0 &&
          // renderPerTxn* 仍按“每事务渲染密度”判断；
          // renderPerSecond 只用于与 txnPerSecond 一起触发整体危险阈值。
        ...
        >
          {buckets.map((bucket) => {
            ...
            const isHighlighted =
              !isActive &&
              highlightMs > 0 &&
              bucket.lastChangedAt != null &&
              now - bucket.lastChangedAt < highlightMs
            ...
            // 默认柱子颜色（txn density）：
            // - ok: primary（绿色）保证在深色背景下也可见；
            // - warn/danger: 使用 warning/danger 强色做强提醒。
