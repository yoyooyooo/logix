...
describe('@logixjs/devtools-react integration with @logixjs/react', () => {
  ...
  it('materializes dirty rootPaths from canonical staticIrDigest + staticIrByDigest.fieldPaths on evidence import', async () => {
    ...
    await waitFor(() => {
      ...
      expect(imported?.meta?.stateWrite?.source).toBe('boundApi.update')
      expect(imported?.meta?.stateWrite?.coverage).toBe('precisePatch')
    })
  })

  it('computes stateWrite degradeRatio / degradeUnknownShare with stable read coverage from imported evidence', async () => {
    render(<LogixDevtools position="bottom-left" initialOpen={true} />)

    const protocolVersion = Logix.Observability.protocolVersion
    const evidence = {
      protocolVersion,
      runId: 'run-state-write-ledger',
      createdAt: 5,
      source: { host: 'test' },
      events: [
        {
        ...
        },
        {
          ...
          payload: {
            ...
            meta: {
              state: { count: 3 },
              stateWrite: {
                source: 'trait.externalStore',
                coverage: 'customMutation',
                degradeReason: 'unknownWrite',
                anchor: {
                  instanceId: 'i1',
                  txnSeq: 7,
                  opSeq: 0,
    ...
    await waitFor(() => {
      const state = devtoolsRuntime.runSync(devtoolsModuleRuntime.getState as any) as DevtoolsState
      const ledger = state.operationSummary?.stateWrite
      expect(ledger).toBeDefined()

      expect(ledger?.observedCount).toBe(3)
      expect(ledger?.missingCount).toBe(1)
      expect(ledger?.degradedCount).toBe(2)
      expect(ledger?.unknownReasonCount).toBe(1)
      expect(ledger?.readCoverage).toBeCloseTo(0.75, 6)
      expect(ledger?.degradeRatio).toBeCloseTo(2 / 3, 6)
      expect(ledger?.degradeUnknownShare).toBeCloseTo(0.5, 6)

      expect(ledger?.bySource.reducer.total).toBe(1)
      expect(ledger?.bySource.reducer.degraded).toBe(1)
      expect(ledger?.bySource.boundApiUpdate.total).toBe(1)
      expect(ledger?.bySource.boundApiUpdate.degraded).toBe(0)
      expect(ledger?.bySource.traitExternalStore.total).toBe(1)
      expect(ledger?.bySource.traitExternalStore.degraded).toBe(1)
      expect(ledger?.bySource.traitExternalStore.unknownReason).toBe(1)
    })
  })
