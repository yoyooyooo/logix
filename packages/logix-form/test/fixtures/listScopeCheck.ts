...
import type { FormProgram } from '../../src/Form.js'
...
}): FormProgram<string, Values> =>
  (() => {
    return Form.make(
      options?.id ?? 'Form.ListScopeCheck',
      {
        values: ValuesSchema,
              ...
              uniqueWarehouse: {
                ...
                validateChanged: (rows: ReadonlyArray<Row>, changedIndices: ReadonlyArray<number>) => {
                  ...
                  for (const changedIndex of changedIndices) {
                    const changedValue = String(rows[changedIndex]?.warehouseId ?? '').trim()
                    if (!changedValue) continue
                    for (let i = 0; i < rows.length; i++) {
                      const v = String(rows[i]?.warehouseId ?? '').trim()
                      if (v !== changedValue) continue
                      const bucket = indicesByValue.get(v) ?? []
                      bucket.push(i)
                      indicesByValue.set(v, bucket)
                    }
