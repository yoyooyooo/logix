...
- **Browser perf matrix suite（推荐）**：真实浏览器长链路/端到端口径；产物进入 `PerfReport/PerfDiff`，可做基线对比与回归判定。
  ...
  - 矩阵/契约落点：`@logixjs/perf-evidence/assets/*`（物理路径：`packages/logix-perf-evidence/assets/*`）
...
- **Node bench（补充）**：微基准/算法/内核对比；默认不进入 `PerfReport/PerfDiff`（除非你专门做一个 collect runner）。
  - 脚本落点：`packages/logix-perf-evidence/scripts/bench.*.ts` 或 `scripts/<NNN-*>.ts`

## 1) 新增一个 Browser suite（最常见路径）

### 1.1 在矩阵里新增 suite（定义“你要测什么”）

编辑：`packages/logix-perf-evidence/assets/matrix.json`
...
- collect 落点：`packages/logix-perf-evidence/scripts/collect.ts`
- diff 落点：`packages/logix-perf-evidence/scripts/diff.ts`
- 调参实验场：`packages/logix-perf-evidence/scripts/tuning.recommend.ts`

### 2.2 纯 micro-bench（不进入 PerfReport/PerfDiff）时

新增 `packages/logix-perf-evidence/scripts/bench.*.ts`，输出建议：

- 顶层 `meta`（node/platform/iters 等）
- `results`（每个 case 的 ok/error + 统计）

并把入口加到 `packages/logix-perf-evidence/package.json`，保证“一条命令可跑完且自动退出”。
