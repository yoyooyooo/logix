...
新增/扩展场景（suite/矩阵/用例/脚本）见：`packages/logix-perf-evidence/references/authoring.md`
...
- 决策层 smoke：`pnpm -C packages/logix-perf-evidence run ci:decision-smoke`
- 夹具回放（capacity collect + summary）：`pnpm -C packages/logix-perf-evidence run ci:replay`
- 预检总入口（默认=smoke+replay）：`pnpm -C packages/logix-perf-evidence run ci:preflight`
- 需要端到端最小采样时：`pnpm -C packages/logix-perf-evidence run ci:preflight -- --with-tiny-collect`
...
- `full`：完整采集 + auto-collect，上限探索优先（成本最高）。
...
> 解释口径提醒：diff 里的 evidence 是跨点位聚合值（见实现 `packages/logix-perf-evidence/scripts/diff.ts`）：
...
- 矩阵（SSoT）：`packages/logix-perf-evidence/assets/matrix.json`
- Schema：`packages/logix-perf-evidence/assets/schemas/perf-report.schema.json`、`packages/logix-perf-evidence/assets/schemas/perf-diff.schema.json`
- 采集脚本：`packages/logix-perf-evidence/scripts/collect.ts`
- diff 脚本：`packages/logix-perf-evidence/scripts/diff.ts`
- 调参推荐脚本：`packages/logix-perf-evidence/scripts/tuning.recommend.ts`
