...
const makeRuntime = (args: {
  ...
  const rootProgramBase = Logix.Program.make(Root, {
    initial: undefined,
    capabilities: {
  ...
  const rootProgram =
    processes.length > 0
      ? Logix.Module.assembleProgram(Root, {
          initial: undefined,
          imports: [RuntimeContracts.getProgramRuntimeBlueprint(sourceProgram)],
          processes,
        })
      : rootProgramBase

  return Logix.Runtime.make(rootProgram, {
    label: `Perf012Process:${args.mode}:${args.processCount}:${args.diagnosticsLevel}`,
    layer: Debug.noopLayer as Layer.Layer<any, never, never>,
  })
}
...
const runOnce = async (args: {
  ...
  try {
    ...
    const events = (await runtime.runPromise(RuntimeContracts.getProcessEvents() as any)) as ReadonlyArray<any>
