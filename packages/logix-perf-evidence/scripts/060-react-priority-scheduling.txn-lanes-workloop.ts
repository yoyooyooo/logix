...
}): { readonly M: any; readonly program: any } => {
...
const benchOnce = (args: {
  ...
  const { M, program } = makeModule({ steps: args.steps, iters: args.iters })
...
const main = async (): Promise<void> => {
  ...
  const diff = {
    meta: {
      ...
      generator: 'packages/logix-perf-evidence/scripts/060-react-priority-scheduling.txn-lanes-workloop.ts',
      envId,
      config: {
        runs: RUNS,
        warmupDiscard: WARMUP_DISCARD,
