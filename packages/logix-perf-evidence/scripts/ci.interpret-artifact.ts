...
const findArtifactFiles = async (artifactDir: string): Promise<{
  ...
  readonly dispatchShellTaxReportMd?: string
  ...
  const dispatchShellTaxReportMd = files.find((f) => path.basename(f).startsWith('runtime-dispatch-shell-tax-migration-report.') && f.endsWith('.md'))
  const diffJsons = files
    .filter((f) => f.endsWith('.json') && path.basename(f).startsWith('diff.'))
    .sort((a, b) => a.localeCompare(b))

  return { summaryMd, dispatchShellTaxReportMd, diffJsons }
}
...
const renderFromDiff = (diff: PerfDiff): string => {
  ...
  const dispatchShellTaxReport = suites.find((suite) => suite.id === 'dispatchShell.fixedCost')
  const dispatchShellHasTaxEvidence =
    Boolean(
      dispatchShellTaxReport?.evidenceDeltas?.some(
        (delta: any) => typeof delta?.name === 'string' && delta.name.startsWith('runtime.txnPhase.'),
      ),
    ) ||
    Boolean(dispatchShellTaxReport?.metricDeltas?.some((delta) => delta.metric === 'runtime.txnCommitMs'))

  if (dispatchShellHasTaxEvidence) {
    lines.push(
      '- dispatchShell.fixedCost: run `pnpm perf ci:dispatch-shell-tax-report -- --diff <diff.json> --profile default --out <report.md>` before making a tax-removed claim',
    )
  }
...
const main = async (): Promise<void> => {
  ...
  const { summaryMd, dispatchShellTaxReportMd, diffJsons } = await findArtifactFiles(artifactDir)
  ...
  const taxReportText = dispatchShellTaxReportMd ? await readTextIfExists(dispatchShellTaxReportMd) : undefined
  if (taxReportText && taxReportText.trim().length > 0) {
    const rendered = `### logix-perf artifact 解读\n\n${taxReportText.trimEnd()}\n`
    if (out) {
      await fs.mkdir(path.dirname(out), { recursive: true })
