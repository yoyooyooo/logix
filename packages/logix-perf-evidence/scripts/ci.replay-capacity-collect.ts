...
const { decideCapacityCollectAction } = require('../../../.github/scripts/lib/logix-perf-capacity-decision.cjs') as {
  readonly decideCapacityCollectAction: (input: Record<string, unknown>) => {
    readonly decision: string
    readonly shouldExtend: boolean
    readonly topSaturated: boolean
  }
...
const readFixture = async (): Promise<Fixture> => {
  const fixturePath = path.resolve(
    process.cwd(),
    'packages/logix-perf-evidence/fixtures/perf-sweep/capacity-collect-replay-cases.json',
  )
...
const runBudgetControllerCase = (item: BudgetControllerCase): void => {
  ...
  for (const [index, step] of item.timeline.entries()) {
    ...
    if (out.ok !== step.expect.ok) {
      throw new Error(
        `[capacity-collect-replay:${item.id}] timeline[${index}] ok mismatch, expect=${String(step.expect.ok)} got=${String(out.ok)}`,
      )
    }
    if (step.expect.reasonCode !== undefined) {
      if (out.reasonCode !== step.expect.reasonCode) {
        throw new Error(
          `[capacity-collect-replay:${item.id}] timeline[${index}] reasonCode mismatch, expect=${String(step.expect.reasonCode)} got=${String(out.reasonCode)}`,
        )
      }
    }
  }
...
const runDecisionCase = (item: DecisionCase): void => {
  const out = decideCapacityCollectAction(item.input)
  if (out.decision !== item.expect.decision) {
    throw new Error(
      `[capacity-collect-replay:${item.id}] decision mismatch, expect=${item.expect.decision} got=${out.decision}`,
    )
  }
  if (out.shouldExtend !== item.expect.shouldExtend) {
    throw new Error(
      `[capacity-collect-replay:${item.id}] shouldExtend mismatch, expect=${String(item.expect.shouldExtend)} got=${String(out.shouldExtend)}`,
    )
  }
  if (out.topSaturated !== item.expect.topSaturated) {
    throw new Error(
      `[capacity-collect-replay:${item.id}] topSaturated mismatch, expect=${String(item.expect.topSaturated)} got=${String(out.topSaturated)}`,
    )
  }
}
...
const main = async (): Promise<void> => {
  ...
  console.log(
    `[logix-perf] capacity collect replay passed (budgetCases=${fixture.budgetControllerCases.length}, decisionCases=${fixture.decisionCases.length})`,
  )
}
