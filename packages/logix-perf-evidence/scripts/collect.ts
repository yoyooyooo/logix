...
const readMatrixMeta = async (): Promise<MatrixMeta | undefined> => {
  const file = path.resolve('packages/logix-perf-evidence/assets/matrix.json')
  try {
...
const mergeReports = (parts: ReadonlyArray<PerfReport>, matrixMeta: MatrixMeta | undefined): PerfReport => {
  ...
  return {
    ...
    meta: {
      ...
      generator: 'packages/logix-perf-evidence/scripts/collect.ts',
...
const main = async (): Promise<void> => {
  ...
  const childEnv: NodeJS.ProcessEnv = {
    ...
    ...(process.env.LOGIX_TXN_SHELL_FASTPATH && !process.env.VITE_LOGIX_TXN_SHELL_FASTPATH
      ? { VITE_LOGIX_TXN_SHELL_FASTPATH: process.env.LOGIX_TXN_SHELL_FASTPATH }
      : {}),
  }
