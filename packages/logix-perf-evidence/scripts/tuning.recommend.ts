...
const parseArgs = (
  ...
  const matrixFile = get('--matrix') ?? 'packages/logix-perf-evidence/assets/matrix.json'
...
const main = async (): Promise<void> => {
  ...
  const confirmationSummary = confirmation
    ? {
        ...
        ...('skippedReason' in confirmation ? { skippedReason: confirmation.skippedReason } : {}),
        ...(confirmedWinner
          ? {
  ...
  const recommendation = {
    ...
    meta: {
      ...
      generator: 'packages/logix-perf-evidence/scripts/tuning.recommend.ts',
      profile,
