...
const reactDevExportKeys = new Set(['./dev/lifecycle', './dev/live', './dev/vite', './dev/vitest'])

const toNormalizedRelativePath = (relativePath: string): string => relativePath.split(path.sep).join('/')

const shouldIncludePackageFile = (packageName: string, relativePath: string): boolean => {
  if (packageName !== '@logixjs/react') return true

  const normalizedPath = toNormalizedRelativePath(relativePath)
  return (
    !normalizedPath.startsWith('dist/dev/') &&
    !normalizedPath.startsWith('src/dev/') &&
    !normalizedPath.startsWith('dist/internal/dev/') &&
    !normalizedPath.startsWith('src/internal/dev/')
  )
}

const toPlaygroundPackageJson = (packageName: string, pkgJson: any): any => {
  const packageJson = toSyntheticPublishPackageJson(pkgJson)
  if (packageName !== '@logixjs/react' || !packageJson?.exports || typeof packageJson.exports !== 'object') {
    return packageJson
  }

  const exportsEntries = Object.entries(packageJson.exports).filter(([exportKey]) => !reactDevExportKeys.has(exportKey))
  return {
    ...packageJson,
...
const addPackageFiles = async (
  ...
  const packageJson = mode === 'workspace-dist' ? toPlaygroundPackageJson(packageName, pkgJson) : pkgJson
  ...
  if (mode === 'workspace-dist') {
    ...
    for (const absolutePath of typeFiles) {
      ...
      if (!shouldIncludePackageFile(packageName, relativePath)) continue
