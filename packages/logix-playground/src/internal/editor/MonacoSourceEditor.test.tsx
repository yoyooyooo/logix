...
describe('MonacoSourceEditor', () => {
  ...
  it('keeps React dev carrier declarations out of the Playground type bundle', async () => {
    const { monacoTypeBundleFiles } = await import('./types/monacoTypeBundle.generated.js')
    const fileNames = Object.keys(monacoTypeBundleFiles)
    ...
    const hits = Object.entries(monacoTypeBundleFiles).flatMap(([fileName, source]) =>
      forbiddenMarkers.flatMap((marker) => (source.includes(marker) ? [`${fileName} :: ${marker}`] : [])),
    )

    expect(fileNames).not.toEqual(
      expect.arrayContaining([
        'file:///node_modules/@logixjs/react/dist/dev/lifecycle.d.ts',
        'file:///node_modules/@logixjs/react/dist/dev/lifecycle.d.cts',
        'file:///node_modules/@logixjs/react/dist/dev/live.d.ts',
        'file:///node_modules/@logixjs/react/dist/dev/live.d.cts',
        'file:///node_modules/@logixjs/react/dist/dev/vite.d.ts',
        'file:///node_modules/@logixjs/react/dist/dev/vite.d.cts',
        'file:///node_modules/@logixjs/react/dist/dev/vitest.d.ts',
        'file:///node_modules/@logixjs/react/dist/dev/vitest.d.cts',
      ]),
    )
    expect(hits).toEqual([])
  })
  ...
  it('keeps package build free from local worker query imports', () => {
    ...
    expect(source).not.toContain('monaco-editor/esm/vs/language/json/json.worker?worker')
    expect(source).not.toContain('monaco-editor/esm/vs/language/css/css.worker?worker')
    expect(source).not.toContain('monaco-editor/esm/vs/language/html/html.worker?worker')
  })
