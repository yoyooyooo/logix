...
const bootstrapMonaco = async (): Promise<MonacoBootstrap> => {
  ...
  bootstrapPromise = (async () => {
    const [{ Editor, loader }, monaco, typescript] = await Promise.all([
      import('@monaco-editor/react'),
      import('monaco-editor/esm/vs/editor/editor.api.js') as unknown as Promise<typeof MonacoTypes>,
      import('monaco-editor/esm/vs/language/typescript/monaco.contribution.js') as unknown as Promise<MonacoTypeScriptLanguageNamespace>,
    ...
    return { Editor, loader, monaco, typescript, typeBundle }
  })()
...
const installTypeBundle = (
  ...
  typescript: MonacoTypeScriptLanguageNamespace,
  typeBundle: typeof import('./types/monacoTypeBundle.generated.js'),
): void => {
  if (typeBundleInstalled) return
  typeBundleInstalled = true
  const compilerOptions = getPlaygroundTypeScriptCompilerOptions(typescript)
  ...
  for (const [fileName, content] of Object.entries(typeBundle.monacoTypeBundleFiles)) {
    typescript.typescriptDefaults.addExtraLib(content, fileName)
    typescript.javascriptDefaults.addExtraLib(content, fileName)
  }
...
export function MonacoSourceEditor({
  ...
  React.useEffect(() => {
    ...
    void bootstrapMonaco().then(
      (result) => {
        if (canceled) return
        installTypeBundle(result.monaco, result.typescript, result.typeBundle)
