...
function useModuleInternal(
  ...
  if (isProgramRuntimeBlueprint(normalizedHandle)) {
    ...
    const reflectionManifest = React.useMemo(
      () => CoreReflection.extractRuntimeReflectionManifest(normalizedHandle, { programId: blueprintId }),
      [normalizedHandle, blueprintId],
    )
    React.useMemo(
      () =>
        bindInstalledRuntimeHostLifecycle(runtimeBase as unknown as any, {
          ownerId,
          runtimeInstanceId: moduleRuntime.instanceId,
          reflectionManifest,
          moduleRuntime: {
            getState: moduleRuntime.getState as Effect.Effect<unknown, unknown, any>,
            dispatch: moduleRuntime.dispatch as (action: unknown) => Effect.Effect<void, unknown, any>,
          },
          fieldInspect: {
            getSnapshot: () => CoreDebug.getModuleFieldSnapshot(moduleRuntime as any),
            getGraph: () => CoreDebug.getModuleFieldProgram(normalizedHandle.module).graph,
          },
        }),
      [runtimeBase, ownerId, moduleRuntime, reflectionManifest],
    )
