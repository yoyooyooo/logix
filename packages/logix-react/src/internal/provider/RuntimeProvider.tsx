...
import { bindInstalledRuntimeHostLifecycle } from './runtimeHostLifecycleBridge.js'
...
export const RuntimeProvider: React.FC<RuntimeProviderProps> = ({
  ...
  const hostLifecycleBinding = useMemo(() => bindInstalledRuntimeHostLifecycle(baseRuntime), [baseRuntime])
  ...
  const effectiveLayer = useMemo(() => {
    const hostLifecycleLayer = hostLifecycleBinding?.layer
    if (hostLifecycleLayer && layer) {
      return Layer.provideMerge(hostLifecycleLayer, layer) as Layer.Layer<any, any, never>
    }
    return (layer ?? hostLifecycleLayer) as Layer.Layer<any, any, never> | undefined
  }, [hostLifecycleBinding?.layer, layer])
