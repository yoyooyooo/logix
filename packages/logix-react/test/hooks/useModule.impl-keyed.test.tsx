...
describe("useModule(ModuleImpl, { key })", () => {
  ...
  it("reuses the same local instance for the same key across components", async () => {
    ...
    const EnvTag = Context.GenericTag<{ readonly name: string }>(
      "@logix/react-test/useModuleImplKeyed/env"
    )
    ...
    const UseCounter = () => {
      ...
      return { count, inc: counter.actions.increment, id: String(counter.runtime.id) }
    }
  ...
  it("does not share instances across different RuntimeProvider.layer scopes even with the same key", async () => {
    const runtime = makeRuntime()
    const EnvTag = Context.GenericTag<{ readonly name: string }>(
      "@logix/react-test/useModuleImplKeyed/scope-env"
    )
