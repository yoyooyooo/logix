  ...
  return new Proxy(handle as any, {
    get: (target, prop, receiver) => {
      if (prop === "changes") {
        return (...args: any[]) => {
          onChangesCall()
          return (target as any).changes(...args)
        }
      }
...
describe("useSelector(shared subscription)", () => {
  it("does not start one changes() subscription per useSelector call", async () => {
    ...
    const baseHandle = runtime.runSync(Counter as any) as Logix.ModuleRuntime<
      { count: number },
      CounterAction
    >
    ...
    // 即使同一组件内有多个 useSelector，也应共享同一条底层 changes() 订阅。
