...
describe('ReactModuleConfig (defaults)', () => {
  ...
  it('gcTime should respect ConfigProvider override', () => {
    const layer = ConfigProvider.layer(ConfigProvider.fromUnknown({ 'logix.react.gc_time': '1000' }))
  ...
  it('initTimeoutMs should expose overridden timeout as Some(value)', () => {
    const layer = ConfigProvider.layer(ConfigProvider.fromUnknown({ 'logix.react.init_timeout_ms': '30000' }))
  ...
  it('ReactRuntimeConfig.replace should override existing config based on current value', () => {
    ...
    const envLayer = ConfigProvider.layer(ConfigProvider.fromUnknown({ 'logix.react.gc_time': '800' }))
