...
describe("Logix Runtime → ModuleRuntime → RuntimeProvider → useModule chain", () => {
  ...
  const TestImplWithProcess = TestModule.implement({
    ...
    processes: [
      Effect.gen(function* () {
        ...
        const runtime = (yield* TestModule) as Logix.ModuleRuntime<
      ...
      () => {
        ...
        React.useEffect(() => {
          void runtime
            ...
            .then(() =>
              runtime.runPromise(moduleRuntime.getState as any).then((s) =>
                setState(s as { initialized: boolean; label: string }),
              ),
            )
  ...
  it("should allow React Provider.layer to override Env for logic executed via useRuntime", async () => {
    ...
    const providerLayer = Layer.succeed(EnvTag, { label: "ENV_FROM_PROVIDER" }) as Layer.Layer<
      any,
      never,
      never
    >
    ...
    const wrapper = ({ children }: { children: React.ReactNode }) => (
      <RuntimeProvider runtime={appRuntime} layer={providerLayer}>
        {children}
      </RuntimeProvider>
    )
  ...
  it("processes should see root Env even when React Provider overlays EnvTag", async () => {
    ...
    await waitFor(async () => {
      const state = (await appRuntime.runPromise(
        (result.current as any).getState,
      )) as { initialized: boolean; label: string }
