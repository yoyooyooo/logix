  ...
  async () => {
    ...
    const code = `
      ...
      const program = Effect.gen(function* () {
        ...
        // 触发 UI_INTENT
        const emitUi = (globalThis as any).__logixSandboxUiIntent;
        if (emitUi) {
          emitUi({
            id: "ui-1",
            component: "Button",
        ...
        // 触发 Spy 观测
        const emitSpy = (globalThis as any).__logixSandboxSpy;
        if (emitSpy) {
          emitSpy({ name: "spy-call", target: "sdk-demo", payload: { ok: true } });
        }
    ...
    const byKind = (kind: string) =>
      runResult.traces.find((t) => (t.attributes as any)?.kind === kind)
