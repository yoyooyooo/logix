...
export const getActionsByTag = <Sh extends Logix.AnyModuleShape>(
  ...
  result.actions.filter(
    (action) =>
      typeof (action as any)._tag === "string" &&
      (action as any)._tag === tag
  )

export const hasError = <Sh extends Logix.AnyModuleShape>(
  result: ExecutionResult<Sh>
): boolean => result.trace.some((event) => event._tag === "Error")

export const getErrors = <Sh extends Logix.AnyModuleShape>(
  result: ExecutionResult<Sh>
): ReadonlyArray<Extract<TraceEvent<Sh>, { _tag: "Error" }>> =>
  result.trace.filter(
    (event): event is Extract<TraceEvent<Sh>, { _tag: "Error" }> =>
      event._tag === "Error"
  )
...
export const expectNoError = <Sh extends Logix.AnyModuleShape>(
  result: ExecutionResult<Sh>
): void => {
  const errors = getErrors(result)
  if (errors.length > 0) {
    throw new Error(
      `Expected no errors, but found ${errors.length}. First error cause: ${String(
        errors[0]?.cause
      )}`
    )
  }
