...
- 业务 authoring 默认不生成验证对象、report 对象、evidence 对象或 scenario 对象。
- 测试文件、验证文件或 repo-local proof 需要控制面时，只生成 `Logix.Runtime.check(Program, options?)` 与 `Logix.Runtime.trial(Program, { mode: "startup", ... })`。`compare` 只作为评审阶段名出现，禁止生成 root `Runtime.compare` facade。
- harness/proof/fixture/support 资产遵循 `docs/standards/harness-and-proof-assets-standard.md`：文件名表达最终行为或边界，编号只作测试/spec 追溯，不进入 `src/**`、public API、CLI machine output 或 runtime payload。
- shell/CI 自验证默认交给 `logix-cli` skill：`logix check`、`logix trial --mode startup`、必要时 `logix compare`。需要 active runtime context 时，也交给 `logix-cli` 的 172 live inspect drilldown，不在业务代码或 React host 里生成私有调试读取。
...
- 验证控制面不进入业务 authoring surface。测试/验证代码只用 `Runtime.check` 与 `Runtime.trial(mode="startup")`；shell/CI 自验证走 `logix-cli`；`compare` 只作为 CLI/control-plane repair closure 阶段，不生成 root `Runtime.compare` facade。
- Live debugging 不进入业务 authoring surface。需要 active runtime context 时走 `logix live <task>`，172 后优先使用 `logix live state/actions/events/timeline/fields/field-graph/field-summary/summary` 获取 owner-backed inspect facts 或 structured gaps。live output 只能提供 evidence/clues，不能直接产出 repair verdict。
...
- Verification：业务实现文件不写 verification/report/evidence 对象。测试文件或 repo-local proof 才使用 `Logix.Runtime.check(Program, options?)` 与 `Logix.Runtime.trial(Program, { mode: "startup", ... })`。shell/CI 走 `logix-cli`。行为脚本需要交互验证时，才在验证文件内使用交互试运行。`compare` 只作为 CLI/control-plane repair closure 阶段，不生成 root `Runtime.compare` facade。
...
- 不生成 root `Runtime.compare` facade、public scenario object、public report object、public evidence object 或 raw evidence default compare。
- 不从 `logix live` 结果里读取 `repairHints`、`nextRecommendedStage` 或 verification verdict；live-derived evidence 必须先进入 `runtime.trial` 或 `runtime.compare`。
- 不在 CLI、daemon、browser adapter 或业务代码里拼 Runtime inspect truth。state、actions、timeline、field graph 等必须来自 owner-backed `LiveInspectArtifact` 或 structured gap；`actions/dispatch` 必须引用 `LiveManifestBindingRef`，timeline `stateAfter` 禁止 latest-state 历史回填，field graph 禁止 raw `nodes[] / edges[] / from / to` dump。
- 不把 `R172-*`、`TASK-*`、`CAP-*`、`PF-*`、`matrixRow`、temporary `Probe/Witness/Pressure` helper 写进生产源码或 machine contract。若当前任务触及同一目录或证明链，发现这类旧资产应按 harness/proof 标准顺手修正或记录阻塞。
