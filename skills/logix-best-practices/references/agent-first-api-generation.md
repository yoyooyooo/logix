...
Verification control plane is adjacent to authoring, not part of business authoring. In tests, proof files, or repo-local validation code, use:

```ts
Logix.Runtime.check(CounterProgram, options?)
Logix.Runtime.trial(CounterProgram, { mode: "startup", ...options })
```

For shell/CI Agent self-verification, use the `logix-cli` skill and run `logix check`, `logix trial --mode startup`, and explicit `logix compare` only for repair closure.
...
业务实现文件默认不生成 verification/report/evidence/scenario 对象。

测试文件或 repo-local proof 需要验证控制面时，只生成 `Logix.Runtime.check` 与 `Logix.Runtime.trial`。

不要生成 root `Runtime.compare` facade。`compare` 只作为 CLI/control-plane 阶段名出现在评审文本或 `logix-cli` repair closure 中。
...
await Logix.Runtime.check(CheckoutForm)
...
| 静态合同检查 | `Logix.Runtime.check(CounterProgram)` | 暗中启动 trial |
| 启动试运行 | `Logix.Runtime.trial(CounterProgram, { mode: "startup" })` | 交互脚本协议 |
| 验证文件内交互脚本 | 只在验证文件内使用交互试运行 | public scenario business asset |
| CLI repair closure | `logix compare` with before/after reports | root `Runtime.compare` facade、raw evidence default compare、benchmark truth owner |
| Live runtime debugging | `logix live <task>` then feed exported evidence to `trial` or `compare` | live-owned repair verdict、live-owned repair hints、flat `logix status/capture/trigger` roots |
...
Verification 只走 runtime control plane。下面片段只适用于测试文件、验证文件或 repo-local proof，不能写进业务 authoring surface。Core proof 使用 Core program 变量：

```ts
const checkOutcome = await Logix.Runtime.check(CounterProgram)
...
Form proof 使用 Form object 变量：

```ts
const checkOutcome = await Logix.Runtime.check(CheckoutForm)
...
compare 只作为 control-plane stage label 处理标准化 outcome 或关键工件摘要。业务实现文件不生成 compare 调用。需要 before/after repair closure 时走 `logix compare`。测试/proof 代码只保留 startup trial：
...
| verification boundary | `check / trial / compare` 只作为 runtime control plane，不进入业务 authoring；业务实现没有 report/evidence/scenario 对象；root `Runtime.compare` 未出现 |
