...
- 业务 authoring 不生成 verification/report/evidence/scenario 对象。
- 测试/proof 只生成 `Logix.Runtime.check` 与 `Logix.Runtime.trial(mode="startup")`，且 `check` 不带 `mode: "static"`。
- shell/CI 自验证走 `logix-cli`；repair closure 用 `logix compare`，不生成 root `Runtime.compare` facade。
- live debugging 走 `logix live <task>`，且 live output 不拥有 `repairHints / nextRecommendedStage / verdict`。
- 不新增 Form hook family、public path carrier、public row owner token、second fact namespace、public scenario/report noun。
- harness/proof/fixture/support 遵循 `docs/standards/harness-and-proof-assets-standard.md`，编号只作测试/spec 追溯，不进入生产源码、CLI machine output 或 runtime payload。
...
rg -n "second hook family|public path carrier|public row owner token|public scenario/report noun|root compare facade|Runtime\\.compare" <skill-root>/SKILL.md <skill-root>/references
rg -n "Module\\.logic|Program\\.make|Runtime\\.make|source|companion|useSelector|Logix\\.Runtime\\.trial|logix live|LiveCommandResult" <skill-root>/SKILL.md <skill-root>/references
rg -n "Runtime\\.check\\([^\\n]*mode: \\\"static\\\"" <skill-root>/SKILL.md <skill-root>/references
