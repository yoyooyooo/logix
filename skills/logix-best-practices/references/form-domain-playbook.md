...
业务实现文件不生成 verification/report/evidence/scenario 对象。测试文件、验证文件或 repo-local proof 需要验证控制面时，只生成静态快检与启动试运行：

```ts
await Logix.Runtime.check(FormProgram)
await Logix.Runtime.trial(FormProgram, { mode: "startup" })
```

shell/CI Agent 自验证走 `logix-cli` skill：`logix check`、`logix trial --mode startup`，必要时再显式 `logix compare`。
