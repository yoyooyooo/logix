...
7. 业务 authoring 不生成 verification/report/evidence/scenario 对象；测试/proof 只用 `Runtime.check` 与 `Runtime.trial(mode="startup")`；shell/CI 走 `logix-cli`；`compare` 只作为 CLI/control-plane repair closure 阶段。
...
- 为了便利新增第二 host gate、第二 report object、第二 evidence envelope、第二 live truth 或第二 authoring carrier。
- 从 `logix live` output 直接读取 repair verdict、`repairHints` 或 `nextRecommendedStage`。
