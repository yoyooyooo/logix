# Contracts: 000-module-fields-runtime
...
1. `FieldKernel.from(StateSchema)`
   - 职责：将模块 State Schema 收敛为可用的 Field DSL 入口，约束合法字段路径与字段类型。
2. `FieldKernel.build(stateSchema, traitsSpec)`
   - 职责：在不依赖 Runtime 环境的前提下，把 Module 图纸中的 `state + fields` 编译为 `FieldProgram`，输出包含 `stateSchema` / `FieldKernelSpec` / `FieldGraph` / `FieldPlan`。
   - 约束：纯函数；对致命结构错误直接 fail，对非致命问题产生 warnings（参见 spec Clarifications 与 FR-015）。
3. `FieldKernel.install($, program)`
   - 职责：在 Logix Runtime 中，根据 `FieldProgram` 将 computed / source / link 行为编译为 Bound API 上的 watcher / Flow / Effect 行为，并通过 EffectOp/Middleware 总线对接 Debug / Resource / Query 等能力。
...
5. Devtools 数据契约
   - FieldProgram / FieldGraph 导出格式；
   - moduleId + instanceId 的 runtime envelope；
   - Graph 节点与 EffectOp 事件之间的关联元数据（源位置、Field 节点引用等）。
