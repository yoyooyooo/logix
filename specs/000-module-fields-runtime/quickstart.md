# Quickstart: 在 Module 图纸中使用 FieldKernel（computed / source / link）

> 目标：用两个尽量小的例子说明「`state + actions + fields`」图纸如何配合 `FieldKernel` 工作，模块作者需要写什么、Runtime 会做什么。

---

## 示例一：只用 computed + link 的 CounterWithProfile 模块

### 1. Module 图纸：state / actions / fields
...
- `fields` = 用 FieldKernel 为字段打 computed / link / source 标签。

> 类型提示补充：在 `FieldKernel.from(StateSchema)({...})` 中书写 key 时，IDE 会基于 `StateSchema` 提供所有合法路径（例如 `"profile.name"`、`"profile.id"` 等）的自动补全，写错路径会直接在类型层报错；`FieldKernel.computed` / `FieldKernel.link` 等 API 也会基于路径推导对应字段的类型，保证 derive 函数与联动两端类型一致。

### 2. Runtime 会做什么（概念）

在 Runtime 初始化这个 Module 时：

1. Logix Runtime 调用：`const program = FieldKernel.build(StateSchema, traitsSpec)`：
   - 构建 FieldProgram：包括 FieldGraph（字段拓扑）和 FieldPlan（执行计划）。
2. 在创建 ModuleRuntime / Logic 时调用：`FieldKernel.install($, program)`：
...
在实现层，FieldKernel 不会直接操作 Bound API，而是先把 PlanStep 映射成 EffectOp，再交给 Runtime 注入的 MiddlewareStack 执行：
...
const runtime = Logix.Runtime.make(impl, {
  layer: appLayer,
  middleware: Middleware.withDebug([]),
})
```

这样 Devtools / 日志系统就可以统一消费 EffectOp 时间线，而 FieldKernel / Logic 只负责声明“要做什么”。
...
### 1. Module 图纸中的改动：给 profileResource 加 source Field
...
export const CounterWithProfile = Logix.Module.make("CounterWithProfile", {
  ...
  fields: FieldKernel.from(StateSchema)({
    ...
    profileResource: FieldKernel.source({
...
export const UserProfileResource = CoreReadContracts.Resource.make({
  id: "user/profile",
...
// 这里以 TanStack QueryClient 作为默认引擎实现（也可以注入自定义 Engine）。
const queryClient = new QueryClient()

const runtimeLayer = Layer.mergeAll(
  UserProfileLive,
  CoreReadContracts.Resource.layer([UserProfileResource]),
  Query.Engine.layer(Query.TanStack.engine(queryClient)),
  // 基于 EffectOp(kind="field-source") + resourceId + keyHash 决定是否由引擎接管。
  Query.Engine.middleware({ useFor: (id) => id === "user/profile" }),
)
...
在更细粒度的路由 / 子树中，可以通过 RuntimeProvider 叠加或移除 `Query.Engine.layer` + `Query.Engine.middleware`，从而做到“这一片都走查询引擎 / 这一片直接调用 Service Tag”，而不需要修改任何 Module / Field 声明。

### 3. FieldKernel.install 在 source 上会做什么（概念）

当 FieldPlan 中包含 source 步骤时，`FieldKernel.install` 大致会：

1. 从 Plan 中取出 `resourceId = "user/profile"` 与 `key(state)` 规则，并为每个 source 字段生成一个标准的加载入口（例如在 Bound API 上挂载 `$.fields.source.refresh("profileResource")`），该入口在被调用时执行一次对应的 `source-refresh` 计划。
2. 当 Logic / Action / Devtools 显式调用该入口时：
   - 构造一个 `EffectOp`，`kind = "field-source"`, `name = "user/profile"`；
   ...
   - 由 Resource / Query 相关 Middleware 读取 `resourceId + key`，选择：
     ...
     - 通过外部引擎（如 TanStack QueryClient）以 key 作为 queryKey 发起/复用请求；
...
- Timeline：每次 `kind = "field-source"` 的 `user/profile` 调用，以及 Resource / Query / 其他中间件（重试/超时/日志等）的参与情况。

这就是 FieldKernel + Resource / Query + Middleware 整条链路在一个最小场景里的形态。实际实现时，Field 内核与 Resource/Query 命名空间的具体 API 会按 `data-model.md` 中的模型补全，但模块作者的心智始终停留在 Module 图纸这一层。
...
- `packages/logix-core/test/FieldKernel.ComputedLink.test.ts`
  验证在简单 State Schema 上使用 `FieldKernel.from` + `computed/link` 时，类型与行为（基于 meta）符合预期。

- `packages/logix-core/test/FieldKernel.QuickstartExample.test.ts`
  直接导入 `examples/logix-react/src/modules/counter-with-profile.ts`，检查：
  - Module 图纸（id / state / actions / fields）结构正确；
  - `CounterTraits` 中的 computed / link entry 与 Quickstart 示例描述一致（`sum` 为 `a + b`，`profile.name` 跟随 `profileResource.name`）。

在后续演进中，如需调整示例代码或 FieldKernel API 形状，应同步更新上述测试，确保 Quickstart 文档始终可运行、可验证。
...
在未来的 Query 集成包中，我们可以提供一些**完全基于 FieldKernel.source 的语法糖**，例如：
...
export const CounterWithProfile = Logix.Module.make("CounterWithProfile", {
  ...
  actions: Actions,
  fields: FieldKernel.from(StateSchema)({
    sum: FieldKernel.computed({
      deps: ["a", "b"],
      get: (s: Readonly<State>) => s.a + s.b,
    }),

    // 显式写法：等价于上面的 FieldKernel.source（不引入第二套事实源）
    profileResource: FieldKernel.source({
      deps: ["profile.id"],
      resource: "user/profile",
...
- `Query.source` 只是对 `FieldKernel.source` 的薄包装，不改变 FieldProgram / FieldGraph 的结构，也不引入新的 Field kind；
- 所有与缓存 / 重试 / queryClient 相关的行为仍然由 Runtime 侧的 `Query.Engine.layer` + `Query.Engine.middleware` 决定；
- 即使完全移除 Query 相关 Layer/Middleware，这类语法糖也必须退化为普通的 FieldKernel.source 行为，确保 Module 图纸与 Field IR 保持稳定。

随着 Field + Middleware 主线落地，后续可以在 Query 命名空间下继续探索更多语法糖（例如基于路径的 key 推导、带失效策略的 cachedSource 等），但它们都应遵守上述原则：**不修改 FieldKernel 核心协议，只在 Query / Middleware 层叠加便利性。**
...
- **端到端验证现状**
  - `packages/logix-core` 下的 `FieldKernel.Build/Install/RuntimeIntegration/EffectOpIntegration` 等测试已经覆盖了 FieldProgram 构建、Plan 安装以及 EffectOp/Middleware 总线的关键路径；
  - `FieldKernel.QuickstartExample.test.ts` 直接导入本 Quickstart 中的 `CounterWithProfile` 模块，验证了 `CounterStateSchema` / `CounterTraits` 与文档示例一致，并对 computed/link 行为做了最小行为校验；
  - Query / Resource 集成通过 `Resource.test.ts` 与 `ResourceQuery.Integration.test.ts` 做了基础回归。

- **Devtools 体验观察（当前阶段）**
  - Devtools UI 仍处于 PoC/演进阶段，当前主要通过 FieldProgram / FieldGraph / EffectOp Timeline 的结构与日志来观测行为；
...
- **后续改进建议（留给 Devtools / Studio 特性）**
  ...
  - 随着 US5/US6/US7 完成后，再回收这些观察与截图，将本 Quickstart 升级为端到端“从 Fields 到 Devtools”的权威样板。
