# Reference: Devtools 与 Debug 在 Field + EffectOp 下的收敛

> 作用：拆开说明 Devtools / Debug 如何在 FieldProgram + EffectOp 总线上工作，并给出旧 Debug 模块的迁移路径。
...
1. 结构侧：`FieldProgram`（含 `FieldGraph` 与 `FieldPlan`）；
2. 事件侧：`EffectOp` 流（通过统一 Middleware 总线发出）。

Runtime 负责：

- 在构建 ModuleRuntime 时调用 `FieldKernel.build`，得到 Program / Graph / Plan；
- 在运行时通过 `FieldKernel.install` 将 Plan 转译为若干 EffectOp 触发点；
...
## 2. 结构视图：基于 FieldGraph 的模块图

FieldGraph 提供了字段级别的依赖拓扑信息，Devtools 可以据此渲染：
...
- 点击节点可高亮其所有入边/出边；
- 支持按 Field kind 过滤（只看 source/link/computed）；
- 支持从资源节点出发查看“依赖该资源的所有字段/模块”；
- 节点上展示源码位置 hint（文件路径 + 行号），便于一键跳回 fields 定义。

这些能力全部建立在 FieldGraph / Program 中的静态信息之上，不依赖 Runtime 当前状态。

### 2.1 数据入口与结构视图协议

- `@logixjs/core` 暴露内部调试 API：`CoreDebug.getModuleFields(module)`：

  ```ts
  interface ModuleFieldsDebug {
    readonly program?: FieldProgram<any>
    readonly graph?: FieldGraph
    readonly plan?: FieldPlan
  }

  const fields = CoreDebug.getModuleFields(SomeModule)
  // fields.graph / fields.plan 仅在该 Module 使用了 fields 槽位时存在
  ```

- Devtools / Studio 只依赖上述协议获取结构信息，而不直接访问 `__stateTraitProgram` 等内部字段；
- 未来扩展 ActionTrait / FlowTrait / ModuleTrait 时，约束：
  - 可以在 Program 的 meta 中增加其他 Field 家族的入口，但 **不改变** 现有 FieldProgram / Graph / Plan 的形状；
  - Devtools 结构视图组件（例如 `FieldGraphView`）只消费 FieldGraph，不感知具体 runtime 实现细节。
...
时间线只消费 EffectOp，不需要知道 Field 的细节；Field 相关信息通过 meta 中的字段与 Graph 做关联。
...
为了实现 FR-013 中提到的“玻璃盒调试”体验，需要让 Graph 与 Timeline 相互联动，并支持跳回 fields 源码。
...
- 在时间线点击某个事件：
  - Graph 高亮对应字段/资源节点；
  - Devtools 提供“打开代码”按钮：跳转到 fields 定义的位置。
...
- 结构侧：FieldProgram / FieldGraph（由 FieldKernel.build 提供）。
...
2. **结构收敛**：对依赖 ModuleRuntime / Store 内部结构做状态快照或拓扑分析的旧实现，应改为消费 FieldProgram / FieldGraph，并以此作为唯一结构事实源。
...
2. 从 Graph 节点可以跳到相关事件，从事件可以跳回 fields 定义；
