...
> 作用：细化 EffectOp 类型与 Middleware 总线设计，以及 FieldKernel.install 与总线之间的接缝。
...
## 2. FieldKernel.install 与 EffectOp 的接缝

### 2.1 从 Plan 到 EffectOp

FieldPlanStep → EffectOp 映射原则：
...
- `Logix.Runtime.make(program, { layer, middleware })` 在构建 Runtime 时接受一个 `MiddlewareStack` 配置（`program` 为 `Program.make(...)` 的结果）：
...
- FieldKernel.install / Debug / Resource / Query 等运行时代码，通过 Env 中的 `EffectOpMiddlewareTag` 获取当前总线配置：
  - 典型用法：
    ```ts
...
- DebugObserver 作为一种 Middleware/Observer：
  ...
  - 将 FieldGraph 信息带入事件上下文（通过 meta 中的节点/边 ID）。

- FieldGraph / Plan 中的 debugInfo 字段应至少包含：
  - 对应 Module ID；
  - 对应字段路径；
  - 源代码位置（文件/行号的粗粒度信息）；
