# Reference: Parser / Studio 与 Module Fields 的对齐

> 作用：说明 Parser / Studio（包括未来的全双工平台侧）如何理解 Module 图纸与 FieldKernel，并与 SDD / v3 文档对齐。
...
- 在不运行业务代码的前提下，从 TS 源码中提取：
  ...
  - fields 结构（FieldKernelSpec）与对应的 Program/Graph；
- 以统一 IR 形式暴露给 Studio / 平台侧：
  - 用于渲染模块图谱（Module Graph + FieldGraph）；
...
2. 在第二参数中查找：
   ...
   - `fields: FieldKernel.from(<StateSchemaRef>)(<SpecObject>)`：
     - 要求 `from` 的参数与 `state` 使用的 Schema 在语义上对应（同一个引用或可静态推断等价）。

Parser 侧可以做两级识别：

1. 只基于 AST 结构识别“这是一个 Module 图纸 + FieldKernel.from 的组合”；
2. 在需要时，调用共享的 `FieldKernel.build` 逻辑（见下节），在安全环境中构建 Program/Graph。

这样可以避免在 Parser 中重新实现一套“解析 fields → Graph”的规则。

---

## 3. 共享 FieldKernel.build：避免“双实现”

spec 要求 `FieldKernel.build` 是纯函数，只依赖：

- `StateSchema`（Schema 对象）；
- `FieldKernelSpec`（由 `FieldKernel.from(StateSchema)({ ... })` 得到的 spec 对象）。

实现建议：

- 将 FieldKernel 内核设计为可在“非 Runtime 环境”下运行：
  - 不访问全局可变状态；
  - 不依赖 RuntimeProvider / Layer 环境；
  - 失败时只返回结构化错误或抛出同步错误。
- Parser / Studio 可通过两种方式复用该逻辑：
  1. 在 Node 环境中直接 import `@logixjs/core/FieldKernel`，对选定模块执行 build；
  2. 将 FieldKernel 内核编译为可独立运行的 bundle（甚至 WASM），由 Studio 进程/浏览器侧调用。

通过共享 `build`：

- 可以确保 Runtime 与 Studio/Parser 对 fields 的理解完全一致；
- 避免出现“Studio 画出来的 Graph 和真正 Runtime 行为不一致”的情况。

---

## 4. Studio 视图：从 Module Graph 到 Field Graph

在 Platform / Studio 侧，Module Graph 与 FieldGraph 可以以嵌套方式呈现：

- Module 节点：来源于 v3 / Runtime-Logix 的 Module/Flow 图谱；
- 每个 Module 节点下挂一张 FieldGraph：
  - 字段节点、Field 类型（computed/source/link）、外部资源节点；
  - 可选组合 Action/Flow/EffectOp 信息，构成完整“模块内部视图”。

Studio 可以提供的操作包括：

- 只读视图：浏览现有 Module 的 FieldGraph，理解字段依赖与资源依赖；
- 引导式编辑：在图上为字段添加/删除 Field（例如从“普通字段”升级为“computed”或“source”）；
- 导出/同步：把修改后的 Field 变更回写到 TS 源码（见下一节）。

这些能力应与 SDD / IntentSpec 保持对齐：
- Module 图纸 + FieldGraph 可视为实现侧对“状态与数据依赖”的落地表达；
...
1. Studio 只允许在有限范围内修改代码：
   - 添加/删除/修改 `fields` 槽位中的 entries；
   - 在必要时对 `StateSchema` 做有限扩展（例如新增 computed 字段）。
2. 回写方式：
   - 基于 AST 对原文件做结构化编辑（不使用字符串替换）；
   - 保留用户手写的注释和格式尽量不变。
3. 冲突处理：
   - 若用户手动修改了同一段 fields 定义，导致 Studio 的 IR 与实际源码不一致：
...
- FieldGraph：描述“状态字段之间、与外部资源之间的依赖关系”；
- EffectOp 流：描述“实际运行时发生了什么”。

Parser / Studio 的职责是：

- 把 Module 图纸与 FieldGraph 作为“实现端”的结构视图；
- 与 SDD / IntentSpec 建立映射（例如某些 Field 与某些 Quality/Constraint 相关）；
- 为全双工平台提供：
  - 从 Intent/SDD 跳到具体 Module / Field 的能力；
  - 从运行时事件回跳到 Intent/SDD 的能力（间接通过 Module / Field）。
...
- 在真正开始实现前，应先：
  - 确认 FieldKernel.build 内核稳定；
  - 确认 EffectOp / Devtools 已经能稳定提供 Graph + Timeline 信息。

这样可以保证 Parser / Studio 不是在“流沙地基”上做集成，而是站在已经稳定的 Field / Runtime 契约之上。
