# Research Notes: 统一 Module Fields（FieldKernel）与 Runtime Middleware/EffectOp

**Branch**: `000-module-fields-runtime`
**Spec**: `specs/000-module-fields-runtime/spec.md`
**Scope (Phase 0)**: 本文档当前优先聚焦 Field / FieldKernel 相关决策，Middleware/EffectOp 只在与 Field 接缝处简单标注。

> 相关 reference：
> - FieldKernel 内核与 Program 结构：`references/field-kernel-core.md`
> - EffectOp 总线与 Middleware 设计：`references/effectop-and-middleware.md`
> - Resource / Query 命名空间与 FieldKernel.source 集成：`references/resource-and-query.md`
> - Devtools / Debug 与 Graph + Timeline 视图：`references/devtools-and-debug.md`
> - Parser / Studio 与 Module 图纸 / Field IR 对齐：`references/parser-and-studio.md`

---

## 1. Field 抽象与 FieldKernel Engine 形态

### Decision 1.1 · Field 生命周期与抽象层级

- **Decision**: Field 的标准生命周期统一为：
  - `Spec`（Module 图纸中的声明，例如 `fields: FieldKernel.from(StateSchema)({ ... })`）
...
- **Alternatives considered**:
  - 暴露统一的 `RuntimeTraitEngine<Subject, Spec, Program>` 抽象作为 public API；
    - 结论：保留为 internal 接口（`internal/state-field/engine.ts`），对外只暴露 `FieldKernel.build/install`，避免在 v1 就锁死过重的泛型抽象。

### Decision 1.2 · FieldProgram 的最小必要字段

- **Decision**: `FieldProgram<S>` 至少包含：
  - `stateSchema: Schema.Schema<S, any>`
  - `spec: FieldKernelSpec<S>`（Module 图纸中的 fields）
  - `graph: FieldGraph`（字段 + Field 的结构视图）
  - `plan: FieldPlan`（内部执行计划，供 `install` 使用）
- **Rationale**:
...
## 2. FieldKernelSpec / Entry 类型边界

### Decision 2.1 · from + Spec 的基本形态

- **Decision**:
  - 对模块作者暴露的入口为：`FieldKernel.from(StateSchema)(spec)`；
  - `FieldKernelSpec<S>` 的形态为：
    ```ts
    type FieldKernelSpec<S> = {
      [Path in StateFieldPath<S>]?: FieldKernelEntry<S, any>
    }
    ```
  - `FieldKernelEntry<S, A>` 至少包含：
...
### Decision 2.3 · 与 Field 相关类型的命名规范

- **Decision**:
  - 所有代表 Field 概念的核心类型名都显式带上 `Field` 前缀/后缀，以便在代码阅读与 IDE 搜索中一眼识别其职责，例如：
    - `FieldKernelSpec` / `FieldKernelEntry` / `FieldKernelField` / `FieldKernelFieldTrait` / `FieldKernelResource`；
    - 后续如引入 ActionTrait / FlowTrait 等，也沿用同一命名模式。
  - 仅在 Module 图纸层保留无 Field 前缀的简洁 API（例如 `fields: FieldKernel.from(StateSchema)({...})`），internal 层尽量显式化。
- **Rationale**:
  - Field 体系本身就是“为现有主体加能力标签”的元编程设施，命名中明确带上 `Field` 有助于在引擎与业务代码之间划清边界；
  - 方便未来在代码中快速按 `*Field*` 聚合/检索相关实现，支撑长期演进与重构。
- **Alternatives considered**:
  - 采用更短的前缀（如 `ST*`）或不带前缀的通用名（如 `FieldSpec`）：
    - 容易与非 Field 概念混淆，且不利于跨 Field 家族（State/Action/Flow）做统一认知；
    - 因此明确选择在类型层面坚持 `Field` 命名。
...
- **Decision**:
  ...
  - `FieldKernelSpec<S>` 形式为：
    ```ts
    type FieldKernelSpec<S> = {
      [Path in StateFieldPath<S>]?: FieldKernelEntry<S, Path>
    }
    ```
    这样在 `FieldKernel.from(StateSchema)({ ... })` 字面量中书写 key 时，TS 会将 `"profile.name"` 等路径视为字面量类型并约束在 `StateFieldPath<S>` 内，写错路径（例如 `"profile.nam"`）会直接报类型错误。
  - `FieldKernelEntry<S, P>` 中可以使用 `StateAtPath<S, P>` 约束 derive 函数与 link 的类型，例如：
...
- **Rationale**:
  ...
  - 让 `FieldKernel.from(StateSchema)({...})` 成为 IDE 体验的核心入口：路径可补全、路径写错能报错、derive 函数参数和结果都有强类型约束。
- **Alternatives considered**:
...
## 3. FieldGraph / Plan 与 Middleware/EffectOp 的接口

### Decision 3.1 · Graph 作为结构 IR，一等公民

- **Decision**:
  - Graph 是 FieldProgram 的一部分，类型为 `FieldGraph`，包含节点与边：
    - 节点：State 字段 + 其挂载的 FieldKernelFieldTrait；
    - 边：依赖关系（computed/link/source → 依赖字段）。
  - Graph 是 Devtools / Studio 的主入口，对外文档应以 Graph 解释 Field 结构。
- **Rationale**:
  - Graph 自然适合作拓扑可视化与 diff；
  - 将 Graph 明确为 Program 的一部分，避免后续 Debug / Studio 再造自己的视图结构。

### Decision 3.2 · Plan 与 EffectOp 的关系（Field install 接缝）

- **Decision**:
  - FieldPlan 主要用于 `FieldKernel.install` 在 Bound API 上安装行为；
  - Plan 中每个 `FieldPlanStep` 应具备构造 `EffectOp` 所需的最小上下文，例如：
  ...
  - `FieldKernel.install($, program)` 不直接跑副作用，而是：
    - 从 Plan 构造 `EffectOp<A, E, R>`（`meta.kind = "state"` 或 `"service"`）；
    - 将 `EffectOp` 交给 Middleware 总线（`composeMiddleware(...)(op)`）执行。
- **Rationale**:
  - 保证 Field 层不依赖具体 Middleware 实现，只依赖 “有一条 EffectOp 总线”；
  - 允许未来将 Plan 的某些部分完全下沉到 Middleware 配置，而不破坏 FieldKernel API。
- **Alternatives considered**:
  - 在 Field 层直接操作 Bound API，不经过 EffectOp；
    - 这种做法会让 Field 与 Middleware 形成两套独立的执行体系，不利于 Debug/可观测与策略统一；
    - 因此统一要求 FieldKernel.install 通过构造 EffectOp 交由总线执行。

---

## 4. FieldKernelResource ↔ ResourceRegistry + Service Tag

### Decision 4.1 · resourceId 的语义与命名

- **Decision**:
  - FieldKernel.source 中的 `resource` 字段在内部规范为 `resourceId: string`；
...
- 在 FieldPlan 中，source 型步骤只保留 resourceId 与 key 规则，`FieldKernel.install` 根据 Plan 生成 `EffectOp(kind = "field-source")` 并调用 ResourceRegistry。
- **Rationale**:
  - 保证 Module/Field 只负责声明“要什么（resourceId + key）”，而不是“怎么拿”；
...
- **Decision**:
  - `FieldKernel.source({ resource, key })` 中的 `key(state)` 是**资源标识 + 参数**的纯函数描述，语义类似“强类型的 queryKey”；
  - 推荐返回结构化对象而不是裸数组/字符串，例如 `{ userId: s.profile.id }`，并在内部以只读、可比较的结构存储；
  - FieldPlan 中保留的是 “如何从 State 计算 key” 的规则，`FieldKernel.install` 在每次触发 source 时重新计算 key，并将其附带在 `EffectOp.meta` 或 payload 中；
...
- **Decision**:
  - 对外不直接暴露 ResourceRegistryTag / QueryClientTag（内部概念），而是通过命名空间封装：
    ...
    - `Query.Engine.middleware(config)`：基于 `EffectOp(kind = "field-source")` + `resourceId` + `keyHash` 决定哪些资源走查询引擎、如何做缓存/去重等策略。
  - Module/Field 层只依赖 `resourceId + key` 的字面值与类型，完全不知道 Tag/Registry 的存在；
  - Resource/Query 命名空间所在模块归属于 `@logixjs/core`，作为 Runtime 基础设施的一部分。
- **Rationale**:
  - 保持 Field / Module 图纸层的 API 简洁、稳定，避免暴露内部 Tag/Registry 的实现细节；
  - 让 Resource / Query 能够被 RuntimeProvider 在不同子树中灵活组合，同时不破坏 FieldKernel 的“声明即事实源”属性；
  - 通过 Middleware 统一对接查询引擎，使得是否使用 queryClient 成为 Runtime 配置，而非写死在 Field 中。
- **Alternatives considered**:
  - 直接在 FieldKernel.source 中加入 `driver: "query" | "direct"` 之类配置：
    - 会把运行策略硬编码进 Field Spec，削弱 RuntimeConfig/Middleware 的角色；
...
- **Decision**:
  - FieldKernel 层不对 `resourceId` 做唯一性约束：同一逻辑资源（例如 `"user/profile"`）可以被多个模块、多个字段复用，这些引用在 FieldProgram / FieldGraph 中被视为“共享同一资源类型”的依赖；
...
- **Rationale**:
  - 将唯一性与冲突检测放在 Resource/Runtime 层，而不是 Field 层，既允许不同模块共享资源定义，又能避免在同一作用域内出现难以察觉的实现冲突；
  - TraitProgram 仍然可以把 `resourceId + key` 作为纯事实源输入，Middleware/Resource 层负责保证“同名资源”的语义一致性。
- **Alternatives considered**:
  - 在 FieldKernel.build 阶段全局收集并检查 resourceId 唯一性：
    - 不利于按 Runtime scope 做差异配置，也会把实现策略硬编码进 Field 引擎；
...
- **Decision**:
  - FieldKernel.build 必须在构建 FieldGraph 时检测 link 组成的依赖环；
...
- **Decision**:
  - 在 FieldGraph 中，link 表达为一条从源字段到目标字段的边（`kind = "link"` 或等价）；
  - 在 FieldPlan 中，link 表达为 `kind = "link-propagate"` 的步骤，包含源/目标路径（或字段 id），`FieldKernel.install` 基于此安装 watcher 与状态更新逻辑。
- **Rationale**:
...
- **OQ-1**: 是否需要在 FieldKernel API 层对外暴露统一的 Field 抽象（例如 `Field.System` / `RuntimeTraitEngine`），供 ActionTrait / FlowTrait 等复用？
  - 当前倾向：先让 FieldKernel 独立跑通，抽象保留在 internal，等第二个 Field 家族出现时再看是否值得对外稳定此抽象。

- **OQ-2**: FieldPlan 与 Middleware 配置的边界
  - 当前假设：FieldPlan 主要描述“watch/更新”的结构，具体运行策略（节流/批处理等）通过 Middleware 配置实现；
  - 后续是否需要在 Plan 中显式编码某些策略（例如 computed 批量更新）仍需实践验证。

- **OQ-3**: link 的 map/条件/优先级等高级能力
  - 当前决定不在首版 DSL 中支持；
  - 未来可以将其设计为 FieldKernelEntry.meta 的扩展，并让 Plan/install 在保证可读性的前提下支持这些规则。
...
- **目标**：
  - 在 FieldKernel.source + Resource / Query + Middleware 主线稳定之后，为常见的远程资源场景提供一批轻量的语法糖（helpers），降低样板代码数量、增强可读性；
  - 典型形式包括：`Query.source` / `Query.cachedSource` / `Query.sourceByPath` 等。
- **边界约束**：
  - 所有语法糖都必须在类型和 IR 上等价于某种 `FieldKernel.source` 调用，即：
    - 不引入新的 Field kind；
    - 不改变 FieldProgram / FieldGraph 的结构；
    - 不要求 FieldKernel 引擎理解任何 Query 专有字段。
  - 所有与缓存 / 重试 / 失效策略 / queryClient 相关的行为，必须通过 `Query.Engine.layer` / `Query.Engine.middleware` 在 EffectOp 层实现，而不是写入 FieldKernel.meta。
...
- **Query.source 作为别名糖**：
  ...
  // Query.namespace 内部
  export const Query = {
    source: <S extends object, P extends FieldKernel.StateFieldPath<S>>(options: {
      resource: string
      deps: ReadonlyArray<FieldKernel.StateFieldPath<S>>
      key: (state: Readonly<S>) => unknown
    }) => FieldKernel.source<S, P>(options),
  }
  ```
  ...
  - 对 FieldProgram 而言，这与直接写 `FieldKernel.source` 完全等价。
...
- **更高阶语法糖的约束**：
  ...
  - 但这些配置必须通过 ResourceSpec / QueryConfig / Middleware 传入，不允许直接修改 FieldKernelEntry 的核心 meta 结构。

### 7.3 与 Middleware 的协同

- Middleware 仍是行为的唯一落点：
  - 对于由 Query 语法糖声明的 source 字段，最终在 Runtime 中观察到的，应是 `EffectOp(kind = "field-source", meta.resourceId, meta.keyHash)`；
  - `Query.Engine.middleware` 可以基于 resourceId + key + 自己的配置，决定是否走 queryClient、如何缓存/重试；
  - DebugTraitObserver / Devtools 也只依赖 EffectOp + FieldGraph，无需理解语法糖的存在。

- 语法糖的**可撤销性**：
  - 即使完全移除 Query 相关 Layer/Middleware，使用 Query 语法糖定义的模块仍然必须作为“单纯的 FieldKernel.source 模块”正常工作；
  - 这条约束保证了 Field 稳定性与 Query 集成的“可插拔”特性。
