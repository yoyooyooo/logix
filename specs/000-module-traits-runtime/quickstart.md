...
const runtimeLayer = Layer.mergeAll(
  ...
  // 基于 EffectOp(kind="trait-source") + resourceId + keyHash 决定是否由引擎接管。
...
在更细粒度的路由 / 子树中，可以通过 RuntimeProvider 叠加或移除 `Query.Engine.layer` + `Query.Engine.middleware`，从而做到“这一片都走查询引擎 / 这一片直接调用 Service Tag”，而不需要修改任何 Module / Trait 声明。
...
2. 当 Logic / Action / Devtools 显式调用该入口时：
   - 构造一个 `EffectOp`，`kind = "trait-source"`, `name = "user/profile"`；  
...
- Timeline：每次 `kind = "trait-source"` 的 `user/profile` 调用，以及 Resource / Query / 其他中间件（重试/超时/日志等）的参与情况。  
...
export const CounterWithProfile = Logix.Module.make("CounterWithProfile", {
  ...
  traits: StateTrait.from(StateSchema)({
    ...
    // 显式写法：等价于上面的 StateTrait.source（不引入第二套事实源）
    profileResource: StateTrait.source({
      deps: ["profile.id"],
      resource: "user/profile",
