...
**Spec**: `specs/001-module-traits-runtime/spec.md`  
...
- **Decision**:
  ...
  - 在 StateTraitPlan 中，source 型步骤只保留 resourceId 与 key 规则，`StateTrait.install` 根据 Plan 生成 `EffectOp(kind = "service")` 并调用 ResourceRegistry。
- **Rationale**:
...
- **Decision**:
  - 对外不直接暴露 ResourceRegistryTag / QueryClientTag，而是通过命名空间封装：
    - `Resource.make({ id, keySchema, load })`：定义资源规格（ResourceSpec）；  
    - `Resource.layer([specA, specB, ...])`：返回 Layer，在内部注册到 ResourceRegistryTag；  
    - `Query.layer(queryClient)`：注册查询客户端实例（可基于 TanStack Query 或其他实现）；  
    - `Query.middleware(config)`：基于 `EffectOp(kind = "service")` + `resourceId` + `key` 决定哪些资源走查询引擎、如何做缓存/重试等策略。
...
- **Alternatives considered**:
  - 直接在 StateTrait.source 中加入 `driver: "query" | "direct"` 之类配置：  
    ...
    - 当前选择让驱动策略停留在 ResourceSpec/meta 与 Query.middleware 的组合里。
...
- **Decision**:
  ...
  - Query 集成同样依赖 RuntimeProvider：在某个子树中装配 `Query.layer` + `Query.middleware`，就可以让该子树下的所有 source 以统一策略走 queryClient；移除该 Layer 即可恢复为直接调用 Service Tag 的模式。
- **Rationale**:
...
- **边界约束**：
  ...
  - 所有与缓存 / 重试 / 失效策略 / queryClient 相关的行为，必须通过 `Query.layer` / `Query.middleware` 在 EffectOp 层实现，而不是写入 StateTrait.meta。
...
- **更高阶语法糖的约束**：
  - 类似 `Query.cachedSource`、`Query.sourceByPath` 等更高阶 helper，可以在内部完成：
    ...
    - 给 Query.middleware 填充额外的配置（例如在 ResourceSpec 或 QueryConfig 中登记 cacheGroup）；  
...
- Middleware 仍是行为的唯一落点：
  - 对于由 Query 语法糖声明的 source 字段，最终在 Runtime 中观察到的，仍然应是 `EffectOp(kind = "service", meta.resourceId, meta.key)`；  
  - Query.middleware 可以基于 resourceId + key + 自己的配置，决定是否走 queryClient、如何缓存/重试；  
