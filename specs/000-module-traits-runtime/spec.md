...
- 在需要的 Runtime 范围（根 Runtime 或某个 RuntimeProvider 子树）装配 `Query.Engine.layer(engine)` + `Query.Engine.middleware(config)`（例如 `engine = Query.TanStack.engine(queryClient)`），让该范围内的 `StateTrait.source` 调用自动获益于查询引擎的缓存 / 去重 / 失效等能力；
...
- Query 语法糖的运行时行为完全通过 `Query.Engine.layer` / `Query.Engine.middleware` 与 EffectOp/Middleware 总线实现，移除对应 Layer/Middleware 后，模块仍然是一个纯粹依赖 StateTrait.source 的模块。
...
- **FR-025 · Trait DSL Kernel 收敛（统一 verbs + check 语义糖）**  
  StateTrait 必须作为所有 *Trait 领域 DSL 的唯一内核（kernel）形状：  
...
- **FR-026 · 数组一等公民：StateTrait.list**  
  Kernel DSL 必须把数组字段作为一等公民支持，提供 `StateTrait.list(...)`（或等价 node）描述数组字段的 per‑item / per‑list 规则：  
  ...
  - list/check 只在 DSL 层存在，build 后仍然产出标准的 StateTraitProgram/Graph/Plan（computed/source/link 步骤）。

- **FR-027 · 领域 Trait DSL 的同源落脚点（Form 等）**  
  未来任何 `xxxTrait`（例如 Form/Permission/User/Workflow 等）在 DSL 层都必须遵循：  
  - 只提供“领域语义的甜味语法糖/预置库”，其产物为可直接 `spread` 进 `StateTrait.from(StateSchema)({ ... })` 的 kernel fragment；  
  - 领域 DSL 不得引入新的运行时 IR、不得绕开 EffectOp/Middleware 总线；  
  - 领域 DSL 对数组/校验/异步的表达必须编译为 `StateTrait.list + check/source/computed/link` 的等价 kernel 结构；  
