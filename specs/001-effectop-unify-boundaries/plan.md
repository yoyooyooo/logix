...
**Project Type**: pnpm monorepo；改动主落点在 `packages/logix-core`，并联动 `apps/docs` 与 `docs/specs/runtime-logix`  
**Performance Goals**: 中间件总线对高频边界不引入显著额外开销；默认路径应为“零配置接近零成本”（空 stack 走直通）  
**Constraints**:
- docs-first：任何对 Runtime 契约的改变需同步更新 `docs/specs/runtime-logix` 与 `apps/docs`；
...
- Intent → Flow/Logix → Code → Runtime 映射：  
  - Intent/规范侧：`docs/specs/runtime-logix` 与 `specs/001-effectop-unify-boundaries/spec.md` 定义“边界操作统一模型”；  
  - Flow/Logix 侧：业务通过 Flow/Bound 等 API 触发操作；  
  - Code/Runtime 侧：所有操作统一提升为 EffectOp 并进入 middleware stack，形成可回放/可观测链路（含操作链路 id）。
- 依赖 / 修改的上游 specs（文档先行）：  
  - `docs/specs/runtime-logix/core/04-logic-middleware.md`（将“统一总线”作为唯一主线叙事）；  
  - `docs/specs/runtime-logix/core/05-runtime-implementation.md` 与相关 impl 备忘（记录接线与防呆约束）；  
...
└── docs/specs/runtime-logix # 规范文档：把总线收敛为唯一主线
```
...
- 该入口负责：补齐 meta（moduleId/runtimeId/txnId/linkId/trace...）、串行化（如适用）、并执行 middleware stack（空 stack 直通）；
...
- 该入口必须：  
  - 自动补齐 moduleId/runtimeId/txnId/linkId；  
  - 在空 stack 时直通；  
...
- 对照 `specs/001a-module-traits-runtime/*` 与 `docs/specs/runtime-logix/*`：  
  - 标注哪些承诺已由本次实现覆盖；  
  - 哪些需要修订文字/迁移示例；  
  - 哪些属于后续主题（明确延期理由与锚点）。  
...
- Effect/Logix 契约变更：已将全局/局部策略优先级与拒绝语义写死为可测试约束，后续实现必须以此为准并同步更新 `docs/specs/runtime-logix/*` 与 `apps/docs/*`。  
