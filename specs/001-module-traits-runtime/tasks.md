...
- [X] T050 [P] [US3] 在 Devtools 包中新增一个简单视图组件/脚本，用于展示某 Module 的 StateTraitGraph（可先用 JSON tree 替代图形）：`packages/logix-devtools-react/src/StateTraitGraphView.tsx`
...
- [X] T060 [P] [US7] 在 Devtools React 包中新增 Timeline 视图组件，消费 EffectOp 流并支持基础过滤：`packages/logix-devtools-react/src/EffectOpTimelineView.tsx`
- [X] T061 [P] [US7] 将 StateTraitGraph 视图与 Timeline 视图通过 moduleId/fieldPath/resourceId 联动起来（点击 Graph 节点自动过滤 Timeline）：`packages/logix-devtools-react/src/StateTraitGraphView.tsx`
- [X] T062 [US7] 为 Devtools/Debug 页面添加入口导航与 UX 微调，使 Debug/Trait/Middleware 信息集中在一个面板内展示：`packages/logix-devtools-react/src/LogixDevtools.tsx`
- [X] T073 [US7] 调整 EffectOp Timeline 视图交互逻辑，实现“默认不选中事件、无选中时展示最新事件详情、再次点击已选中事件取消选中”的行为：`packages/logix-devtools-react/src/Inspector.tsx`
