# Data Model: 设计 Module DSL 接入 `@logix/data`，让 `@logix/core` 可消费字段能力（隶属已归档 spec，仅供参考）
...
> 提示：本文件隶属于已归档的 `specs/002-logix-data-core-dsl` 特性，当前主线已由 `specs/000-module-traits-runtime` 收敛到 `@logix/core` 内部的 StateTrait 数据模型。此处内容只作为早期 Module DSL + `@logix/data` 协作模型的参考。
...
- **关键属性**：
  ...
  - `fieldCapabilities`: 针对某些字段附加的能力声明集合（Computed / Source / Link 等），从模块作者视角看是 DSL 片段，从运行时视角看是 `@logix/data` 的 Blueprint 输入；
...
- **描述**：由 `@logix/data` 从字段能力蓝图推导出的中间结构，描述在运行时应如何为模块字段挂载 Computed / Source / Link 行为。  
- **关键属性**：
...
- 一个 ModuleRuntimePlan **依赖 1..N 个** Field / FieldCapability 实体（定义于 `@logix/data` 内部数据模型中）。
...
- **关键属性**：
  - `moduleId`: 模块标识；  
  - `fields`: 基于 `@logix/data` Field 实体的字段列表（包含路径、显示名、类型等元信息）；  
...
- Module DSL 定义中的 `fieldCapabilities` 必须能被 `@logix/data` 的扫描逻辑解析为合法的 `Field` 与 `FieldCapability` 实体，否则视为编译/构建期错误；  
