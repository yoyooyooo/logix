# Phase 0 Research: 设计 Module DSL 接入 `@logix/data`，让 `@logix/core` 可消费字段能力（隶属已归档 spec，仅供参考）

**Branch**: `002-logix-data-core-dsl`  
**Date**: 2025-12-09  
**Source Spec**: `specs/002-logix-data-core-dsl/spec.md`

> 提示：本文件隶属于已归档的 `specs/002-logix-data-core-dsl` 特性，记录的是早期「Module DSL ↔ `@logix/data` ↔ `@logix/core`」的决策草图。当前主线已改为 StateTrait / `@logix/core`，此处内容仅作为历史思路参考。
...
以「保持现有 `@logix/core` Module API 尽量稳定」为前提，采用**渐进增强式 DSL**：
...
- 该扩展点负责构造 `@logix/data` 需要的 Schema/Blueprint，供扫描与 Plan 生成使用。
...
2. **完全在 `@logix/data` 内部提供独立 DSL**，再通过适配层注入 core。  
   - 缺点：模块作者需要同时理解两套 DSL，且 core 侧类型推导会更绕。

---

## 2. `@logix/core` 与 `@logix/data` 依赖关系

### Decision

维持「core 依赖 data，data 不依赖 core」的单向依赖关系：

- `@logix/core` 在编译期引用 `@logix/data` 的类型与 IR（Field / FieldCapability / RuntimePlan 等），并在 Runtime 中消费这些结构；
- `@logix/data` 仍然只是字段能力与状态图的描述层，不引入对 core 类型或 Runtime 的反向依赖。
...
1. **双向依赖**：在 `@logix/data` 中直接引用 core 的 Module 类型以获取更强类型推导。  
...
在 `@logix/core` 的 ModuleRuntime 构建流程中插入一个「字段能力挂载阶段」：

- 在模块初始化时，调用 `@logix/data` 的扫描与 Plan API，生成该模块的 `ModuleRuntimePlan`；
