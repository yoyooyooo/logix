# Data Model: FieldKernel 状态事务与生命周期分层（Devtools 升级）

## 1. Core Runtime Entities

### 1.1 Module

- **Description**: 逻辑模块蓝图，承载 `state / actions / fields` 三个槽位定义。Devtools 以 Module 为根节点组织实例与事务视图。
- **Key Fields**:
  - `moduleId: string` — 模块唯一标识（与 ModuleTag.id 对齐）；
  - `name: string` — 用于 Devtools 展示的模块名称；
  - `stateSchema: any` — 状态 Schema 描述（供 TraitProgram 构建使用）；
  - `traitsSpec: FieldKernelSpec` — Field 声明蓝图。
- **Relationships**:
  - 1:N 到 `ModuleRuntime`；
  - 1:1 到 `FieldProgram` / `FieldGraph`（蓝图层）。
...
- **Key Fields**:
  ...
  - `setupStatus: SetupStatus` — Field 蓝图是否全部正确接线的状态枚举：
...
- **Description**: 某次事务内部对状态的原子变更记录，用于在 Devtools 中展示字段级差异和 Field 的影响范围。
- **Key Fields**:
  ...
  - `reason: PatchReason` — 变更原因：
    ...
    - `"field-computed"` — 由 Field 的 computed 步骤引起；
    - `"field-link"` — 由 Field 的 link 步骤引起；
    - `"source-refresh"` — 由某个 source 刷新结果写回引起；
    - 其他可扩展类型（如 `"devtools"` 回放）。
  - `traitNodeId?: string` — 若来自 Field 步骤，对应 FieldGraph 节点 ID；
  - `stepId?: string` — 若来自 TraitPlan 步骤，对应 Plan 中的 step ID。
- **Relationships**:
  - N:1 到 `StateTransaction`；
  - 可选 1:1 到 `FieldGraphNode` / PlanStep。

### 1.5 TraitRuntimeEvent / RuntimeDebugEventRef

- **Description**: Devtools 消费的统一 Runtime Debug 事件视图，是对底层 `CoreDebug.Event` / EffectOp 事件的轻量抽象，用于在事务视图和时间线中做聚合和过滤。虽然名称中仍保留 Field，但语义上覆盖所有与模块运行相关的事件（动作、状态提交、Field 步骤、服务调用、React 渲染、Devtools 操作等）。
- **Key Fields**:
  - `eventId: string` — 事件唯一标识（在单进程内唯一，通常由 DebugSink 生成）；
  - `txnId?: string` — 所属事务 ID（若事件可以归属到某个 StateTransaction，则必填；例如 `action:dispatch` / Field 步骤 / `state:update` / 由事务引发的 `react-render`；若是全局 Devtools 操作或单纯布局事件，则可为空）；
  - `moduleId?: string` — 所属模块（如有）；
  - `instanceId?: string` — 所属实例（如有）；
  - `timestamp: number` — 事件时间戳（ms）；
  - `kind: "action" | "state" | "service" | "field-computed" | "field-link" | "field-source" | "lifecycle" | "react-render" | "devtools" | string` — 归一化事件类别，用于时间线分组与筛选；
  - `label: string` — 简要描述（例如 `"action: changeName"`、`"field: meta.isDirty computed"`、`"react: FieldForm render"`）；
  - `meta: any` — 事件特定的附加信息，典型字段包括：
    - `traitNodeId?: string` / `planStepId?: string` — 若为 Field 步骤，关联的图节点 ID 与 Plan 步骤 ID；
...
- **Relationships**:
  ...
  - 与底层 Debug 事件（`CoreDebug.Event`）之间应有可逆或可追溯映射，便于后续扩展更细粒度的诊断能力。
...
## 2. Field Blueprint & Lifecycle Entities

### 2.1 FieldProgram

- **Description**: 从 `stateSchema + traitsSpec` 构建出的 Field 蓝图程序，对 Devtools 来说是「TraitGraph / Plan」的数据来源。
- **Key Fields**（高层抽象）：
  - `moduleId: string` — 所属模块；
  - `stateSchema: any` — 状态 Schema 类型描述；
  - `spec: FieldKernelSpec` — Field 规格；
  - `graph: FieldGraph` — 字段与 Field 的图结构视图；
  - `plan: FieldPlan` — 运行阶段应执行的步骤计划列表。

### 2.2 FieldGraph

- **Description**: Field 蓝图的图结构表示，用于 Devtools 绘图和结构分析。
- **Key Fields**：
  - `nodes: FieldGraphNode[]` — 节点集合，节点可以是字段节点、Field 节点等；
  - `edges: FieldGraphEdge[]` — 边集合，描述字段依赖和 Field 链接；
  - `metadata: any` — 附加元信息，如模块文件路径、源位置等。

### 2.3 FieldPlan

- **Description**: 从 Program 中导出的执行计划，用于指导 setup/run 如何安装和执行 Field 行为。
- **Key Fields**：
  - `steps: FieldPlanStep[]` — 有序步骤列表，每个步骤有 kind（computed-update / link-propagate / source-refresh 等）、目标字段路径、依赖字段集合等。

### 2.4 FieldKernelLifecycleState

- **Description**: 某个 Field 节点在蓝图 / setup / run 三阶段的状态视图，用于 Devtools 上高亮和过滤。
- **Key Fields**：
  - `moduleId: string`；
  - `instanceId?: string` — 若与具体实例绑定；
  - `traitNodeId: string` — 来自 FieldGraph 的节点 ID；
...
- **Key Fields**：
  - `moduleId: string`；
  - `instanceId: string`；
  - `label: string` — 简要说明实例来源（例如 “FieldForm@Sandbox”）；
...
- **Key Fields**：
  - `mode: "basic" | "deep"` — 观测模式，控制是否展示 Field 细节、`react-render` 事件、时间旅行控件等（与 FR-016 一致）；
  - `showTraitEvents: boolean` — 是否在 Timeline 中展示 Field 级事件（在 `"basic"` 模式下通常为 false）；
...
- FieldLifecycleState 必须满足：
  - `blueprintPresent = true` 时，`traitNodeId` 必须在 FieldGraph.nodes 中存在；
...
- `specs/003-field-txn-lifecycle/references/future-devtools-data-model.md`
