# Implementation Plan: 003-field-txn-lifecycle（FieldKernel 状态事务 / 生命周期分层 + Devtools 升级）

**Branch**: `003-field-txn-lifecycle` | **Date**: 2025-12-11 | **Spec**: `specs/003-field-txn-lifecycle/spec.md`
**Input**: Feature specification from `/specs/003-field-txn-lifecycle/spec.md`

## Summary

本特性围绕「Field + 状态事务 + Devtools」打通从 Runtime 到调试视图的完整链路，并为后续表单/复杂联动等高 Field 密度场景打好运行时地基：

- 在 `@logixjs/core` 中引入显式的 `StateTransaction / StateTxnContext` 内核，将一次逻辑入口（dispatch / fields.source.refresh / service 回写 / devtools 操作）视为一个状态事务，在事务内部聚合所有 Reducer / Field / Middleware 的状态修改，只在 commit 时写入底层 store 并对订阅者发出一次聚合通知。
- 将 FieldKernel 生命周期拆分为「蓝图 → setup → run」三段：蓝图层只负责基于 stateSchema + traitsSpec 构建 Program/Graph/Plan，setup 层只做 Env 无关的结构接线（source 刷新入口、Debug/Devtools 锚点），run 层在 StateTransaction 内执行 Field 步骤与 Effect 行为。
...
- 在 React 集成层纳入 `kind = "react-render"` 的 Debug 事件，将组件渲染纳入事务视图，使 Field + 事务下的 UI 行为可回放、可比较。

研究结论与数据模型已分别固化在 `research.md`、`data-model.md` 与 `contracts/devtools-runtime-contracts.md` 中，`quickstart.md` 给出了从 FieldForm Demo 出发的一条完整验证路径，`tasks.md` 将实现拆分为 Phase 1/2 + US1–US5 + Polish 的任务列表。
...
- 包内：`pnpm test --filter @logixjs/core` / `--filter @logixjs/react` / `--filter @logixjs/devtools-react`
  ...
  **Performance Goals**:
- 对齐 spec 中 SC-001：在包含 ~50 Field 节点的示例模块中，一次用户交互对应的单次 StateTransaction 执行时间通常 < 50ms；对外状态提交 1 次，订阅通知 1 次，React 渲染次数不超过 3 次（StrictMode 额外渲染除外）。
...
- Devtools 所有影响 Runtime 开销的观测开关（模式、Field 明细、时间旅行控件、overview 维度等）必须通过统一的设置面板暴露给用户，并按浏览器 localStorage 持久化，localStorage 不可用时不得影响当前会话功能（回退到内存态）。
- StateTransaction 的观测策略（Instrumentation）配置面 MUST 收敛到 Runtime / ProgramRuntimeBlueprint 两层：
  - 在 `@logixjs/core` 中通过 `Logix.Runtime.make(program, { stateTransaction?: { instrumentation?: "full" \| "light" } })` 提供应用级默认观测级别（`program` 为 `Program.make(...)` 的结果）；
  - 在 `Program.make(ModuleDef, { initial, logics?, stateTransaction?: { instrumentation?: "full" \| "light" } })` 上允许为少数高频/性能敏感模块覆写观测级别；
  - 优先级约束：Program 级配置 > Runtime.make 级配置 > `getDefaultStateTxnInstrumentation()`（基于 `NODE_ENV` 的默认值）；
  - React 层的 `RuntimeProvider` / `LogixProvider` 仅负责透传已构造好的 Runtime 与 Layer，不得在 Provider 级别引入新的事务观测模式或关闭事务模型。
    **Scale/Scope**:
- 首批覆盖 `examples/logix-react` 中 FieldForm 等 Demo（包括高 Field 密度版本）；
...
- 本特性如何映射到 `Intent → Flow/Logix → Code → Runtime` 链路？
  - Intent：模块作者与平台维护者希望在 Field 规模化后仍然可以以“事务 + 生命周期”的视角推理与调试状态变化；业务开发者希望以 Timeline/Origin 视角观察 Field 行为与渲染成本，而不是在零散日志中摸索。
  - Flow/Logix：在 Module / FieldProgram / ModuleRuntime 层引入 StateTransaction 与 Field 生命周期语义；在 Devtools 层以事务视图 + 时间线 + 性能概览条暴露观测与时间旅行能力。
...
- 依赖或修改哪些 `docs/specs/*` 规范？是否已按文档先行更新？
  - 直接依赖：
    - `docs/ssot/runtime/logix-core/concepts/10-runtime-glossary.md`（StateTransaction / Field 生命周期 / Devtools 术语需在实现后登记）；
    - `docs/ssot/runtime/logix-core/*`（Module / Logic / State / Debug 契约）；
    - `docs/ssot/runtime/logix-core/impl/README.md`（运行时实现要点）；
    - `docs/specs/drafts/topics/field-system/*`（作为高 Field 场景参考与回归样本清单）。
...
- 是否引入/修改 Effect/Logix 契约？相关规范是否同步？
  ...
  - 修改：对 Field 生命周期（蓝图/setup/run）的约束、对 ModuleRuntime 订阅语义（以事务 commit 为粒度）的约束。
...
- 质量门槛：在合并前会运行哪些脚本？什么算“通过”？
  ...
  - 示例验证：按 `quickstart.md` 路径对 FieldForm Demo 进行手工验证，检查事务视图、TraitGraph、时间线游标、时间旅行、渲染事件与 overview strip 的表现；
...
```text
specs/003-field-txn-lifecycle/
├── spec.md          # 需求与 FR/SC（StateTransaction + Field 生命周期 + Devtools 功能）
...
├── quickstart.md    # Phase 1：从 FieldForm 等 Demo 出发的验证路径
...
└── references/      # 扩展设计（如 future-devtools-data-model、Field-first Form 模式）
```
...
│       ├── state-field.ts            # FieldProgram / Graph / Plan + 生命周期拆分
...
│       │   ├── graph/
│       │   │   └── FieldGraphView.tsx   # Field Graph 结构视图
│       │   ├── inspector/
...
└── logix-react/
    └── src/
        ├── modules/
        │   ├── field-form.ts             # 现有 Field Demo
        │   └── field-form-advanced.ts    # （可选）高 Field 密度压测场景
        └── demos/
            └── field-txn-devtools-demo.tsx   # Demo Page：挂载 Field 模块 + Devtools 面板，用于手工体验事务/时间旅行/性能视图
...
- 本特性主要落在三个核心包：`@logixjs/core`（事务 / Field 生命周期 / DebugSink / DevtoolsHub + `Runtime.make({ devtools })` 一键启用）、`@logixjs/react`（React 渲染事件上报与 devtools 开关联动）、`@logixjs/devtools-react`（Devtools 视图模型与 UI，包括 overview strip；不再内置 snapshot/sink）。
...
- **DevtoolsHub（core 侧全局单例）**
  ...
  - 通过 `packages/logix-core/src/internal/debug-api.ts` 暴露只读 API：
...
- **ProgramRuntimeBlueprint 级入口（按模块覆写）**
  - 扩展 `packages/logix-core/src/internal/runtime/core/module.ts` 中的 `ModuleTag["implement"]` 与 `ProgramRuntimeBlueprint` 类型，在 `implement` 的 config 中新增可选字段
    `stateTransaction?: { readonly instrumentation?: "full" | "light" }`；
  - 在 `packages/logix-core/src/internal/runtime/ModuleFactory.ts` 的 `Module.implement` 实现中：
    ...
    - 确保优先级逻辑与 spec/FR-015 中的“ProgramRuntimeBlueprint > Runtime > NODE_ENV 默认”保持一致。
...
- **使用边界与风险**
  - `run*Task` 仅面向 run 段 watcher（`$.onAction/$.onState/$.on`），**不得在 Reducer/Field-run 的同步事务 body 内直接调用**，否则可能与内部队列等待语义冲突；dev 下应给出诊断提示。
