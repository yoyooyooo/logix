# Quickstart: 升级 Devtools 面板以支持 StateTransaction 与 Field 生命周期

本特性聚焦在现有 Devtools 面板基础上，补齐「模块 → 实例 → 事务」层级导航以及 Field 蓝图/setup/run 分层视图。下面是从“如何在本仓验证”的角度给出的快速指引。
...
1. 在仓库根目录安装依赖并构建：
   - `pnpm install`
   - `pnpm build`
2. 启动包含 Field 示例的文档站或示例应用（视当前集成方式而定，例如 `pnpm dev --filter apps/docs` 或运行 `examples/logix-react` 对应的 dev 命令）。
3. 在浏览器中打开示例页面，确保 `FieldForm` 等带 Field 的 Demo 能正常加载，并在界面上显示 Devtools 入口（例如一个可展开的调试面板或侧栏）。

## 2. 在 Devtools 中浏览「模块 → 实例 → 事务」

1. 打开 Devtools 面板，找到左侧导航区域：
   - 顶层应按 Module 维度列出已注册的模块，例如 `FieldFormModule`；
...
## 3. 通过 Field 示例验证状态事务语义（US1）

以 `FieldForm` 示例为例，完整走一遍“一次交互 = 一次事务 = 一次状态提交”的链路：

1. 在 Devtools 左侧选择 `FieldFormModule` 的某个实例。
...
3. 点击该事务条目，观察事务详情：
   - 时间线视图中应展示：
     ...
     - 若干 `field-computed` / `field-link` 步骤（用于维护 `meta.dirtyCount` / `meta.isDirty`）；
     - 最终一条 `state:update` 事件，表示事务提交；
   - Patch 列表中应只包含与本次输入相关的字段（例如 `form.name`、`meta.dirtyCount`、`meta.isDirty`），而不会包含无关字段；
   - 事务详情中应能看到统一的 `txnId`，同一事务内的 `action` / `field-*` / `state` / `react-render` 事件都共享这个 `txnId`；
   - 对外状态提交次数应为 **1 次**：
...
>
> - 使用 `TestProgram.runProgram(programModule, ...)` 或 React `RuntimeProvider` + `useModule/useDispatch` 驱动一次 `FieldForm` 交互；
> - 通过挂载 DebugSink 并调用 `CoreDebug.internal.toRuntimeDebugEventRef`，统计某个 `txnId` 下 `kind = "state"` 的事件数应为 1；
> - 断言交互前后 `FieldFormState` 仅在期望字段上发生变化，与 Devtools 中看到的 Patch 聚合结果一致。

## 4. 浏览 Field 蓝图与生命周期状态

1. 在 Devtools 左侧 Sidebar 中：
   - 在某个 Runtime 下，带有 FieldKernel 蓝图的模块会在实例计数旁显示一个 `Field` 徽标；
   - 若模块存在运行中的实例且已挂载 FieldKernel 程序，则该徽标会以高亮样式展示（表示「蓝图 + 运行中」）。
2. 选中带 Field 的模块与实例后，在右侧 Inspector 的 `Fields / FieldGraph` 区块中：
   - 可以看到 FieldGraph 的节点列表与依赖边：
     - 节点通常对应某个字段路径（例如 `form.name`、`meta.isDirty`）；
     - 每个节点右侧会展示该字段上挂载的 Field 类型列表（`computed` / `link` / `source` 等）。
...
如果某个示例模块引入了 FieldKernel.source 字段：
...
3. 若资源访问失败，应在 Patch / 事件详情中看到错误信息，并且字段保留上一次成功值或默认值（符合 fields-runtime 规范）。

## 6. 使用时间旅行调试 Field 行为

1. 在 Field 示例中进行多次交互，产生一串 Transaction（例如多次修改表单字段、触发 source 刷新等）。
...
2. 打开带 Field 的表单示例（如 `FieldForm`），在输入框中缓慢输入几次文本，产生多条 StateTransaction。
3. 在 Devtools 中选中某个事务，观察时间线：
   - 除了 `action` / `field-*` / `state` 等事件外，应看到若干 **`kind = "react-render"`** 的事件条目；
...
4. 尝试在 Devtools 中按事件类别过滤/折叠视图：
   ...
   - 结合 `state` / `action` 事件一起查看，可以分析在 Field 数量增加后，单次交互对应的渲染次数与字段更新范围是否仍然在可接受范围内。

## 8. 使用时间轴总览条与模式切换分析性能

1. 在 Dev 模式下打开带 Field 的 Demo 页面（如 `FieldForm` 或后续的 `field-txn-devtools-demo` 页面），确保 Devtools 面板已挂载。
...
4. 切换 Devtools 观测模式（例如从 `"basic"` 切到 `"deep"`）：
   - 在 `"basic"` 模式下，应只看到 action/state/service 事件与粗粒度 `react-render` 事件，Field 细节和时间旅行 UI 被隐藏，以降低噪音；
   - 切换到 `"deep"` 模式后，应额外看到 field-\* 事件与时间旅行按钮，同时 overview strip 可能展示更多维度的指标；
   - 切换模式时，Runtime / Debug 层应减少当前模式不需要的 Debug 事件与统计工作（例如在 `"basic"` + `"light"` 下只对 `react-render` 做采样埋点），你可以在高 Field 密度场景下观察模式切换对 Devtools 自身开销的影响。
5. 在高频输入场景（如快速输入表单字段）和低频操作场景（如间隔点击按钮）下，对比 overview strip 的形态与颜色变化，判断当前模块是否接近或超过预设性能阈值；如某段时间显著偏红，可结合事务视图和渲染事件进一步分析是否需要引入中间件防抖/节流或调整 Field 粒度。
...
3. 手工验证：
   - 打开带 Field 的 Demo，确认：
     ...
     - 在包含较多 Field 节点的示例模块中，简单体验“一次输入/点击”的响应速度：在本地 dev 环境下，单次交互对应的事务执行应无明显卡顿，React 渲染次数大致不超过 3 次（StrictMode 双调用除外），作为性能调优与自检的参考；
