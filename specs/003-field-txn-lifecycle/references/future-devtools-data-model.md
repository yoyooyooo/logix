...
- 未来可能增加：
  - `groupId`：将多个 Patch 归为同一“逻辑步骤”（例如同一次 Field 派生内部对多个字段的修改）；
...
- 潜在需求：将一段时间内的事务序列录制下来，并在 Devtools 中重放，观察 Field 行为与中间状态。
- 数据模型扩展点：
