# Research: FieldKernel 状态事务与生命周期分层（003-field-txn-lifecycle）
...
- 在 Runtime 级通过 `Logix.Runtime.make(program, { stateTransaction?: { instrumentation?: "full" | "light" } })` 配置应用级默认观测策略；
- 在 Program 级通过 `Program.make(ModuleDef, { ..., stateTransaction?: { instrumentation?: "full" | "light" } })` 为单个 Program 覆写观测策略；
- 优先级：**Program 配置 > Runtime.make 配置 > 基于 `NODE_ENV` 的默认值**（`getDefaultStateTxnInstrumentation()`）。
...
- 当前 StateTransaction 内核本身开销可控，真正的大头在 React 渲染和复杂 Field 逻辑，通过节流与 `"light"` 模式即可显著降低开销；
...
- 保持 runtime SSoT 的契约单一：观测策略的来源集中在 Runtime 与 ProgramRuntimeBlueprint，而不是在 React 层再加一层配置；
...
- Runtime 与 ProgramRuntimeBlueprint 层新增的配置类型统一引用同一枚举类型，并保证语义相同：
...
export interface ProgramRuntimeBlueprintConfig<R> {
  // ...
...
- 当 RuntimeOptions 或 ProgramRuntimeBlueprint 显式给出 `instrumentation` 时，优先使用显式配置，不再依赖默认推导。
