# Data Model: Field 生命周期桥接 × FieldKernel（支点）× Form（Rules/ErrorTree/安装）

**Branch**: `004-field-bridge-form`
**Source Spec**: `specs/004-field-bridge-form/spec.md`
**Source Quickstart**: `specs/004-field-bridge-form/quickstart.md`

> 作用：从「概念实体 + 关系」角度，把本特性在 field 内核链路上要固化的通用形状梳理清楚：
> - 以 **FieldKernel** 作为支点跑通数组/错误树/资源/事务；
> - 在其上构建 **Form 领域系统**（`@logixjs/form`：Rules/Error helpers/迁移映射）；
> - 同时把“field 生命周期桥接”下沉成可复用能力，便于未来更多 `xxxTrait` 复用同一条链路（install / refs / validate / cleanup）。
> 优先面向：Field/Runtime 实现者、Devtools / Studio 维护者。
>
> 相关 reference：
> - FieldKernel kernel 主模型与 Program/Graph/Plan：`specs/000-module-fields-runtime/data-model.md`
> - FieldKernel kernel API 形状（computed/source/link/check/list）：`specs/000-module-fields-runtime/references/field-kernel-core.md`
> - Resource / Query 约束：`specs/000-module-fields-runtime/references/resource-and-query.md`
> - Txn / Runtime 主线（表单最终落点）：`specs/003-field-txn-lifecycle/spec.md`
>
> 平行领域（Query）的外链规划：
> - Query 领域规划：`specs/004-field-bridge-form/references/02-query-module-plan.md`
> - Query 数据模型：`specs/004-field-bridge-form/references/03-query-data-model.md`
> - Query quickstart：`specs/004-field-bridge-form/references/04-query-quickstart.md`

---

## 1. 总览：Form 领域能力如何回落到 field kernel

本特性不引入第二套表单运行时。**唯一运行时主线仍是 FieldKernel → Program/Graph/Plan → Runtime/Txn/EffectOp。**

概念链路：

```text
（可选）Form 领域包（基于 field 生命周期桥接）
  - Form.fields(...)
  - Form.Rule.make(...)
  - （未来可扩展）RHF 映射/迁移 helper
        │  编译为
        ▼
FieldKernelSpec<S>（含 node/list/$root 组合子）
        │
        ▼
FieldKernel.build
        │
        ▼
FieldProgram<S> + Graph + Plan
        │
        ▼
...
- `Form.fields / Form.Rule / Form.Error` 只负责生成/组合 kernel 友好的 `FieldKernelSpec` 片段与 ErrorTree helper；
- FieldKernel.build/install 继续作为唯一的编译/安装入口；
...
## 2. 数组 Field：FieldKernel.list IR

### 2.1 FieldKernelListSpec<S, TItem>

**角色**：描述某个数组字段在 `item` / `list` 两个输入域上的 Field 能力集合（单行 / 整列）。
...
// 运行时约束（与内部 ResourceRegistry owner 对齐）：
...
// - 局部场景也可在 Program capabilities.imports 中注入 Layer（更贴近模块实现），语义等价。
...
```ts
/**
 * FieldKernel.node：统一 DSL 形状（纯编译期组合子）。
 *
 ...
 */
type FieldKernelDeps = ReadonlyArray<string>
 ...
 */
type FieldKernelComputedSpec<Input, Ctx> = {
  readonly deps: FieldKernelDeps
  readonly get: (...depsValues: Array<unknown>) => unknown
  readonly equals?: (prev: unknown, next: unknown) => boolean
}

type FieldKernelSourceSpec<Input, Ctx> = {
  readonly deps: FieldKernelDeps
...
type FieldKernelNodeSpec<Input, Ctx> = {
  ...
  readonly computed?: Record<
    ...
    string,
    FieldKernelComputedSpec<Input, Ctx>
  >
  ...
  readonly source?: Record<
    string,
    FieldKernelSourceSpec<Input, Ctx>
  >
...
/**
 * FieldKernel.list：数组字段一等公民。
 *
 * - item：单行 scope（数组元素）；
 * - list：整列 scope（跨行/列表级）。
 */
type FieldKernelListSpec<S, TItem> = {
  readonly kind: "list"
  ...
  readonly item?: FieldKernelNodeSpec<TItem, ItemCtx<S, TItem>>
  readonly list?: FieldKernelNodeSpec<ReadonlyArray<TItem>, ListCtx<S, TItem>>
}
```
...
- `FieldKernel.source` 与 `node.source`（字段级 / list.item / list.list）写回的目标字段类型 SHOULD 是 `ResourceSnapshot<Key, Out, Err>`；
...
`FieldKernel.node` / `FieldKernel.list` / `$root` 都是 **DSL 组合子**：build 阶段必须把它们展开成 kernel 可执行的规则集合（computed/source/link + check 写 errors），以保证 Runtime/Devtools 只有一条主线。
...
- `@logixjs/form` 负责维护 `state.ui` 的写入，以及在需要时触发 FieldKernel 的局部/全量校验（validate）。

---

## 8. Form.fields 领域糖（可选）

Form.fields 用于把“表单视角的配置”收敛为可 spread 的 FieldKernelSpec 片段，提升可读性，但**不增加新的 verbs**。
...
namespace Form {
  function fields<S>(
    ...
    [P in StateFieldPath<S>]?: FieldKernelEntry<S, P> | FieldKernelListSpec<S, any>
  }) => FieldKernelSpec<S>
}
```
...
- fields 只是帮助编排 list/check/source 的小工具；
- 输出仍然是 kernel 的 `FieldKernelSpec<S>`，最终交给 FieldKernel.build/install。

---

## 9. FieldLifecycle.install（桥接 UI 事件 → state.ui → scoped validate）

本小节刻意把 “install” 从 Form 领域叙事里抽出来：它应该是 **Field 生命周期**提供的通用桥接能力（见 spec FR-020/FR-021），用于把“上层领域事件”接到某个具体 ModuleRuntime 上执行。

Form 只是第一个吃到这套桥接能力的领域包：`@logixjs/form` 可以选择性地 re-export 这一能力（例如 `Form.install = FieldLifecycle.install`），但 API 的语义与所有权属于更底层的 Field 生命周期。
...
// install 的最小产物必须是可直接塞进 Program logics 的一段逻辑（ModuleLogic）
declare const FieldLifecycle: {
...
- `FieldLifecycle.install` 必须发生在 **Program/Runtime 作用域内**，因为 scoped validate 需要读取该模块的 FieldProgram/Graph/Plan 并对当前 state 执行；
