# Quickstart: Field→FieldKernel（支点）→Form（领域包）——数组/校验/异步/可回放（RHF≥）

> 本文以 **Field→FieldKernel（支点）** 为主线，展示如何在不引入第二套运行时的前提下构建 Form 领域系统：
> 规则/错误树/异步约束由 FieldKernel（node/list/check/source）承载；Form 领域包（`@logixjs/form`）只提供语法糖与 helper；Field 生命周期的桥接能力（install/refs/validate/cleanup）下沉为可复用能力，未来更多 `xxxTrait` 也可复用同一形状。
>
> 备注：`FieldKernel` 是本 spec 选定的“第一个支点 Field”，但不是强制中间层；未来某些领域也可以直接走 **Field → Form（领域包）** 的链路。
>
> Query（平行领域）的 quickstart 与数据模型见：
> - `specs/004-field-bridge-form/references/04-query-quickstart.md`
> - `specs/004-field-bridge-form/references/03-query-data-model.md`
>
> Kernel 只保留一套 verbs：
>
...
>
> 数组字段通过 `FieldKernel.list({ item, list })` 一等公民化；`item/list` 两个 scope 内部用 `FieldKernel.node({ ... })` 统一声明形状。
> `Form.fields / Form.Rule` 仅作为**可选领域糖**，最终都会回落为等价的 kernel 结构。
>
> 备注：`FieldKernel.list` 的 `item` / `list` 两个 scope 都是可选的。大多数表单只需要 `item`；只有在需要“跨行/列表级”规则或摘要时才声明 `list`。
>
> 如果你只关心“业务怎么用”（Blueprint/Controller/React），只读第 0 节即可；后续场景 1/2/3 主要展示 Fields 写法与语义边界。
>
> 进一步阅读（同一事实源）：
> - Form 业务 API（Blueprint + Controller）：`specs/004-field-bridge-form/references/06-form-business-api.md`
...
>
> 关键点：`FieldKernel.node / FieldKernel.list` 属于 **FieldKernel 基础体系（@logixjs/core）**；`@logixjs/form` 只负责把它们装配成“业务默认入口”（Blueprint + Controller），并提供少量领域糖与 helper。

### 0.1 创建 FormBlueprint（组件外定义，不会偷跑运行时）

```ts
import { FieldKernel } from "@logixjs/core"
...
- `useField` 只做 selector 投影 + DOM 事件适配；`state.ui` 与 scoped validate 仍由 FieldLifecycle/默认 logics 维护。
...
const RootProgram = Logix.Program.make(Root, {
  initial: { /* ... */ },
  capabilities: {
    imports: [EmailFormProgram],
  },
})
```

> 如果 fields 使用了 `source`（异步资源），对应的 `ResourceSpec` 需要在 runtime scope 内注册（`Resource.layer([...])`）；可放在 `EmailFormProgram` 自身 layer 或 `Logix.Runtime.make(..., { layer })` 的 layer 合并里。

后续章节的“场景 1/2/3”主要聚焦在 **fields 写法**（computed/source/check/list scope 等）；但把它们装配进项目的方式，都遵循本节的 Blueprint → Program → Runtime 的主线。
...
export const OrderTraits = FieldKernel.from(OrderFormStateSchema)({
  items: FieldKernel.list<OrderFormState, OrderItem>({
...
export const OrderTraitsWithRules = FieldKernel.from(OrderFormStateSchema)({
  items: FieldKernel.list<OrderFormState, OrderItem>({
    item: FieldKernel.node({ check: orderItemCheck }),
    list: FieldKernel.node({ check: orderListCheck }),
  }),
})
```
...
export const LanguageTraits = FieldKernel.from(LanguageFormStateSchema)({
  // 列表级 meta：写在独立字段上（不在 list 内做 patch）
  "meta.canAddLanguage": FieldKernel.computed({
    deps: ["languages"],
    get: (languages) => {
  ...
  "meta.languageTypeOptions": FieldKernel.computed({
    deps: ["languages"],
    get: (languages) => {
      const hasNone = languages.some((i) => i.type === "无")
      const count = languages.length
  ...
  languages: FieldKernel.list<LanguageFormState, LanguageItem>({
    item: FieldKernel.node({
      computed: {
        "ui.hideLevelAndScore": FieldKernel.computed({
          deps: ["type"],
          get: (type) => type === "无", // R4
        }),
      },
...
```ts
import { Resource, FieldKernel } from "@logixjs/core"
...
// 资源 id 常量（纯数据：供 Fields/Devtools/生成使用；不承载 load 实现）
...
export const ContractTraits = FieldKernel.from(ContractFormStateSchema)({
  items: FieldKernel.list<ContractFormState, ContractLine>({
	    item: FieldKernel.node({
	      // 1) 同步联动 + 远程结果派生（纯派生：每个字段各自产值）
		      computed: {
		        supplierCodeDisabled: FieldKernel.computed({
		          deps: ["contractId"],
		          get: (contractId) => !!contractId,
		        }),

		        // 合同 -> 供应商编码（禁用时强制回填；否则保持用户输入）
		        supplierCode: FieldKernel.computed({
		        ...
		        feeSiteOptions: FieldKernel.computed({
		          deps: ["supplierInfo"],
		          get: (supplierInfo) => {
		        ...
		        categoryOptions: FieldKernel.computed({
		          deps: ["supplierInfo"],
		          get: (supplierInfo) => {
		            const snap = supplierInfo
		            const info = snap && snap.status === "success" ? snap.data : undefined
...
| 维度 | RHF 写法示例 | FieldKernel 写法（概念） |
|------|--------------|-------------------------|
| 字段注册 | `register('items.0.amount')` | `FieldKernel.list({ item: FieldKernel.node({ ... }) })` 中直接访问 `row.amount` |
...
Fields / Rules 只面对 FormView 字段名；字段名映射与深度业务校验由 Schema/transformer 在边界统一处理。

---

## 6. 与 Module / Logic 的集成示意（概念）

Form 场景的 Fields 仍然是 Module 图纸 fields 槽位的一部分：

```ts
import { Module, FieldKernel } from "@logixjs/core"
import { Schema } from "effect"
...
const OrderTraits = FieldKernel.from(OrderFormStateSchema)({
  items: FieldKernel.list({
    item: FieldKernel.node({ /* computed/source/link/check */ }),
    list: FieldKernel.node({ /* computed/source/link/check */ }),
  }),
})
...
- Module/Runtime 只有一条主线（FieldKernel → Txn/EffectOp/Resource）；
- Form 相关能力只是 fields 中的一类领域用法；
- 可选的 `Form.fields / Form.Rule` 只是提升可读性，输出仍是等价的 kernel 结构。
...
>
> 说明：以下代码按本 spec 的规划给出“真实项目会怎么写”的代码形状：**动作（Action）是 UI→Runtime 的唯一入口**，FieldLifecycle 负责把动作桥接为 `state.ui` 与 scoped validate（写回 `state.errors`）。

```ts
import { Module, FieldKernel, FieldLifecycle } from "@logixjs/core"
import * as Form from "@logixjs/form"
import { Schema } from "effect"
...
const Fields = FieldKernel.from(StateSchema)({
  "profile.email": FieldKernel.node({
    check: {
      required: (email) => (email ? undefined : "必填"),
      format: (email) => (/^\\S+@\\S+$/.test(email) ? undefined : "邮箱格式不正确"),
...
export const FormModule = Module.make("FormModule", {
  state: StateSchema,
  actions: {
    // 领域动作：FieldLifecycle.install 监听并桥接为 state.ui + scoped validate（写回 state.errors）
...
// 生成 Program 时安装 Field 生命周期桥接（Form 只是领域包：可选择性 re-export）
const TraitBridgeLogic = FieldLifecycle.install(FormModule, {
  uiPath: "ui",
  errorsPath: "errors",
})

export const FormProgram = Logix.Program.make(FormModule, {
  initial: {
    profile: { email: "" },
    errors: { profile: {} },
...
- 规则写法仍然只依赖 fields 的 `check`（写 `state.errors`），不在 UI 层手动 setError/clearError。
