...
> **Field（内核）→（可选）FieldKernel（支点）→ 领域包（Form/Query/…）→ UI 适配层**，用于压力测试 004 的假设，避免“为了 Form 假设一个不存在的世界”。

---

## 1. 三种产物形态（先定边界）

### 1.1 `@logixjs/core`（Field kernel）
...
在 004 的语境里：`FieldKernel.*` 与 `FieldLifecycle.*` 是 kernel 归属。
...
如果必须稳定 identity，说明：
- 仍然可以复用 `FieldLifecycle` 的“桥接模式”，但 Ref 的形状可能需要扩展（例如允许 `id` 锚点）；
...
> 结论先行：**Form 不是“超级类库”**。我们要把“通用桥接能力”下沉到 FieldLifecycle，把“领域语义”留在各自领域包里。
...
- **Form 体系（`@logixjs/form`）只收口“可提交草稿 + 字段错误 + touched/dirty”这一类强语义能力**
  其余领域应该复用 FieldLifecycle/FieldKernel 的“同源形状”，但用自己的领域包话术。
