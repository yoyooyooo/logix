...
> **Field（内核）→（可选）FieldKernel（支点）→ 领域包（Form/Query）→ UI 适配层**。
>
...
- Logix Field/Runtime 不重复实现缓存，但仍负责：
...
- 一套新的运行时（仍然回落到 FieldKernel 的 `source + computed/check`）；
- 一套与 Resource 体系并行的“自建缓存/自建请求层”。

一句话：`@logixjs/query` 是 **Resource/FieldKernel 的领域糖 + 最佳实践协议**，与 `@logixjs/form` 平行。
...
- `FieldKernel`：`computed/source/link/check` + `node/list/$root`
- `FieldLifecycle`：通用桥接 helper（install/ref/scoped execute/cleanup）
...
> - `FieldLifecycle`：把 UI 事件桥接为触发/刷新/取消/清理

### 2.2 Query 领域包（归属 `@logixjs/query`）

建议的最小表面积（与 Form 对齐但不共享语义）：

- `Query.make(...)`：产出 QueryBlueprint（组合 Module + 默认 logics + fields/bridge wiring），作为业务侧默认入口（参见 `references/07-query-business-api.md`）
- `Query.fields(schema)(spec)`：可选语法糖，输出可 spread 的 `FieldKernelSpec` 片段
- `Query.Ref`：Ref helper（可选择性 re-export `FieldLifecycle.Ref`，但语义属于 FieldLifecycle）
...
- （可选）`Query.install = FieldLifecycle.install`：为 DX 做 re-export，但所有权仍在 kernel
...
- Query 的 IR 与 Form 共享 kernel 心智，未来扩展其它领域 field 也有模板。
...
- **FR-Q06 · 失效/刷新（invalidate/refetch）**
  ...
  注意：这不要求新增 kernel kind；可通过 FieldLifecycle + ResourceRegistry + EffectOp middleware 实现。
...
- 具体示例代码：`specs/004-field-bridge-form/references/04-query-quickstart.md`

---

## 6. Data Model（见外链）

- Query 的核心实体（QueryKey/QuerySnapshot/InvalidateRequest）：`specs/004-field-bridge-form/references/03-query-data-model.md`

---

## 7. TanStack Query 集成契约（见外链）

- `@logixjs/query` 与 TanStack Query 的职责边界与映射：`specs/004-field-bridge-form/references/05-query-tanstack-integration.md`
