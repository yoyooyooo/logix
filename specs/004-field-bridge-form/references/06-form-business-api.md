...
> 本文是 `specs/004-field-bridge-form/spec.md` 的辅助材料：把 “业务侧怎么用 Form” 的 API 形态定下来，同时保证所有语义都能回落到 FieldLifecycle + FieldKernel IR（单一事实源），不引入第二套运行时/第二套事实源。
...
- 业务开发默认只需要面对 `@logixjs/form` 的 **高层入口**，而不是每次手写 `Logix.Module.make + actions + reducers + FieldKernel.install wiring`。
- Form 的 **values/errors/ui** 都落在 Module state 上，满足全双工可回放：不允许在 React 组件本地维护第二套错误/交互态事实源。
- 允许可控的 escape hatch：可以局部插入 raw `FieldKernel.node/list` 与自定义 Logic，但不破坏主线。
...
- **Layer 0 · Kernel（`@logixjs/core`）**
  Module/Runtime/Flow/Bound + FieldKernel IR（computed/source/link/check）+ Resource/EffectOp/Middleware。

- **Layer 1 · Module（`@logixjs/form`）**
  业务 API：`Form.make`（Blueprint）、`Form.Controller`、`Form.Rule/Form.Error/Form.Field`（领域糖与 helper；默认入口是 `rules/fieldArrays/derived`）、默认 logics（可选）；`Form.fields/Form.node/Form.list` 仅作为最小 escape hatch（示例不推荐）。
...
1) **Blueprint（构造期）**：一次性把模块图纸（state/actions/fields）+ 默认 logics + bridge wiring 组合出来；
...
export interface FormBlueprint<TValues> {
  ...
  /**
   * 默认 Program（桥接“模块定义 → 可 imports 的装配蓝图”）：
   * - 等价于 `Logix.Program.make(form.module, { initial: form.initial(), logics: form.logics })`
   * - 业务侧可直接用于 Root imports，避免每次手写 Program.make。
   */
  readonly program: Logix.Program<any, any, any>
  ...
  // 纯数据：由 `rules/fieldArrays/derived` 编译得到的 field 声明（归一化为等价 FieldKernelSpec；允许业务 mix-in 少量 raw FieldKernel 片段作为 escape hatch）
...
export interface FormMakeConfig<TValues> {
  ...
  // escape hatch：业务注入少量 raw FieldKernelSpec 片段（不建议作为默认写法）
  readonly fields?: unknown
}
...
- Blueprint 必须保证：最终落到 Module 的 `fields` 槽位仍然是 **等价的 FieldKernelSpec**（或可归一化为它的结构）。
...
- “局部校验 / scoped validate” 需要能对接 FieldLifecycle（见主 spec 的 FR-021）。
...
- 所有校验最终回落为 `FieldKernel.check`（写 `state.errors` 同构错误树）。
- 异步资源约束最终回落为 `FieldKernel.source`（写回 `ResourceSnapshot`），并遵守 keyHash 门控与并发策略。
...
- `form.program`：`Logix.Program`（`Logix.Program.make(form.module, ...)` 的结果，可被 Root `capabilities.imports` 引入）

### 5.1 Blueprint 本质上就是 `Logix.Module`

`Form.make` 的职责是把「业务表单需要的模块图纸」一次性组装好：

- 内部等价于 `Logix.Module.make(id, { state, actions, reducers?, fields })`，并把 fields（由 rules/fieldArrays/derived 编译 + 可选 raw FieldKernel mix-in）归一化为等价的 `FieldKernelSpec`。
- 只要 `fields` 槽位存在，`Logix.Module.make` 当前实现会在模块构造期 build `FieldProgram`，并在 `Program.make(...)` 时自动注入 `FieldKernel.install`（无需业务侧再写 wiring）。参见：`packages/logix-core/src/Module.ts`。
- Blueprint 的 `logics` 只负责 Form 领域协议：把 `field/change|blur|focus|array/*|form/*` 映射为 `state.ui` 与（可选）scoped validate/submit 流程；不重复承担 FieldKernel 的安装职责。

### 5.2 AppRuntime（全局模块）接入

把 Blueprint 变成可被根模块 imports 的 `Program`：

```ts
const RootProgram = Logix.Program.make(Root, {
  initial: { /* ... */ },
  capabilities: {
    imports: [form.program],
  },
})
```
...
- Form Program 的 `logics` 由 Blueprint 提供默认值；业务可以追加/替换其中某些逻辑，但不应绕开 “领域事件 → state” 的主线。
- Root Program 仍然是唯一的 `Runtime.make(root, ...)` 入口；Form 只是被 imports 的普通模块，不单独拥有 Runtime。
...
非 React 代码可以直接使用 Blueprint 的 module 或 Program：

- 构造 Layer：`form.module.live(form.initial(), ...form.logics)`
- 或构造 Program：`Logix.Program.make(form.module, { initial: form.initial(), logics: form.logics })`
