# Feature Specification: Field 生命周期桥接 × Form（以 FieldKernel 为支点：数组/校验/异步，RHF≥）

**Feature Branch**: `004-field-bridge-form`
**Created**: 2025-12-11
**Status**: Draft
...
- 以 **Field** 作为内核抽象，明确 Field 的生命周期与桥接契约；
- 以 **FieldKernel** 作为“第一个支点 Field”（最先跑通：数组、错误树、异步资源、可回放事务）；
- 在此之上建立一套 **Form 领域系统**（`@logixjs/form`），并保证同一条链路未来可以自然扩展到更多 `xxxTrait`：
  上层领域包只是在复用 Field 的标准化桥接能力（install / refs / validate / cleanup），而不是发明第二套运行时或第二套心智。
...
2) Field 生态需求（标准桥接模式 + 最小 API 表面积 + 可回放/可生成/可诊断）。
...
本 spec 的核心目标之一是“**把 Form 做成 Field 生态的第一个示范，而不是额外一套体系**”。为此我们把对外 API 的所有权与表面积做如下分层收口（拒绝向后兼容，优先寻求新的完美点）：

- **Field Kernel（应归属 `@logixjs/core`）**
  - `FieldKernel`（支点 Field）：`from/build/install` + kernel verbs（`computed/source/link`）与表单语义糖 `check`
  - `FieldKernel.node / FieldKernel.list / $root`：纯编译期组合子（build 阶段展开为 kernel entries）
  - `FieldLifecycle`：标准桥接 helper（`install / Ref / scoped validate / cleanup`），用于把上层领域事件（如表单 change/blur/unregister/submit）接到 Module/Logic 上执行

- **Form 领域包（归属 `@logixjs/form`，业务默认入口，方案 B：Blueprint + Controller）**
  - `Form.make(...)`：产出 FormBlueprint（组合 Module + 默认 logics + fields/bridge wiring），用于业务侧以最少样板代码创建表单模块
  - `Form.Controller.*`：把 ModuleRuntime 投影为业务可用的 Controller（field/array/submit/validate 等），并保持全双工事实源
  - `Form.fields(...)`：可选语法糖入口（输出可 spread 的 `FieldKernelSpec` 片段，可与 raw `FieldKernel.node/list` 混用）
  ...
  - （可选）re-export：`Form.install = FieldLifecycle.install`、`Form.Ref = FieldLifecycle.Ref`（仅为体验，不改变所有权）

- **UI 适配层（如 `@logixjs/react`）**
  - 负责把真实 UI 事件（onChange/onBlur/…）映射为领域 action，并调用/触发 `FieldLifecycle` 维护 `state.ui` 与 scoped validate
  - 不得引入第二套不可回放“本地错误/交互态事实源”

## References

- 领域放置决策（Form vs 独立领域包 vs 最佳实践）：`specs/004-field-bridge-form/references/01-module-placement.md`
- 链路叙事（Field/FieldKernel → Module → UI）：`specs/004-field-bridge-form/references/00-field-stack-narrative.md`
- Query 领域规划（与 Form 平行，外链）：`specs/004-field-bridge-form/references/02-query-module-plan.md`
- Query 数据模型（外链）：`specs/004-field-bridge-form/references/03-query-data-model.md`
- Query quickstart（外链）：`specs/004-field-bridge-form/references/04-query-quickstart.md`
- Query × TanStack Query 集成契约（外链）：`specs/004-field-bridge-form/references/05-query-tanstack-integration.md`
- Form 业务 API（方案 B：Blueprint + Controller）：`specs/004-field-bridge-form/references/06-form-business-api.md`
- Query 业务 API（方案 B：Blueprint + Controller）：`specs/004-field-bridge-form/references/07-query-business-api.md`

## Clarifications

### Session 2025-12-11

- Q: 数组元素的「身份」默认是按 index 还是按显式 id 绑定（影响错误归属与 Field 语义）？ → A: 默认按 index 语义理解数组元素身份（与 RHF 一致），Field 与错误模型都以 `items[index]` 为基准；如需稳定 identity，可在业务层维护 `id` 字段用于 UI key 或展示，但本 spec 的核心 IR 不依赖该字段。
...
- Q: 首版正式支持的嵌套数组深度是多少，`sections[].items[]` 是否作为一等公民纳入 scope？ → A: 首版明确支持「两层嵌套」数组（例如外层 `sections[]` 下的内层 `items[]`），规范中将其视为一等公民场景，并要求 FieldKernel.list / 错误模型 / Devtools 在该形态下工作良好；更深层级（3 层及以上）不禁止，但视为高级用法，不承诺首版 Devtools 具备完备可视化。
...
- Q: 根级规则与 Rules 命名空间如何对齐，是否需要单独的 Form.build / FormSpec 包壳？ → A: 不需要。根级跨字段/跨列表校验直接落为 `FieldKernel.check`（写入 `errors` 子树），Rules 命名空间提供 `Form.Rule.make(config)` 作为默认根级规则工厂（产出等价的 `check(state, ctx)`），Form.fields 仅是可选语法糖入口，最终都编译为 FieldKernelSpec。
- Q: Rules 工厂是否继续保留在 `Form.Rule` 命名空间，而不是提升到 Form 顶层？ → A: 是。Rules 作为**可选领域糖**保留在 `Form.Rule`，Form 顶层（如存在）只提供 `Form.fields(...)` 这一入口用于生成可 spread 的 Field 片段，避免引入新的运行时 kind 或第二套 verbs。
...
- Q: 表单语义层是否保留独立的 async 槽位 / AsyncRule？ → A: 不保留。所有异步约束与远程派生统一通过 kernel 的 `source + computed/check` 组合表达；如 Form.fields 提供便利写法，也只能映射为等价的 `source` 规则，不引入 onSuccess/onError 回调式 IR。
- Q: Field / Form 领域糖里是否允许直接返回任意 Effect 作为异步执行体？ → A: 不允许。IO/异步实现必须收敛在 ResourceSpec.load（Effect）中，由 Runtime/Middleware 统一触发与接管；Field 侧只做资源依赖声明与纯派生（computed/check）。
...
- Q: Form 场景声明的 source 是否默认自动触发？触发策略叫什么，React 的 blur/submit 这类 UI 事件如何接？ → A: 默认启用自动触发，但 kernel 不再暴露 `triggers` 枚举；统一使用 `FieldKernel.source.autoRefresh`（对齐 076：`onMount` / `onDepsChange` + 可选 `debounceMs`）。UI 事件（blur/submit）不得进入 kernel `autoRefresh` 枚举，应由 `@logixjs/react` 的表单绑定层在对应事件发生时显式调用同一 `source.refresh` 入口（或通过 action/controller 间接触发 refresh）。
...
- Q: field 触发的 resource 请求在竞态下是否允许选择并发策略（switch/exhaust）？ → A: 允许。每个 source 规则可选 `concurrency: "switch" | "exhaust"`（默认 `switch`）：`switch` 以“最新 key”为准（尽量取消旧 in-flight，且无论能否取消都必须丢弃 stale 结果）；`exhaust` 在 in-flight 期间忽略新的触发，但会记录一次 trailing refresh，当前请求结束后自动补一次“最新 key”的刷新（仍需 stale 丢弃）。

### Session 2025-12-12

- Q: 是否引入 `FieldKernel.node` 作为统一 DSL 形状，并将数组 scope 命名从 perItem/perList 收敛为 item/list？ → A: 是。`FieldKernel.node` 作为纯编译期组合子统一承载 `computed/check/source/link` 的声明形状；数组字段通过 `FieldKernel.list({ item, list })` 表达两种输入域（单行/整列），不再使用 per* 前缀，降低心智负担并为未来 *Field 同源形状铺路。
- Q: Form.fields 声明的 source 默认触发是否包含 onMount（用于编辑/回填时初始拉齐依赖资源）？ → A: 包含。默认 `autoRefresh`（未提供）等价于 `{ onMount: true, onDepsChange: true, debounceMs: 0 }`：既保证初始有值时能自动拉齐依赖资源，也保证输入变化时实时更新；若需仅显式 refresh，请设置 `autoRefresh: false`，或配合 `concurrency: "exhaust"` 与 `autoRefresh.debounceMs` 收敛触发频率。
- Q: 是否允许“仅手动触发”，以及能否与自动触发混用？ → A: 允许 `autoRefresh: false` 表示 manual-only，且它与 `autoRefresh: { ... }` 在类型上互斥（不得同时设置），避免“既自动又手动”的歧义。
...
- Q: keySelector 是否允许返回 undefined 作为通用的“禁用/无有效 key”语义（不仅 Form.fields）？ → A: 允许。kernel 的 `FieldKernel.source` 允许 `keySelector` 返回 `undefined` 表示“当前无有效 key / 禁用”，此时不得触发 IO，并将目标字段写回为 idle 快照（Form 场景为 `{ status: "idle" }`）；Form.fields 只是默认给出更甜的 autoRefresh 默认值，但 key=undefined 的禁用语义是 kernel 的通用能力。
...
- Q: Form 领域糖（`Form.fields` / `Form.Rule`）的落点是新增包还是放在 core？ → A: 新增独立包 `@logixjs/form`：只承载“表单领域糖与 helper”（Rules / fields / 迁移映射），不引入第二套运行时；最终必须编译为等价的 `@logixjs/core` FieldKernelSpec（kernel 主线仍是 FieldKernel → Program/Txn/EffectOp/Resource）。
...
- Q: `FieldKernel.list` 的 `item/list` 两个 scope 是否必须同时声明？ → A: 不必须。`item` 与 `list` 都是可选的：默认只声明 `item`（每行规则/派生/资源）；只有在需要“跨行/列表级”规则或摘要（如唯一性、单调性、最少 N 行）时才补充 `list`，避免无谓的 API 表面积。
- Q: `autoRefresh` 的命名应更偏“状态语义”还是更贴近 UI 事件？ → A: kernel 统一使用状态语义命名：`onDepsChange`（指显式 deps 变化带来的 refresh 触发），避免与 UI 层事件混淆；UI 事件由 React 适配层通过显式调用 refresh/action 来接入，不进入 kernel `autoRefresh` 枚举。
...
- Q: touched/dirty/focus/blur/unregister/滚动到首错/局部校验等“表单交互态”是否需要进入全双工可回放链路？ → A: 需要。交互态 SHOULD 存放在 Module state 的 `state.ui`（或等价命名）的专用子树中，由 `@logixjs/form` 维护其更新（通过 action/reducer/logic），并与 FieldKernel 的校验触发（validate）形成稳定配合；React 层仅负责把 DOM 事件映射为“值变化/失焦/注销”等领域事件，不在组件本地维护第二套不可回放的交互态事实源。
...
- Q: `QueryObserver` 的生命周期/作用域我们定为哪种？ → A: 每条“Query 领域的 `FieldKernel.source` 规则”在其 runtime scope 内各自维护一个 `QueryObserver`，scope 结束时统一 cleanup。
- Q: 004 是否需要在本轮明确 `@logixjs/form` 的业务 API 形态（面向业务开发的默认入口），而不仅仅是 kernel/bridge/IR？ → A: 是。SSoT 仍以 FieldLifecycle + FieldKernel IR（`node/list/source/computed/link/check`）为底，但同时选择方案 B：Blueprint + Controller（`Form.make`）；由 Form 领域包组合 Module + 默认 logics + fields/bridge wiring，React 侧只做投影与事件适配，不引入第二套事实源（详见 `references/06-form-business-api.md`）。
- Q: Query 领域包是否也采用与 Form 一致的“Blueprint + Controller”业务入口（`Query.make`），并通过分形 Module 的 imports/useLocalModule 接入？ → A: 是。`@logixjs/query` 与 `@logixjs/form` 平行：业务默认入口均为 Blueprint + Controller，Query 的 TanStack 集成与触发/并发语义仍回落到 FieldKernel/Resource 的主线约束（详见 `references/07-query-business-api.md`）。

---

## User Scenarios & Testing *(mandatory)*

> 本特性站在「Field 生态」视角，目标是固化一条可复用的链路：
> **Field（内核）→（可选）FieldKernel（支点）→ Form（领域包）**，并让这条链路未来可以自然扩展到更多 `xxxTrait`。
>
> 说明：
> - `FieldKernel` 是本 spec 选定的“第一个支点 Field”（率先跑通数组/错误树/异步资源/事务可回放），但它不是强制中间层；某些领域也可以走 **Field → Form** 的直接链路。
> - Form 的目标不是“又一套表单运行时”，而是通过 `@logixjs/form` 提供自解释、可组合、可回放的 API 体系，并把可复用的“桥接玩法”下沉到 Field 生命周期层（install/refs/validate/cleanup）。

### User Story 1 - Field 作者沉淀可复用的桥接玩法（P1）

作为 Field/Runtime 维护者（或领域能力作者），我希望：

- Field 能提供一套标准化的“生命周期桥接”能力（如 `FieldLifecycle.install / Ref / validate / cleanup`），把上层领域事件接到 Module/Logic 上执行；
- 这套桥接能力不与 Form 绑定：Form 只是第一个领域包示范；未来的 `QueryTrait` / `PermissionTrait` / `WorkflowTrait` 也能复用同一套形状；
- Devtools 能以同一套心智观测：上层事件 → state.ui/state/errors 变更 → field 规则执行（可回放、可追踪、可解释）。
...
- 让两位工程师只看文档就能写出它们的“安装点”（Program logics）与“调用点”（dispatch 领域 action）的代码草图。

**Acceptance Scenarios**

1. **Given** 一个新的领域 Field（非 Form），**When** 它选择复用该桥接玩法，**Then** 它能在不发明第二套 Runtime 的情况下，把领域事件接入 Module/Logic，并保持可回放与可诊断。
2. **Given** Form 与另一个领域 Field 同时存在，**When** 工程师阅读其安装/调用方式，**Then** 能在 10 分钟内理解它们共享的桥接结构（install/ref/validate/cleanup），而不需要重新学习一套概念。
...
### User Story 3 - 模块作者在复杂表单中声明数组级 Fields（P1）

作为 Logix 模块作者，我希望在包含数组字段（如订单行、动态联系人列表）的复杂表单中，能够像 RHF 一样自然地：

- 声明针对数组元素的计算字段（如每行小计）、联动逻辑（如勾选某行自动影响汇总），以及校验逻辑（行级错误）；
- 使用统一的「字段路径 / DSL」来表达这些 Field，而不是为数组单独写一套 ad-hoc 逻辑；
- 同时保留良好的类型安全与可读性，至少不弱于我在 RHF 中使用 `Path<TFieldValues>` / `FieldArrayPath<T>` 的体验。

**Why this priority**
数组 + 联动是 ToB 表单的高频难点场景，如果 FieldKernel 在这里「短板明显于 RHF」，会直接影响 Logix 被前端团队采纳的可能性。因此需要优先在概念层和规范层把能力和边界定义清楚。
...
- 给出一个典型「订单 + 收货信息」表单的 StateSchema + TraitSpec 示例，包含：
  ...
  - 至少一个数组级计算 Field（如总金额）；
  - 至少一个数组级联动 Field（如选中行控制汇总）；
  - 至少一个行级错误 Field（如某一行字段不合法）。
...
2. **Given** 一份基于本 spec 编写的 TraitSpec，**When** 不同模块作者阅读该 TraitSpec 与配套文档，**Then** 可在 10 分钟内准确说出每个 Field 针对的是哪个数组字段、哪个元素层级、与哪些字段存在依赖关系。
...
- 不需要为 Logix 单独维护一套错误对象形状，而是将 Field 定义的错误模型与 UI 库（如 RHF、内部 Form 组件）进行自然对接；
...
### User Story 5 - 架构师从 SSoT 理解 Field 链路与边界（P2）

作为负责 Logix 平台规划的架构师/规范维护者，我希望：

- 在 docs/specs 中，有一份清晰的章节描述 “Field →（可选）FieldKernel → Form” 这条链路的角色与边界；
- 这份规范既能与 RHF 的字段路径/错误模型做对照，也能作为未来 `xxxTrait` 的模板（桥接玩法、错误/交互态事实源、可回放策略）；
- 当后续实现/演进 Field 生命周期桥接、FieldKernel 数组支持、Form 集成与 Devtools 视图时，都可以直接引用本 spec 作为唯一事实源，而不是每次重新解释。

**Why this priority**
Field 链路一旦落实现，会牵动 StateSchema、TraitSpec、Runtime/Devtools、表单库适配、未来其他领域 Field 等多个层面。如果没有清晰的 SSoT，后续实现容易「各说各话」，导致难以维护。
...
1. **Given** 本 spec 与现有的 FieldKernel SSoT 文档，**When** 新工程师在不看代码的情况下阅读这些文档，**Then** 能够区分：
   - 「Field 生命周期桥接」与「领域包语法糖」的差别；
   - `FieldKernel` 作为“支点 Field”的角色，以及何时可以直接走 Field→领域包；
   - 「Field 表达能力 ≥ RHF」意味着什么（能力上限 vs 路径语法兼容）。
...
- 当数组为空时（例如 `items.length = 0`），数组级 Field（汇总、错误）如何定义「0 行」的语义（如总金额=0、是否视为错误等）？
- 当数组中存在大量元素（例如 100+ 行）时：
  - Field 对每一行进行计算/校验是否有「节流 / 采样」语义约束？
  - 增删/重排时 Field 执行次数和事务边界如何控制？
- 对于嵌套数组（如 `sections[].items[]`），本 spec 明确将「两层嵌套」（外层 section 列表 + 内层 items 列表）纳入首版正式 scope：
  - 需要在概念层定义「两层数组元素级 Field」的行为语义，并确保 Field DSL（FieldKernel.list）/ 错误模型可以自然覆盖该结构；
  - 要求 FieldKernel.list 与 Devtools 至少能覆盖「外层 section + 内层 items」这类典型两层嵌套结构；更深层级（3 层及以上）可以在实现上支持，但视为高级用法，不作为本轮 Devtools 视图与文档的强制要求。
...
- **FR-001 · 数组字段路径模型**
  规范 MUST 定义「数组字段 / FieldArray」在 State 模型中的路径语义，至少能够表达「数组元素级」的 Field 作用范围（例如通过显式数组段或等价 DSL），并给出与 RHF 字段路径（如 `items.0.name` / `items[index].name`）的概念映射关系。

- **FR-002 · 数组元素级 Field 能力**
  FieldKernel 模型 MUST 支持在数组元素层级挂载以下类别的 Field：
...
- **FR-004 · 数组操作语义（增删 / 重排）**
  规范 MUST 明确数组增删、重排时 Field 与错误模型的语义，包括但不限于：
  - 行被删除后，该行相关 Field / 错误的清理策略；
...
- **FR-005 · Devtools / SSoT 可视化视图**
  规范 MUST 要求 Devtools / runtime SSoT 文档中，以清晰的方式呈现数组字段上的 Field 结构与行为：
  - TraitGraph 能区分「列表字段节点」与其 Fields；
  - StateTransaction / Timeline 能在高层概览中反映「一次操作对哪些行产生了 Field 更新」；
...
- **FR-006 · RHF 对齐边界定义（能力 ≥，路径不强制兼容）**
  ...
  - 可以在字段路径语法与错误对象结构上与 RHF 存在差异（例如通过 kernel DSL（FieldKernel.list）/ Helper 表达数组元素级行为，而不直接使用 `'items.0.name'` 这类路径作为规范的一部分）；
  - 必须提供一份面向实际项目的「迁移指南」，说明如何将典型 RHF 表单（含 FieldArray）迁移到 FieldKernel 模型（含 list/check），包括字段路径、错误结构、增删/重排语义的对照表；
  - 若未来提供 RHF 适配层，可在适配层处理 path/错误结构的兼容转换，但本 spec 不将「字符串路径兼容 RHF」视为核心契约。

- **FR-007 · FieldKernel.source 与 Resource/Query 复用约束（表单异步约束）**
  规范 MUST 要求所有「字段/表单语义」层面的异步行为（例如根据供应商编码拉取供应商信息、根据远程配置约束下拉选项）统一通过 **Resource/Query + kernel source** 表达：
  - 异步依赖只能以 `FieldKernel.source`（或 list.item/list.source 的等价 kernel 形式）声明「哪个字段依赖哪个逻辑资源、key 如何从 state/row/rows 推导」；
  - 资源结果如何影响表单视图（值/错误/meta/options）必须通过 `computed/check` 派生完成，保持单向数据流与事务可回放；
  - Field 侧 **不得直接返回或嵌入任意 Effect/IO**。所有真实 IO 必须收敛在统一的 Resource 事实源（`ResourceSpec.load: Effect`）中，由 Runtime/Middleware 触发与接管；
  - 资源本身的定义（包括访问方式、错误模型等）必须在 Resource 事实源中集中管理并以 Layer 注入 Runtime，而不是在表单 Field 层定义「影子资源」。
...
- **FR-008 · Resource 组织与作用域的最佳实践指引**
  ...
  - 推荐在应用级或特性级 Layer 中组合 ResourceLayer，并在 Runtime.make 的 layer 中统一注入；允许在 Program capabilities.imports 中挂载局部 ResourceLayer，但必须避免在同一 Runtime 作用域内用不同实现重复注册同一 resourceId；
...
- **FR-009 · Rules 语法糖与内置校验库（对标 RHF rules，基于 check）**
  ...
  - Rules 输出始终是 IR 层统一的 `check` 函数（本质为写 errors 子树的 computed），FieldKernel / Runtime / Devtools 只感知 `check`，不直接依赖具体 Rule 配置结构。
...
- **FR-010 · effect/Schema 集成与字段名映射（Form ↔ 后端模型边界）**
  ...
  - 无论是否使用 Schema，Form 层最终暴露给 FieldKernel / Runtime 的仍然是统一的 `check` 函数与错误树结构（本质为写 errors 子树的 computed），Schema/transformer 只在 Form ↔ 后端模型边界与深度校验路径出现。

- **FR-011 · 根级 check 槽位（跨字段 / 跨列表规则）**
  为了承载跨字段、跨列表甚至跨 Section 的纯校验逻辑，本规范要求支持可选的「根级 check」：
  - 根级 check 直接面对**整份 Module/State**，签名为 `(state: Readonly<S>, ctx: FormCtx<S>) => GlobalErrorTree<S>`，并以 `FieldKernel.check` 的形式写入 `errors` 子树（具体 error path 由实现层约定，例如 `errors.$form`）；
...
- **FR-015 · Form 场景 source 的自动触发（autoRefresh）**
  为了让“输入变化 → 资源约束变化”的表单链路足够甜且可演进，规范 MUST 满足：
  - kernel `FieldKernel.source` 的自动触发语义统一由 `autoRefresh` policy 表达（对齐 `076-logix-source-auto-trigger-kernel`），替代旧的 `triggers/debounceMs` 反射式解释口径；
  - 默认 `autoRefresh`（未提供）等价于 `{ onMount: true, onDepsChange: true, debounceMs: 0 }`：
    - `onMount` 用于“编辑/回填”场景：初始 state 已有有效 key 时，自动触发一次 refresh；
    - `onDepsChange` 用于“输入变化/依赖变化”场景：当显式 deps 推导的 key 发生变化时触发 refresh（实现层可做 keyHash 去重与 stale 丢弃）；
  - `autoRefresh: false` 表示“仅显式 refresh”（manual-only）；
  - 可选配置 `autoRefresh.debounceMs`：用于合并高频的 depsChange 触发（onMount 通常不需要 debounce）；
  - keySelector 允许返回 undefined 表示“当前无有效 key / 禁用”：此时不得触发 IO，并将 snapshot 置为 `{ status: "idle" }`（该禁用语义是 kernel 的通用能力，不仅限于 Form.fields）；
  - `autoRefresh` 的命名以语义为准：`onDepsChange` 描述“显式依赖变化”而不是 UI 事件；UI 事件映射由 React 适配层负责，且不得被编码为 kernel `autoRefresh` 枚举。
...
- **FR-016 · source 的写回形态（ResourceSnapshot）**
  ...
  - Devtools MUST 能将 source 的刷新过程体现在 Timeline（进入 loading、成功/失败落盘）与 FieldGraph（字段依赖）中。
...
- **FR-019 · ResourceRef（resource id 的领域常量 + 蓝图元信息，不影响语义）**
  为了让 Module/Fields 在保持“只依赖 resourceId + key”边界清晰的同时，仍能具备更好的可读性、可诊断性与可生成性，本规范要求：
  ...
  - 用户文档 SHOULD 给出推荐工程组织：Fields/Module 图纸层优先依赖 `resources/*.id.ts`（只导出 ResourceRef 常量），Runtime/实现层在 `resources/*.spec.ts` 中用 `Resource.make({ id: ResourceRef.id, ... })` 定义并通过 `Resource.layer([...])` 注入；以降低 bundle/循环依赖风险。
...
- **FR-021 · FieldLifecycle：Field 与上层领域的标准桥接 API（FieldRef + scoped validate + cleanup）**
  为了让上层（`@logixjs/form` / `@logixjs/react` / 未来更多 `xxxTrait`）能以最小 glue 实现 RHF 级体验，Field kernel MUST 提供可组合的桥接能力（本 spec 暂称 `FieldLifecycle`）：
...
**1. FieldKernel.node：统一 DSL 形状（纯编译期组合子）**

为了让「标量字段 / 数组字段 / 根级规则」共享同一套声明形状，本 spec 引入 `FieldKernel.node(...)`：
...
```text
FieldKernel.from(StateSchema)({
  "profile.email": FieldKernel.node({
    check: {
...
**3. 数组字段：FieldKernel.list + item/list 两个 scope**

对任意数组字段 `path = "items"`，通过 `FieldKernel.list({ item, list })` 一等公民化数组语义：

```text
items: FieldKernel.list({
  item: FieldKernel.node({
    computed: { /* 行级派生：按字段产值 */ },
...
为了承载跨字段/跨列表的整表不变量，允许在 fields spec 中声明一个保留键 `$root`：

```text
$root: FieldKernel.node({
...
- 代码生成层：可以机械地将 node/list 结构映射为 `FieldKernel.node` / `FieldKernel.list({ item, list })`（或可选的 Form.fields 领域糖）调用，保持 IR 等价；
...
- **数组元素 Field（Array Element Field）**
  - 作用于某个数组元素及其子字段的 Field 抽象；
  - 可用于描述 per-item 的计算、联动与校验行为；
  - 与当前 FieldKernel 的 computed/source/link 语义保持兼容。
...
- **字段定位（FieldRef / ListRef）**
  - 用于唯一定位一个“字段实例”（而不仅是字段路径字符串），是 `@logixjs/form` / `@logixjs/react` 与 Runtime/FieldKernel 的桥接货币；
...
- **SC-001 · 覆盖典型 RHF 场景**
  给出至少 3 个来自 RHF 文档或内部实践的典型 FieldArray 场景（如「动态教育经历列表」「订单行 + 折扣」「多级嵌套地址表单」），经小组评审确认，本 spec 中定义的 FieldKernel 数组模型在概念上可以一一覆盖这些场景，无需引入额外的 ad-hoc 状态。评审结果需记录在 spec 旁的 notes 中。
...
- **SC-003 · 迁移指导有效性**
  选取一个现有 RHF 表单（含 FieldArray）作为样本，用本 spec 提供的模型和指南重写一版「Logix + FieldKernel」设计文档：
...
- **SC-004 · 设计一致性约束**
  ...
  - 所有新引入的数组 Field 场景要么明确引用本 spec 概念，要么在本 spec notes 中登记「计划中的偏离/扩展」。
