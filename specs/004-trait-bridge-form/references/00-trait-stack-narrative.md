...
- `packages/logix-core/src/Runtime.ts`：`Runtime.make(root, ...)` 的 AppRuntime 入口（`root` 可为 program module 或其 `.impl`）
...
- `packages/logix-core/src/StateTrait.ts`：对外 StateTrait 命名空间（当前仍是 Phase 占位形态）
...
- React 集成：`useLocalModule(form, ...)` / `useModule(form)` + `useSelector/useDispatch` 做投影
...
2) StateTrait：`packages/logix-core/src/StateTrait.ts` + `packages/logix-core/src/internal/state-trait/*`  
