...
### 3.1 QueryClient 的注入（Layer/Provider）

`@logix/query` SHOULD 提供：

- `QueryClientTag`（Effect Context.Tag）
- `QueryClientLayer`（在某个 runtime 作用域内提供 QueryClient）

语义：

- 同一 runtime 作用域内一个 QueryClient（与 ResourceRegistry 的 scope 对齐）
- 跨 runtime 允许不同 QueryClient（缓存隔离）
