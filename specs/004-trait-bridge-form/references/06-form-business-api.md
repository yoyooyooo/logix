...
- **Layer 1 · Domain（`@logix/form`）**  
  业务 API：`Form.make`（Blueprint）、`Form.Controller`、`Form.traits/Rule/Error`（领域糖与 helper）、默认 logics（可选）。
...
export interface FormBlueprint<TValues> {
  ...
  // 纯数据：用于 Traits 的声明（允许业务 mix-in 自己的 raw StateTrait.node/list）
...
export interface FormMakeConfig<TValues> {
  ...
  // 领域层配置：校验策略、触发策略、以及（可选）领域默认 Traits
  readonly mode?: "onChange" | "onBlur" | "onSubmit" | "all"
  readonly debounceMs?: number

  // 可选：业务注入的 trait 片段（Form.traits 或 raw StateTrait.node/list 的 mix-in）
  readonly traits?: unknown
}
...
- `form.module`：`Logix.ModuleInstance`（模块图纸/Tag，本质是 `Module.make(...)` 的返回值）
...
- 内部等价于 `Logix.Module.make(id, { state, actions, reducers?, traits })`，并把 traits（领域糖 + raw StateTrait mix-in）归一化为等价的 `StateTraitSpec`。
...
- RootImpl 仍然是唯一的 `Runtime.make(rootImpl, ...)` 入口；Form 只是被 imports 的普通模块，不单独拥有 Runtime。
