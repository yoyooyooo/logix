...
- [ ] T001 更新 SSoT：在 `docs/specs/runtime-logix/core/09-debugging.md` 固化 005 的协议/导出导入/Worker-first/Recording Window 语义
- [ ] T002 建立 core 公共入口：新增 `packages/logix-core/src/observability.ts` 并在 `packages/logix-core/src/index.ts` 导出为 `Logix.Observability`
...
- [ ] T007 [P] 实现 payload 降级与摘要工具：`packages/logix-core/src/internal/observability/payload.ts`
- [ ] T008 [P] 实现 ObservationEnvelope codec（validate/parse/compat）：`packages/logix-core/src/internal/observability/envelope.ts`
- [ ] T009 [P] 实现 EvidencePackage codec（export/import）：`packages/logix-core/src/internal/observability/evidence.ts`
...
**Checkpoint**: 组件形态导出证据包 → 插件形态导入后核心计数/顺序一致（SC-002）
...
## Phase 5: User Story 3 - Chrome 扩展实时连接 + 更低干扰（Priority: P1）

**Goal**: 扩展面板连接到当前页面会话并实时展示关键视图，同时将高成本处理移出被测页面主线程（SC-001/US3）
...
3. US1（MVP）：导出/导入 + 组件/插件离线一致性
4. US4：Recorder/Brush/TimeTravel + Worker-first 达标路径
5. US3：扩展实时连接 + 低干扰（复用 Worker-first）
