# Implementation Plan: 006-optimize-fields（Field 事务内收敛“极致优化”）

**Branch**: `006-optimize-fields` | **Date**: 2025-12-13 | **Spec**: `specs/006-optimize-fields/spec.md`
**Input**: `specs/006-optimize-fields/spec.md`

## Summary

本特性的目标不是“把单条 Field 算快一点”，而是把 Field 的派生更新**收敛到同一个 Operation Window（= 一次 action 派发）**里完成，从而把性能上限绑定到事务体系上，并兑现以下硬承诺：
...
- Intent → Flow/Logix → Code → Runtime 映射：
  - Intent/规范侧：本特性以 `specs/006-optimize-fields/spec.md` 为验收口径，明确 Operation Window 与 0/1 commit；
  - Flow/Logix 侧：业务仍用 `fields` 槽位声明规则；
  - Runtime 侧：在 `ModuleRuntime.dispatch` 的 StateTransaction 内完成派生收敛与一次性提交；Devtools 以 txnId 作为窗口锚点观测。
- 需要同步更新的事实源（docs-first）：
  - `docs/ssot/runtime/logix-core/runtime/05-runtime-implementation.md`（Field/事务收敛语义、失败语义、预算/降级）；
...
```text
packages/logix-core/
├── src/internal/state-field/         # Field Program/Graph/Plan 与运行期执行（本特性主落点）
...
examples/logix-react/                 # 真实复杂表单基准（ComplexFieldForm…）+ 合成压力基准
...
1. **基准场景落点确认**
   - 真实复杂表单基准：以 `examples/logix-react/src/demos/field-txn-devtools-demo.tsx`（ComplexFieldForm…）为基准入口，固化可重复的输入序列与采集口径。
...
3. **把“真实复杂表单基准”固化为自动化回归用例**
   - 新增一个 headless integration/regression test：`examples/logix-react/test/complex-field-form.baseline.integration.test.ts`。
   - 测试体用 `@logixjs/test` 跑 `ComplexFieldFormModule + ComplexFieldFormImpl`，执行一段固定的 action 序列（模拟输入/增删行/修改价格等），并断言两类结果：
...
### Phase 1（Core）：事务内 Field 收敛引擎（依赖图 + 脏集传播 + 固定点）

1. **规则编译与静态校验**（安装期一次性完成）
   - 从 `FieldProgram` 编译出运行期 Rule IR（仅包含会写回的派生：computed/link；source 不进入派生收敛循环）。
   - 构建索引：
...
2. **移除 watcher 驱动的派生写回**（`FieldKernel.install`）
   - computed/link 不再注册 `onState` watcher（避免 commit 后再开新事务）。
   - source 仍保留显式入口 `$.fields.source.refresh(fieldPath)`（它是独立 Operation Window），并与本特性保持一致的诊断/patch 语义。
...
3. **Devtools 对齐**
   ...
   - Inspector 增加“本窗口 Field 收敛摘要”展示（与 spec 中 SC-007 对齐）。
