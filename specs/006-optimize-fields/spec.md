# Feature Specification: Field 运行时性能上限提升（极致优化）

**Feature Branch**: `006-optimize-fields`
**Created**: 2025-12-13
**Status**: Draft
**Input**: User description: "把能想到的优化做到极致，field 决定了以蓝图意图为核心的体系的性能上限"

## Assumptions

- “Field”指：由基础状态字段派生/联动出校验、汇总、默认值、联动字段等的声明式规则集合；Field 是蓝图意图落地到运行时的核心机制之一。
- 本特性以“高频用户输入 + 多字段联动/校验/汇总”的典型 ToB 表单场景为主基准，不以一次性覆盖所有场景为目标。
- 性能优化必须以语义正确为前提：在相同输入与相同 Field 定义下，最终对外可观察的状态结果一致且可解释。
- 允许为追求性能与语义一致性进行大规模重构；不要求向后兼容历史实现细节。

## Out of Scope

- 不新增新的业务能力（例如新增一种 Field 类型或全新 DSL）；本特性聚焦于让既有 Field 能力“更快、更稳、更可观测”。
- 不承诺把任何长耗时/外部 IO 的业务流程塞进 Field；Field 仍以短窗口、可收敛的派生为主。
- 不以“某个具体 UI 框架/渲染器的优化”为目标；只关注运行时 field/事务语义与其可观测结果。
...
- Q: 多条 Field 同时写回同一目标字段如何处理？ → A: 视为配置错误：阻止本次收敛/提交，并输出明确诊断。
...
- 输入不丢字符、页面保持可用；
- 由 Field 驱动的联动字段、校验提示与汇总信息在可接受时间内稳定更新；
- 同一段交互的表现可重复（不会时快时慢、或出现偶发“抖动”）。

**Why this priority**: 这是“蓝图意图可用于真实业务”的底线体验；如果高频输入场景不稳，Field 的上层体系无从谈起。
...
- 所有由该操作引起的 Field 派生、联动与校验更新能够在同一操作窗口内收敛；
...
1. **Given** 一个启用了多条 Field 的模块实例，**When** 用户触发一次字段修改操作，**Then** 所有关联的联动/校验/汇总结果在一次可观察的最终状态提交后达到一致且正确的最终状态。
...
作为业务开发者与运行时维护者，我希望 Field 的派生触发具备“最小必要性”与“可解释性”：

- 只有当依赖字段发生变化时，相关 Field 才会执行；
...
**Independent Test**: 在真实复杂表单基准场景中只修改一个与部分 Field 无关的字段，验证无关 Field 不会被触发；再构造一个循环依赖/不收敛示例，验证系统能给出明确提示且不会导致卡死。

**Acceptance Scenarios**:

1. **Given** 一个包含多条 Field 的场景，且其中一部分 Field 与字段 X 无依赖关系，**When** 用户只修改字段 X，**Then** 与 X 无关的 Field 不会执行，且不会产生额外的状态提交噪音。
2. **Given** 一个存在循环依赖或无法收敛的 Field 组合，**When** 触发相关输入，**Then** 系统不会卡死或无限更新，并提供明确提示说明问题与影响范围。
3. **Given** 一个存在多条 Field 同时写回同一目标字段的场景，**When** 触发相关输入，**Then** 系统阻止本次操作窗口产生新的状态提交，并给出明确诊断说明冲突目标字段与涉及规则范围。

---

### Edge Cases

- Field 存在循环依赖、振荡或无法收敛：视为配置错误，阻止本次收敛/提交，并输出明确诊断，且不得造成持续卡顿。
- 多条 Field 同时写回同一目标字段：视为配置错误，阻止本次收敛/提交，并输出明确诊断（至少包含目标字段与涉及规则范围）。
...
- **FR-001**: 系统必须将 Field 视为“可声明依赖”的派生规则，并基于依赖关系最小化触发范围：只有依赖发生变化的 Field 才允许执行。
- **FR-002**: 系统必须避免“无意义写回”：当某个 Field 的派生结果与目标字段现值等价（以对外可观察的值相同为准）时，系统不得产生新的对外可观察状态提交。
...
- **FR-014**: 系统必须检测同一 Operation Window 内多条 Field 试图写回同一目标字段的冲突，并视为配置错误：本次 Operation Window 不得产生新的对外可观察状态提交，同时必须输出明确诊断信息（至少包含冲突目标字段与涉及规则集合的可读标识）。
...
- **Field Rule**: 一条派生/联动/校验/汇总规则，包含目标字段与其依赖字段集合。
- **Dependency Graph**: Field 规则间的依赖关系图，用于确定最小触发范围与传播顺序。
- **Operation Window**: 一次 action 派发对应的“收敛窗口”，用于界定本次操作的派生与最终提交边界。
- **Transaction (Observable Commit)**: 对外可观察的一次最终状态提交，订阅者以此为单位接收状态更新。
- **Base Fields**: 在一次 Operation Window 中由用户输入或业务动作直接写入的字段，不由 Field 规则维护。
- **Derived Fields**: 由 Field 规则维护/写回的字段，用于联动、校验、汇总等派生信息。
...
- **SC-004**: 在至少一个“只改动局部字段”的场景中，与该字段无依赖关系的 Field 触发次数 = 0（以诊断信号或等价可验证方式衡量）。
