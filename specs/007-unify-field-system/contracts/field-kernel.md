# Contract: FieldKernel（支点 DSL）× Kernel（Graph/Txn）

> 目标：把 FieldKernel 固化为“强表达但安全”的中间层语言：最小集合、可组合、可诊断、可完全降解到 Field 最小 IR。业务层不应直接接触 FieldKernel；它服务于领域包实现与生成器/工具链。

## 0. API 分层定位（Form > FieldKernel > Field）

- Form/Query（领域包）：业务默认入口，负责把常见组织形状（field/list/check/source）固化为可推导中间表示，并提供默认 logics。
- FieldKernel：中间层 DSL（强表达但安全），暴露最小集合但保持可组合/可诊断；是领域包“降级落点”与 codegen 的主要目标语言。
- Field（最小 IR）：内核实现细节，负责统一收敛、冲突检测、回放与诊断口径；仅供工具链/生成器/内核使用。

## 1. DSL（编译前）契约

### 1.1 基本组合子

- `FieldKernel.from(schema)(spec)`：仅用于类型收窄与路径约束；不改变运行语义。
- `FieldKernel.node({...})`：在某个 scope 下声明 `computed/source/link/check`。
- `FieldKernel.list({ item?, list? })`：数组字段一等公民，item/list 两个 scope 均可选。
...
- FieldKernelSpec MUST 支持“分片组合”（例如领域默认片段 + 业务片段 + 覆盖片段），且合并语义必须确定性。
- 合并后必须进行一致的冲突检测：
...
- `FieldKernel.build(schema, spec)` MUST 产出：
...
> 推荐落点：以上桥接协议由 `FieldLifecycle` 作为 kernel 归属统一提供与实现（见 `contracts/field-lifecycle.md`），领域包应复用而不是各自发明一套 glue。
