...
- `Query.fields({ queries })` SHOULD 存在作为可选语法糖：输出可 spread 的 `FieldKernelSpec` 片段，用于声明“参数 → 异步快照”的依赖关系；实现必须能降解为 `FieldKernel.source + computed/check`（不引入第二套运行时）。
...
- `@logixjs/query` MUST 提供便捷 Layer：`Query.Engine.layer(engine)`，语义等价于 `Layer.succeed(Query.Engine, engine)`。
  - 供应用在 root `Program.make(..., { capabilities: { services } })` 或 `Runtime.make(...)` 层统一注入。
- 作用域约束：
...
## 6. FieldLifecycle（统一下沉接口）

- Query 的默认 logics MUST 基于 FieldLifecycle 的 scoped execute（见 `contracts/field-lifecycle.md`）：
