# Data Model: 007 Field 系统统一（Kernel/Perf）×（Form/Query）×（可回放/可解释）
...
**Source Research**: `specs/007-unify-field-system/research.md`
...
```text
Field（内核：computed/source/link/check）
        │  （可选）
        ▼
FieldKernel（支点 DSL：node/list/$root）
        │  编译为
        ▼
...
- `FieldKernel.node({...})`：在某个 scope（field/item/list/root）下声明 `computed/source/link/check`。
- `FieldKernel.list({ item?, list? })`：数组字段一等公民，提供 item/list 两个可选 scope。
...
type FieldKernelSpec<S> = Record<string, unknown> // 由 build 阶段归一化成 Entry
```
...
type QuerySnapshot<Params, Out, Err> = {
  ...
  readonly lastTrigger?: { readonly kind: "onMount" | "onDepsChange" | "manual" }
}
```
...
## 10. FieldLifecycle：install / Ref / scoped validate / scoped execute / cleanup

> 作用：把“领域事件（Form/Query）”统一桥接到 Field IR 的执行语义上，避免领域各自发明 glue，保证回放/诊断口径一致。
...
### 10.5 FieldLifecycle.install（模块逻辑）

- `FieldLifecycle.install(module, opts)` 产出可通过 `Program.make(..., { logics })` 装配的 `ModuleLogic`；
