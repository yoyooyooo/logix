# Implementation Plan: 007 Field 系统统一（Form 形状 × Kernel 性能 × 可回放）

**Branch**: `007-unify-field-system` | **Date**: 2025-12-13 | **Spec**: `specs/007-unify-field-system/spec.md`
**Input**: `specs/007-unify-field-system/spec.md`

## Summary

007 的目标是把 004 的“对外形状”（Form/Query 领域入口、数组 index 语义、同构错误树、交互态全双工、Blueprint/Controller + imports 同源组合）与 006 的“内核红线”（显式依赖 + 依赖图、反向闭包 scoped validate、事务内 0/1 次提交、诊断/预算/降级）合并为一套无冲突的 Field 系统。落地策略：

- 以 `FieldKernel` 作为首个“可推导中间表示”，强制 `computed/source/check` 显式声明 `deps`，并构建可解释的 `Program + DependencyGraph(+reverseAdj) + Plan`。
- 以内核归属的 `FieldLifecycle` 统一承载 `install / Ref / scoped validate / scoped execute / cleanup`，让 Form/Query 默认逻辑同源下沉，避免各领域各造一套 glue。
...
研究结论与对外契约已固化为资产：`specs/007-unify-field-system/research.md`、`specs/007-unify-field-system/contracts/*`、`specs/007-unify-field-system/data-model.md`、`specs/007-unify-field-system/quickstart.md`。
...
- `Intent → Flow/Logix → Code → Runtime` 映射：
  - Intent/需求：以 `specs/007-unify-field-system/spec.md` 的用户故事与成功标准为验收口径（Form/Query 双对照 + 0/1 commit + 回放/诊断）。
  - Flow/Logix：以 Module/Logic/FieldKernel 作为对外可写的声明入口；领域事件通过 `FieldLifecycle` 下沉到 scoped validate/execute/cleanup。
  - Code：主要落点在 `packages/logix-core`（FieldKernel build/graph/plan、StateTransaction/dispatch、诊断/回放事件）；领域包落点在 `packages/logix-form` 与（新增）`packages/logix-query`；React 适配在 `packages/logix-react` 与领域包的 `*/react` 薄糖；Devtools 在 `packages/logix-devtools-react`。
...
- Effect/Logix 契约变更与落档：
  - `FieldKernel` 的“显式 deps + Graph + Reverse Closure + 事务内收敛”需要在 `docs/ssot/runtime` 的实现与调试章节固化后再落代码。
  - `FieldLifecycle` 作为 kernel 新公共契约，需要在 `docs/ssot/runtime/logix-core/api/02-module-and-logic-api.md` 或新增对应章节中明确其定位与与 ModuleLogic 的关系。
- 质量门（merge 前必须通过）：
...
```text
packages/logix-core/                  # Field/FieldKernel 内核、事务、诊断、回放基础能力（主落点）
...
### Phase 1：Kernel（FieldKernel → Program/Graph/Plan）与事务内收敛
...
- RowID 映射层必须先用高强度测试矩阵锁死语义与清理：覆盖 insert/delete/move/swap 等数组操作与 `FieldLifecycle.cleanup`、以及 in-flight 结果在“删除/重排后”的门控与归属正确性，避免引入隐蔽一致性 bug。

### Phase 2：FieldLifecycle（统一下沉接口）与回放/诊断口径

- 固化 `FieldLifecycle.install / Ref / scoped validate / scoped execute / cleanup` 的数据模型与事件口径（对应 `contracts/field-lifecycle.md`）。
...
- Form：以 Blueprint/Controller 为主线（对齐 004），默认 logics 基于 FieldLifecycle；提供 `Form.Rule.make` 与 `errorMap` 逃生舱；并沉淀一套默认的 Schema Path Mapping（覆盖大多数常见重命名/结构映射/数组内对齐场景，减少 errorMap 手写负担）；React 侧仅薄投影。
