# Quickstart: 007 Field（内核）→（可选）FieldKernel（支点）→ Form/Query（领域包）→ UI
...
- 业务默认只使用 `@logixjs/form` 的领域入口（`Form.make({ rules, fieldArrays, derived })` + `Form.Rule/Form.Error/Form.Field`），无需直接接触 FieldKernel/Field；
- 所有高层声明会 **完全降解** 到统一的最小中间表示（Field IR），并在降解后仍执行一致的冲突检测与合并；
...
- 默认桥接（update `state.ui` + scoped validate/cleanup）由 `FieldLifecycle.install`（kernel 归属）承载，Form 只做领域语义与 DX 封装。
...
> `EmailFormProgram` 是 `Program.make(EmailForm.module, { initial: EmailForm.initial(), logics: EmailForm.logics })` 的等价默认装配；Form 不单独拥有 Runtime。
