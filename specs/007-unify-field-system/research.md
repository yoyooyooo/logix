# Research: 007 Field 系统统一（Form 形状 × Kernel 性能 × 可回放）

**Branch**: `007-unify-field-system`
**Source Spec**: `specs/007-unify-field-system/spec.md`
**Source Plan**: `specs/007-unify-field-system/plan.md`

> 目标：把 007 的“对外行为约束”落到一组可实现、可验证、可交接的技术决策上，并为 Phase 1 的 `data-model.md` / `contracts/*` / `quickstart.md` 提供依据。

## 0. 现状盘点（代码事实）

- `packages/logix-core/src/FieldKernel.ts` 已有最小 DSL（`computed/source/link/build/install`），但当前：
  - `computed/source` 依赖字段并不要求显式声明，`FieldKernel.build` 也不会为 computed/source 构建可用于最小触发的 deps；
...
上述事实意味着：007 要求的“显式 deps → 依赖图 → 最小触发/反向闭包/诊断解释/回放重赛”仍需要在 FieldKernel/Runtime 层做结构性升级；这与 006 的方向一致。
...
- 触发语义（autoRefresh：onMount/onDepsChange + debounce；以及 autoRefresh=false 的显式 refresh）与并发策略选择；
...
### D10 — 包边界：Field（内核）→ FieldKernel（可选支点）→ Module（Form/Query）→ UI

**Decision**: 维持分层：

- `@logixjs/core`：Field/FieldKernel 内核、Resource/事务/诊断/回放基础能力；
...
### D11 — FieldLifecycle：Form/Query 的统一下沉接口

**Decision**: 将 “install / Ref / scoped validate / scoped execute / cleanup” 固化为 kernel 归属的 `FieldLifecycle`：Form 与 Query 的默认 logics 都必须基于它实现；领域包可以为 DX 做 re-export，但语义与演进所有权属于 kernel。
**Rationale**: 让 Form 与 Query 共享同一套“下沉到 Field IR 的桥接语义”，避免各领域各造一套 glue，确保诊断/回放口径一致且可维护。
**Alternatives considered**:
...
**Decision**: `Form.Rule.make` / `Form.Error.*` / `Query.Result` 等领域糖只负责“组织与可读性”，其产物必须可完全降解为 FieldKernelSpec/Field IR，并继续受同一套冲突检测、合并、诊断与回放口径约束；领域糖不得引入第二套 store/第二套状态事实源。
**Rationale**: 业务侧需要高表达但稳定的 API，同时必须避免出现“Form 与 Store 何时同步”的双事实源难题。
**Alternatives considered**:

- 直接要求业务写 FieldKernel/Field：门槛高且易写错；
