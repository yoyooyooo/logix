...
- **Todo**：为 Field Spec 引入 `zod` 或 `effect/Schema` 运行时校验。在 `FieldKernel.build` 阶段对配置结构进行严格验证（禁止环、禁止无效引用），这也是 006 要求的“分级失败”的基础。
...
- **Todo**：在 `@logixjs/form` 实现一个编译器宏或预处理 helper，允许：
  ...
  ```
  运行时自动展开为标准的 `FieldKernel.list` 结构。这也是用户感知最明显的 DX 提升。

### 6. CLI Codegen: Schema -> Scaffold (Code-Behind Pattern)

- **痛点**：面对后端给的 5 层嵌套 JSON Schema，手写 FieldKernel 依然痛苦；且纯 CLI 手动运行体验断裂。
- **Todo**：开发 `logix codegen` (Core) + `unplugin-logix-codegen` (Trigger)。
  ...
  - **Output**: 自动生成物理文件 `UserForm.fields.generated.ts`（可被 gitignore 或提交），供业务代码显式 import。
...
- **Todo**：在 `FieldKernel.list` 支持 `trackBy: (item) => item.id`。
...
- **痛点**：Query Field 知道资源的 status，也知道数据结构。
- **Todo**：利用 Field 的元信息，自动生成 Skeleton UI 组件。`Form.LoadingState` 直接根据 Schema 形状渲染骨架屏。
