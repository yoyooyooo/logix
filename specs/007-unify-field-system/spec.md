# Feature Specification: Field 系统统一（Form 形状 × Kernel 性能 × 可回放）

**Feature Branch**: `007-unify-field-system`
**Created**: 2025-12-13
**Status**: Draft
**Input**: User description: "整合 004-field-bridge-form 与 006-optimize-fields，并吸收 runtime/DX review 建议，消除冲突，生成统一的 007 spec。"
...
- Form 与 Query 是首批对照领域：007 必须能同时支撑“复杂表单联动/校验/数组”和“参数化查询的自动触发/竞态/缓存/失效”，以验证 Field 系统的可推广性。
...
- 不引入新的运行时能力类别：Field 仍以「派生 / 联动 / 异步资源状态 / 错误写回」为主，不把长耗时业务流程塞进 Field。
...
**Why this priority**: Query 是与 Form 平行的第二个对照领域；它能暴露表单语境外的缓存、竞态与触发语义问题，逼迫 Field 系统在更广谱的真实场景中自洽。
...
- 查询场景中 `autoRefresh` 与显式 refresh 共存时：如何避免语义歧义（例如 `autoRefresh=false` 表示仅显式），并让诊断能解释每次触发来源？
...
- **FR-001**: 系统必须提供统一的 Field 行为模型，支持“派生/联动/异步资源状态/错误写回”，并允许被上层领域能力（如表单）以标准化桥接方式接入（安装、字段引用、局部校验、清理），且该桥接方式不与某一领域强绑定。
...
- **FR-017**: 系统应提供“生成优先（codegen-first）”的降门槛路径：开发者可以从 Schema/蓝图输入生成可编辑的表单骨架与 Field 声明模板，从而避免手工维护深层嵌套场景下的高门槛声明；生成结果必须与本 spec 的对外行为契约一致，并应优先生成可维护的 deps 声明（或等价的可追溯来源），以降低显式 deps 的手写负担。
- **FR-018**: 系统必须支持查询场景的受限自动触发（Π_source；对齐 076）：至少包含 `autoRefresh`（`onMount`/`onDepsChange` + 可选 `debounceMs`）以及显式关闭 `autoRefresh=false`（仅显式 refresh）。触发来源必须可被诊断与回放；当 `autoRefresh=false` 时，系统不得自动触发新的请求，且默认应保留最后一次可观察结果快照（除非 key 被显式清空并进入 idle 语义）。
...
- **FR-022**: 系统必须固化并文档化链路分层：Field（内核）→（可选）FieldKernel（支点）→ 领域包（Form/Query/…）→ UI 适配层。上层所有领域糖与 UI 事件桥接都必须可还原为内核语义，且不得引入第二套不可回放事实源（错误、交互态与资源快照必须走同一条全双工链路）。
...
- **FR-027**: 系统必须定义 Form 场景的资源触发语义：内核以 `FieldKernel.source.autoRefresh` 表达受限自动触发（对齐 076：`onMount`/`onDepsChange` + 可选 `debounceMs`；以及 `autoRefresh: false` 的 manual-only）。UI 事件（如 blur/submit）不得被编码为 kernel `autoRefresh` 枚举，应通过显式 action/refresh 入口触发；当 key 变为空时必须遵循 FR-013 的同步 idle 清空语义；触发来源必须可诊断与可回放。
...
- **FR-033**: 系统必须提供“可选的 FieldKernel 支点”来承载领域 DSL：至少支持字段 scope、数组 scope（item/list 两层）与根级 scope；该 DSL 必须可被展开为等价的内核原语集合，并保持可回放、可诊断与可生成。
...
- **Field Rule**: 一条派生/联动/校验/资源状态写回规则，包含目标与显式依赖集合。
