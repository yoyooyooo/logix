...
- `@logix/query` 暴露 `QueryClientTag`（或等价 `QueryEngineTag`）作为运行时依赖；
- `@logix/query` 提供 `Query.layer(queryClient)` 便捷 Layer，用于宿主在全局 Runtime Layer 注入 QueryClient；
- 同一 Runtime 作用域内 SHOULD 只有一个 QueryClient；跨 Runtime 作用域允许不同 QueryClient（缓存隔离），但 keyHash 门控与回放/诊断口径必须一致。
