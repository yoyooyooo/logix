...
export const EmailForm = Form.make("EmailForm", {
  ...
  traits: Form.traits(ValuesSchema)({
    // 业务侧应只使用 Form 的领域 DSL；内部会降解到 StateTrait/Trait IR。
    // 如确需 escape hatch，可在局部 mix-in raw StateTrait.node/list，但不建议作为默认写法。
    "profile.email": Form.node({
      check: Form.Rule.make<string>({
        validate: {
...
- 业务默认只使用 `@logix/form` 的领域 DSL（例如 `Form.traits/Form.node/Form.Rule.make/Form.Error.*`），无需直接接触 StateTrait/Trait；
...
export const SearchQuery = Query.make("SearchQuery", {
  ...
  queries: {
    search: {
      ...
      concurrency: "switch",
      key: (params, ui) =>
        ui.query.autoEnabled ? { q: params.q, page: params.page } : undefined,
    },
  },
})
```
...
### 1.5 Runtime 注入（全局 Layer）：注册资源 + 注入 QueryClient

> Query 领域通过 effect 的 DI 获取 QueryClient；建议在应用 Runtime 层一次性注入。
