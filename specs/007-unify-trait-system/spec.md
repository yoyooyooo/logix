...
- **FR-027**: 系统必须定义 Form 场景的资源触发语义：内核以 `StateTrait.source.autoRefresh` 表达受限自动触发（对齐 076：`onMount`/`onDepsChange` + 可选 `debounceMs`；以及 `autoRefresh: false` 的 manual-only）。UI 事件（如 blur/submit）不得被编码为 kernel `autoRefresh` 枚举，应通过显式 action/refresh 入口触发；当 key 变为空时必须遵循 FR-013 的同步 idle 清空语义；触发来源必须可诊断与可回放。
