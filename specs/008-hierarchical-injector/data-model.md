...
- **ModuleToken**：`Logix.ModuleTagType<Id, Sh>`（同时也是 `Context.Tag`；用于解析 `ModuleRuntime<StateOf<Sh>, ActionOf<Sh>>`）。
...
type ScopeId = string // 诊断友好：可由 moduleId/instanceId/key 等拼出
...
type ResolutionError = {
  readonly name: 'MissingModuleRuntimeError' | 'MissingImportedModuleError' | 'AmbiguousModuleRuntimeError'
