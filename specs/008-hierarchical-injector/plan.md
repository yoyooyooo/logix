...
- `Intent → Flow/Logix → Code → Runtime` 映射：
  ...
  - Flow/Logix：Logic 侧以 Bound API（`$.use`）与跨模块协作入口（`orchestration link alias`）作为入口；React 侧以 `useModule/useLocalModule/useImportedModule` 与 `imports.get` 作为入口；这些入口必须共享同一套“作用域边界”语义。
...
- 依赖或修改的 `docs/specs/*`（docs-first & SSoT）：
  - 需要把“strict 默认 + 显式 root/global + 最近 wins”的对外契约回写到：
    - `docs/ssot/runtime/logix-core/api/02-module-and-logic-api.md`（`$.use` 与 `orchestration link alias`/跨模块协作 的语义边界）
...
- 收敛跨模块协作入口：移除 `$.useRemote` 作为公共 API；跨模块协作/IR 承载统一使用 `orchestration link alias`，业务层默认只使用 `$.use`（strict）。
...
- 明确 imports.get 的适用范围：root 也是一种模块实例；只要某个 host Program 确实提供了 imports（`Program.make(Host, { capabilities: { imports: [ChildProgram] } })`），则 strict 下 `host.imports.get(Child.tag)` / `useImportedModule(host, Child.tag)` 均应可解析（不区分 root/local）。
