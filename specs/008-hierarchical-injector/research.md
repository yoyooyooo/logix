...
### D09 — 收敛跨模块协作入口：删除 `$.useRemote`，统一为 `orchestration link alias`

**Decision**: 跨模块协作（读取对方 state、监听变化、触发对方 actions）的默认入口只保留 `orchestration link alias`；`$.useRemote` 不再作为公共 API 暴露并将被删除。
...
- 若目标模块不在 imports 关系中且属于“跨模块胶水逻辑”：将协作逻辑提升为显式 `orchestration link alias(...)`（process），并挂到 root 模块的 `processes` 中；示例在 `specs/008-hierarchical-injector/quickstart.md`。
  **Alternatives considered**:
- 同时保留但分层：仍会带来协作分裂与长期维护成本；
- 保留 `$.useRemote` 删除 `orchestration link alias`：会削弱工具链/IR 承载的表达能力与可解释性边界。
