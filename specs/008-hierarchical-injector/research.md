...
**Decision**: 解析失败错误在 Logic/React 两侧保持同构字段与修复建议：包含请求 token、入口（Logic/React）、当前 scope 标识（moduleId/runtimeId/key/parentId）、以及最少两条可执行修复路径；prod 环境保持短消息与稳定 error name。  
