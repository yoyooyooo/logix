...
- Q: `Module.implement({ imports: [...] })` 的 imports 应接收 `Child.impl` 还是 `Child.module`？ → A: 只接收 `Child.impl`（ModuleImpl）作为“提供实现/构造子实例”的声明；`Child.module`（ModuleTag）只用于解析（`$.use`/`imports.get`/`Root.resolve`）。
