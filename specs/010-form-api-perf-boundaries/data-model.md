...
- `fields`：统一的声明入口（对齐 `FieldKernelSpec`），用于表达字段级校验（`check`）与数组字段的 list 语义（`{ identityHint, item, list }`），以及 `computed/link/source` 等能力。
- `derived`：联动/派生声明入口（业务侧不直接写 `FieldKernel`），用于表达可完全降解为 field `computed/link/source` 的能力；默认只允许写回 `values/ui`（不写 `errors`）。

因此出现 `values.userList` 与 `fields.userList = { identityHint, item, list }` 并不是“重复定义”，而是“对 `values.userList` 这个数组字段附加列表语义（rowId/校验 scope/触发推导）”。
...
type FormConfig<TValues extends object> = {
  ...
  // fields：统一声明入口（字段 check / list 语义 / computed-link-source）
  fields?: FieldKernelSpec<TValues>

  // (提案) 联动/派生入口：key = 写回路径（valuePath/uiPath），value = `Form.Field.*` 产物（同形状于 `FieldKernel.*`）
  derived?: Readonly<Record<string, unknown>>
}
```

> 注：终态业务写法默认只用 `fields/derived` + `Form.Rule/Form.Error/Form.Field`（仍可完全降解到 FieldKernelSpec/IR）；数组字段的 list 语义通过 `fields.<listPath> = { identityHint, item, list }` 显式声明。

### 0.2 RuleDecl（用户声明形态）

默认写法支持“直写形态”：在 `fields.<path>.check`（含 list 的 `item.check/list.check`）里直接用对象声明规则；当需要复用/组合时，可提取为 `Form.Rule.make/merge` 的产物复用。
...
`Form.Rule.make/merge` 的产物必须可直接挂到 `FieldKernel.node({ check })`，并可完全降解为 kernel `CheckRule`：
...
ValidateTarget 是“scoped validate”的目标表达（与 `FieldLifecycle.FieldRef` 对齐）：
...
list-scope check 的最小诊断事件（Slim & 可序列化）应包含（对齐 Spec 009 DynamicTrace.events 的事件信封，`kind="field:check"`）：
...
详细字段以 `specs/010-form-api-perf-boundaries/contracts/schemas/field-check-event.schema.json` 为准。
