...
- `[DEFER]` **跨模块 Form 消费 Query 的示例与共享缓存**：010 先只覆盖“本模块 source 写回快照 → local deps 消费”；跨模块消费通过 Link/Logic 显式投影到 Form `ui.*`，以及按 `resourceId+keyHash` 的跨模块缓存/in-flight 去重，后续在 `FieldKernel.source`/`@logixjs/query` 跑道单独加强并补齐示例。
