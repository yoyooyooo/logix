...
本目录保留 014 spec 对“可对比证据”契约的镜像与说明；契约 SSoT 统一为 `@logix/perf-evidence/assets/schemas/*`（物理：`.codex/skills/logix-perf-evidence/assets/schemas/*`）：
...
- schema 的 `schemaVersion` 发生 breaking change 时，必须同步更新：
  ...
  - diff 工具与 `perf.md` 的固化流程说明（以及 `@logix/perf-evidence/assets/schemas/*`）；
  - 需要的话补迁移说明（不保留兼容层）。
