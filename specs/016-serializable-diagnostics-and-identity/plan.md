...
**Primary Dependencies**: effect v3、`@logixjs/core`（DebugSink/DevtoolsHub/Runtime identity）、`@logixjs/react`（RuntimeProvider onError）、`specs/005`（协议 contracts）、`specs/009`（稳定标识与 trace）、`specs/011`（error summary + lifecycle status）、`specs/013`（field:* 证据）
...
packages/logix-core/src/internal/state-field/meta.ts              # TraitMeta（Slim/可导出）+ sanitize（裁剪规则）
