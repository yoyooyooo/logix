...
- `Query.make(id, config)`：创建 Query Module（与 Form 一样，`make` 返回 `Logix.Module.Module`，而不是 Blueprint）。
  ...
  - 类型目标：`deps` 在不牺牲“字符串路径声明式”的前提下，尽量收窄到 `FieldKernel.StateFieldPath<{ params; ui }>`（深度上限 4）。
...
- `Query.fields(...)`：提供 Query 规则 → FieldSpec 的降解入口（不引入第二套事实源；写回到 `state.queries.*`）。
...
- 高级入口（20%）：业务将 `Query.fields(...)` + 领域 logics 作为 building blocks，把 query 快照字段挂到自定义主模块 state 上（仍保持单一事实源与可回放链路）。
