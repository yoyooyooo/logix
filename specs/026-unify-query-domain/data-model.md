...
- `impl`（ProgramRuntimeBlueprint）：可装配蓝图（Layer/imports/processes 等），供 Runtime/React 消费。
...
- Query Module 通过 fields 降解到 FieldKernel/Resource 主线（不维护第二套事实源）。
...
- Dev 诊断：FieldKernel 全体系（computed/source/link/check）若声明的 `fieldPath/deps/link.from/check.writeback.path` 不存在于 `stateSchema`，dev 环境应给出可定位的 warning（不影响生产语义，且 diagnostics off 接近零成本）。例如：宿主模块忘了声明 `queries.<name>`（或 `queries` 命名空间被占用）时，应提前暴露配置错误而不是静默写出新字段。
...
### 4) Query Rule / QuerySourceConfig（规则声明 → 降解为 FieldKernel）
...
- `deps`：依赖字段路径数组（如 `params.q` / `ui.query.autoEnabled`），用于图构建与触发收敛；类型建议收窄为 `FieldKernel.StateFieldPath<TState>`（`TState` 为模块 state；深度上限 4）。
...
> 说明：`deps` 与 `key(state)` 的 `state` 根一致——`Query.make` 场景下是 Query 模块自己的 state（默认包含 `params/ui`）；`Query.fields` 场景下是宿主模块 state（因此可以直接读取宿主 state 的任意值）。
...
**Degradation (Query → FieldKernel)**:

- 每条 Query Rule 必须能降解为一条 `FieldKernel.source`，写回目标字段为 queryName（同构到模块 state 上）。
