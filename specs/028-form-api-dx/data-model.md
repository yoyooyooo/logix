...
- **高级入口**：`fields`（直接注入底层 `FieldSpec`），用于 escape hatch；不作为默认文档路径。
- **不引入第二套运行时**：`rules/derived/fields` 必须可 100% 降解为 kernel `FieldSpec`，并通过 `@logixjs/core` 执行与产出 trace（Static IR + Dynamic Trace）。
...
2) **FieldPath（无索引）**：面向 field/依赖推导/诊断锚点，必须是“去索引”的 canonical segments（对齐 Spec 009）。
...
- field/规则锚点（scope）与诊断契约必须使用 FieldPath（segments array）。
...
```ts
// schema-scope：统一类型收窄入口（对齐 Logix.FieldKernel.from）
...
type FormMakeConfigV2<TValues extends object> = {
  ...
  // 高级：直接注入底层 FieldSpec（escape hatch）
...
- `rules/derived/fields` 三者的 path 锚点冲突必须“稳定失败”（抛错）并给出迁移建议：需要组合能力时回退到 `fields` 高级入口。
...
其中 `RuleInput` 对齐现有 `Form.Rule.make` 的输入能力（RHF-like 内置规则 + validate 函数 + deps + validateOn），并最终归一化为可挂载到 `FieldKernel.node({ check })` 的 `RuleSet`（见 `packages/logix-form/src/rule.ts` 的 `RuleSet/RuleEntry`）。
...
## 6) 诊断事实源：RulesManifest（Static IR）+ field:check（Dynamic Trace）

为保证“可解释/可对比/可回放”，规则系统必须导出一个不含函数体的静态清单（RulesManifest）：

- 静态：有哪些规则、scope 锚点、deps、validateOn、meta（JsonValue）。
- 动态：复用 `field:check`（010）的事件口径，给出 ruleId、scopeFieldPath、触发源、扫描/写回摘要、rowIdMode 与降级原因。
