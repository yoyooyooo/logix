...
## Decision 1: 推荐路径引入 `rules` 顶层入口；`fields` 退为高级入口

**Decision**
- `Form.make` 的配置顶层新增 `rules`（推荐路径默认只配 `rules + derived`），`fields` 仍保留但明确定位为“底层/高级入口”。
- `rules` 的目标不是引入第二套运行时：它必须可完全编译/降解为等价的 `FieldSpec`（最终仍由 `@logixjs/core` 的 state-field build/validate 执行）。
- 推荐引入 schema-scope 的入口 `Form.from(ValuesSchema)`：把 `derived/rules/fields` 的“类型收窄、路径提示、语法糖”统一挂在同一个 `$` 上（对齐 `Logix.FieldKernel.from` 的命名习惯）。
- `Form.make` 内部仍采用“合并后的 field spec + validateOn wrapper + install wiring”链路（现有链路在 `packages/logix-form/src/form.ts` 已存在），只是把“规则声明入口”从 `fields.<path>.check` 的深嵌套组织，提升为 `rules` 的独立概念。

**Rationale**
- 当前 `Form.make` 对外主入口是 `fields?/derived?`（`packages/logix-form/src/form.ts`），业务侧为了表达校验必须进入 `fields` 的结构，导致概念耦合与嵌套偏深（尤其是 list scope：`items: { item, list }`）。
...
1) **维持“全靠 fields 组织（含 list 嵌套）”**：最少改动，但继续让推荐路径暴露底层结构细节（DX 与 spec 的 FR-001/004 冲突）。
2) **新增 `FormSpec` 二次包壳（把 values/derived/fields/rules 全包进去）**：会引入第二套概念命名空间，且更难做到“只配 rules 的最短路径”。
...
- `rules` 的核心形态为“声明列表（decl list）”，每条声明只表达一个职责（字段规则 / 列表规则 / 根规则），例如：
  ...
  `const $ = Form.from(ValuesSchema)` 后，`Form.make({ rules: $.rules(...decls) })` 负责把 decls 编译为统一的 field spec。
...
- 诊断与回放必须能解释：本次 list 采用了什么 identity 策略、是否发生降级、原因是什么；并复用既有 `field:check` 事件字段（`rowIdMode/degraded` 对齐 010 的契约）。

**Rationale**
- 当前 `Form.fields` 会把“plain object 且包含 item/list/identityHint”的值归一化为 `FieldKernel.list(...)`（`packages/logix-form/src/dsl/fields.ts` 的 `normalizeNodeOrListOrEntry`），因此 `items: { item, list }` 即使没有 `identityHint` 也会被当作 list field；这会让“写法能跑”但“身份与错位风险不透明”。
...
- 说明：上述“隐式 list 识别”只允许存在于 `fields` 高级入口（escape hatch）；`rules` 推荐路径必须显式声明 list 语义与 identity。
...
**Alternatives considered**
1) **不做 wrapper，完全在 validate 引擎里按 metadata 过滤**：需要深入修改 `@logixjs/core` 的 state-field validate hot path，风险更高；不符合“先收敛 API、后逐步优化”的增量策略。
...
- Devtools/调试面使用 RulesManifest + DynamicTrace（`field:check`）建立解释链路：
  - 静态：有哪些规则、锚点在哪、依赖是什么；
...
- 拆分目标：单文件 ≤ 400 行（FR-009），并形成“可单测的纯函数内核”。建议边界：
  ...
  - fields/rules/derived 的 normalize + merge + validateOn wrapper（含 rulesValidateOn 汇总）
...
**Rationale**
- 当前单文件混杂 reducer、controller、path utils glue、field wrapper 与 schema mapping，难以测试与演进；拆分后可针对规则触发、列表写回、错误优先级等关键行为补齐测试（FR-010）。
...
- Spec 010 已预留 `ValidateTarget.listIndexPath` 与 `FieldLifecycle.Ref.fromValuePath(...)` 的多层 index 解析能力，但 `@logixjs/core` 当前的 state-field validate 尚未消费 listIndexPath（且 rowIdStore / list runtime 仅支持单层）；028 直接把这条链路补齐即可实现“递归 list scope”而不引入第二套运行时。
...
**Decision**
- 028 的目标是做“表单规则 DSL（可编译到 Logix/FieldKernel）”，而不是“新的通用 schema validator”。
- 我们会在 authoring 体验上对齐 zod（`object/array/field + refine/superRefine` 的组合直觉），但 **不** 把 zod 作为内部实现依赖；实现采用自有的轻量 RulesNode AST → decl list → field spec 编译链路。
...
- 028 需要“编译到现有内核”（FieldKernel/check/writeback）并输出 slim 证据；依赖第三方校验器会让 IR 口径和诊断链路更难收敛。
...
- `rules` 保持为表单领域 DSL：负责**交互约束**（依赖 deps、触发 validateOn、跨字段/跨行、list identity 与 scoped validate），并编译到 decl list / field spec / RulesManifest + trace。
