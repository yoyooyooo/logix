...
> 范围：028 推荐路径下的 `@logixjs/form` + `@logixjs/core`（FieldKernel / FieldLifecycle）当前实现口径。

## 0) 一屏 TL;DR（先记结论）

1. **deps 是唯一依赖事实源**：Graph / ReverseClosure / 增量校验 / 诊断解释都只认 `deps`。
2. **触发是 action→scopedValidate**：`setValue/blur/submit` 由 `Form.install` 映射为 `FieldLifecycle.scopedValidate`。
...
9. **诊断事件是 trace 证据**：`trace:field:validate`（选了多少 check、为何范围变大）+ `trace:field:check`（list 扫了多少行、耗时）。
...
- `packages/logix-core/src/internal/state-field/validate.ts`（mode 在规则 ctx 中可见）

### 1.2 ValidateTarget / FieldRef（scope 表达）

`FieldLifecycle.Ref.*`（可序列化、可比较）：
- `Ref.field("a.b")`
- `Ref.list("items")`
...
关键落点：`packages/logix-core/src/internal/field-lifecycle/index.ts`
...
关键落点：`packages/logix-core/src/internal/state-field/validate.ts`（`toPatternPath` / `toGraphTargets`）
...
构图阶段（build）会把每条 field 的 `deps` 写入 Graph 边：
- `check-dep: dep -> fieldPath`
- 运行时对 target 求 reverse-closure：从 target 出发，沿“谁依赖我”方向扩散，得到最小需执行集合。

关键落点：
- `packages/logix-core/src/internal/state-field/build.ts`（从 deps 产出 Graph.edges）
- `packages/logix-core/src/internal/state-field/graph.ts` / `reverse-closure.ts`
- `packages/logix-core/src/internal/state-field/validate.ts`（`scopesToValidate` 计算）
...
关键落点：`packages/logix-core/src/internal/state-field/validate.ts`（`toGraphTargets`）
...
关键落点：`packages/logix-core/src/internal/state-field/validate.ts`（`extractIndexBindings`）

### 3.4 Phase 2 限制（避免对“深层 list”产生误解）

- `FieldLifecycle.Ref.fromValuePath(...)` 支持把多层 index 解析为 `listIndexPath`（协议层已预留）。
- 但当前 state-field validate 的实现 **尚未消费 `listIndexPath`**，并且 item-scope 逻辑明确标注为“Phase 2：仅支持最浅层 `items[]`”。
...
`Form.make` 会把 field 上的 check 规则包一层：
...
关键落点：`packages/logix-core/src/internal/state-field/validate.ts`（`RULE_SKIP` 处理）
...
- rules：`errors.<path>`（FieldKernel check 写回）
- schema：`errors.$schema`（`Schema.decodeUnknownEither(valuesSchema)` 映射后写回）

优先级约定：`$manual > rules > $schema`

关键落点：
- 写回：`packages/logix-core/src/internal/state-field/validate.ts`
...
关键落点：`packages/logix-core/src/internal/state-field/validate.ts`（`evalListScopeCheck` / `touchedKeys` / row patch 应用）
...
- `packages/logix-core/src/internal/state-field/validate.ts`（field-check 写回时的 delta）
...
FieldKernel validate 会读取当前 diagnostics level：
- `off`：不产出 trace（接近零成本）
- `light/full`：产出 `trace:*` 事件

关键落点：`packages/logix-core/src/internal/state-field/validate.ts`（`Debug.currentDiagnosticsLevel`）

### 6.2 关键事件（建议先看这两个）

- `trace:field:validate`：一次 validate 的概要（mode、请求数、选中 check 数、是否 root）
- `trace:field:check`：list-scope 规则的运行摘要（scannedRows、affectedRows、changedRows、durationMs、rowIdMode、degraded…）
...
- 诊断：`trace:field:validate`（selectedCheckCount≈1，mode=valueChange）
...
- 诊断：`trace:field:check`（scannedRows=200、durationMs、changedRows…），可用于解释“为什么这次重”
