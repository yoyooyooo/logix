...
- **包边界**：新增 `@logix/i18n` 作为 i18n 领域特性包；`@logix/core` 保持最小运行时内核（含 `$.root.resolve`），不引入对特定 i18n 引擎/资源加载的强绑定。
...
**Primary Dependencies**: `effect` v3、`@logix/core`、`@logix/react`、`@logix/i18n`（外部 i18n 以 i18next 风格实例为典型注入值；`@logix/i18n` 不依赖具体引擎；不新增 `@logix/i18n-react`）  
...
- `Intent → Flow/Logix → Code → Runtime` 映射：
  ...
  - Code：核心落点在 `packages/logix-core`（Bound API `$` 的类型/实现、Root provider 解析、`$.root.resolve` 语法糖）与 `packages/i18n`（`@logix/i18n`：I18n Service / I18nModule / message token / 最小形状注入）；必要时补 `packages/logix-react` 的示例/文档对齐（不改变现有语义，只增补 DX）。
...
packages/i18n/                                                     # `@logix/i18n` 领域特性包（Service/Module/token/driver）
...
### Phase 1：Core 增量能力（`$.root.resolve`）+ `@logix/i18n` 领域特性包
...
- 为国际化提供“可注入服务契约”的最小入口（落点在 `@logix/i18n`，类似 Query 的 layer 思路）：
