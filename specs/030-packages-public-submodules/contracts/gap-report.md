...
|--------|----------|----------|
| `@logixjs/core` | A | 少量命名/导出一致性问题（`Reflection`、`InternalContracts` 落点） |
| `@logixjs/react` | C | index 直接导出实现目录；`./*` 使 `shallow.ts` 等 helper 变 public |
| `@logixjs/devtools-react` | C | `./*` 导出 `.tsx` 导致非概念入口可被 subpath import；实现目录未下沉 |
| `@logixjs/sandbox` | B | 对外概念明确但文件命名/落点不符合“概念入口 PascalCase”约束；internal 屏蔽策略缺失 |
| `@logixjs/test` | B | 仅 `.` 单入口；缺少概念子模块入口文件与 internal 屏蔽策略 |
| `@logixjs/form` | C | `./*` 通配导出 + root 低层实现文件（`*.impl.ts` 等）导致严重泄漏 |
| `@logixjs/query` | C | `./*` 通配导出暴露 `typecheck.ts` 等；`@logixjs/query/react` 空壳入口 |
| `@logixjs/i18n` | B | 单文件聚合实现（index 既是实现又是入口）；缺少概念级子模块入口文件 |
| `@logixjs/domain` | B | 概念入口为 `crud.ts`（lowercase）且被 `./*` 导出；应收敛为 `Crud` 概念入口 |
...
- **C**：`@logixjs/query/react` 子路径入口为空壳（`src/react/index.ts` 为空），按规则必须删除或实现为稳定能力后再开放。
