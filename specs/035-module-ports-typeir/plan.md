# Implementation Plan: Module Ports & TypeIR（035：端口/类型 IR 作为平台 SSoT）

**Branch**: `035-module-ports-typeir` | **Date**: 2025-12-26 | **Spec**: `specs/035-module-ports-typeir/spec.md`  
...
- **链路映射（Intent → Flow/Logix → Code → Runtime）**
  - 035 是平台侧“引用空间与类型事实源”：由 trial-run 从最终可运行模块对象导出，供编辑/验收/Agent 迭代使用；运行时不读取这份协议。
