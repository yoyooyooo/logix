...
035 的目标是定义平台“模块引用空间事实源”的唯一真相源：只覆盖 **引用空间导出**（`ModulePortSpec@v1` / `TypeIR@v1` / `PortAddress`）。
...
表达式/校验资产协议已拆分为 034：`specs/034-code-asset-protocol/`。
...
- **链路映射（Intent → Flow/Logix → Code → Runtime）**
  - 035 是平台侧“模块引用空间事实源”：引用空间（PortSpec/TypeIR）由 trial-run 从最终可运行模块对象导出；运行时不读取这份协议。
...
- 资产协议（CodeAsset/Deps/Digest/Anchor）的治理边界在 034 定义：`specs/034-code-asset-protocol/`。
...
- 资产协议 schemas（CodeAsset/Deps/Anchor）已拆分为 034：`specs/034-code-asset-protocol/contracts/schemas/*`
- 在 `data-model.md` 固化：
...
- PortSpec/TypeIR 的导出/归一化/diff，优先做成纯函数 + schema 驱动，以便被 Workbench/CI/Agent 复用。
