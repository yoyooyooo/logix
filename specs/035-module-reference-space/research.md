...
**Existing evidence**：runtime 里已在 `packages/logix-core/src/internal/state-field/converge.ts` 使用 `effect/SchemaAST` 做 fieldPath 静态判定与诊断（`schemaHasPath/schemaHasFieldPath`）。
