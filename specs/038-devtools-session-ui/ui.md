...
- 子节点建议默认顺序（可按数据可得性退化）：
  ...
  3. Field evidence（converge/check/validate）
...
- 同一时间刻度下展示多条 pulse 轨：
  ...
  - EffectOp pulses（service/source/field 等）
...
- **收拢噪音**：若某个 txn 仅包含 `state:update` 且无副作用/无 field 证据，默认折叠为一个胶囊（例如：`State Patch x1`）
- **高亮主角**：若泳道内出现 `trace:field:converge`（或等价 converge 证据），使用 `--intent-highlight-focus` 的指示条/表面着色高亮（禁止“金色边框”）
