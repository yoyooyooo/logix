# Research: Field Derived Converge Perf & Diagnostics Hardening
...
- Exec IR 不改变对外契约：仍以 `field:converge` 的证据字段与稳定标识作为事实源。
...
**Decision**: `field:converge` 的事件契约继续以 `specs/013-auto-converge-planner/contracts/` 为单一事实源；本特性默认不修改 shape，只优化产出成本与必要时的字段裁剪（light/full 由 `DebugSink` 控制）。
...
- 以 Node runner 作为 converge 专项基线的高频迭代跑道，并将 Before/After/Diff 证据固化到 `specs/039-field-converge-int-exec-evidence/perf/*`；
