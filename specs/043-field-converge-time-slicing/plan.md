# Implementation Plan: 043 Field 收敛 Dirty Checks 的 Time-slicing（显式 Opt-in）

**Branch**: `043-field-converge-time-slicing` | **Date**: 2025-12-29 | **Spec**: `specs/043-field-converge-time-slicing/spec.md`
**Input**: Feature specification from `specs/043-field-converge-time-slicing/spec.md`

## Summary

在不改变默认行为（每个操作窗口内完成收敛、对外 0/1 commit、事务窗口禁止 IO/async）的前提下，为 FieldKernel 的 converge/dirty-check 引入显式 opt-in 的 time-slicing：

- fields 显式分为 `immediate`（每 txn 仍收敛）与 `deferred`（可延迟到后续窗口合并执行）；
...
**Performance Goals**: 在 1000+ fields、90% deferred 的高频输入场景下，操作窗口内 converge/dirty-check 的 p95 显著下降（目标 ≥2×）；默认关闭时无回归
...
- **IR & anchors**：不改变统一最小 IR，仅在 Dynamic Trace（`trace:field:converge`）增加可序列化摘要字段；稳定锚点继续使用 `moduleId/instanceId/txnSeq/opSeq`。
...
- **Dual kernels（core + core-ng）**：该特性位于 FieldKernel/ModuleRuntime 内，预期 core/core-ng 都支持；不引入 consumer 对 `@logixjs/core-ng` 直依赖。
...
- profile：`default`
- collect（before）：`pnpm perf collect -- --profile default --out specs/043-field-converge-time-slicing/perf/before.<worktree>.<envId>.default.json --files packages/logix-react/test/browser/perf-boundaries/*time-slicing*`
- collect（after）：`pnpm perf collect -- --profile default --out specs/043-field-converge-time-slicing/perf/after.<worktree>.<envId>.default.json --files packages/logix-react/test/browser/perf-boundaries/*time-slicing*`
- diff：`pnpm perf diff -- --before specs/043-field-converge-time-slicing/perf/before...json --after specs/043-field-converge-time-slicing/perf/after...json --out specs/043-field-converge-time-slicing/perf/diff.before...__after...json`
...
└── field-convergence-policy.md

specs/013-auto-converge-planner/contracts/schemas/
└── field-converge-data.*.schema.json
```

**Structure Decision**:

- 语义扩展（immediate/deferred + scheduler）在 `packages/logix-core/src/internal/state-field/*` 与 ModuleRuntime transaction 层闭合；
- 配置入口统一收敛到既有 StateTransaction 控制面（runtime_default / runtime_module / provider），避免新增并行旋钮；
- 可解释证据继续复用 `trace:field:converge` 事件（在 data 字段内追加 time-slicing 摘要），保持 Devtools/Sandbox 消费面稳定。
