# Feature Specification: Field 收敛 Dirty Checks 的 Time-slicing（显式 Opt-in）

**Feature Branch**: `043-field-converge-time-slicing`
**Created**: 2025-12-27
**Status**: Done
**Input**: User description: "Fields 收敛 dirty checks 的 time-slicing（帧/间隔节流）与可解释证据"

## Assumptions

- 默认不启用：未配置时保持现有行为（每个操作窗口内即时收敛、对外 0/1 commit）。
- time-slicing 只影响“是否在本窗口做收敛检查/执行”的调度策略；不改变单次收敛内部语义（等价于把部分工作移动到后续窗口）。
- “可延迟 field”允许短暂读到过期值；是否可延迟必须由业务显式声明（禁止自动推断/猜测）。
- 事务窗口禁止 IO/async 仍是红线；延迟执行发生在后续独立操作窗口中，并拥有自身稳定标识链路（`instanceId/txnSeq/opSeq` 等）。

## Out of Scope

- 不在本特性中实现 `specs/039-field-converge-int-exec-evidence/` 的“整型执行链路打穿”；本特性只解决“高频窗口 O(N) 检查/触发”的调度策略。
- 不自动推断 field 的重要性/可延迟性；不提供“自动分级”的魔法策略。
- 不提供跨模块的全局优先级系统（priority inversion/多队列调度等）；只定义最小可验收闭环。

## Dependencies

- 可复现的浏览器基准场景：可构造 1000+ fields，其中绝大多数属于 UI 装饰/可延迟类，用于验证“跳过检查/合并触发”的收益。
- Devtools/trace 能消费并展示 Slim 的 field 证据（沿用既有 `field:*` 事件链路）。

## User Scenarios & Testing _(mandatory)_

### User Story 1 - 高频输入下仍流畅 (Priority: P1)

作为业务开发者，我希望在 fields 数量很大（例如 1000+）的表单/交互场景中，连续输入时系统不会因为每次变更都全量检查/收敛所有 fields 而产生卡顿。

**Why this priority**: “每次窗口 O(N) 全量检查”在大 N 下会成为硬上限，直接限制 Field 系统可用规模。

**Independent Test**: 构造“大量 fields + 高频事务”场景，对比开启前后单次操作窗口 p95 时间；关键字段（即时 fields）仍在同窗口收敛。

**Acceptance Scenarios**:

1. **Given** 一个含 1000 fields 的模块，其中 90% 标记为可延迟，**When** 连续触发高频输入，**Then** 每次操作窗口内仅对“即时 fields”做收敛检查/执行，并将可延迟 fields 合并到后续调度窗口中。
2. **Given** 同一初始状态与相同输入序列，**When** 重复运行多次，**Then** 最终可观察状态一致，且延迟 fields 的最大滞后不超过一个可配置的上界。

---

### User Story 2 - 延迟策略可解释 (Priority: P2)

作为运行时维护者，我希望当某个 field 没有在当前窗口被收敛时，Devtools/证据能解释“为什么被跳过、何时会被补算、是否出现饥饿/积压”。
...
1. **Given** 某次窗口只触发了延迟 fields 的依赖，**When** 诊断开启，**Then** 证据中能看到：延迟策略生效、跳过数量/类别、以及下一次计划执行的窗口摘要信息。
...
- 延迟 fields 依赖即时 fields 时，延迟窗口是否需要包含其所有依赖的补算？
- 同一 field 被不同策略标记（模块级 vs field 级）时，优先级如何裁决？
...
- **FR-001**: 系统必须支持显式 opt-in 的“延迟收敛策略”，允许把部分 fields 从“每窗口立即检查/执行”降级为“按调度窗口合并执行”。
- **FR-002**: 系统必须保证默认行为不变：未启用时仍满足“每操作窗口内完成收敛、0/1 commit、事务窗口禁止 IO、稳定标识、最小 IR”。
- **FR-003**: 系统必须支持 fields 至少分为“即时（immediate）”与“可延迟（deferred）”两类，且分类来源必须是显式声明（禁止自动推断）。
- **FR-004**: 系统必须提供“最大滞后上界”与“饥饿保护/降级策略”，确保 deferred fields 不会无限期不收敛。
...
- **SC-002**: immediate fields 的行为不变（仍在同窗口收敛），deferred fields 的最大滞后不超过配置上界（p95）。
- **SC-003**: 未启用本特性时，既有基线与回归无回归（时间/分配开销在阈值内）。
- **SC-004**: Devtools 能解释一次“跳过/补算/降级”事件的原因链路（至少包含策略、生效范围摘要、是否触发饥饿保护）。

## Notes

- 与 `specs/039-field-converge-int-exec-evidence/` 的关系：039 不改变对外语义，只优化执行链路；本特性改变触发/调度语义，因此必须独立交付并提供 opt-in 与迁移说明。
