# Implementation Plan: 044 Field 收敛诊断的低成本采样（计时/统计）

**Branch**: `044-field-converge-diagnostics-sampling` | **Date**: 2025-12-29 | **Spec**: `specs/044-field-converge-diagnostics-sampling/spec.md`
**Input**: `specs/044-field-converge-diagnostics-sampling/spec.md`
...
- **Intent→Flow/Logix→Code→Runtime**：属于 Runtime 可观测性（field converge 的执行诊断/证据）能力增强；不改变用户侧 Intent/Flow/DSL。
- **SSoT / docs-first**：采样新增字段/level 的协议裁决落在 `specs/009-*`（DynamicTrace level 枚举）与 `specs/013-*`（field:converge data/event schema）；代码只实现，不在 feature 目录复制 schema。
...
- **Performance budget**：通过 `packages/logix-perf-evidence/assets/matrix.json` 的 `diagnostics.overhead.e2e` suite 验证 off/light/sampled/full 四档开销曲线。
...
├── src/internal/state-field/model.ts
└── test/FieldKernel/FieldKernel.ConvergeAuto.DiagnosticsLevels.test.ts
...
└── contracts/schemas/field-converge-*.schema.json
```

**Structure Decision**: 本特性是 runtime observability + field converge 诊断协议扩展；代码落在 `packages/logix-core`（DebugSink + converge），浏览器证据复用现有 perf boundary（`packages/logix-react/test/browser/perf-boundaries`）与 perf matrix SSoT（`@logixjs/perf-evidence/assets/matrix.json`）。
