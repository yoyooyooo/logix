# Feature Specification: Field 收敛诊断的低成本采样（计时/统计）

**Feature Branch**: `044-field-converge-diagnostics-sampling`
**Created**: 2025-12-27
**Status**: Planned
**Input**: User description: "Fields 收敛诊断的低成本采样（计时/统计）与生产可观测性"
...
- 不在本特性中改变 `specs/039-field-converge-int-exec-evidence/` 的“整型执行链路”与其证据门禁；本特性聚焦“计时/统计如何更低成本”。
...
- 既有 `field:*` 证据链路可用于承载采样摘要（或提供明确迁移方案）。
...
1. **Given** 启用 sampled 模式，**When** 运行高频收敛基准，**Then** 系统能输出采样摘要（例如 Top-N step/field、采样次数、总览分位数）且可序列化。
2. **Given** 构造某个特定 field 明显变慢的回归，**When** 运行基准并开启 sampled，**Then** 采样摘要能把该 field 归入 Top-N（允许统计误差，但应在稳定阈值内）。
...
- **FR-001**: 系统必须提供一种“采样诊断模式”，用于对 field 收敛过程的计时/统计进行抽样采集，并输出 Slim、可序列化的摘要证据。
...
- **SC-002**: sampled 模式相对 full 的开销显著更低（time/alloc），且在注入“单 field 变慢”的场景下能稳定把目标归入 Top-N（允许统计误差）。
- **SC-003**: diagnostics=off 在新版本中仍保持近零开销（无回归）。

## Notes

- 与 `specs/039-field-converge-int-exec-evidence/` 的关系：039 解决执行链路本身的性能与证据门禁；本特性解决“诊断计时/统计本身如何足够便宜”，避免“为了观测而伤害被观测对象”。
