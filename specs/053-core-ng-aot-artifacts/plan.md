...
- **已有地基（runtime-only）**：
  - Static IR：`packages/logix-core/src/internal/reflection-api.ts`（export/manifest）+ `packages/logix-core/src/internal/state-field/converge-ir.ts`
  - Exec IR（JIT-style）：`packages/logix-core/src/internal/state-field/converge-exec-ir.ts`（TypedArray tables）
...
```text
packages/logix-core/src/internal/reflection-api.ts                 # Static IR export/manifest（事实源）
packages/logix-core/src/internal/state-field/*        # converge Static/Exec IR
