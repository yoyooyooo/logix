...
- reachability 当前落点（JS）：`packages/logix-core/src/internal/state-field/converge.ts`（plan compute / reachability 判定）
- Exec IR tables（TypedArray）：`packages/logix-core/src/internal/state-field/converge-exec-ir.ts`
...
```text
packages/logix-core/src/internal/state-field/*        # JS baseline reachability（对照）
