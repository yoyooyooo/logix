# Quickstart: 067 core 纯赚/近纯赚性能优化（怎么验收）
...
以 `specs/067-core-pure-perf-wins/plan.md` 的 `Perf Evidence Plan（MUST）` 为准：Node + Browser 都要跑 before/after/diff，并确保 `meta.comparability.comparable=true && summary.regressions==0`。
