...
- Runtime SSoT entry points：
  ...
  - `docs/ssot/runtime/logix-core/api/README.md`：补齐 ExternalStore/FieldKernel 的代码落点。
  - `docs/ssot/runtime/logix-core/concepts/02-long-chain-tour.md`：修正 React deep dive 路径。
- 用户文档（apps/docs）：
  - `apps/docs 当前 guide/runtime 口径文档` + `apps/docs/content/docs/guide/recipes/meta.json`
  - `apps/docs 当前 core/guide 口径文档` + `apps/docs/content/docs/api/core/meta.json`
...
- 改动（落点）：
  - `packages/logix-perf-evidence/scripts/diff.ts`：新增 `metricDeltas`（按 suite+metric 输出 median/p95 的 delta 与 top 变化点）；修正 `unit=count` 证据聚合（避免跨点位相加导致误解）。
  - `packages/logix-perf-evidence/assets/schemas/perf-diff.schema.json`：补齐 `metricDeltas` schema。
...
- 改动：`packages/logix-core/src/internal/state-field/external-store.ts`
  - `getSnapshot()` 抛错时熔断（保持既有“停止后续写回”语义），并补齐 `external_store::snapshot_threw` 诊断事件（Slim、可序列化；diagnostics=off 近零成本）。
- 测试：`packages/logix-core/test/internal/FieldKernel/FieldKernel.ExternalStoreTrait.Runtime.test.ts`
  - 扩展用例：断言熔断时会产生上述诊断事件（ring buffer sink）。

### FR-011：nonUrgent lane → commit priority=low

- 改动：
  - `packages/logix-core/src/internal/state-field/external-store.ts`：field 安装写回事务 origin 注入 `details.stateCommit.priority`（nonUrgent→low）。
