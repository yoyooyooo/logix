...
- **Level 1（模块内强一致）**：引入 `ExternalStore<T>` 归一化外部输入，并提供 `FieldKernel.externalStore` 将外部输入声明式接入 State Graph：
...
- **Level 2（跨模块强一致 + React 无 tearing）**：引入 Runtime 级 TickScheduler + RuntimeStore（单一 snapshot 真理源 + 分片订阅）：
  ...
  - 提供 DeclarativeLink IR（强一致可识别的跨模块依赖表达），并定义与 `orchestration process link surface` 黑盒的边界；
...
- **受限绑定（Boundaries / Geometry）**：`FieldKernel.externalStore` / `FieldKernel.source` 属于“边界条件绑定”而不是自由工作流：
  - 绑定必须可 IR 化、可预算、可诊断（Static IR + Dynamic Trace）；
  - 写回必须进事务窗口并受治理（external-owned/单 writer/txn 禁 IO）。
- **自由编排（Control Laws / Workflows）**：多步协议、分支、时间算子（delay/retry/timeout）、跨服务协调等属于“自由度”，不应被塞进 field 的静态 meta。它们应由独立的 Flow/Action Program 表达，并通过 tick 的证据链（`trace:tick` + `EffectOp`）进入可回放、可解释、可预算的轨道。
...
- 本特性不扩展/不固化 `FieldKernel.source` 的 `meta.triggers/debounceMs` 反射式解释入口；后续由 `076-logix-source-auto-trigger-kernel` 收敛替换，避免 073 把 tick 参考系意外绑定到旧接口上。
...
- Decision: `ExternalStore.getSnapshot()` 永远返回 raw current；coalesce 在 field 写回层做 pending/raw 与 committed 分离，保证 committed 才可观测（source: `spec.md#Clarifications`）
...
- Decision: blackbox `orchestration process link surface` 不进入同 tick fixpoint（Next Tick best-effort），并必须在 diagnostics 中标注边界（source: `spec.md#Clarifications`）
...
- Decision: Module-as-Source：支持把模块 selector 结果当作 ExternalStore 来源（`ExternalStore.fromModule(...)`），并用 `FieldKernel.externalStore` 声明式写回到下游模块字段；但必须被编译为 IR 可识别依赖（module readQuery → field writeback）并参与同 tick 稳定化，禁止退化为 runtime 黑盒订阅（source: `spec.md#Clarifications` / FR-012 / SC-005）
- Decision: `ExternalStore.fromModule` 不做值拷贝：Field 写回存的是 selector 返回值本身（按引用共享，不深拷贝/不结构化拷贝）。因此禁止用 fromModule “镜像大状态”；保持 selector 小且稳定，必要时在 selector 内显式投影/拷贝并把成本计入预算（source: `spec.md#Clarifications`）
...
- Decision: Field 下沉边界：`FieldKernel` 只负责“模块内字段能力 + 静态治理 + Static IR 导出”；`TickScheduler/RuntimeStore` 只消费 IR 做调度与快照一致性。禁止把 tick/React 订阅逻辑塞进 fields（SRP + no-dual-truth）。
...
- Batch A：
  - Q001/Q005：DeclarativeLinkIR 必须落到明确 TS 类型（internal），写侧仅允许 `dispatch`；黑盒 `orchestration process link surface` 的写入不参与同 tick fixpoint（视为 Next Tick best-effort）。
...
- Batch C：
  ...
  - Q005：黑盒 `orchestration process link surface` 的 “Next Tick best-effort” 对依赖同 microtask 生效的存量存在 breaking 风险，必须写入 migration 口径并给出迁移建议。
...
- Batch D：
  ...
  - Q003：ExternalStore.getSnapshot 同步抛错不会崩溃 runtime：熔断该 field，保留 last committed 值，并记录诊断（默认 Warn）。
  - Q004：topic facade 默认“归零即删”，不引入 TTL；若业务场景出现 GC churn，将以 perf evidence 驱动再引入 keep-alive（必须有 retained/budget 防线）。
  - Q005：Root Reset 不直接覆盖 external-owned 字段：externalStore field 以当前快照为准保持外部值（避免 reset 写入逃逸/闪烁）。
...
- **Docs-first & SSoT**：依赖/对齐既有 specs：`007-unify-field-system`、`057-core-ng-static-deps-without-proxy`（ReadQuery/static lane）、`060-react-priority-scheduling`（priority notify/lanes）、`027-runtime-observability-hardening`（token/订阅不变量）。若新增对外契约与诊断协议，需同步更新 `docs/ssot/runtime/**` 的 runtime SSoT。
...
- **Transaction boundary**：externalStore 写回/派生收敛必须在事务窗口内完成；tick 稳定化期间不得引入 IO；IO 只能通过既有 `FieldKernel.source` 两阶段语义（loading → async writeback）落地。
...
│   ├── FieldKernel.ts               # ADD: FieldKernel.externalStore DSL（public）
│   └── internal/
...
│       ├── runtime/core/process/ProcessRuntime.ts   # CHANGE: orchestration process link surface boundary (best-effort) + tick integration (as needed)
│       ├── runtime/core/process/ProcessRuntime.*.ts # NEW: split ProcessRuntime(>1000 LOC) into mutually exclusive modules（见下文）
│       ├── state-field/model.ts                 # CHANGE: add externalStore kind + plan step
│       ├── state-field/build.ts                 # CHANGE: emit externalStore plan steps + writer/ownership governance
│       ├── state-field/ir.ts                    # CHANGE: export externalStore policy into Static IR
│       ├── state-field/external-store.ts        # NEW: externalStore field runtime/install (internal)
│       ├── state-field/source.ts                # CHANGE: deps-as-args + writeback semantics (T009)
│       ├── state-field/source.*.ts              # NEW: split source(>1000 LOC) into mutually exclusive modules（见下文）
│       ├── state-field/converge-in-transaction.ts    # CHANGE: external-store-sync plan step + tick scheduling hooks (T014/T020)
│       ├── state-field/converge-in-transaction.*.ts  # NEW: split converge-in-transaction(>1000 LOC) into mutually exclusive modules（见下文）
│       ├── state-field/validate.ts              # CHANGE: ownership + new kind validation (T019)
│       └── state-field/validate.*.ts            # NEW: split validate(>1000 LOC) into mutually exclusive modules（见下文）
...
- `packages/logix-core/src/internal/state-field/source.ts`（1173）
- `packages/logix-core/src/internal/state-field/converge-in-transaction.ts`（1031）
- `packages/logix-core/src/internal/state-field/validate.ts`（1384）
...
- `ModuleRuntime.fields.ts`：traitState/rowIdStore/selectorGraph 装配 + field converge time-slicing state（只做组装，不做 tick）
...
#### 2) `state-field/source.ts`（1173）→ `source.*.ts`
...
- `source.depsMismatch.ts`：deps mismatch 计算/格式化与 `state_field::deps_mismatch` 诊断发射
...
- `validate.diagnostics.ts`：`trace:field:validate`/diagnostic 事件门禁与 slim payload 组装
...
目标：在明确 “黑盒 orchestration process link surface = Next Tick best-effort（不进入同 tick fixpoint）” 的边界时，把 link 相关逻辑从 runtime 组装里拆出去，降低后续演进风险。
...
- 容错：ExternalStore.getSnapshot() 同步抛错必须被 field 层捕获；熔断该 field（保留 last committed 值），并通过 diagnostics 记录 Warn（不得崩溃整个 runtime）。
...
- `FieldKernel.externalStore` 只负责“写回 state field”，派生/联动用 `computed/link/source` 表达（保持 SRP）。
- Field 下沉（做到位，避免“Runtime 猜语义”）：
  - `ExternalStore` sugar 必须携带内部 descriptor（至少 `kind="module"` 时包含 `moduleId + ReadQueryStaticIr`），供 field build/IR export/门禁消费（不允许在 runtime 侧通过 subscribe 黑盒识别）。
  - `FieldKernel.source` 的 `key` 改为 **deps-as-args**：`key(...depsValues)`（不再接收 `key(state)`），与 computed 对齐，避免“key 读集漂移/隐式依赖”导致 deps/IR 不一致；实现层通过 DSL 将其 lower 为 `key(state)` 供 source runtime 调用。
  - FieldKernel 必须把 externalStore 作为一等 kind 进入 `Program/Graph/Plan`（新增 plan step，如 `external-store-sync`），并在 `FieldKernel.exportStaticIr` 导出 `source/ownership/lane` 等 policy，确保 Static IR digest 覆盖结构变化。
  ...
  - lane hint 下沉到 field policy：externalStore 可声明 `lane`（urgent/nonUrgent），TickScheduler 仅做 lane→notify-priority 映射（urgent→normal，nonUrgent-only→low），notify 节流策略仍由 RuntimeStore topic facade 承担。
  - Non-goals：fields 不负责跨模块稳定化、topic version 增量或 React 订阅；这些属于 RuntimeStore/TickScheduler/SelectorGraph。
...
- `coalesceWindowMs`：**Pre-Write** 聚合（写回前聚合）：底层 ExternalStore.getSnapshot 仍为 raw current；coalesce 发生在 field 写回层（pending/raw 与 committed 分离），只有 committed 值进入 state 与 RuntimeStore snapshot（避免“未 notify 但可观测值已变化”的 tearing）。外部源的 subscribe 回调不延迟，仅延迟 committed 写回与 tick flush。
...
- Lane 判定与入口：
  ...
  - 显式降级：允许通过 `dispatchLowPriority` 与 `FieldKernel.externalStore({ priority: "nonUrgent" })` 把“可延后/可合并”的链路放入 **nonUrgent**；预算降级只推迟 nonUrgent backlog。
...
- “全是 urgent”风险控制：默认把 origin（dispatch/externalStore writeback）视为 urgent，但派生扩散（例如跨模块 declarative link 执行、source refresh）应允许按 policy/field 标注降为 nonUrgent；并通过 `coalesceWindowMs`/`Runtime.batch(...)` 合并高频触发，保持 budget 与 lanes 仍有实际调度意义。
...
- 强一致仅对 declarative IR 生效：ExternalStoreTrait + FieldProgram + DeclarativeLinkIR；**黑盒 `orchestration process link surface` 的写入不进入同 tick fixpoint**（Next Tick best-effort），并在 diagnostics 中显式标注边界。
...
- C｜黑盒心智负担：强一致只覆盖 declarative IR；黑盒 `orchestration process link surface` 的写入明确为 Next Tick best-effort。预算降级允许 partial fixpoint，但必须通过 `trace:tick.result.stable=false` 与 `triggerSummary` 解释“为何本次不是强一致”。
