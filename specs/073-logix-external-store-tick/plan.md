...
- Decision: selector-topic activation ownership 继续下沉到 core retain/release：`RuntimeExternalStore` 在 static readQuery topic 上只保留 topic facade + grace teardown，不再自己 `runDrain(changesReadQueryWithMeta(...))` 持有 activation；activation lease 由 core 内部 retain/release 提供，避免短 listener gap 与高频 mount/unmount 下重复启停 selector drain。
- Decision: selector retain/release 不再引入外部 `Scope.make()` 包装：`SelectorGraph.ensureEntry(...)` 自身不依赖调用方 scope，retain 冷路径禁止再为 static selector activation 额外创建/关闭无效 scope，避免给每次冷激活叠固定税。
- Decision: selector entry / hub 在最后一个订阅者 release 后进入 core 缓存态：active 集合归零后仍可 `hasAnyEntries=false`，但后续同 selectorId 的 ensure 必须复用既有 entry / hub，避免 long-gap cold activation 重复分配 `PubSub`、reads 索引与 entry 壳对象。
- Decision: readQuery topic facade 若创建后始终没有 listener，必须按同一条 grace teardown 路径自动从 runtime cache 移除；禁止把“只创建 store、未进入 subscribe”留成常驻 cache。
