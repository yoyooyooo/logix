# PR：命名收敛提案（历史命名 → Workflow）——对齐 073/075 与平台口径

> 目的：把“自由编排层（Π）”的命名在 **对外 DX** 与 **对内/平台** 之间收敛为同一个词：`workflow`，避免在同一条链路里出现多套历史词导致心智噪音与文档漂移。

## Status

- **Accepted（075 已吸收）**：本提案已在 `specs/075-workflow-codegen-ir/*` 内完成命名收敛；本文保留为“当时需要拍板的牵引点清单”，用于同步教程/索引与避免二次漂移。
...
- 对外 DX 值对象已统一为 `Workflow`（不再引入历史别名作为第二套名）
...
4. **与 012 Program→Process 的交叉：是否会把“Program”词重新引回公共 API？**
   ...
   - 这也是倾向统一为 `Workflow` 的直接动机之一

## 建议落点（如果采纳）

1. 在 `specs/075-workflow-codegen-ir/*` 内完成“命名裁决”更新（spec/contracts/quickstart/tasks）
2. 同步 Workbench 文档措辞（`docs/specs/sdd-platform/workbench/*` 中的历史表述）
