...
>
> 公式基线：`docs/ssot/platform/foundation/01-the-one.md`（含 `Σ_t=(S_t,I_t)` 扩展与 Tape 口径）。
...
- `serviceId` 必须是稳定字符串：按 `specs/078-module-service-manifest/contracts/service-id.md` 从 `Context.Tag` 派生（单点 helper），禁止用 `toString()` 充当身份。
