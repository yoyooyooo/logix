...
- FlowProgram 的 Static IR 是 Root IR 的 `workflowSurface` slice（$\Pi$ 的可导出形态）；手写 watcher 允许存在，但必须降级为 `opaque effect`（显式登记，禁止静默黑盒）。
...
2. **体验层（DX）**：对外暴露一个 Effect 风格的“值对象”`FlowProgram`（未来可更名为 `Workflow`），它本身可 `toJSON()` 导出 `WorkflowDef`，并提供冷路径方法 `validate()/exportStaticIr()/install(...)`。其方法只是对同一份 def 的编译操作，不引入第二语义。
...
1. **Given** 模块声明了 submit FlowProgram，**When** dispatch `submit`，**Then** 按声明顺序执行 `call → dispatch(success) → navigate`，且所有事件能通过 `tickSeq` 关联。
...
- FlowProgram 有可导出的 Workflow Static IR（$\Pi$ slice；结构可视化、diff、审查），并能被 `ControlSurfaceManifest` 收口引用；
...
1. **`call`（原 `serviceCall`）不提供结果数据流（v1）**：FlowProgram 只表达控制流（success/failure、并发/取消/时间）；任何“基于 service 结果计算后续 payload/条件分支”的需求必须下沉到 service（由 service 自己 dispatch/写 state），或拆成多个 Program 通过 action 串联。
...
8. **Root IR 收口（v1）**：平台/Devtools/Alignment Lab 只消费 `ControlSurfaceManifest`（digest + 最小索引；按需加载 slices）。FlowProgram 的 Workflow Static IR 必须作为 `workflowSurface` slice 被 Root IR 引用；禁止把“完整 IR 图”塞进运行期事件流。
...
- **FR-001**: 系统 MUST 提供 FlowProgram 的声明式 DSL 与可导出的 Workflow Static IR（见 `contracts/ir.md`），并能被 mount 为运行时 watcher（复用既有 FlowRuntime 语义）；同时该 IR 必须能作为 `ControlSurfaceManifest.workflowSurface` slice 被 Root IR 引用（见 `docs/ssot/platform/contracts/03-control-surface-manifest.md`）。
...
- **FR-003**: FlowProgram MUST 支持至少三类步骤：`dispatch`、`call`、`delay`；其中：
...
- **FR-005**: FlowProgram 的 Workflow Static IR MUST JSON 可序列化、带版本号与 digest；digest 算法与裁剪口径必须与 Root IR 一致（Stable JSON + budgets；见 `docs/ssot/platform/contracts/03-control-surface-manifest.md`）。
...
- **FR-007**: FlowProgram MUST 提供可被 Root IR 收口的最小索引信息：至少包含 `effectId/programId/trigger/sourceKey?` 等可回链字段；Root IR 默认只存 digest + `effectsIndex`，不得内嵌全量 nodes/edges 表（见 `docs/ssot/platform/contracts/03-control-surface-manifest.md`）。
...
- **NFR-004**: 事务窗口禁 IO：FlowProgram 不得在 reducer/txn 内执行 call；任何违反必须 fail-fast 并产出诊断。
