# Research: Workflow IR（从命令式 Flow 到可编译控制律）
...
- TickScheduler ≈ 求解器/积分器；Workflow ≈ 控制律；FieldProgram ≈ 约束/势能（受限绑定）。
- “影子时间线”≈ 参考系之外的非协变项：局部看似合理，但全局无法保持因果与可解释性。

## 设计裁决（本 spec 的最小集合）

- Workflow 是“自由编排”的结构化表示：它不是 FieldKernel 的一种 meta（避免语义混装）。
- Workflow 的最小节点集先收敛为：trigger(action/lifecycle) + step(dispatch/call/delay) + concurrencyPolicy；timer fire 属于 `delay/timeout/retry` 等时间算子的内部调度结果（需进入 tick 参考系与 trace/tape）。
...
- ✅ API 落点：v1 先做可挂载的 `ModuleLogic`；当前推荐装配入口收敛为 `Program.make(Module, { workflows: [...] })`，底层仍保留 `program.install(Module.tag)` 作为高级装配能力；蓝图 `flows` 槽位作为后续 DX 演进（forward-only）。
...
- ✅ 取消/补偿：v1 不引入 compensation 节点；取消语义先以 `cancelReason/cancelledByRunId` + 业务 action/reducer（或 service 自己 dispatch）表达；compensation 作为 v2/backlog 议题（需先定义可回放/可预算的最小语义）。
...
- `specs/022-module/*`、`specs/023-logic-fields-setup/*`：蓝图/setup/run 的生命周期约束
