...
- [ ] T001 GATE: Define `FieldKernel.source` auto-trigger policy (replace `triggers/debounceMs` reflection) in `packages/logix-core/src/FieldKernel.ts` + `specs/076-logix-source-auto-trigger-kernel/contracts/public-api.md`
- [ ] T002 [P] Export policy in FieldKernel Static IR (`policy.autoRefresh`) in `packages/logix-core/src/internal/state-field/ir.ts` + `specs/076-logix-source-auto-trigger-kernel/contracts/ir.md`
...
- [ ] T010 GATE: Precompute depsIndex during field build in `packages/logix-core/src/internal/state-field/build.ts` (must be `O(totalDeps)` at build time; no per-commit scan)
...
- [ ] T020 GATE: Migrate `@logixjs/query` to rely on kernel auto-trigger; remove `packages/logix-query/src/internal/logics/auto-trigger.ts` usage; update `packages/logix-query/src/Fields.ts` lowering
- [ ] T021 [P] Migrate `@logixjs/form` to rely on kernel auto-trigger; remove dependence on `FieldLifecycle.makeSourceWiring`
...
- [ ] T033 既有文档措辞同步（延后到本需求收尾阶段）：在 `docs/ssot/platform/appendix/concepts/01-frontend-pattern-landscape.md` 补充 post-076 “next patterns to consider” 指针（仅措辞/导航对齐）
