...
- **Docs-first & SSoT**：Manifest IR 属于平台可消费资产；需要在 `docs/specs/sdd-platform/ssot` 补充/更新“模块 Manifest IR（含 servicePorts）”的裁决（避免仅代码里暗自演化）。
- **Effect/Logix contracts**：变更 `ModuleManifest`（reflection contract）与试运行报告（TrialRunReport 的 environment/alignment 部分）；需要同步更新 runtime-logix 参考文档（反射/试运行）与本 spec 的 `contracts/*`。
