...
- Decision: 静态识别以 `docs/specs/sdd-platform/ssot/ir/00-codegen-and-parser.md` 的 Platform-Grade AST 规则为准；Loader/TrialRun 只作为证据/校验输入。
...
- **Docs-first & SSoT**：静态识别规则以 `docs/specs/sdd-platform/ssot/ir/00-codegen-and-parser.md` 为准；Loader 相关约束参考 `docs/specs/sdd-platform/workbench/15-module-runtime-reflection-loader.md`；与 `specs/078-module-service-manifest/spec.md` 形成闭环（显式声明→Manifest→诊断/回放）。
