...
logix describe --runId desc1 --json --profile trace
```

### 标准 JSON 样例（最小可解析）

`CommandResult@v1`（失败示例，含 `reasonCodes`）：
  ...
  "artifacts": [
    {
      ...
      "budgetBytes": 512,
      "actualBytes": 410,
      "reasonCodes": ["MISSING_REQUIRED_FILE:control-surface.manifest.json"],
      "inline": {
        "schemaVersion": 1,
        "kind": "IrValidateReport",
        "status": "violation"
...
`contract-suite.verdict.json`（最小字段示例）：
...
```json
{
  ...
  "reasons": [
    {
      "code": "MISSING_RULES_MANIFEST",
      "severity": "error",
      "message": "rulesManifest artifact is required but missing"
    }
  ]
}
```
...
pnpm -C packages/logix-cli measure:startup:ci
```

`measure:startup*` 输出 `CliStartupBudgetReport@v1`（包含样本、均值、p95、runner 标签、阈值与 pass/fail）。
