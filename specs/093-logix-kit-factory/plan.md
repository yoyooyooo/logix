...
- `forService(tag)`：把一个稳定 `Context.Tag` 同时接到 Field/Logic/Workflow 三个入口（等价组合，不新增语义）。
- `forModule(module, readQuery)`：把 Module-as-Source 标准化为 InputKit，供 `FieldKernel.externalStore` 写回使用。
...
**Primary Dependencies**: `effect` v3、`@logixjs/core`（FieldKernel/ExternalStore/Workflow/ReadQuery）
**Storage**: N/A
...
- **Intent → Flow/Logix → Code → Runtime**：本特性属于“组合层/造糖层”，目标是把既有运行时原语（ExternalStore/FieldKernel/Logic/Workflow）用确定性模板拼装成可复用 sugar，减少胶水并避免双真相源；不新增运行时语义与调度路径。
- **Docs-first & SSoT**：语义与身份规则以既有 SSoT 为准，主要依赖：
  - `specs/073-logix-external-store-tick`（ExternalStore + externalStore field + Module-as-Source 边界）
...
- **Effect/Logix contracts**：新增 `@logixjs/core` 公共子模块（`Kit`），仅提供 build-time 组合 API；不改变既有 ExternalStore/FieldKernel/Workflow 契约。
...
- **Transaction boundary**：Kit 不在事务窗口内执行 IO；写侧通过显式 command/service port（Effect）在事务外执行，读侧通过 `FieldKernel.externalStore` 写回进入事务窗口。
- **React consistency（no tearing）**：不触及 React 订阅模型；但强制遵守“外部源必须通过 ExternalStore→FieldKernel.externalStore 接入，UI 不双读”口径（避免双真相源）。
