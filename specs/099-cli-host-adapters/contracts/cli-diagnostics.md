...
当命令进入 `mode=write`（例如 `anchor autofill --mode write`）时：

- 至少输出 1 条 `DiagnosticV1`，用于提示“写回已授权/已执行/需复核”；
- 写回失败必须有 `severity=error` 的诊断，并提供可执行补救动作；
- 推荐 code：
  - `AUTOFILL_WRITE_APPLIED`
  - `AUTOFILL_WRITE_FAILED`
  - `AUTOFILL_REPORT_ONLY`（显式说明仍是只读路径）
