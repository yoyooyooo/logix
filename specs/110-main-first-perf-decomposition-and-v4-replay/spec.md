...
- 当前实时里程碑：
  ...
  - `2026-03-29` `selector_nonreact_plane_dedupe` 的 declarative-link fanout verify 已完成，当前 cheap-local 结论仍保持 `poc_positive_in_cheap_local`
...
| `postmerge-nonreact-fanout-writeback-fusion` | `main_control` | `provisional` | `heavier_local` | `blocked` | `not_applicable` | `unknown` | same-target Module-as-Source probe 已证明 target commits 按 `1 / 8 / 32` 线性增长；module-side writeback fusion PoC 已把 same-target focused 与 heavier local p95 都压到近常数 |
...
- 当前 `111` 已完成 cheap local 与 heavier local；旧 shadow candidate 的 candidate-specific regression 已由 cutdown v2 在 representative points 上清掉。当前进入 `shadow_local_recovery_candidate` 状态，live candidate 继续 blocked。
...
- 当前总控下一步改为：
  ...
  - post-merge big-cut identify 当前改为：
    ...
    - `external-store.ts` 的 module-side writeback fusion 已拿到 `heavier_local_positive_on_module_side`
