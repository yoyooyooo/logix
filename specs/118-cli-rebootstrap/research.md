...
## Decision 1: verification 一级命令面只保留 `check / trial / compare`

- **Decision**: 新 CLI 的 verification 一级命令面收敛为 `check`、`trial`、`compare`。
- **Rationale**: 这三条已是 verification control plane 的正式主干。
- **Current note**: 171 后续新增 `logix live <task>` 作为 live runtime collaboration lane；本历史 decision 不覆盖 live lane。
