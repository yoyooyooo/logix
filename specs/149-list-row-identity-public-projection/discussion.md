...
> **Final cutover note (2026-05-11):** This stopped spec is now fully consumed by `specs/155-form-api-shape/spec.md` and `docs/ssot/form/13-exact-surface-contract.md`. Earlier wording that did not freeze exact noun/import shape is superseded: current row-facing route is `fieldArray(...).byRowId(...)` plus row-scoped selectors, with no complete `useFormList` core/helper family.
