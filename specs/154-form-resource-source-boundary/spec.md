...
> **Final cutover note (2026-05-11):** The accepted user-view for form-local remote dependency is only `field(path).source({ resource, deps, key, triggers?, debounceMs?, concurrency?, submitImpact? })`. `Form.Source`, form-level source families, public source refresh helpers, and component-side remote sync remain rejected.
