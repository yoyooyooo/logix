...
> **Final cutover note (2026-05-11):** This plan predates the final single-track cleanup. Replace any `AC3.3` baseline wording with the final `Form.make / field.source / field.companion / rule-root-list-submit / useModule-useSelector` authority. `examples/logix-form-poc/**` is now deleted and must not be recreated or preserved as compatibility material.
...
- 不重建 `examples/logix-form-poc`；该旧 POC / package-local hook teaching route 已在 final single-track cutover 中删除
