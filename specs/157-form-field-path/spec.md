...
> **Final cutover note (2026-05-11):** This companion implementation spec is consumed by the final exact surface. `field(path).companion({ deps, lower })` remains accepted, but public `Form.Path`, public schema path mapping, carrier-bound selector routes, and Form-owned hook families remain rejected. Returned-carrier typing is the canonical exact inference route; void callback remains runtime-valid / honest-unknown.
