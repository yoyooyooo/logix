...
verification command surface 只保留 check / trial / compare。
...
171 后续已把 live runtime collaboration lane 追加为 `logix live <task>`。本页的三命令裁决只约束 verification lane，不否定 `15` 当前冻结的 public live namespace。
