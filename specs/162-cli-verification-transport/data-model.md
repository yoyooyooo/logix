...
- Runtime-stage results point to a `VerificationControlPlaneReport` artifact.
- Pre-control-plane gate failures may point to a transport error artifact; that artifact must not carry stage, verdict, repair truth or scheduling authority.
...
- `primaryReportOutputKey`, when present, references a transport error artifact, not a `VerificationControlPlaneReport`.
