...
**Decision**: `CommandResult` continues to be the stdout envelope. Runtime-stage results must point to the primary `VerificationControlPlaneReport` artifact; pre-control-plane gate failures may point to a transport error artifact that is not report truth.
