...
`160` 已把 CLI verification lane 切到 Agent First control-plane route，verification public commands 固定为 `check / trial / compare`。171 后续新增的 `logix live <task>` 属于 live runtime collaboration lane，不改变本页对 `CommandResult` 与 verification transport 的约束。`runtime/16` 继续压出了 CLI 层必须补齐的闭环能力：
...
- 输入坐标可重建同一命令或升级命令。
- runtime-stage primary machine report 始终来自 `VerificationControlPlaneReport` artifact。
- pre-control-plane gate failure 可以产出 primary transport error artifact；该 artifact 不拥有 report verdict、repair truth 或 stage authority。
...
- **Primary Report Output Key**: The artifact outputKey selected by CLI transport. Runtime-stage results point to the primary `VerificationControlPlaneReport`; pre-control-plane gate failures may point to a transport error artifact.
