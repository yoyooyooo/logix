...
- canonical live evidence event or artifact facets with stable runtime coordinate
...
| `degradation-notice` | stable canonical evidence authority ref | mark dropped, oversized, incomplete, inconclusive, degraded evidence |
...
- Live operation, selector-route, host-commit, profile and snapshot evidence cannot directly create diagnostic findings unless a control-plane report, evidence gap, or degradation authority supports that finding.
- Runtime debug event batches can only be projected when they are canonical evidence event facets, canonical evidence artifact refs, or temporary producer feeds that have already been normalized into authority input.
...
### 171 Live Evidence Projection Addendum

Live bridge inputs 只能通过 canonical evidence 或 owner-approved artifact refs 进入 Kernel。

| 输入 | Kernel lane | 允许投影 | 禁止投影 |
| --- | --- | --- | --- |
| canonical evidence envelope | `truthInputs` | evidence package session、artifact、metric、degradation 与 gap nodes | second report 或 verdict |
| live operation `accepted/completed/failed/denied` facet | `truthInputs` | session/debugEvidence/artifact/degradation nodes；binding 或 authority 不足时，denial 也可以产生 evidence gap | standalone diagnostic finding 或 Workbench-owned operation truth |
| static-live binding header | `contextRefs` 或 evidence facet 内的 authority metadata | manifest/action/schema digest guard、drilldown label、mismatch gap | reflection manifest mutation 或 new runtime coordinate |
| 带 `stageClass` 的 selector-route observation facet | `truthInputs` 加可选 `contextRefs` | drilldown、artifact、degradation、evidence gap，或有 owner authority 时的 scenario/host-harness backed finding | 缺少 control-plane、scenario 或 host-harness authority 的 check/startup finding |
| 带 `stageClass` 的 host-commit、profile 或 snapshot facet | `truthInputs` | artifact、metric、drilldown、degradation notice、evidence gap | 默认 `runtime.check` 或 startup verdict |
| capture budget、sampling、dropped、degraded 或 redaction marker | `truthInputs` | degradation notice 或 evidence gap | 静默省略 |
| selection manifest 或 selected target coordinate | `selectionHints` | initial focus、filtering、ordering | session/finding/artifact identity 或 severity |

规则：

- Kernel 不接受 raw live transport envelopes 作为 truth inputs。
- Repo-internal debug drilldown feeds 在归一成 canonical evidence facets、artifact refs、degradation notices 或 evidence gaps 之前，不能作为 Kernel truth inputs。
- 171 targeted implementation fixes the first normalization path: DVTools debug drilldown live facets normalize to `live-evidence` truth inputs and drilldown gaps normalize to `evidence-gap` truth inputs before projection. Host view models over the projection index remain allowed; private session/finding/artifact truth remains forbidden.
- Operation denial 是 operation facet result。它可以支撑 evidence gap 或 degradation notice，但不是第五类 finding。
- Selector 或 host quality evidence 缺少 `stageClass` 时，输入降级为 drilldown 或 evidence gap。
- `runtime.check`、`runtime.trial` 和 `runtime.compare` 的 verdict authority 保持在 Kernel 之外。

### 171 Researchability Header Addendum

171 的 researchability data 只能作为 comparable evidence header metadata、metric refs、degradation notices、drilldown refs 或 evidence gaps 进入 Kernel。
...
- `metricRef[]` 是引用列表，不是 Workbench-owned metric family registry。
- detailed decision traces 必须作为 artifact refs、drilldown refs、redaction markers 或 evidence gaps 进入。
- Kernel 可以展示 comparability gaps，但不能产生 adopt/discard verdicts。
- AutoResearch adoption algebra、adoption ledger 和 autonomous loop state 不是 171 的 Workbench projection inputs。
...
- Driver/Scenario execution outputs may enter `truthInputs` only after they become Run result, control-plane report, canonical evidence package, evidence artifact ref or canonical live evidence facet.
