...
- `VerificationDependencyCause` browse nodes
...
- Reflection bridge 必须把 167 manifest 展开成 action、payload 与 `VerificationDependencyCause` browse nodes；不能只停留在 `reflectionManifest` artifact ref。
...
- Live operation, selector-route, host-commit, profile and snapshot facts consumed by Playground must come from canonical live evidence event or artifact facets. Playground must not infer them from UI state, preview state, or reflection manifest fields.
...
### TD-007A: Playground Live Bridge Dogfood Host

171 之后，Playground 可以作为 live bridge dev-only dogfood host。
...
- Playground 只提交 adapter offers 与 source/context refs。它不拥有 runtime identity、operation authority、evidence envelope 或 Workbench projection。
- Playground dogfood 可在 Batch 6 gates 后使用 allowlisted operation slot。operation implementation 存在前，仍可证明 discovery、capture、export、projection 和 compare handoff。
- Product scenario playback、preview state、source editor selection 和 demo metadata 不能创建 live operation、selector-route、host-commit、profile、snapshot、report、verdict 或 finding truth。
- Playground 消费的 live evidence 必须是 canonical evidence facet、artifact ref、degradation notice 或 evidence gap。
- Unsupported product-only facts 必须表现为 host view state、viewer-local gap 或 discussion item。
- 当前 examples/logix-react targeted dogfood route 是 `logix-react.live-bridge`。它只作为 Logic-first Playground proof fixture，输出 live target coordinate、structured denial clue 和 canonical evidence package shape；它不得输出 `repairHints`、`nextRecommendedStage` 或 verification verdict。171 已关闭 full playground browser route suite；compare before/after deep closure 属于 future extension。
...
- **FR-013A**: Playground live evidence projection MUST consume canonical live evidence facets for operation, selector-route, host-commit, profile and snapshot facts. It MUST NOT derive those facts from reflection manifest, UI state or product scenario metadata.
- **FR-013B**: Playground live bridge dogfood MUST follow the 171 path from adapter offer to canonical evidence export, Workbench projection and verification/compare handoff. Controlled operation steps MUST use Batch 6 allowlisted operation slots only.
