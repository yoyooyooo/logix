# Hotpath Diagnostics-Off Internal Counters Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 为 source / validate / selector / converge fallback 建内部可观测计数，同时守住 diagnostics=off 无 trace 分配。

**Architecture:** 新增 test-only/internal hotpath audit sink。生产热路径默认没有 sink，因此只有空指针/undefined 分支；diagnostics 或 tests 显式安装 sink 时才记录 counters。该需求不是 benchmark，不采集耗时，只记录 structural decision counts 和 fallback reason counts。
...
### Create
- `packages/logix-core/src/internal/runtime/core/KernelHotPathAudit.ts` — new focused artifact for this requirement; keep it internal unless explicitly a docs/script artifact.
- `packages/logix-core/test/Contracts/KernelHotPathAudit.contract.test.ts` — new focused artifact for this requirement; keep it internal unless explicitly a docs/script artifact.
...
- `packages/logix-core/test/Debug/Debug.OffSemantics.NoDrift.test.ts` — update only for this requirement's stated responsibility; avoid opportunistic cleanup.
...
```bash
pnpm -C packages/logix-core test test/Contracts/KernelHotPathAudit.contract.test.ts
pnpm -C packages/logix-core test test/Debug/Debug.OffSemantics.NoDrift.test.ts
pnpm -C packages/logix-core test test/Debug/Debug.DiagnosticsLevels.test.ts
...
### Task 1: Write audit contract test
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Create a test that installs a sink, triggers source dirty fallback, and expects a count by reason.
  ```
...
- [ ] **Step 6: Commit**
  ...
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 1"
  ```

### Task 2: Create audit module
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Add `KernelHotPathAudit.ts` with `withKernelHotPathAuditSink`, `recordKernelFallback`, and snapshot helpers.
  ```
...
- [ ] **Step 6: Commit**
  ...
  ```bash
  git add <changed-files-for-this-task>
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 2"
  ```
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Record source dirty gate fallback only if sink exists.
  ```
...
- [ ] **Step 6: Commit**
  ...
  ```bash
  git add <changed-files-for-this-task>
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 3"
  ```
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Record static-ir selection/fallback decisions only if sink exists.
  ```
...
- [ ] **Step 6: Commit**
  ...
  ```bash
  git add <changed-files-for-this-task>
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 4"
  ```
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Record dirty fallback reason on commit only if sink exists.
  ```
...
- [ ] **Step 6: Commit**
  ...
  ```bash
  git add <changed-files-for-this-task>
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 5"
  ```
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Record legacy/unknown dirty only if sink exists.
  ```
...
- [ ] **Step 6: Commit**
  ...
  ```bash
  git add <changed-files-for-this-task>
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 6"
  ```
...
- [ ] **Step 2: Write or update the failing guard/contract test**
  ...
  ```text
  Run audit, diagnostics-off, and fallback tests.
  ```
...
- [ ] **Step 6: Commit**
  ...
  ```bash
  git add <changed-files-for-this-task>
  git commit -m "chore(kernel): hotpath-diagnostics-off-internal-counters task 7"
  ```
## Final Verification
